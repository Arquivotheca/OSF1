...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: WmWinConf.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/07/16 21:40:10 $"
#endif
#endif
/*
	...
	if (pev->type == KeyPress) 
	{
		...
		case XK_Escape:
		    ...
		    return;
                ...
                /* Fix for escape key on DEC keyboards */
                case XK_F11:
                  /* Is escape known on this keyboard ? */
                  if ( !XKeysymToKeycode( DISPLAY, XK_Escape ))
                    /* No, Map it to F11 */
                    {
                      CancelFrameConfig (pcd);
                      CheckEatButtonRelease (pcd, pev);
...
int error_code;
	 ...
	 */
#ifdef DEC_MOTIF_EXTENSION
	error_code = XGrabPointer( DISPLAY, grab_win, FALSE, pmask,
	...
	{	
            ...
            if ( wmGD.debug )
              {
                fprintf( wmGD.debug_file, "**** grab pointer failed %d \n",
                         error_code );
                fflush( wmGD.debug_file );
              }
	    return(FALSE);
    ...
    {
#ifdef DEC_MOTIF_EXTENSION
	error_code = XGrabKeyboard( DISPLAY, grab_win, FALSE, GrabModeAsync,
		         	    GrabModeAsync, grabTime );
        if ( wmGD.debug )
          {
            ...
            if ( wmGD.debug )
              {
                fprintf( wmGD.debug_file, 
                         "Grab keyboard failed, ungrab pointer %d \n", 
                         error_code );
                fflush( wmGD.debug_file );
              }
	    return(FALSE);
	    ...
	    (!(pCD->pSD->moveOpaque)))
	{
        ...
        if ( wmGD.debug )
          {
            fprintf( wmGD.debug_file, "Grab server\n" );              
            fflush( wmGD.debug_file );
          }
        }
    }
...
#endif /* _NO_PROTO */
{
	...
	XUngrabServer(DISPLAY);
        ...
        if ( wmGD.debug )
          {
            fprintf( wmGD.debug_file, "ungrab server\n" );
            fflush( wmGD.debug_file );
          }
#endif
    }
	...
	XUngrabKeyboard (DISPLAY,CurrentTime);
        ...
        if ( wmGD.debug )
          {
            fprintf( wmGD.debug_file, "Ungrab keyboard\n" );
            fflush( wmGD.debug_file );
          }
      }
#endif
    ...
    if ( wmGD.debug )
      {
        fprintf( wmGD.debug_file, "Ungrab pointer\n" );
        fflush( wmGD.debug_file );
      }
#endif
    XFlush (DISPLAY);
...
#endif /* _NO_PROTO */
{
			 ...
			 pev->xbutton.time) == GrabSuccess)
	{
            ...
            if ( wmGD.debug )
              {
                fprintf( wmGD.debug_file, "Grab pointer, checkeatbuttonrelease\n" ); 
                fflush( wmGD.debug_file );
              }
#endif
...
#endif /* _NO_PROTO */
{
    ...
    if ( wmGD.debug )
      {
        fprintf( wmGD.debug_file, "grab pointer, regrabpointer\n" );
        fflush( wmGD.debug_file );
      }
#endif

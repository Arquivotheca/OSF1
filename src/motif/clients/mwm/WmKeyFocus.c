...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: WmKeyFocus.c,v $ $Revision: 1.1.4.4 $ $Date: 1993/10/18 17:18:04 $"
#endif
#endif
/*
...
static Boolean removeSelectNumLockGrab = True;
static Boolean removeSelectModGrab = True;
#endif
...
#ifdef DEC_MOTIF_EXTENSION
			    removeSelectNumLockGrab = False;
   			    if ( wmGD.ignoreModKeys || wmGD.ignoreAllModKeys )  
			        removeSelectModGrab = False;
			}
			else 
                        {
                          if (buttonSpec->state == LockMask)
			        removeSelectLockGrab = False;
   			  if (( wmGD.ignoreModKeys || wmGD.ignoreAllModKeys ) &&
                              (( buttonSpec->state == Mod2Mask ) || 
            		       ( buttonSpec->state == Mod3Mask ) ||
            		       ( buttonSpec->state == Mod4Mask )))
    			       removeSelectModGrab = False;         
			  if ( wmGD.ignoreNumLockMod &&
			       buttonSpec->state == NumLockMask )
			      removeSelectNumLockGrab = False;
#else
			}
		    ...
		    (pCD->clientState == MAXIMIZED_STATE)))
	{
                  ...
                  && removeSelectModGrab )
	    {
                if ( wmGD.ignoreAllModKeys || 
                     ( ModeSwitchOfDisplay( DISPLAY ) & Mod2Mask ))
    	            XUngrabButton( DISPLAY, FOCUS_SELECT_BUTTON, Mod2Mask,
		                   pCD->clientBaseWin);
                if ( wmGD.ignoreAllModKeys ||
                     ( ModeSwitchOfDisplay( DISPLAY ) & Mod3Mask ))
    	            XUngrabButton( DISPLAY, FOCUS_SELECT_BUTTON, Mod3Mask,
		                   pCD->clientBaseWin);
                if ( wmGD.ignoreAllModKeys || 
                     ( ModeSwitchOfDisplay( DISPLAY ) & Mod4Mask ))
    	            XUngrabButton (DISPLAY, FOCUS_SELECT_BUTTON, Mod4Mask,
		    pCD->clientBaseWin);
	    } else if ( wmGD.ignoreNumLockMod && removeSelectNumLockGrab ) {
		XUngrabButton (DISPLAY, FOCUS_SELECT_BUTTON, NumLockMask,
			       pCD->clientBaseWin);
	    }
    ...
    if ( wmGD.ignoreModKeys || wmGD.ignoreAllModKeys )

      {
        if ( wmGD.ignoreAllModKeys || ( ModeSwitchOfDisplay( DISPLAY ) & Mod2Mask ))
            /* Grab MB1 with all the modifiers */
            XGrabButton( DISPLAY, FOCUS_SELECT_BUTTON, Mod2Mask, window, False, 
                         ButtonReleaseMask, GrabModeSync, GrabModeAsync, 
                         None, None );
        if ( wmGD.ignoreAllModKeys || ( ModeSwitchOfDisplay( DISPLAY ) & Mod3Mask ))
            XGrabButton( DISPLAY, FOCUS_SELECT_BUTTON, Mod3Mask, window, False, 
                         ButtonReleaseMask, GrabModeSync, GrabModeAsync, None,
                         None );
        if ( wmGD.ignoreAllModKeys || ( ModeSwitchOfDisplay( DISPLAY ) & Mod4Mask ))
            XGrabButton( DISPLAY, FOCUS_SELECT_BUTTON, Mod4Mask, window, False, 
                         ButtonReleaseMask, GrabModeSync, GrabModeAsync, None,
                         None );
      } else if ( wmGD.ignoreNumLockMod ) {
	  XGrabButton( DISPLAY, FOCUS_SELECT_BUTTON, NumLockMask, window, False,
		       ButtonReleaseMask, GrabModeSync, GrabModeAsync, None,
		       None );
      }
#endif /* DEC_MOTIF_EXTENSION */

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: WmWinList.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/07/16 22:04:56 $"
#endif
#endif
/*
...
/* This has been fixed in Xm and Mwm 1.2 (See WmCEvent.C
   HandleCPropertyNotify).  However, this code fix in 1.1 for losttransients 
   needs  to remain for backward compatibility for XUI and Motif 1.1 applications. */
    ClientData *lost;
    ClientData *prev;
#endif
    ...
    /* Check if this window has any transient children that
       were previously mapped. */
    lost = wmGD.transientLost;
    prev = wmGD.transientLost;
    while( lost != NULL )
      {
        /* Is this the parent for the lost transient ? */
        if ( lost->transientFor == pCD->client )
          /* Yup, do some re-adding. */
          {        
            /* Remove the client from the list */
            DeleteClientFromList( pCD->pSD->pActiveWS, lost );
            lost->transientLeader = pCD;
            /* Make it a proper transient. */
            AddTransient( pCD->pSD->pActiveWS, lost );
            /* Restack the windows. */
	    RestackTransientsAtWindow ( lost );
            /* Done */
            break;
          }
        /* Is it the first one ? */
        if ( wmGD.transientLost != lost )
            /* No, move the lost one */
            prev = prev->transientLost;
        lost = lost->transientLost;
      }  /* while */
...
#endif /* _NO_PROTO */
{
...
#ifdef DEC_MOTIF_BUG_FIX
    /* If this was on the global lost transient list,
       then remove it. */
    /* Check if this window has any transient children that
       were previously mapped. */
    lost = wmGD.transientLost;
    ...
    while( lost != NULL )
      {
        /* Is this client on lost transient list ? */
        if ( lost == pCD )
          /* Yup, remove it from the list. */
          {        
            /* Is it the first one ? */
            if ( wmGD.transientLost == lost )
                /* Yes, remove the first */
                wmGD.transientLost = wmGD.transientLost->transientLost;
            /* No, remove a middle one */
            else prev->transientLost = lost->transientLost;
            /* Done */
            break;
          }
        /* Next ! */
...
#ifdef DEC_MOTIF_BUG_FIX
/* This fixes a problem when an application has several dialog boxes and
   one of them is full application modal.  This used to cause
   an accvio or allow dialog boxes other than the modal one to
   be raised. */
        /* Are there any application full modal dialog boxes ? */
        if ( pcdLeader->fullModalCount )
            /* Yes, do not restack the application */
            return( False );
#endif /* DEC_MOTIF_BUG_FIX */

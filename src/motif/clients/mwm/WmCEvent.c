...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: WmCEvent.c,v $ $Revision: 1.1.4.5 $ $Date: 1993/10/18 17:18:52 $"
#endif
#endif
/*
...
/*************************************<->*************************************
 *
 *  WmModBtnCheck( buttonSpec, grabWindow, grabState, eventMask, ModMask )
 *
 *
 *  Description:
 *  -----------
 *  Grab the keys button on the mod masks.  Look at the
...
#ifdef _NO_PROTO
void WmModBtnCheck( buttonSpec, grabWindow, grabState, eventMask, ModMask )
...
#else /* _NO_PROTO */
void WmModBtnCheck( ButtonSpec *buttonSpec, Window grabWindow, 
                    unsigned int grabState, unsigned int eventMask, 
                    unsigned int ModMask )
...
/********************************/
    ...
    if ( wmGD.ignoreAllModKeys ||                    
      ...
      {
        XGrabButton( DISPLAY, buttonSpec->button, ( grabState | ModMask ),
                     grabWindow, False, eventMask, GrabModeSync,
                     GrabModeAsync, None, None );
        XGrabButton( DISPLAY, buttonSpec->button, 
                     ( grabState | ModMask | LockMask),
                     grabWindow, False, eventMask, GrabModeSync,
                     GrabModeAsync, None, None );
      }
    ...
    /* If this has changed since the last mod key, 
      ...
      { 
        XUngrabButton( DISPLAY, buttonSpec->button, ( grabState | ModMask ),
                       grabWindow );
        XUngrabButton( DISPLAY, buttonSpec->button, 
                       ( grabState | ModMask | LockMask),
                       grabWindow );
      }
	    ...
	    (buttonSpec->subContext & F_SUBCONTEXT_W_CLIENT))
	{
	    ...
	    XGrabButton (DISPLAY, buttonSpec->button, (grabState | LockMask),
            ...
            if ( wmGD.ignoreModKeys || wmGD.ignoreAllModKeys )
              {
                WmModBtnCheck( buttonSpec, window, grabState, eventMask, 
                               Mod2Mask );
                WmModBtnCheck( buttonSpec, window, grabState, eventMask, 
                               Mod3Mask );
                WmModBtnCheck( buttonSpec, window, grabState, eventMask, 
		...
		XGrabButton (DISPLAY, buttonSpec->button, 
			     (grabState | NumLockMask),
			     window, False, eventMask, GrabModeSync,
			     GrabModeAsync, None, None);
		XGrabButton (DISPLAY, buttonSpec->button, 
			     (grabState | LockMask | NumLockMask),
			     window, False, eventMask, GrabModeSync,
			     GrabModeAsync, None, None);
	    }
#endif /* DEC_MOTIF_EXTENSION */
...
#endif /* _NO_PROTO */
{
	...
	if (wmGD.keyboardFocusPolicy == KEYBOARD_FOCUS_EXPLICIT)
	{
        ...
        /*
         * Dont want to assign input focus to a window thats being
         * minimized
         */
            if (!(subContext & F_SUBCONTEXT_W_MINIMIZE))
#endif
...
#ifdef DEC_MOTIF_BUG_FIX
	/* If the next event is a FocusIn or FocusOut event, then
           do not install a new colormap.  This avoid the 
           colormap flashing when the default
           colormap is installed for the root window.
           Do this only when the keyboard focus policy is explicit, 
           and the colormap focus policy follows the keyboard. */
	if (( wmGD.colormapFocusPolicy == CMAP_FOCUS_KEYBOARD ) &&
	    ( XEventsQueued( DISPLAY, QueuedAlready ) > 0 ))
          {
	    XPeekEvent( DISPLAY, &next_event );
            if (( next_event.type == FocusIn ) ||
                ( next_event.type == FocusOut ))
	      {
		if ( wmGD.debug )
                  {
                    fprintf( wmGD.debug_file, "Colormap ignored on FocusOut\n" );
                    fflush( wmGD.debug_file );
                  }
	        focusFlags |= IGNORE_COLORMAP_FOCUS;
	      }
	  }
#endif

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: UilLstLst.c,v $ $Revision: 1.1.4.2 $ $Date: 1993/05/07 00:35:14 $"
#endif
#endif
...
**  FUNCTIONAL DESCRIPTION:
**
**      VMS specific routine to create the listing file.
**
**  FORMAL PARAMETERS:
**
**      az_fcb          fcb for the listing file
**
**  IMPLICIT INPUTS:
**
...
{
    status              l_open_status;
    static char const   c_default_file_spec[]= ".lis";
    char                interim_file_name[ NAM$C_MAXRSS+1 ];

    az_fcb->fab = cc$rms_fab;
    az_fcb->nam = cc$rms_nam;
    az_fcb->rab = cc$rms_rab;
     az_fcb->fab.fab$l_dna = c_default_file_spec;
    az_fcb->fab.fab$b_dns = (sizeof c_default_file_spec)-1;
    az_fcb->fab.fab$b_org = FAB$C_SEQ;
    az_fcb->fab.fab$b_rfm = FAB$C_VAR;
    az_fcb->fab.fab$b_rat = FAB$M_CR;
    az_fcb->fab.fab$l_fop = (FAB$M_SQO | FAB$M_OFP);
    az_fcb->fab.fab$b_fac = FAB$M_PUT;
    az_fcb->fab.fab$l_fna = Uil_cmd_z_command.ac_listing_file;

    if (Uil_cmd_z_command.ac_listing_file != NULL)
        az_fcb->fab.fab$b_fns = strlen(Uil_cmd_z_command.ac_listing_file);
    else
        az_fcb->fab.fab$b_fns = 0;
    ...
    */
    ...
    az_fcb->nam.nam$l_rlf = src_az_source_file_table[ 0 ]->fab.fab$l_nam;

    az_fcb->rab.rab$l_fab = &(az_fcb->fab);
    az_fcb->rab.rab$b_rac = RAB$C_SEQ;
    az_fcb->rab.rab$l_rop = RAB$M_WBH;

    /* create the output file */

    l_open_status = sys$create( &(az_fcb->fab) );

    if (_error( l_open_status ))
    {
    ...
    l_open_status = sys$connect( &(az_fcb->rab) );

    if (_error( l_open_status ))
        return src_k_open_error;

    /* call lib$lp_lines to find out the number of lines on a page */

    lst_l_usable_lines = lib$lp_lines() - 9;
...
**  FUNCTIONAL DESCRIPTION:
**
**      VMS specific function to write a line to the listing file.
**
**  FORMAL PARAMETERS:
**
...

{
    ...
    */
    ...
    /*
    **  Check to see if we are at the end of the page.  If so put
    **  start a new page with a new banner.
    */
    ...
    if ((lst_l_lines_left <= 0) || v_new_page)
    {
        ...
        lst_az_fcb->rab.rab$w_rsz = 1;
        lst_az_fcb->rab.rab$l_rbf = form_feed;

        sys$put( &(lst_az_fcb->rab) );

        sprintf(buffer, "%s%d", lst_c_title1, lst_l_page_no);

        lst_az_fcb->rab.rab$w_rsz = strlen( buffer );
        lst_az_fcb->rab.rab$l_rbf = buffer;

        sys$put( &(lst_az_fcb->rab) );

        lst_az_fcb->rab.rab$w_rsz = strlen( Uil_lst_c_title2 );
        lst_az_fcb->rab.rab$l_rbf = Uil_lst_c_title2;

        sys$put( &(lst_az_fcb->rab) );

        lst_az_fcb->rab.rab$w_rsz = 1;
        lst_az_fcb->rab.rab$l_rbf = blank_line;

        sys$put( &(lst_az_fcb->rab) );

    }

    lst_az_fcb->rab.rab$w_rsz = strlen( ac_line );
    lst_az_fcb->rab.rab$l_rbf = ac_line;

    write_status = sys$put( &(lst_az_fcb->rab) );

    if (_error( write_status ))
    {
...
**  FUNCTIONAL DESCRIPTION:
**
**      VMS specific function to return ASCII date and time in VMS format.
**
**  FORMAL PARAMETERS:
**
**      void
...
static  $DESCRIPTOR( date_desc, date_buffer );

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: UilDB.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/10/18 16:49:00 $"
#endif
#endif
...
String get_root_dir_name()
{
	...
	tmp1 = (char *)getenv("HOMEDRIVE");
	tmp2 = (char *)getenv("HOMEPATH");

	ptr = XtMalloc(strlen(tmp1) + strlen(tmp2) + 1);
	strcpy(ptr, tmp1);
	strcat(ptr, tmp2);
#else
    ...
    String	filename ;
{
    ...
    if (path  == NULL)
        {
        wmd_path = XtCalloc(1, strlen(WMDPATH_DEFAULT));
        strcpy (wmd_path, WMDPATH_DEFAULT);
        }
    else
        {

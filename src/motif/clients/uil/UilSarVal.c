...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: UilSarVal.c,v $ $Revision: 1.1.4.2 $ $Date: 1993/05/07 00:38:23 $"
#endif
#endif
...
**      This function takes a value frame which is the contents
**      of a table of strings and appends it to a value entry which is
**      the next entry in that table.  It is used by solely by the nested
**      compound string (ncs) value type.
**
**  FORMAL PARAMETERS:
**
**      value_frame     pointer to current table entry stack frame
...
void    sar_append_ncs_value( value_frame, table_frame )

yystype     *value_frame;
yystype     *table_frame;

{
    sym_value_entry_type    *value_entry, *next_value_entry, *table_entry;
    int                     value_type;
    ...
    /*
    ** Check for a forward reference. If the
    ** table entry is named, than a valref entry linked to it is placed
    ** in the table. If the new entry is a forward reference, than a
    ** valref is always required, but we can use the value node which
    ** is supplied as it is a parser artifact, and has no other use.
    ...
    */
    ...
    if (table_entry != NULL)
        {
        next_value_entry = table_entry;
        while (next_value_entry->az_next_table_value != NULL)
            {
            next_value_entry = next_value_entry->az_next_table_value;
            }
        next_value_entry->az_next_table_value = value_entry;
        }
    else
        {
    ...
    ** Save source information
    */
    _sar_save_source_pos ( &value_entry->header, value_frame);
    ...
    */
    ...
    if (table_entry == NULL)
        ...
        {
        value_frame->b_type = table_entry->b_type;
        value_frame->b_flags = table_entry->obj_header.b_flags;
        value_frame->value.az_symbol_entry = (sym_entry_type *) table_entry;
        }
...
**  FUNCTIONAL DESCRIPTION:
**
**      This function set the properties of a ncs CHAR_8_LITERAL
**      such as direction and seperation.
**
**  FORMAL PARAMETERS:
**
**      value_frame     pointer to string value
...
void    sar_add_ncs_char_8_attr
    ( value_frame, attr_frame )
...
yystype     *attr_frame;
{
    ...
    if ((value_frame->b_flags & sym_m_forward_ref) != 0)
        sym_make_value_forward_ref (value_frame,
            &(value_entry->az_exp_op1), sym_k_patch_add);
    else
        {
        value_entry =
            (sym_value_entry_type *) value_frame->value.az_symbol_entry;
    ...
    /*
    **  If the attr_frame is not null, it must be a value frame with contains
    **  a pointer to the value entry for the userdefined charset, or a token fr
    **  which contains the charset token subclass.
    */
    switch (attr_frame->b_tag)
    {
    case sar_k_value_frame:
        ...
        */
        ...
        value_entry->b_aux_flags |= (attr_frame->b_type & sym_m_separate);
        break;
...
**  FUNCTIONAL DESCRIPTION:
**
**      This function makes a nested COMPOUND_STRING and sets the properties
**      of the string.
**
**  FORMAL PARAMETERS:
**
...
void    sar_make_ncs
    ( target_frame, value_frame, right_to_left)
...
Boolean     right_to_left;
{
    ...
    */
    ...
    cstr_entry = (sym_value_entry_type *) sem_create_cstr();
    cstr_entry->b_expr_opr = sym_k_comp_str_op;
    cstr_entry->b_arg_type = sym_k_nested_cs;
    value_entry =
        (sym_value_entry_type *) value_frame->value.az_symbol_entry;
    cstr_entry->az_exp_op1 = value_entry;
    cstr_entry->az_first_table_value = value_entry;


    /*
    **  Set the direction of the string.
    */
    if (right_to_left)
        {
        cstr_entry->b_direction = XmSTRING_DIRECTION_R_TO_L;
        }
    else
        {
        cstr_entry->b_direction = XmSTRING_DIRECTION_L_TO_R;
        }
    ...
    */
    ...
    _sar_move_source_info( target_frame, value_frame );

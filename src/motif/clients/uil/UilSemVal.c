...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: UilSemVal.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/07/15 18:42:48 $"
#endif
#endif
...
#if defined(VMS) && defined(VAXC) && defined(BOGUS_MB_MAX)
/* from <Xm/XmosP.h> */
#undef MB_LEN_MAX
#define MB_LEN_MAX 1
#undef MB_CUR_MAX
	...
	for (list_member=(sym_obj_entry_type *)list_node->obj_header.az_next;
		...
		break;
                ...
                case sym_k_argument_entry:
                    arg_entry = (sym_argument_entry_type *) list_member;
                    sem_validate_node
                        ((sym_entry_type *)arg_entry->az_arg_value);
                    break;
#endif
		}
	break;
...
/*
 * If this is a forward reference, and that forward reference has already
 * been evaluated, then don't re-evaluate.
 */
if ( ( ( value_node->obj_header.b_flags & sym_m_forward_ref) != 0 ) &&
     (value_node->az_exp_op1 != NULL) )
    {
    if (value_node->az_exp_op1->obj_header.b_flags & sym_m_validated)
        {
        /*
         * Set the node to validated in case there is more forward references
         */
...
/* END HAL Fix CR 4774 */
	...
	sem_evaluate_value_wcs(val_entry);
	break;
...
/*
 *      If this is a nested compound string do the evaluation in another
 *      routine.
 */
if (csval_entry->b_arg_type == sym_k_nested_cs)
    {
    sem_evaluate_value_ncs(csval_entry);
    return(csval_entry);
    }
#endif
...
**  FUNCTIONAL DESCRIPTION:
**
**      This function evaluates a nested compound string value node, and
**      returns the resulting compound string.
**
**  FORMAL PARAMETERS:
**
...
sym_value_entry_type *sem_evaluate_value_ncs (csval_entry)
    sym_value_entry_type        *csval_entry;
...
_assert(csval_entry->az_first_table_value != NULL,
        "nested compound string with no segments" );

/*
 *  Create the direction the string will begin with
 */

cstr_r = XmStringDirectionCreate (csval_entry->b_direction);

/*
 **  Loop through the rest of the segments of the string and append
 **  them to the first direction of the string. There are two types.
 **  The simplest type is char_8_value which just needs to have a
 **  compound string created for it and concatenate with the compound
 **  string that has already been created.
 **
 **  The second type is a nested compound string. To evaluate a nested
 **  compound string just recursively call this routine.
 */
for (next_segment = csval_entry->az_first_table_value;
     next_segment != NULL;
     next_segment = next_segment->az_next_table_value)
    {
    switch (next_segment->b_type)
        {
        case sym_k_char_8_value:
             ...
             **  Create this segment, then concatenate to the result string.
             **  Free the two inputs now that a concatenated result string
             **  exists. Create a separartor if needed, concatenate to the
             **  result string and free the two inputs.
             */
            cstr_2 = XmStringCreate (next_segment->value.c_value, csetptr);
            cstr_1 = cstr_r;
            cstr_r = XmStringConcat (cstr_1, cstr_2);
            XmStringFree (cstr_1);
            XmStringFree (cstr_2);
            if ((next_segment->b_aux_flags & sym_m_separate) != 0 )
                {
                cstr_1 = cstr_r;
                cstr_2 = XmStringSeparatorCreate();
                cstr_r = XmStringConcat (cstr_1, cstr_2);
                XmStringFree (cstr_1);
                XmStringFree (cstr_2);
                }
        ...
        case sym_k_compound_string_value:
            /*
             **  If this isn't a nested compound string scream about it.
             */
            _assert(next_segment->b_arg_type == sym_k_nested_cs,
                    "unexpected compound string in nested compound string" );
            /*
             **  Recall this routine evaluate the nested compound string.
             **  The result will be a compound string which can be concatenated
             **  to the result string. Free the two inputs now that the concate
             **  result string exits.
             */
            sem_evaluate_value_ncs(next_segment);
            cstr_1 = cstr_r;
            cstr_r = XmStringConcat (cstr_1, (XmString) next_segment->value.c_value );
            XmStringFree (cstr_1);
            XmStringFree ((XmString) next_segment->value.c_value);
            next_segment->value.c_value = NULL;
            break;
        default:
            _assert(FALSE, "unexpected type in nested compound string" );
        } /* end switch */
    } /* end for loop */

/*
 * Wrap things up by balancing out the direction of the string with a REVERT
 * Do the standard concatenation to finish up.
 */
cstr_2 = XmStringDirectionCreate (XmSTRING_DIRECTION_REVERT);
cstr_1 = cstr_r;
cstr_r = XmStringConcat (cstr_1, cstr_2);
XmStringFree (cstr_1);
XmStringFree (cstr_2);
...
**  FUNCTIONAL DESCRIPTION:
**
**	This function evaluates a wide_character string value node, and
**	returns the resulting wide_character string.
**
**  FORMAL PARAMETERS:
**
**	wcsval_entry	pointer to the value node to be evaluated
**
**  IMPLICIT INPUTS:
**
**      none
...
sym_value_entry_type *sem_evaluate_value_wcs (wcsval_entry)
    sym_value_entry_type	*wcsval_entry;

{
wchar_t 	*wcstr_r;
size_t		max_size, str_size;

_assert( (wcsval_entry->header.b_tag == sym_k_value_entry) &&
	(wcsval_entry->b_type == sym_k_wchar_string_value),
	"value not wide_character string" );

/*
 **	You can't do anyting about imported wide_character strings so return.
 */
if ((wcsval_entry->obj_header.b_flags & sym_m_imported) != 0) 
    return(wcsval_entry);

/** Convert multibyte string to wide_character string. **/

/* Allocate memory. */
max_size = wcsval_entry->w_length * MB_CUR_MAX;
wcstr_r = (wchar_t *)XtMalloc(sizeof(wchar_t)*max_size);

/* Convert */
str_size = mbstowcs(wcstr_r, wcsval_entry->value.c_value, max_size);

/* Reallocate memory. */
wcstr_r = (wchar_t *)XtRealloc((char *)wcstr_r, sizeof(wchar_t)*str_size);

/* Store result. */
XtFree(wcsval_entry->value.c_value);

wcsval_entry->value.c_value = (char *) wcstr_r;
wcsval_entry->w_length = str_size;

_assert(wcsval_entry->w_length <= MrmMaxResourceSize, 
	"wide character string too long" );
...
sym_value_entry_type		*cat_str_entry;
    ...
    */
...
#ifdef DEC_MOTIF_EXTENSION
        if (value_entry->b_arg_type == sym_k_nested_cs)
            {
            sem_evaluate_value_ncs(value_entry);
	...
	switch (op1_type)
	{
	    ...
	    value_entry->az_first_table_value->b_aux_flags =
		value_entry->b_aux_flags;
	    sem_evaluate_value_wcs(value_entry);

 ...
 * HISTORY
 */
static char SccsId[] = "@(#)gepolio.c	1.2\t1/31/90";
/*
*****************************************************************************
**                                                                          *
**                         COPYRIGHT (c) 1988 BY                            *
...
**  FACILITY:
**
**	GEPOLIO	             	       Polygon object IO handler
**
**  ABSTRACT:
**
**
...
switch (geVersionIn)
  {case 8:
   case 7:
	  ...
	  {c = geFgetc(geFdI);                  /* Get seg STRT char  - "{"  */
	   if (geFeof(geFdI)) break;
	    if ((char)c != '{')                 /* Either there is a problem */
	      {geUngetc((char)c, geFdI); break;}/* or this poly is done      */
	...
	if (PSegP)                              /* Poly sub-segs=grp private */
	  {if (PSegP->Next) PSegP->Next->Prev = NULL;
   ...
   case 3:
         ...
         *    - 3 is reading and writing the Z coord and Visible flag
         */
	if (PSegP = ASegP)
	  {geFscanf2(geFdI, " %d %d",
		     &ASegP->Z,

 ...
 * HISTORY
 */
static char SccsId[] = "@(#)gelinio.c	1.4\t4/17/89";
/*
*****************************************************************************
**                                                                          *
**                         COPYRIGHT (c) 1988 BY                            *
...
**  FACILITY:
**
**	GELINIO	             	       Line object IO handler
**
**  ABSTRACT:
**
**
...
if (cmd == GEWRITE)
  {if (geVersionOut == GECURRENTVERSION)
     {/*
      ...
      fprintf(geFdO, " %d", 
	      ASegP->Z);
      if (geGenSIOCo (cmd, &priv->Co))                        return;
...
switch (geVersionIn)
  {case 8:
   case 7:
      ...
      */
     ASegP->Private    = geMalloc(sizeof(struct GE_LIN));
     priv 		  = (struct GE_LIN *)ASegP->Private;
     if (geGenSIOTM (cmd, &geCTM))  		                return;
     geFscanf1(geFdI, " %d",
	       &ASegP->Z);
     if (geGenSIOCo (cmd, &priv->Co))                        break;
   ...
   case 3:
         ...
         *    - 3 is reading and writing the Visible flag, and LineFillStyle
	 *        and LineFillHT
         */
        ASegP->Private    = geMalloc(sizeof(struct GE_LIN));
	priv 		  = (struct GE_LIN *)ASegP->Private;

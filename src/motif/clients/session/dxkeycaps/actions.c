...
void popup_menu_action ();
void left_arrow_action ();
...
void
popup_menu_action (widget, event, argv, argc)
     KeyboardWidget widget;
     XEvent *event;
     char *argv[];
     int *argc;
{
  ...
  if (event->xany.type == KeyPress)
    {
      if (XtIsManaged(global_menu))
	    {
	      XtUnmanageChild(global_menu);
	    }
      else 
	{
	  XmMenuPosition(global_menu, event);
	  XtManageChild(global_menu);
	}
    }
  else 
    {
  ...
  if (key)
    {
      ...
      sprintf (b, " Key %s (0x%02X) ", key->key.key_name,
	       key->key.key->keycode);
      XtSetArg (av[0], XmNlabelString, XmStringCreate(b, def_char_set)); ac++;
    }
  else
      XtSetArg (av[0], XmNlabelString,
		XmStringCreate("----No Key----", def_char_set)); ac++;
  
  XtSetValues (menu, av, 1);

  if (key || CurrentKey)
      sensitize_menu (widget, menu, True);
  else
      sensitize_menu (widget, menu, False);
     ...
     int *argc;
{
    extern KeyWidget CurrentKey;
    /*
    **  'menuIsUp flag is set/cleared in commands.c (where it is
    **  defined). Its presence prevents this code from de-selecting
    **  a key being edited when the user moves the mouse into the
    **  edit menu -bg.
    */
    extern int menuIsUp;
  ...
  if (event->xany.type != MotionNotify)
    return;
    /*
    **  don't select a key if we're editing it...-bg
    */
    if (menuIsUp) return;
  {
    ...
    CurrentKey =  widget->keyboard.key_under_mouse;
...
void
keyboard_handle_mapping_notify (widget, client_data, event)
     KeyboardWidget widget;
     caddr_t client_data;
     XEvent *event;
{
  ...
  if (!events_queued_p && widget->keyboard.documented_key)
    /* refresh, in case the documented key has been modified */
    {
      describe_key (widget->keyboard.documented_key);
...
void
keyboard_handle_keymap_notify (widget, client_data, event)
     KeyboardWidget widget;
     caddr_t client_data;
     XEvent *event;
{
  ...
  if (! bcmp (event->xkeymap.key_vector,
	      widget->keyboard.key_state_vector, 32))

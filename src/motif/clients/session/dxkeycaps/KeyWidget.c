...
/*
**	This function updates the KeyWidget's key_name field
**	It needs to be called after a new keycap has been manufactured.
**	If it's not called, the describe_key function gets confused,
**	and prints the wrong keyname for the KeyWidget.
*/
make_key_name (KeyWidget w)
{
	char	buf[255];
	char	*b;
	char	*top;
     ...
     XSetWindowAttributes *attr;
{
  ...
  w->key.keycode_gc = XtGetGC (gw, (unsigned) GCFont|GCForeground, &gcvalues);
	...
	/*
	**	make_key_name() was extracted from the 'else' part of this
	**	code... -bg
	*/
	make_key_name (w);
}
     ...
     KeyWidget w;
{
  ...
  if (top && !*top) top = (char *)0;
  if (bot && !*bot) bot = (char *)0;
...
#define PERCHAR(font,c) \
  (font->per_char \
   ? &font->per_char[(int)(unsigned char)(c) - (int) font->min_char_or_byte2] \
   : &font->max_bounds)
...
#define MAXSTRHEIGHT(font, font2, var) \
  ...
  if (bot)
    {
      /*
      **	these structure references (bot[0]->???) were 
      **	changed from 'top[0]->???' -bg
      */
      x = inner_margin - PERCHAR (bot_font, bot[0])->lbearing;
      y = inner_margin
	+ (top ? MAXSTRHEIGHT (keycap_font, bot_font, bot) : 0)
...
KeyHighlight (keyw)
     KeyWidget keyw;
{
  if (keyw->key.highlighted_p || !keyw->key.key->keycode) return;

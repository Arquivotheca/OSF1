 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: xsetroot_cust.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/11/05 15:47:44 $";
#endif
    ...
    char **argv;
{
    ...
    wmroot = WmRootWindow(dpy, root);
    wmroot = WmRootWindow(dpy, wmroot);
	...
	XDefineCursor(dpy, wmroot, cursor);
	...
	if (!cursor_file) {
	    ...
	    XUndefineCursor(dpy, wmroot);
	}
	if (!excl) {
	    XSetWindowBackgroundPixmap(dpy, root, (Pixmap) None);
	    XSetWindowBackgroundPixmap(dpy, wmroot, (Pixmap) ParentRelative);
	    XClearWindow(dpy, wmroot);
	    unsave_past = 1;
	}
    }
    ...
    unsigned int width, height;
{
    ...
    gc_init.foreground = NameToPixel
    (
	fore_color,
	NameToPixel
	(
    ...
    gc_init.background = NameToPixel
    (
	back_color,
	NameToPixel
	(
    ...
    XSetWindowBackgroundPixmap(dpy, wmroot, pix);
    XFreeGC(dpy, gc);
    ...
    char *cursor_file, *mask_file;
{
    ...
    fp = NameToPixel("DXmDynamicPointerForeground", BlackPixel(dpy, screen));
    fp = NameToPixel(fore_color,fp);
    bp = NameToPixel("DXmDynamicPointerBackground", WhitePixel(dpy, screen));
    bp = NameToPixel(back_color,bp);
    fg = PixelToXColor(fp);
    bg = PixelToXColor(bp);
    if (reverse) {
    ...
    unsigned long pixel;
{
    ...
    if (!name || !*name)
	return pixel;
    if (strncmp(name, "DXmDynamic", 9) == 0)
    {
        Atom type;
        int format;
        ...
        int i, index = -1;
        Atom name_atom = XInternAtom (dpy, name, TRUE);
	...
	{
	    if ((Atom) colors[i] == name_atom) index = i;
	}

	if (index >= 0)
	{
	    ecolor.pixel = colors[index+1];
	}
	else
	{
	    return (pixel);
    ...
    else if (!XAllocNamedColor(dpy, DefaultColormap(dpy, screen), name,
			  &scolor, &ecolor))
    {

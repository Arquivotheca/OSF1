 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: xgifload.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/11/05 18:10:10 $";
#endif
/* 
...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: xgifload.c,v $ $Revision: 1.1.2.2 $ $Date: 1993/11/05 18:10:10 $"
#endif
#endif
/*
...
#ifdef VMS
        for (i=0; i<numcols; i++) ccols[i] = (unsigned long) i;
#else
...
char *fname;
{
    ...
    if (nostrip)  {   /* nostrip was set.  try REAL hard to do it */
        ...
        if (j) {		/* failed to pull it off */
            ...
            for (i=0; i<numcols; i++)
#ifdef VMS
                if (used[i] && ccols[i]==0xffff) {  /* an unallocated pixel */
#else
        ...
        while (strip<8) {
            ...
            if (i<numcols) {		/* failed */
                ...
                for (i--; i>=0; i--)
#ifdef VMS
                    if (used[i]) XFreeColors(theDisp,theCmap,ccols+i,1,0L);
#else

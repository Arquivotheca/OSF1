 ...
 */
/*
 * @(#)$RCSfile: dwi18n_lib.c,v $ $Revision: 1.1.5.2 $ (DEC) $Date: 1993/08/03 00:04:11 $
 */
/*
*****************************************************************************
**                                                                          *
...
/*--------------------------------------------------*/
/* Tables of modifier state (Non-Shift, Caps-Lock,  */
/* Shift) to determine which KeySym to rebind.      */
/*--------------------------------------------------*/

static KeySym modlist_tbl1_m1_a[] = { XK_Mode_switch };	/* Non-Shift */

static KeySym modlist_tbl2_m2_a[] = { XK_Mode_switch,   /* Caps-Lock */
				      XK_Caps_Lock };

static KeySym modlist_tbl3_m3_a[] = { XK_Mode_switch,	/* Shift     */
				      XK_Caps_Lock,
				      XK_Shift_L };

static KeySym modlist_tbl3_m3_b[] = { XK_Mode_switch,   /* Shift     */
				      XK_Caps_Lock,
				      XK_Shift_R };

static KeySym modlist_tbl3_m2_c[] = { XK_Mode_switch,	/* Shift     */
				      XK_Shift_L };

static KeySym modlist_tbl3_m2_d[] = { XK_Mode_switch,	/* Shift     */
				      XK_Shift_R };

static KeySym modlist_tbl3_m2_e[] = { XK_Mode_switch,   /* Shift     */
				      XK_Shift_Lock };

static KeySym modlist_tbl3_m3_f[] = { XK_Mode_switch,	/* Shift     */
				      XK_Shift_Lock,
				      XK_Shift_L };

static KeySym modlist_tbl3_m3_g[] = { XK_Mode_switch,	/* Shift     */
				      XK_Shift_Lock,
				      XK_Shift_R };
  ...
  Display   	*dpy;
{
  ...
  int		keycode_length;
  KeySym	*keysym;
  KeySym	rebound_keysym;
  int		num_of_elements;
  int		i;
  unsigned char	str;

					/* See if language specific KeySym */
  if (!(XKeysymToKeycode(dpy, XK_kana_A)) &&
      !(XKeysymToKeycode(dpy, XK_hebrew_aleph)))
      return;				/* No language specific KeySym 
   */
...
/*-------------------------------------------------*/
  ...
  XDisplayKeycodes(dpy, &min_keycodes,  /* Obtain KeyCodes of the Display  */ 
		   &max_keycodes);
  
					/* Get symbols of the KeyCodes     */
  keycode_count = max_keycodes - min_keycodes + 1;
  keysym = XGetKeyboardMapping(dpy, min_keycodes, 
			       keycode_count,
			       &keycode_length);

  num_of_elements = keycode_count * keycode_length;
 
/*- - - - - - - - - - - - - - - - - - - - - - - - -*/
/* 	 << Rebind Non-Shift State KeySyms >>      */
/*						   */
/* ( Rebind to first KeySym, but if that KeySym is */
/*   uppercase alphabetic, then the corresponding  */
/*   lowercase KeySym is used instead. )	   */
/*- - - - - - - - - - - - - - - - - - - - - - - - -*/

  for (i = 0; i < num_of_elements; i += keycode_length)
     {
	if (keysym[i+third_element] != NoSymbol)
	  {
	    str = tolower((int) keysym[i]);    	/* convert to lowercase */
	    XRebindKeysym(dpy, keysym[i+third_element], modlist_tbl1_m1_a, 1,
			  &str, sizeof(str));
	  }
     }

/*- - - - - - - - - - - - - - - - - - - - - - - - -*/
/* 	 << Rebind Caps-Lock State KeySyms >>      */
/*						   */
/*            ( Rebind to first KeySym. )	   */
/*- - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  for (i = 0; i < num_of_elements; i += keycode_length)
	...
	if (keysym[i+third_element] != NoSymbol)
	  {
	    str = (unsigned char) keysym[i];
	    XRebindKeysym(dpy, keysym[i+third_element], modlist_tbl2_m2_a, 2,
			  &str, sizeof(str));
	  }
     }

/*- - - - - - - - - - - - - - - - - - - - - - - - -*/
/* 	  << Rebind Shift State KeySyms >>         */
/*						   */
/* ( Rebind to second KeySym, if that KeySym is    */
/*   NoSymbol, then the first KeySym is used 	   */
/*   instead. )  				   */
/*- - - - - - - - - - - - - - - - - - - - - - - - -*/

  for (i = 0; i < num_of_elements; i += keycode_length)
     {
	if (keysym[i+fourth_element] != NoSymbol)
	   rebound_keysym = keysym[i+fourth_element];
	else
	   rebound_keysym = keysym[i+third_element];

	if (rebound_keysym != NoSymbol)
	  {
	    if (keysym[i+second_element] != NoSymbol)
	    	str = (unsigned char) keysym[i+second_element];
	    else
	    	str = (unsigned char) keysym[i];

	    XRebindKeysym(dpy, rebound_keysym, modlist_tbl3_m3_a, 3,
			  &str, sizeof(str));

	    XRebindKeysym(dpy, rebound_keysym, modlist_tbl3_m3_b, 3,
			  &str, sizeof(str));

	    XRebindKeysym(dpy, rebound_keysym, modlist_tbl3_m2_c, 2,
			  &str, sizeof(str));

	    XRebindKeysym(dpy, rebound_keysym, modlist_tbl3_m2_d, 2,
			  &str, sizeof(str));

	    XRebindKeysym(dpy, rebound_keysym, modlist_tbl3_m3_f, 3,
			  &str, sizeof(str));

	    XRebindKeysym(dpy, rebound_keysym, modlist_tbl3_m3_g, 3,
			  &str, sizeof(str));
	  }
     }
...
}	/* end of DWI18n_RebindIrregularKeysym */

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: BaseClass.c,v $ $Revision: 1.1.6.4 $ $Date: 1993/10/18 15:24:20 $"
#endif
#endif
/*
...
externaldef(xmqmotif) XrmQuark	XmQmotif;
externaldef(_xm_fastptr) XmBaseClassExt	*_Xm_fastPtr;
#else
XrmQuark	XmQmotif;
...
#endif /* _NO_PROTO */
{
    ...
    if(    !*wcePtr    )
    {   *wcePtr = (XmBaseClassExt) XtCalloc( sizeof( XmBaseClassExtRec), 1) ;
    ...
    {   (*wcePtr)->wrapperData = (XmWrapperData) XtCalloc(
                                                sizeof( XmWrapperDataRec), 1) ;
        } 
...
#endif /* _NO_PROTO */
{
    ...
    wrapperData = (XmWrapperData) 
      XtMalloc(sizeof( XmWrapperDataRec)) ;
    /*
...
#endif /* _NO_PROTO */
{
    ...
    if (wrapperData = (*wcePtr)->wrapperData)
...
{
#ifdef notdef
    XtWarning("_XmBaseClassPartInitialize is an unsupported routine");
#endif /* notdef */
}
...
#endif /* _NO_PROTO */
{
	...
	if ((isBaseClass) || (scePtr && (*scePtr)))
	{
		if (!(*wcePtr))
		  {
		  inited = False;
		  *wcePtr = (XmBaseClassExt) XtCalloc( 

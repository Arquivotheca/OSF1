...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TearOff.c,v $ $Revision: 1.1.4.4 $ $Date: 1993/07/19 15:38:21 $"
#endif
#endif
/*
...
externaldef(_xmexcludedparentpane) XmExcludedParentPaneRec _XmExcludedParentPane = {0, NULL, 0};
#else
XmExcludedParentPaneRec _XmExcludedParentPane = {NULL, NULL, NULL};
#endif
...
#endif
#ifdef DEC_MOTIF_BUG_FIX
	 _XmStringUpdateWMShellTitle(label_xms, (Widget)to_shell);
#else
...
#endif
{
   ...
   {
      if (shell = (ShellWidget)RC_PopupPosted(ancestor))
   ...
   while (ancestor && (IsPulldown(ancestor) || IsPopup(ancestor)))
   {
      ...
      {
	 XLowerWindow(XtDisplay(ancestor), XtWindow(XtParent(ancestor)));
...
#endif
{
   ...
   {
      if (pane = _XmExcludedParentPane.pane[i])
      {
...
#endif
{
   ...
   if (RC_TornOff(submenu) && RC_TearOffActive(submenu))
   {
	  ...
	  * highlighted.
	  */
	 if (child = submenu->manager.active_child)
	 {
	    if (XtIsWidget(child))

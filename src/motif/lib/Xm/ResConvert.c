...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ResConvert.c,v $ $Revision: 1.1.6.5 $ $Date: 1994/01/18 18:33:42 $"
#endif
#endif
/*
...
#define CS_DEC_CNS11643_1986_2  "DEC.CNS11643.1986-2"
#define CS_DEC_DTSCS_1990_2     "DEC.DTSCS.1990-2"
#endif
...
static void _DXmCvtStringToLayoutDirection();
#endif /* DEC_MOTIF_RTOL */
#ifdef DEC_MOTIF_EXTENSION
static void _DXmCvtStringToFitToScreenPolicy ();
...
#endif /* _NO_PROTO */
{
    ...
    if(    !registered    )
    {
        ...
        XtSetTypeConverter( XmRString, XmRTextPosition,
                            ...
                            XtCacheNone, NULL) ;
#ifdef DEC_MOTIF_RTOL
      XtAddConverter (XmRString, DXmRLayoutDirection,
                      (XtConverter) _DXmCvtStringToLayoutDirection, NULL, 0);
#endif /* DEC_MOTIF_RTOL */
#ifdef DEC_MOTIF_EXTENSION
      /* Automatic I14Y Code */
      XtAddConverter (XmRString, DXmRFitToScreenPolicy,
                      (XtConverter) _DXmCvtStringToFitToScreenPolicy, NULL, 0);
...
/************************************************************************
 *
 *  _DXmCvtStringToLayoutDirection
 *      Convert a string to a Digital layout direction
 *
 ************************************************************************/

/* ARGSUSED */
static void _DXmCvtStringToLayoutDirection (args, num_args, from_val, to_val)
XrmValue * args;
Cardinal   num_args;
XrmValue * from_val;
XrmValue * to_val;

{
   ...
   if (_XmStringsAreEqual (in_str, "left_up"))
      i = DXmLAYOUT_LEFT_UP;
   else if (_XmStringsAreEqual (in_str, "left_down"))
      i = DXmLAYOUT_LEFT_DOWN;
   else if (_XmStringsAreEqual (in_str, "right_up"))
      i = DXmLAYOUT_RIGHT_UP;
   else if (_XmStringsAreEqual (in_str, "right_down"))
      i = DXmLAYOUT_RIGHT_DOWN;
   else
   {
      to_val->size = 0;
      to_val->addr = NULL;
      XtStringConversionWarning ((char *)from_val->addr, DXmRLayoutDirection);
   }
}
#endif
...
/************************************************************************
 *
 *  _DXmCvtStringToFitToScreenPolicy
 *	Convert a string to a Digital fit to screen policy
 *
 ************************************************************************/

/* ARGSUSED */
static void _DXmCvtStringToFitToScreenPolicy (args, num_args, from_val, to_val)
XrmValue * args;
Cardinal   num_args;
XrmValue * from_val;
XrmValue * to_val;

{
   ...
   if (_XmStringsAreEqual (in_str, "none"))
      i = DXmNONE;
   else if (_XmStringsAreEqual (in_str, "as_needed"))
      i = DXmAS_NEEDED;
   else
   {
      to_val->size = 0;
      to_val->addr = NULL;
      XtStringConversionWarning ((char *)from_val->addr, DXmRFitToScreenPolicy);
   }
}
#endif	/* DEC_MOTIF_EXTENSION */
...
#endif /* _NO_PROTO */
{
    ...
    XtStringConversionWarning( (char *) from->addr, XmRStringDirection) ;
...
#endif /* _NO_PROTO */
{
    ...
    if(    from->addr    )
    {   
        tmpStr = XmStringCreateLtoR( (char *) from->addr,
...
#endif /* _NO_PROTO */
{
	 ...
	 * Parse the fontlistentries.
				   ...
				   &fontType, &delim))
        {
            XtFree (newString);
            fontListEntry =
		XmFontListEntryLoad (display, XmDEFAULT_FONT,
				     XmFONT_IS_FONT, XmFONTLIST_DEFAULT_TAG);

            /* alternative try, it should work this time! */
            if (!fontListEntry) {
                fontListEntry =
		    XmFontListEntryLoad (display, "*", XmFONT_IS_FONT,
					 XmFONTLIST_DEFAULT_TAG);
            } 
            if (fontListEntry) {
                got_it = TRUE ;
...
#ifdef DEC_MOTIF_EXTENSION
	    {
                font_struct = (XFontStruct *)_Xt_LoadQueryFont(display, fontName);
                fontListEntry = XmFontListEntryCreate(XmFONTLIST_DEFAULT_TAG,
                                                XmFONT_IS_FONT, font_struct);
		if (fontListEntry)
                {
                   got_it = TRUE;
                   fontList = XmFontListAppendEntry (fontList, fontListEntry);
...
#endif /* _NO_PROTO */
{
	...
	*delim = isspace (**s) ? ',' : **s;	/* Xm1.1 compatibility */
        **s = '\0';
    }
   ...
   for (i = 0; GetNextXmString (&s, &cs); i++)
   {
      if (i >= j)
...
#ifdef I18N_EXTENSION
       {
          long byte_count;
          long status;
          table[i] = DXmCvtFCtoCS ( cs, &byte_count, &status);
       }
#else
      table[i] = XmStringCreateLtoR (cs, XmFONTLIST_DEFAULT_TAG);
#endif
      XtFree(cs);
   }
/****************
...
#endif /* _NO_PROTO */
{
    ...
    {
        fontListEntry =
	    XmFontListEntryLoad (XtDisplay(origw), XmDEFAULT_FONT,
				 XmFONT_IS_FONT, XmFONTLIST_DEFAULT_TAG);

        /* alternative try, it should work this time! */
        if (!fontListEntry) {
            fontListEntry =
		XmFontListEntryLoad (XtDisplay(origw), "*",
				     XmFONT_IS_FONT, XmFONTLIST_DEFAULT_TAG);
	}
        if (fontListEntry) {
            data = XmFontListAppendEntry (data, fontListEntry);
            XmFontListEntryFree (&fontListEntry);
	}
    }
    return (data);
...
#endif /* _NO_PROTO */
{
    ...
    {
        while (*btype_str && isspace(*btype_str)) btype_str++;
        if (*btype_str == '\0')
            break;
...
#ifdef DEC_MOTIF_EXTENSION
/* Bug fix for RtoL nesting support. */
#define _PushDir(ctx, dir)\
    ...
    (ctx)->dirstack[++((ctx)->dirsp)] = dir
        /*
          The flag dircs is used to deal the case when two
          direction escape sequences are encountered, without any
          intervening text. It is now NOT controlled by this macro
        */
#else
#define _PushDir(ctx, dir)\
    if ( (ctx)->dirsp == ((ctx)->dirstacksize - 1) ) {\
...
*/
    if (    (	(ctx->gl_charset == CS_ISO8859_1)
	    ...
	    ||
#ifdef DEC_MOTIF_EXTENSION
            (setMSB =  (   !strcmp(ctx->gl_charset, CS_GB2312_0)
...
#ifdef DEC_MOTIF_EXTENSION
        /*  Turn on the high bit for GB2312, JISX208 & KSC5601  */

        if (setMSB) {
            int i;

            for (i = 0; i < ctx->itemlen; i++)
                tempstring[i] |= 0x80;
        }
#endif
...
#ifdef DEC_MOTIF_EXTENSION
                    /*  If GB2312, JISX208 & KSC5601 is used as GL,
                        change it into GR                            */

                    if (!strcmp(ctx->gl_charset, CS_GB2312_0))
                        new_charset = CS_GB2312_1;
                    else if (!strcmp(ctx->gl_charset, CS_JISX0208_0))
                        new_charset = CS_JISX0208_1;
                    else if (!strcmp(ctx->gl_charset, CS_KSC5601_0))
                        new_charset = CS_KSC5601_1;
                    else
                        new_charset = (String) NULL;
	...
	tempstring[i] = EOS;
#ifdef DEC_MOTIF_EXTENSION
        /*  If GB2312, JISX208 & KSC5601 is used as GL, change it into GR */
...
#ifdef DEC_MOTIF_BUG_EXTENSION
/* RtoL nesting bug fix */
/*
 * If two direction CSIs found and there was no text found between, create an
 * empty segment, with the previous direction. (Do this before pushing the
 * new direction to the stack.)
 */
                if (ctx->flags.dircs)
                    ctx->xmstring = concatStringToXmString(
                                        ctx->xmstring,
...
#endif /* _NO_PROTO */
{
  ...
  char				*unknown_value;
  XmStringComponentType		component=XmSTRING_COMPONENT_UNKNOWN;

  ok = XmStringInitContext(&context, (XmString) from->addr);
  if (!ok) return(False);

  component = XmStringGetNextComponent (context, (char **) &ctext,
                &charset,&direction,  &unknown_tag, &unknown_length,
                &unknown_value );

  if (component == XmSTRING_COMPONENT_DIRECTION) {
...
#ifdef DEC_MOTIF_EXTENSION
            /*
            ** Give an explicit octet length for the DEC private char sets.
            */
            if (!strcmp(tag, CS_DEC_CNS11643_1986_2) ||
                !strcmp(tag, CS_DEC_DTSCS_1990_2))
                *temp++ = 0x32;
            else
#endif
      *temp++ = 0x30;

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MenuShell.c,v $ $Revision: 1.1.8.4 $ $Date: 1993/08/03 21:59:28 $"
#endif
#endif
/*
...
/*
** QAR 00098 from DECW-MOTIF-IFT database
** CS, 3-Aug-1993
** _XmRecordEvent calls ManipulateEvent.  ManipulateEvent has a DEC_MOTIF_BUG_FIX which
** records the display for the event.  This is to fix a problem which has been reported
** to OSF concerning comparing event structure fields that belong to different displays.
** OSF is supposed to fix this for Motif 1.2.3.  This fix can probably be removed at the
** same time the dec bug fix is removed from ManipulateEvent.
*/
...
#endif /* _NO_PROTO */
{
    ...
    if (XmIsRowColumn (widget))
    {
	  ...
	  XtAddEventHandler (widget, EnterWindowMask, FALSE,
        ...
        * with menus being dynamically destroyed
        * and added.  Before this fix the new menu
        * was never mapped because the menu being
        *destroyed was still counted as a child.
        *
        * CS  20 April 1992
        */
...
#endif /* _NO_PROTO */
{
   ...
   if (XtIsManaged (rowcol)) 
     {
       /* First check to make sure our parent is visible when managing */
       parent = XtParent((Widget) popup);
       /* Only check if the parent is realized (CR 5207) */
...
#endif /* _NO_PROTO */
{
   ...
   if (subShell = 
       (ShellWidget) RC_PopupPosted(menushell->composite.children[0]))
   {

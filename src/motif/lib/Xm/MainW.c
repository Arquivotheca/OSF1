...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MainW.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:45:17 $"
#endif
#endif
/*
    ...
    XmMainWindowWidget mw;
{
    ...
    XtSetArg(arglist[0], DXmNlayoutDirection, LayoutM(mw));
    for (i = 0, kid = mw->composite.children;
         i < mw->composite.num_children;
         i++, kid++)
    {
            IsScroll = _XmIsFastSubclass(XtClass(*kid),XmSCROLLED_WINDOW_BIT)
                                        ||
                       _XmIsFastSubclass(XtClass(*kid),XmSCROLL_BAR_BIT);

            if (!(*kid)->core.being_destroyed && !IsScroll)
...
/* set pointer to null for any destroyed component
*/

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: LabelG.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:37:32 $"
#endif
#endif
/*
 ...
 */
   ...
   {
        if (LabG_StringDirection(new_w) != XmSTRING_DIRECTION_R_TO_L)
        {
            if (LabG_StringDirection(new_w) != XmSTRING_DIRECTION_DEFAULT)
                                        _XmWarning (new_w, STR_DIR_MESSAGE_RTOL);
            /* Force Right to Left direction */
            LabG_StringDirection(new_w) = XmSTRING_DIRECTION_R_TO_L;
        }
   }
#endif /* DEC_MOTIF_RTOL */
   if (LabG_StringDirection(new_w) == XmSTRING_DIRECTION_DEFAULT)
...
#ifdef DEC_MOTIF_BUG_FIX
            _Xm_cache_charset(LabG_MnemonicCharset (new_w),
                              strlen(LabG_MnemonicCharset (new_w)) );
#else
...
#endif /* _NO_PROTO */
{
   ...
   needed_width = LabG_TextRect(lw).width + LabG_MarginLeft(lw) + LabG_MarginRight(lw) +
                  (2 * (lw->gadget.highlight_thickness +
                        lw->gadget.shadow_thickness +
                        LabG_MarginWidth(lw)));

   needed_height = Max(LabG_TextRect(lw).height, LabG_AccTextRect(lw).height) +
      ...
      { /*
        ...
        if (LayoutIsRtoLG(lw))
           _XmStringDraw (XtDisplay(lw), XtWindow(lw),
                     ...
                     LabG_Alignment(lw), LabG_StringDirection(lw), NULL);
...
#endif /* _NO_PROTO */
{
...
#ifdef DEC_MOTIF_RTOL
   if (LabG_StringDirection(new_w) != LabG_StringDirection(current))
   {
      if (LayoutIsRtoLG(new_w) &&
                LabG_StringDirection(new_w) != XmSTRING_DIRECTION_R_TO_L)
      {
   ...
   if (LayoutG(new_w) != LayoutG(current))
   {
      ...
      if ((LabG_MarginLeft(current)  == LabG_MarginLeft(new_w))
                                    &&

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Label.c,v $ $Revision: 1.1.6.4 $ $Date: 1993/07/15 16:35:21 $"
#endif
#endif
/*
...
#define STR_DIR_MESSAGE_RTOL "Invalid value in XmNstringDirection for Right to Left layout"
      ...
      if ((newlw->core.sensitive) && (newlw->core.ancestor_sensitive))
      {
         if (Pix(newlw) != XmUNSPECIFIED_PIXMAP)
         {
            ...
            XGetGeometry (XtDisplay(newlw),
                          Pix(newlw),
#ifdef ALPHA_BUG_FIX
                          &junkWin,    /* returned root window */
			  /* sizeof(Window) != sizeof(int) on ALPHA/OSF1 
			     plus poor coding practice the other way :-) */
#else
...
#endif /* _NO_PROTO */
{
...
#ifdef DEC_MOTIF_RTOL
   {
       if (LayoutIsRtoLP(newlw))
       {
          ...
          {
             lp->margin_left = lp->acc_TextRect.width + LABEL_ACC_PAD;
          }
       }
       else
       {
 ...
 */
   ...
   if (LayoutIsRtoLP(lw))
   {
        if (lw->label.string_direction != XmSTRING_DIRECTION_R_TO_L)
        {
           if (lw->label.string_direction != XmSTRING_DIRECTION_DEFAULT)
                       _XmWarning ((Widget) lw, STR_DIR_MESSAGE_RTOL);
   ...
   if (lw->label.string_direction == XmSTRING_DIRECTION_DEFAULT)
    ...
    if  (lw->label.mnemonicCharset != NULL )
...
#ifdef DEC_MOTIF_BUG_FIX
                _Xm_cache_charset(lw->label.mnemonicCharset,
                              strlen(lw->label.mnemonicCharset) );
#else
		_XmStringCharSetCreate (lw->label.mnemonicCharset );
             ...
             lw->label.mnemonicCharset =
#ifdef DEC_MOTIF_BUG_FIX
                _Xm_cache_charset(XmFONTLIST_DEFAULT_TAG,
...
/******************************************************************************/
/*                                                                            */
/* This code is being placed before the call to SetSize() so that the high-   */
/* thickness value is set to 0 initially BEFORE the TextRect.(x,y) are    */
/* calculated.   The problem with this value being set after the call to      */
/* SetSize is that if a call to SetSize came in later thru other means, and   */
/* was only called for certain labels in a menu, then the .x coordinate would */
/* be different than originally calcalated here.                          */
/*                                                                            */
/******************************************************************************/
#ifdef DEC_MOTIF_BUG_FIX
 ...
 */
   ...
   needed_width = lp->TextRect.width + lp->margin_left + lp->margin_right +
                  (2 * (lw->primitive.highlight_thickness +
                        lw->primitive.shadow_thickness +
                        lp->margin_width));

   needed_height = Max(lp->TextRect.height, lp->acc_TextRect.height) +
   ...
   if ((new_w->label.menu_type == XmMENU_POPUP) ||
...
#ifdef DEC_MOTIF_RTOL
    if (new_w->label.string_direction != current->label.string_direction)
    {
       if (LayoutIsRtoLP(new_w) &&
                   new_w->label.string_direction != XmSTRING_DIRECTION_R_TO_L)
       {
          /* Force Right to Left direction */
          new_w->label.string_direction = XmSTRING_DIRECTION_R_TO_L;
          _XmWarning ((Widget) new_w, STR_DIR_MESSAGE_RTOL);
       }
    ...
    if (LayoutP(new_w) != LayoutP(current))
    {
       if (LayoutIsRtoLP(new_w))
          ...
          if (new_w->label.string_direction == current->label.string_direction)
                        new_w->label.string_direction = XmSTRING_DIRECTION_L_TO_R;

       /* if no new margins specified swap them */
       if ((Lab_MarginLeft(current)  == Lab_MarginLeft(new_w))
                                    &&
...
#ifdef DEC_MOTIF_BUG_FIX
              _Xm_cache_charset(new_w->label.mnemonicCharset,
                              strlen(new_w->label.mnemonicCharset) );
#else

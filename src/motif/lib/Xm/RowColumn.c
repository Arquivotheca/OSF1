...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: RowColumn.c,v $ $Revision: 1.1.8.5 $ $Date: 1993/11/10 22:01:18 $"
#endif
#endif
/*
...
/*
    RtoL support:
        Menubar and other rowcolumns execute different code
        1. Menubar
           MoveLeft & MoveRight routines are actually move previous
           and move next son.
           the 'Previous' & 'Next' widgets depend on layout direction
           Therefore we have to swap search of Prev or Next entries when
           dealing with RtoL layout direction: Left arrow - Next (=Right),
           Right arrow - Prev(=Left).
        2. Other rowcolumn - arrows handled by TRAVERSAL.C, routine
           _XmMgrTraversal and module NAVIGMAP.
*/
...
#endif /* _NO_PROTO */
{
   ...
   if (mst->RC_ButtonEventStatus.verified)
   {
      ...
      /******************************************************************/
      /*                                                                */
      /* If owner-events is TRUE, the initial button press does not get */
      /* sent to the grabbing window if this is run on the Sun server.  */
      /* Thus, we set owner-events to FALSE in                          */
      /* AddHandlersToPostFromWidgetand then do an explicit grab        */
      /* (with owner-events set to TRUE) in the ButtonEventHandler.     */
      /*                                                                */
      /*        7-30-91                                                 */
      /*                                                                */
      /******************************************************************/
       if (xbutton_event->button == RC_PostButton(popup))
           XtGrabPointer (w,
                          TRUE,
                          ButtonReleaseMask,
                          GrabModeSync,
                          GrabModeSync,
                          NULL,
                          XmGetMenuCursor(XtDisplay(popup)),
                          xbutton_event->time);
#endif
...
#endif /* _NO_PROTO */
{
   ...
   /* If owner-events is TRUE, the initial button press does not get    */
   /* sent to the grabbing window if this is run on the Sun server.     */
   /* Thus, we set owner-events to FALSE and then do an explicit grab   */
   /* (with owner-events set to TRUE) in the ButtonEventHandler.        */
   /*                                                                   */
   /*   7-30-91                                                         */
   /*                                                                   */
   ...
   XtGrabButton (widget, RC_PostButton(popup), RC_PostModifiers(popup),
      FALSE, ButtonReleaseMask, GrabModeSync, GrabModeSync, None, cursor);
#else
   XtGrabButton (widget, RC_PostButton(popup), RC_PostModifiers(popup), 
...
#endif /* _NO_PROTO */
{
     ...
     */
    if (RCIndex(w) != m->composite.num_children)
...
#endif /* _NO_PROTO */
{
   ...
   if (IsOption(m))
   {
	 ...
	 if ( RC_MemWidget(m) && XtIsManaged (RC_MemWidget (m)))
	 {
	     Widget cb;

	     if (cb = XmOptionButtonGadget( (Widget) m))
...
#endif /* _NO_PROTO */
{
    ...
    if (IsOption(new_w))
    {
       if (RC_OptionSubMenu(new_w) != RC_OptionSubMenu(old)) {
          ...
          if (child = XmOptionButtonGadget( (Widget) new_w))
	      ...
	      !RC_TearOffControl(new_w))
	  {
	     ...
	     if (XtHeight(tw) != 1) tw->label.recompute_size = False;
           ...
           XtAddCallback (RC_TearOffControl(new_w), XmNarmCallback, 
                          ArmDisarmTearOffCB, (XtPointer)0);
           XtAddCallback (RC_TearOffControl(new_w), XmNdisarmCallback, 
                          ArmDisarmTearOffCB, (XtPointer)1);
#endif
...
#endif /* _NO_PROTO */
{
   ...
   /*                                                                   */
   /*********************************************************************/
                             FALSE, (unsigned int) ButtonReleaseMask,
#else
...
static void
UpdateDirection(m)      /* Update Layout Direction */
XmRowColumnWidget m;
{
    Widget  w;
    Arg     arglist[1];
    ...
    XtSetArg(arglist[0], DXmNlayoutDirection, LayoutM (m)  ) ;

    for ( i=0;
          (i < m->composite.num_children);
          i++ )
    {
        w = (Widget) m->composite.children[i];

        if ( !w->core.being_destroyed )
             XtSetValues(w, arglist, 1);        /* Propagate Direction */
    }
}
#endif
...
#ifdef DEC_MOTIF_BUG_FIX
/* make sure number of columns is not zero
*/
   if (!RC_NCol(req))
   {
        RC_NCol(new_w) = 1;
...
#ifdef DEC_MOTIF_RTOL
    need_expose |= (LayoutM( old ) != LayoutM( new_w )) ;
    if (LayoutM( old ) != LayoutM( new_w ) )
    {
       if (IsOption (new_w))  UpdateDirection(new_w) ;
       AdaptToSize (new_w, NULL, NULL ) ;
    }
#endif /* DEC_MOTIF_RTOL */
...
#ifdef DEC_MOTIF_RTOL
    /* Propagate direction */
    XtSetArg(args[n], DXmNlayoutDirection, LayoutM(option)); n++;
#endif /* DEC_MOTIF_RTOL */
...
#ifdef DEC_MOTIF_BUG_FIX    /* jv, 9-Jan-1991 */
        m->manager.shadow_thickness = XmConvertUnits ((Widget) m, XmHORIZONTAL,
                            XmPIXELS, 2, m->manager.unit_type);
#else
...
#endif
{
   ...
   for (pane=rc, depth=0;
        (RC_CascadeBtn(pane) && 
	 (!IsPopup(pane)) && XmIsMenuShell(XtParent(pane)));
	depth++)
   {
   ...
   _XmMenuPopDown((Widget)rc, event, popped_up);
}
...
/* 
 * This routine is lifted from VendorS.c.  It is called in the _XmMenuFocus
 * routine below to ensure that checking of the shell popped_up field is
 * only done for popup shells.  Without this fix, non-popup shells (like
 * application shells) would fail this check and keyboard focus would never
 * return to them after a pulldown or option menu was disarmed.  This
 * problem only shows up when running a 1.2 application against a 1.1.3
 * mwm - it doesn't happen with a 1.2 mwm because that version of the 
 * window manager has fallback code that does an XSetInputFocus itself
 * when the need arises.
 * 
 * Originally we fixed this with a hack to VendorS.c that set the popped_up
 * field to TRUE, but this fix had various unsavory side effects.  A
 * bug report has been filed with OSF, this fix should be replaced with
 * their fix when we receive it.
 */
	  ...
	  if (mst->RC_menuFocus.oldFocus)
	  {
		...
		if (XtIsRealized(mst->RC_menuFocus.oldWidget)) 
		{
		   ...
		   if (!XtIsShell(mst->RC_menuFocus.oldWidget)	||
			  ...
			  shell.popped_up)			||
		       ( XtIsRealized(mst->RC_menuFocus.oldWidget) &&
                         !IsPopupShell((Widget)mst->RC_menuFocus.oldWidget)))
#else
		   if (!XtIsShell(mst->RC_menuFocus.oldWidget) ||
		       (((ShellWidget)mst->RC_menuFocus.oldWidget)->
			  shell.popped_up))
	     ...
	     mst->RC_menuFocus.oldFocus = NULL;
	     mst->RC_menuFocus.oldRevert = NULL;
   ...
   for (i = startIndex; i < count; i++)
   {
       ...
       */
	   ...
	   if (_XmMatchKeyEvent((XEvent *) event, klist[i].eventType,
        ...
        /*
         *  We also want to ignore Mod2, Mod3, and Mod4 which are used
         *  by our servers for I18N keyboard shifting
         */
                (ShiftMask | LockMask | Mod2Mask | Mod3Mask | Mod4Mask)) :
#else
						     (ShiftMask | LockMask)) :
#endif
				klist[i].modifiers)) 
...
#ifdef DEC_MOTIF_BUG_FIX
      /* Fix for QAR 1572.  If the pulldown menu's reference to the cascade   */
      /* button isn't cleared, and this pulldown is then later reattached to  */
      /* the same cascade button, the cascade button routine AttachSubmenu    */
      /* will attempt to break any links the pulldown has to existing cascade */
      /* buttons.  Because the field was never cleared, it will break the     */
      /* link between itself and the pulldown, leading to an access violation */
      /* soon afterwards.                                                     */
      /*                                                  6/10/91    RJ       */

      if (RC_Type(XtParent(cascadeBtn)) == XmMENU_OPTION)
          if (RC_OptionSubMenu(XtParent(cascadeBtn)))
                if (RC_CascadeBtn(RC_OptionSubMenu(XtParent(cascadeBtn))) == cascadeBtn)
                    RC_CascadeBtn(RC_OptionSubMenu(XtParent(cascadeBtn))) = (Widget) NULL;
...
#endif /* _NO_PROTO */
{
   ...
   if (gadget = _XmInputInGadget( rc, relativeX, relativeY))
   {
...
#endif /* _NO_PROTO */
{
   ...
   int thing_type = XmSTRING;		/* init for gcc */
...
#endif /* _NO_PROTO */
{
   ...
   if (IsOption(menu))
   {
      ...
      if (cb = XmOptionButtonGadget( (Widget) menu))
      {
	 ...
	 if (h > subtrahend) 
         {
             ...
             m_top = (h-subtrahend) - BHeight (b); /* The difference is what */
                                                   /* it grows in height */
...
#endif /* _NO_PROTO */
{
   ...
   if (IsHorizontal(menu))
   {
	...
	BX(label_box) = RC_MarginW(menu) + BWidth(button_box) + 
			RC_Spacing(menu);
      else
#endif /* DEC_MOTIF_RTOL */
...
{   
    ...
    {                       /* cascade button, below cascade button */
        ...
        if (LayoutIsRtoLM(m))
        {
           ...
           if (! XtIsRealized (m))  XtRealizeWidget ((Widget) m);

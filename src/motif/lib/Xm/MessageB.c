...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MessageB.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:51:11 $"
#endif
#endif
/*
...
/****************/
    ...
    if (LayoutIsRtoLM(new_w))
       BB_StringDirection(new_w) = XmSTRING_DIRECTION_R_TO_L;
    ...
    */
    ...
    if (LayoutM(new_w) != LayoutM(current))
    {
        if (LayoutIsRtoLM(new_w))
           ...
           BB_StringDirection( new_w) = XmSTRING_DIRECTION_L_TO_R;
...
#ifdef DEC_MOTIF_RTOL
    XtSetArg (al[ac], DXmNlayoutDirection, LayoutM(w)); ac++;
#endif /* DEC_MOTIF_RTOL */

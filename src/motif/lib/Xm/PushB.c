...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: PushB.c,v $ $Revision: 1.1.8.3 $ $Date: 1993/07/15 16:57:04 $"
#endif
#endif
/*
    ...
    XmPushButtonWidget pb;
{
    ...
    if ((pb->core.sensitive) && (pb->core.ancestor_sensitive))
    {
        /* Fill in event time */
        ...
        /* Disarm pushbutton */
#ifndef ALPHA_BUG_FIX
        Disarm((Widget) pb, NULL, NULL, NULL);
#else
        Disarm((Widget)pb, &event, NULL, &zero);
#endif /* ALPHA_BUG_FIX */
    }
}
         ...
         (pb->pushbutton.arm_color == pb->primitive.foreground))
    {
     ...
     if( pb->pushbutton.default_button_shadow_thickness > 0 )
        { saved_shadow = True;
          ...
          pb -> primitive.shadow_thickness =
                     (pb -> primitive.shadow_thickness << 1);
        }

      (* xmLabelClassRec.core_class.expose) ((Widget) pb, event, region) ;

      if (saved_shadow)
        pb -> primitive.shadow_thickness = shadow_thickness;
...
#ifdef ALPHA_BUG_FIX
		    (Window *)&junkWin,    /* returned root window */

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MenuUtil.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:50:29 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
   ...
   if (!_XmIsEventUnique(event) ||
       (!RC_IsArmed(rc) && !(RC_Type(rc) == XmMENU_OPTION))) /* fix CR #5029 */
     return;

   if (!_XmGetInDragMode ((Widget)rc))
   {
     if (gadget = (XmGadget) rc->manager.active_child)
   ...
   {
     if (gadget = (XmGadget) _XmInputInGadget( (Widget) rc, event->xkey.x,
						 event->xkey.y))
        _XmDispatchGadgetInput( (Widget) gadget, event, XmHELP_EVENT);
     else
     {
...
#endif /* _NO_PROTO */
{
   ...
   else
   {
       switch (direction)
       {
        ...
        if ( rc->manager.dxm_layout_direction == DXmLAYOUT_LEFT_DOWN )

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: CascadeB.c,v $ $Revision: 1.1.8.3 $ $Date: 1993/07/15 16:07:36 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
   ...
   if (*_XmExcludedParentPane.pane)
   {
      ...
      {
	 _XmLowerTearOffObscuringPoppingDownPanes(XtParent(cb),
	    *_XmExcludedParentPane.pane);
...
#ifdef DEC_MOTIF_RTOL
       if (LayoutP(new_w) != LayoutP(old))
       {
          if ((Lab_MarginLeft(old)  == Lab_MarginLeft(new_w))
                                    &&
              (Lab_MarginRight(old) == Lab_MarginRight(new_w)))
          {
             /* Swap Margins */
             Lab_MarginRight(new_w) = Lab_MarginLeft(old);
             Lab_MarginLeft(new_w) = Lab_MarginRight(old);
             flag = TRUE;
          }
       }
#endif /* DEC_MOTIF_RTOL */

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ScrollBar.c,v $ $Revision: 1.1.8.3 $ $Date: 1993/07/15 17:09:50 $"
#endif
#endif
/*
...
#define MESSAGE14 "Incorrect processing direction in right to left layout."
...
#endif /* _NO_PROTO */
{
      static Boolean traversal = False ;
      Widget parent = XtParent(widget) ;
      Arg al[1] ;
      unsigned char sp ;
...
#endif /* _NO_PROTO */
{
	...
	if (new_w->scrollBar.orientation == XmHORIZONTAL)
	{
           ...
           if (LayoutIsRtoLP(new_w))
           {
               ...
               {
                  /* force processing direction max on left for RtoL layout */
                  new_w->scrollBar.processing_direction = XmMAX_ON_LEFT;
                  _XmWarning ((Widget) new_w, MESSAGE14);
               }
           }
           else
#endif /* DEC_MOTIF_RTOL */
...
#endif /* _NO_PROTO */
{
	...
	((new_w->scrollBar.processing_direction == XmMAX_ON_LEFT)
	 || (new_w->scrollBar.processing_direction == XmMAX_ON_TOP));
        ...
        if (new_w->scrollBar.orientation == XmHORIZONTAL)
        {
           if (new_w->scrollBar.processing_direction !=
                                current->scrollBar.processing_direction)
           {
              if (LayoutIsRtoLP(new_w) &&
                        new_w->scrollBar.processing_direction != XmMAX_ON_LEFT)
              {
                  /* Force processing direction max on left if layout is RtoL */
                  new_w->scrollBar.processing_direction = XmMAX_ON_LEFT;
                  _XmWarning((Widget) new_w, MESSAGE14);
              }
           }
           if (LayoutP(new_w) != LayoutP(current))
           {
              ...
              /*
               * The current (almost EFT version) set values processing
               * direction algorithm is buggy, until fixed
               * the following statements are making it to work
               * correctly in a RtoL layout environment.
               */
              new_backwards =
                ((new_w->scrollBar.processing_direction == XmMAX_ON_LEFT)
                 || !(new_w->scrollBar.flags & VALUE_SET_FLAG));
...
#endif /* _NO_PROTO */
{
	...
	if (new_value != sbw->scrollBar.value)
	{
        ...
        else    /* value didn't change - callback anyway */
        {
...
#endif /* _NO_PROTO */
{
	...
	if (sbw->scrollBar.flags & KEYBOARD_GRABBED)
	{
		...
		ScrollCallback (sbw, XmCR_VALUE_CHANGED,
		                sbw->scrollBar.value, savedX, savedY, NULL);
...
#endif /* _NO_PROTO */
{
	...
	if (returnFlag = (sbw->scrollBar.value != old_value)) {
		RedrawSliderWindow (sbw);
	    }

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ArrowBG.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 15:49:08 $"
#endif
#endif
/*
...
#define DRAWARROW(t_gc, b_gc, c_gc)    \
    _XmDrawArrow(XtDisplay ((Widget) aw),\
              ...
              aw->rectangle.y +\
                 ...
                 aw -> gadget.shadow_thickness,\
              aw -> rectangle.width\
                 - 2 * aw -> gadget.shadow_thickness\
                 - 2 * aw -> gadget.highlight_thickness,\
              aw -> rectangle.height\
                 - 2 * aw -> gadget.shadow_thickness\
                 - 2 * aw -> gadget.highlight_thickness,\
              2,\
...
#endif /* _NO_PROTO */
{
   ...
   if ((iwidth > 0) && (iheight > 0))
   {
     ...
     DRAWARROW(((aw->arrowbutton.selected && aw->rectangle.sensitive)?
             XmParentBottomShadowGC (aw):
             XmParentTopShadowGC (aw)),
            ((aw->arrowbutton.selected && aw->rectangle.sensitive)?
             XmParentTopShadowGC (aw):
             XmParentBottomShadowGC (aw)),
...
#endif /* _NO_PROTO */
{
   ...
   DRAWARROW(XmParentBottomShadowGC (aw),
	   XmParentTopShadowGC (aw),
	   NULL);
...
#endif /* _NO_PROTO */
{
   ...
   DRAWARROW(XmParentTopShadowGC (aw),
	   XmParentBottomShadowGC (aw),
	   NULL);

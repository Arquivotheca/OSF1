...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: SelectioB.c,v $ $Revision: 1.1.6.4 $ $Date: 1994/01/19 19:25:33 $"
#endif
#endif
/*
...
/****************/
    ...
    if(    !XmRepTypeValidValue( XmRID_CHILD_PLACEMENT,
        ...
        if (LayoutIsRtoLM(new_w))
                 SB_StringDirection(new_w) = XmSTRING_DIRECTION_R_TO_L;
#endif /* DEC_MOTIF_RTOL */
...
/* I18N START */
    if (sel->selection_box.dialog_type == XmDIALOG_FILE_SELECTION)
        SB_ListLabel( sel) = _XmBB_CreateLabelG((Widget) sel,
                               sel->selection_box.list_label_string, "Files") ;
    else
#endif
    SB_ListLabel( sel) = _XmBB_CreateLabelG( (Widget) sel,
...
/*  This could also be DEC_MOTIF_BUG_FIX, since it is hard-wiring in a
    label string in English.
*/
            XmString    prompt;

    if (   (sel->selection_box.dialog_type == XmDIALOG_COMMAND)
        && (sel->selection_box.selection_label_string == (XmString)NULL) )
    {
        if (LayoutIsRtoLM(sel)) {
            prompt = XmStringLtoRCreate("<", XmSTRING_DEFAULT_CHARSET);
        } else {
            prompt = XmStringLtoRCreate(">", XmSTRING_DEFAULT_CHARSET);
        }
...
#ifdef DEC_MOTIF_RTOL
    XtSetArg( al[ac], DXmNlayoutDirection, LayoutM(sel)); ++ac ;
#endif /* DEC_MOTIF_RTOL */
...
#else
#ifdef I18N_EXTENSION
        SB_Text (sel) = DXmCreateCSText ((Widget)sel, "sb_text", al, ac);
#else
...
#ifdef I18N_EXTENSION
            DXmCSTextSetString
                        ( SB_Text (sel), sel->selection_box.text_string );
#else
...
/* I18N START */
    if (sel->selection_box.dialog_type == XmDIALOG_FILE_SELECTION)
        SB_ApplyButton( sel) = _XmBB_CreateButtonG((Widget) sel,
                              sel->selection_box.apply_label_string, "Filter") ;
    else
#endif
    SB_ApplyButton( sel) = _XmBB_CreateButtonG( (Widget) sel, 
...
#else
#ifdef I18N_EXTENSION
        temp.value = DXmCSTextGetString (SB_Text (sel));
#else
...
#ifdef I18N_EXTENSION
        DXmCSTextSetString ( SB_Text (sel), callback->item );
#else
	if(    text_value = _XmStringGetTextConcat( callback->item)    )
	{   
#ifndef USE_TEXT_IN_DIALOGS
            XmTextFieldSetString (SB_Text (sel), text_value);
            XmTextFieldSetCursorPosition (SB_Text (sel),
...
/****************/
        ...
        */
        ...
        if( LayoutM(new_w) != LayoutM(current))
        {
            ...
            if (LayoutIsRtoLM(new_w))
                ...
                SB_StringDirection( new_w) = XmSTRING_DIRECTION_L_TO_R;
        }
#endif /* DEC_MOTIF_RTOL */
...
#ifdef DEC_MOTIF_RTOL
        if( LayoutM(new_w) != LayoutM(current))
            {XtSetArg(al[ac], DXmNlayoutDirection, LayoutM(new_w)); ac++;}
#endif /* DEC_MOTIF_RTOL */
	...
	*/
	...
	{   
#ifdef I18N_EXTENSION
            XtSetArg( al[ac], XmNvalue, new_w->selection_box.text_string ) ;  ac++ ;
#else
            text_value = _XmStringGetTextConcat(
...
/****************/
    ...
    {   
#ifdef I18N_EXTENSION
        XtSetArg( al[0], XmNvalue, &text_string) ;
        XtGetValues( SB_Text( sel), al, 1) ;
#else
...
/****************/
    ...
    {   
        ...
        {   
#ifdef I18N_EXTENSION
            DXmCSTextSetString ( SB_Text (sel) , *items );
#else

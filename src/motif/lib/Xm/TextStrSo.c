...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextStrSo.c,v $ $Revision: 1.1.4.4 $ $Date: 1993/12/03 21:04:18 $"
#endif
#endif
/*
...
static XmTextStatus Replace( 
                        ...
                        XmTextBlock block,
#if NeedWidePrototypes
                        int call_callbacks_flag) ;
#else
                        Boolean call_callbacks_flag) ;
#endif /* NeedWidePrototypes */
static void ScanParagraph( 
                        XmSourceData data,
...
#endif /* _NO_PROTO */
{
   ...
   if (!want_wchar) {
     ...
     /* NOTE: to - from could result in a truncated long. */
	  ...
	  destpos += mbstowcs(&wc_buf[destpos], block.ptr,
...
#endif /* _NO_PROTO */
{
    ...
    if (data->numwidgets == 1)
      ...
      /*
      ** Apply Motif 1.2.3 bug fix.  This fixes several problems in Mail.  CS  23-Nov-93.
      */
...
/* Do something to move the allocated space into the buffer */
   ...
   /* NOTE: the value of delta could be truncated by cast to int. */
    ...
    if (data->hasselection && *start < data->right && *end > data->left) {
	  ...
	  if (*end > data->right) {
	     data->right = *end; /* delete encompasses the right half of the
   ...
   /* convert data from char* to characters and copy into the gapped buffer */
    ...
    if (call_callbacks) {
       ...
       if (initiator->text.value_changed_callback)
          XtCallCallbackList((Widget)initiator,
			  initiator->text.value_changed_callback, (Opaque) &cb);
    }
...
/* END 3145 */
          ...
          if (found) break;
       } else if (mb_char && !isspace(*mb_char)) {
          *last_char = (position) + ddir;
       }
...
#endif /* _NO_PROTO */
{
	...
	case XmSELECT_WORD:
		   ...
		   while (position >= 0 && position <= data->length) {
		       ...
		       if (!cur_is_mb && mb_char && isspace(*mb_char)){
		           if (whiteSpace < 0) whiteSpace = position;
...
#endif /* _NO_PROTO */
{
    ...
    if (is_wchar){
       ...
       (void)wcstombs(tmp_value, wc_value, (num_chars + 1) * char_size);
...
#endif /* _NO_PROTO */
{
    ...
    if (!want_wchar) {
       ...
       while (pos < last_pos) {
        ...
        /* NOTE: ret_pos - pos could result in a truncated long. */
          length += mbstowcs(&wc_temp[length], block.ptr, (int)(ret_pos - pos));
          pos = ret_pos;
       }
...
#endif /* _NO_PROTO */
{
    ...
    if (_XmTextModifyVerify(widget, NULL, &fromPos, &toPos,
       ...
       (void)(source->Replace)(widget, NULL, &fromPos, &toPos, &newblock, True);

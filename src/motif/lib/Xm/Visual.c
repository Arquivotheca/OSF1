...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Visual.c,v $ $Revision: 1.1.6.4 $ $Date: 1993/10/18 15:39:45 $"
#endif
#endif
/*
...
{
   { XtWidgetBaseOffset, (XtPointer) XtOffsetOf( struct _WidgetRec, core.parent),
sizeof (Widget ) }
};
...
static void 
#ifdef _NO_PROTO
_XmCvtStringToBackgroundPixmap( args, numArgs, fromVal, toVal )
        XrmValue *args ;
        Cardinal *numArgs ;
        XrmValue *fromVal ;
        XrmValue *toVal ;
...
#endif /* _NO_PROTO */
{
   ...
   if (!XtIsShell(widget) &&
        !_XmStringsAreEqual((char *) (fromVal->addr), "unspecified_pixmap"))
#else
   if (!XtIsShell(widget))
#endif
...
#endif /* _NO_PROTO */
{
  ...
  */
#ifndef WIN32
  bzero((char*) &widget, sizeof(widget) );
#else
  ...
  if (XmIsGadget(w))
  {
    if (XmIsPushButtonGadget(w) || XmIsToggleButtonGadget(w))
    {
      ...
      XmGetColors(parent->core.screen, parent->core.colormap, 
                  parent->core.background_pixel, NULL, NULL, NULL, &select_ret);

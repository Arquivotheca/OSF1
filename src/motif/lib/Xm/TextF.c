...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextF.c,v $ $Revision: 1.1.8.4 $ $Date: 1993/10/18 15:37:10 $"
#endif
#endif
/*
...
/* USE ITERATIONS OF mbtowc TO COUNT THE NUMBER OF CHARACTERS REPRESENTED
...
#endif /* _NO_PROTO */
{
   ...
   for (bptr = ptr; n_bytes > 0; count++, bptr+= char_size){
      char_size = mbtowc((wchar_t*)NULL, bptr, tf->text.max_char_size);
      n_bytes -= char_size;
   }
   return count;
}
...
#endif /* _NO_PROTO */
{
    ...
    XChangeGC(XtDisplay(widget), tf->text.image_gc, GCStipple, &values);
}
...
#endif /* _NO_PROTO */
{
    ...
    *y = 0;
    ...
    if (TextF_UseFontSet(tf)){
       if (tf->text.max_char_size != 1)
          *x += XwcTextEscapement((XFontSet)TextF_Font(tf),
			          TextF_WcValue(tf), (int)position);
       else
          *x += XmbTextEscapement((XFontSet)TextF_Font(tf),
			          TextF_Value(tf), (int)position);
    } else
       *x += XTextWidth((XFontStruct *)TextF_Font(tf), TextF_Value(tf),
			(int)position);
...
#endif /* _NO_PROTO */
{
   if (TextF_UseFontSet(tf)){
      ...
      if (tf->text.max_char_size != 1)
         XDrawString (XtDisplay(tf), XtWindow(tf), gc, x, y, string, 
                           _XmTextFieldCountCharacters(tf, string, length));
      else /* one byte chars */
...
#endif /* _NO_PROTO */
{
   if (TextF_UseFontSet(tf)) {
      ...
      if (tf->text.max_char_size != 1)
         return (XTextWidth(TextF_Font(tf), string, 
                 _XmTextFieldCountBytes(tf, (wchar_t*)string, length)));
      else /* one byte chars */
...
#endif /* _NO_PROTO */
{
  ...
  if (tf->core.width - (2 * margin_width) <= 0)
    return;
  if (tf->core.height - (margin_top + margin_bottom) <= 0)
    return;
...
#endif /* _NO_PROTO */
{
	...
	TextF_Columns(tf) < tf->text.string_length){
       if (TextF_UseFontSet(tf)) {
          if (tf->text.max_char_size != 1) 
	      tmp = XwcTextEscapement((XFontSet)TextF_Font(tf), 
				      TextF_WcValue(tf), 
				      tf->text.string_length);
          else 
	      tmp = XmbTextEscapement((XFontSet)TextF_Font(tf), 
				      TextF_Value(tf),
				      tf->text.string_length);
       } else {
          tmp = XTextWidth(TextF_Font(tf), TextF_Value(tf), 
			   tf->text.string_length);
       }
...
#endif /* _NO_PROTO */
{
  ...
  if (TextF_UseFontSet(tf)){
     if (tf->text.max_char_size != 1) {
        left_edge = XwcTextEscapement((XFontSet)TextF_Font(tf), 
				      TextF_WcValue(tf),
                                      (int)position) + (int) tf->text.h_offset;
     } else {
        left_edge = XmbTextEscapement((XFontSet)TextF_Font(tf),
				      TextF_Value(tf),
                                      (int)position) + (int) tf->text.h_offset;
     }
  } else {
...
ModifyVerify( tf, event, replace_prev, replace_next,
	      insert, insert_length, newInsert )
        XmTextFieldWidget tf ;
        XEvent *event ;
        XmTextPosition *replace_prev ;
        XmTextPosition *replace_next ;
...
#endif /* _NO_PROTO */
{
	...
	return(True);
	 ...
	 if (TextF_ModifyVerifyCallback(tf)){
            newblock.ptr = (char *) XtMalloc((unsigned)*insert_length+1);
            (void)memcpy((void*)newblock.ptr, (void*)insert, *insert_length);
	    newblock.ptr[*insert_length]='\0';
            do_free = True;
	 }
     } else 
	newblock.ptr = NULL;
	...
	if (do_free){ /* there is a char* modify verify callback; the data we
	   ...
	   block_num_chars = _XmTextFieldCountCharacters(tf, vcb.text->ptr, 
						         vcb.text->length);
	   ...
	   block_num_chars = _XmTextFieldCountCharacters(tf, insert, 
						         *insert_length);
	...
	if (wcs_vcb.doit) {
	   wcs_vcb.text->wcsptr[wcs_vcb.text->length] = 0;
           *insert_length = _XmTextFieldCountBytes(tf, wcs_vcb.text->wcsptr, 
						   wcs_vcb.text->length);
	   *replace_prev = wcs_vcb.startPos;
	   *replace_next = wcs_vcb.endPos;
           *newInsert = wcs_vcb.newInsert;
	   if (*insert_length > 0) {
	      if (old_length < *insert_length)
	         insert = XtRealloc(insert, (unsigned) *insert_length + 1);
	      (void) wcstombs(insert, wcs_vcb.text->wcsptr, *insert_length + 1);
	   }        
	}
     } else { /* use vcb data */
	if (vcb.doit) {
...
void 
#ifdef _NO_PROTO
_XmTextFieldSetCursorPosition( tf, event, position, adjust_flag, call_cb )
        XmTextFieldWidget tf ;
        XEvent *event ;
        XmTextPosition position ;
        Boolean adjust_flag ;
...
#endif /* _NO_PROTO */
{
  ...
  */
  if ((tf->text.string_length + delta) - (TextF_MaxLength(tf)) > 0) { 
 ...
 /* make sure the selection are redisplay, since they were turned off earlier */
  ...
  cb.event = event;
  /* NEEDS WORK TO HANDLE DIFFERENT CALLBACK STRUCTS */
...
#endif /* _NO_PROTO */
{
    ...
    */
    ...
    if (tf->text.string_length > 0) {
       ...
       if (TextF_UseFontSet(tf)){
          if (tf->text.max_char_size != 1)
             next_char_width = XwcTextEscapement((XFontSet)TextF_Font(tf),
					         TextF_WcValue(tf), 1);
          else
             next_char_width = XmbTextEscapement((XFontSet)TextF_Font(tf),
					         TextF_Value(tf), 1);
       } else   
	 ...
	 */
	  ...
	  if (TextF_UseFontSet(tf)){
             if (tf->text.max_char_size != 1)
                next_char_width = XwcTextEscapement((XFontSet)TextF_Font(tf),
				          TextF_WcValue(tf) + position + 1, 1);
             else
                next_char_width = XmbTextEscapement((XFontSet)TextF_Font(tf),
					  TextF_Value(tf) + position + 1, 1);
          } else
             next_char_width = XTextWidth(TextF_Font(tf), 
					  TextF_Value(tf) + position + 1, 1);
       } 
    } /* for */
...
/* if positions aren't adjacent, return False */
   ...
   if (tf->text.max_char_size == 1) { /* data is char* and one-byte per char */
      if (isspace((int)TextF_Value(tf)[pos1]) || 
	  isspace((int)TextF_Value(tf)[pos2])) return True;
   } else {
      size_pos1 = wctomb(s1, TextF_WcValue(tf)[pos1]);
      size_pos2 = wctomb(s2, TextF_WcValue(tf)[pos2]);
      if (size_pos1 == 1 && (size_pos2 != 1 || isspace(*s1))) return True;
      if (size_pos2 == 1 && (size_pos1 != 1 || isspace(*s2))) return True;
   }
   return False;
}
 ...
 /* If there is more data than we can handle, bail out */
  ...
  if (insert_length > 0) {
   /* do not insert non-printing characters */
    if (TextF_UseFontSet(tf)){
       if (!XmbTextEscapement((XFontSet)TextF_Font(tf), insert_string, 
			      insert_length)) 
          return;
    } else {
      if (!XTextWidth(TextF_Font(tf), insert_string, insert_length)) 
	 return;
    }
...
#endif /* _NO_PROTO */
{
  ...
  _XmTextFieldDrawInsertionPoint(tf, False);
  if (NeedsPendingDeleteDisjoint(tf)) (void) XmTextFieldRemove(w);
  else { 
     if (tf->text.has_primary &&
         tf->text.prim_pos_left != tf->text.prim_pos_right) {
...
#endif /* _NO_PROTO */
{
    ...
    if (cursorPos < tf->text.string_length) {
       if (tf->text.max_char_size == 1) {
         if (isspace((int)TextF_Value(tf)[cursorPos]))
...
#endif /* _NO_PROTO */
{
  ...
  if (reset_cursor = !old_has_focus && tf->text.has_focus)
     _XmTextFieldDrawInsertionPoint(tf, False);

  _XmTextFieldSetCursorPosition(tf, event, new_position, True, True);
...
#endif /* _NO_PROTO */
{
   ...
   if (event->xselection.property == None) {
      ...
      if (tf->text.selection_move) {
         if (_XmTextFieldReplaceText(tf, NULL, left, right, NULL, 0, False)) {
...
#endif /* _NO_PROTO */
{
    ...
    if (num_spaces) {
	  ...
	  (void)_XmTextFieldReplaceText(tf, (XEvent *)event, left, right,
	  ...
	  (void)_XmTextFieldReplaceText(tf, (XEvent *)event, left, right,
					(char*)wc_spaces, num_spaces, False);
          if (TextF_CursorPosition(tf) > left)
	     ResetClipOrigin(tf, False);     
...
#endif /* _NO_PROTO */
{
    ...
    if (TextF_UseFontSet(tf)){
       if (tf->text.max_char_size != 1)
          length = XwcTextEscapement((XFontSet)TextF_Font(tf), 
			            TextF_WcValue(tf), tf->text.string_length);
       else
          length = XmbTextEscapement((XFontSet)TextF_Font(tf), TextF_Value(tf),
				     tf->text.string_length);
    } else 
       length = XTextWidth(TextF_Font(tf), TextF_Value(tf),
			   tf->text.string_length);
    ...
    RedisplayText(tf, 0, tf->text.string_length);
    _XmTextFieldSetCursorPosition(tf, event, GetPosFromX(tf, x), True, True);
...
#endif /* _NO_PROTO */
{
   ...
   if (!is_wchar) {
      ...
      temp_str = XtMalloc((unsigned)(str_len + 1));
      start_temp = temp_str;
      curr_str = value;
      if ((tf->text.max_char_size == 1) && TextF_UseFontSet(tf)){
         for (i = 0; i < str_len; i++, curr_str++) {
            if (XmbTextEscapement((XFontSet)TextF_Font(tf), curr_str, 1)) {
               *temp_str = *curr_str;
                temp_str++;
            } else {
               char warn_str[52];
      ...
      } else if ((tf->text.max_char_size != 1) && TextF_UseFontSet(tf)){
         for (i = 0, csize = mblen(curr_str, tf->text.max_char_size); 
	      i < str_len; 
	      i += csize, csize = mblen(curr_str, tf->text.max_char_size)) {
	    if (csize < 0) csize = 1;
            if (XmbTextEscapement((XFontSet)TextF_Font(tf), curr_str, csize)){
	       for (j = 0; j < csize; j++, temp_str++, curr_str++)
                  *temp_str = *curr_str;
            } else {
               char warn_str[52];
               sprintf(warn_str, MSG5, *curr_str);
      ...
      } else { /* no font sets, so use font struct routines to validate */
         for (i = 0; i < str_len; i++, curr_str++) {
	 ...
	 if (tf->text.max_char_size != 1) {
	    ...
	    str_len = 0;
         ...
         for (i = 0, new_len = 0; i < str_len; i++, wcs_curr_str++) {
            if (XwcTextEscapement((XFontSet)TextF_Font(tf), wcs_curr_str, 1)) {
               *wcs_temp_str = *wcs_curr_str;
                wcs_temp_str++;
                new_len++;
            } else {
      ...
      *wcs_temp_str = (wchar_t)0L; /* terminate with a wchar_t NULL */
      ...
      if (tf->text.max_char_size == 1) { /* Need to store data as char* */
         ...
         (void)wcstombs(TextF_Value(tf), wcs_start_temp, tf->text.size_allocd);
         TextF_WcValue(tf) = NULL;
...
#endif /* _NO_PROTO */
{
    ...
    */
    ...
    if (!LoadFontMetrics(tf)) tf->text.max_char_size = 1;
...
#endif /* _NO_PROTO */
{
  ...
  /* Create area in which to save text located underneath I beam */
   MakeIBeamOffArea(tf, tf->text.cursor_width, tf->text.cursor_height);
  ...
  /* Create a new add_mode cursor */
   ...
   XmSetNormGC(tf, tf->text.image_gc, False, False);
}
   ...
   /* When type = NULL, we are assuming a DELETE request has been requested */
    if (*type == XmInternAtom(XtDisplay(transfer_rec->widget), "NULL", False)) {
       if (transfer_rec->num_chars > 0 && transfer_rec->move) {
          ...
          _XmTextFieldSetCursorPosition(tf, NULL, cursorPos, False, True);
...
#endif /* _NO_PROTO */
{
    ...
    if ( new_tf->text.font_list == (XmFontList) NULL ){
        new_tf->text.font_list =
                (XmFontList) XmFontListCreateDefault (new_tf, NULL);
#ifdef DEBUG
        printf("I18n default font is set in XmTextField widget\n");
    } else {
        printf("Use default font in resource for XmTextField widget\n");
#endif
    }
...
#endif /* _NO_PROTO */
{
    ...
    * the i-beam. */
    ...
    if (TextF_CursorPosition(new_tf) != TextF_CursorPosition(old_tf)) {
       ...
       _XmTextFieldSetCursorPosition(new_tf, NULL, new_position, True, True);
    ...
    if (diff_values) { /* old value != new value */
	  ...
	  if (new_tf->text.max_char_size == 1) {
	     mod_ver_ret = ModifyVerify(new_tf, NULL, &fromPos, &toPos,
					TextF_Value(new_tf), 
					&new_tf->text.string_length,
					&newInsert);
	  } else {
	     temp = XtMalloc((unsigned)((new_tf->text.string_length + 1) *
			     new_tf->text.max_char_size));
	     (void)wcstombs(temp, TextF_WcValue(new_tf), 
...
#endif /* _NO_PROTO */
{
   ...
   if (tf->text.string_length > 0) {
	 ...
	 (void) wcstombs(temp_str, TextF_WcValue(tf), 
...
#endif /* _NO_PROTO */
{
   ...
   if (num_chars > 0) {
	 ...
	 (void) wcstombs(buffer, &TextF_WcValue(tf)[start], 
			 n_bytes);
      }
      buffer[n_bytes] = '\0';
   } else
...
#endif /* _NO_PROTO */
{
   ...
   if (tf->text.string_length > 0) {
      ...
      if (tf->text.max_char_size != 1) {
         ...
         (void) mbstowcs(temp_wcs, TextF_Value(tf),
                         tf->text.string_length + 1);
      }
      return temp_wcs;
   } else {
...
#endif /* _NO_PROTO */
{
   ...
   if (num_chars > 0) {
      if (tf->text.max_char_size == 1) {
	 (void) mbstowcs(buffer, &TextF_Value(tf)[start], num_chars);
      } else {
	 (void)memcpy((void*)buffer, (void*)&TextF_WcValue(tf)[start], 
	       (size_t) num_chars * tf->text.max_char_size);
      }
      buffer[num_chars] = '\0';
   } else if (num_chars == 0) {
      buffer[num_chars] = '\0';
...
#endif /* _NO_PROTO */
{
    ...
    XmTextFieldSetCursorPosition(w, 0);
...
#endif /* _NO_PROTO */
{
    ...
    TextF_MaxLength(tf) = MAXINT;
    if (tf->text.max_char_size == 1) {
       (void) _XmTextFieldReplaceText(tf, NULL, from_pos, 
				      to_pos, value, strlen(value), True);
    } else { /* need to convert to wchar_t* before calling Replace */
       wc_value = (wchar_t *) XtMalloc((unsigned) sizeof(wchar_t) *
				       (1 + strlen(value)));
       wc_length = mbstowcs(wc_value, value, (unsigned) (strlen(value) + 1));
       (void) _XmTextFieldReplaceText(tf, NULL, from_pos, to_pos, 
				      (char*)wc_value, wc_length, True);
       XtFree((char *)wc_value);
    }
...
#endif /* _NO_PROTO */
{
   ...
   _XmTextFieldSetCursorPosition(tf, NULL, position, True, False);
}
...
#endif /* _NO_PROTO */
{
   ...
   _XmTextFieldSetCursorPosition(tf, NULL, position, True, True);
}
...
#endif /* _NO_PROTO */
{
   ...
   if (tf->text.max_char_size == 1) {
      (void) mbstowcs(wc_value, TextF_Value(tf) + tf->text.prim_pos_left, 
                      length);
   } else {
...
#endif /* _NO_PROTO */
{
   ...
   if (_XmTextFieldReplaceText(tf, NULL, left, right, NULL, 0, True))
      XmTextFieldSetSelection(w, TextF_CursorPosition(tf),
                              TextF_CursorPosition(tf),
			      XtLastTimestampProcessed(XtDisplay(w)));

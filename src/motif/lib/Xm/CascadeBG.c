...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: CascadeBG.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:08:31 $"
#endif
#endif
/*
...
externaldef(xmcascadebgcacheobjclassrec) XmCascadeButtonGCacheObjClassRec xmCascadeButtonGCacheObjClassRec =
#else
...
#endif /* _NO_PROTO */
{
   if ((CBG_HasCascade(cb)) && (CBG_Cascade_width(cb) != 0))
   {
	 ...
	 offset_y =  (CBG_Cascade_height(cb)- height)/2;
          ...
          if (LayoutIsRtoLG(cb))
            {
              offset_x = (CBG_Cascade_width(cb) - width);
...
#endif /* _NO_PROTO */
{
...

   {
	  ...
	  * The coersion is done to keep binary compatability, although
	  * the right answer is to eventually change the timer field to
	  * XtIntervalId.
	  */
#ifdef DEC_MOTIF_BUG_FIX
	/*
...
#ifdef DEC_MOTIF_RTOL
       if (LayoutG(new_w) != LayoutG(old))
       {
          if ((LabG_MarginLeft(old)  == LabG_MarginLeft(new_w))
                                    &&
              (LabG_MarginRight(old) == LabG_MarginRight(new_w)))
          {
             /* Swap Margins */
             LabG_MarginRight(new_w) = LabG_MarginLeft(old);
             LabG_MarginLeft(new_w) = LabG_MarginRight(old);
             flag = TRUE;
          }
       }
#endif /* DEC_MOTIF_RTOL */
...
#ifdef DEC_MOTIF_RTOL
               /* If the armed pixmap exists, both pixmaps must be arrows */
               ((CBG_ArmedPixmap(old) != XmUNSPECIFIED_PIXMAP) &&
               (LayoutG(new_w) != LayoutG(old))) ||
#endif /* DEC_MOTIF_RTOL */
	        ((CBG_ArmedPixmap(old) != XmUNSPECIFIED_PIXMAP) &&

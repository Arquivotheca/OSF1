...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: PushBG.c,v $ $Revision: 1.1.8.4 $ $Date: 1993/07/15 16:58:11 $"
#endif
#endif
/*
...
externaldef(xmpushbuttongcacheobjclassrec) XmPushButtonGCacheObjClassRec xmPushButtonGCacheObjClassRec =
#else
...
#ifdef DEC_MOTIF_BUG_FIX
	 /*
	 **	This must be an XtIntervalId which is an unsigned long.  Type casting
	 **	this to an int does not work on our 64 bit platform.
	 **
	 **	This also requires the timer field in PushBGP.h to change to XtIntervalId.
	 **	CS, 3-Jun-1993
	 */
...
/*------------------------------------------------------*/
/* called via DXmActivateWidget, this routine simulates */
/* widget activation (Btn1Down & Btn1Up)                */
/*------------------------------------------------------*/

void DXmActivatePBGadget(pb)
    XmPushButtonGadget pb;
{
    XEvent event;
#ifdef ALPHA_BUG_FIX
    Cardinal zero = 0;
#endif
    if ((pb->rectangle.sensitive) && (pb->rectangle.ancestor_sensitive))
    {
        /* Fill in event time */
        event.xbutton.time = XtLastTimestampProcessed(XtDisplay(pb));
        /* Call ArmAndActivate (simulate a Return key) */
#ifndef ALPHA_BUG_FIX
        ArmAndActivate((Widget) pb, &event, NULL, NULL);
#else
        ArmAndActivate((Widget)pb, &event, NULL, &zero);
#endif /* ALPHA_BUG_FIX */
        /* Disarm pushbutton */
        Disarm(pb, NULL);
    }

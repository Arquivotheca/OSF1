...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Scale.c,v $ $Revision: 1.1.6.4 $ $Date: 1993/10/18 15:35:34 $"
#endif
#endif
/*
...
#define MESSAGE10 "Incorrect processing direction for right to left layout"
...
#ifdef DEC_MOTIF_RTOL
        XtSetArg (args[n], DXmNlayoutDirection, DXmLAYOUT_RIGHT_DOWN); n++;
#endif /* DEC_MOTIF_RTOL */
    ...
    ValidateInitialState(req, new_w);
        ...
        if (new_w->scale.orientation == XmHORIZONTAL)
        {
           if (LayoutIsRtoLM(new_w))
           {
              ...
              {
                 /* Force processing direction to be Max on Left */
                new_w->scale.processing_direction = XmMAX_ON_LEFT;
...
#ifdef DEC_MOTIF_RTOL
        if (LayoutIsRtoLM(new_w) && new_w->scale.orientation == XmHORIZONTAL)
        {
           if (new_w->scale.processing_direction !=
                                        cur->scale.processing_direction)
           {
              if (new_w->scale.processing_direction != XmMAX_ON_LEFT)
              {
                 _XmWarning((Widget) new_w, MESSAGE10);
                 /* Force scale processing direction to be Max on Left */
                 new_w->scale.processing_direction = XmMAX_ON_LEFT;
              }
           }
        }

        /* Check if Layout Direction changed */

        if (LayoutM(new_w) != LayoutM(cur))
         {
           if (new_w->scale.orientation == XmHORIZONTAL)
           {
              if (new_w->scale.processing_direction ==
...
#endif /* _NO_PROTO */
{
	...
	else
	{
        ...
        if (LayoutIsRtoLM(sw))
        {
           ...
           _XmMoveObject(sw->composite.children[0], titleBox.x, titleBox.y);
        }
        else
#endif /* DEC_MOTIF_RTOL */

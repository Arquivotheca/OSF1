...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextSel.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/07/15 17:22:34 $"
#endif
#endif
/*
  ...
  /* Don't do replace if there is no text to add */
    ...
    if (_XmTextModifyVerify(tw, NULL, &left, &right,
       ...
       if ((*tw->text.source->Replace)(tw, (XEvent *)insert_select->event,
				 &left, &right, &newblock, True) != EditDone) {
...
#ifdef DEC_MOTIF_BUG_FIX
                          (XtSelectionCallbackProc) HandleInsertTargets,
#else
                          HandleInsertTargets,
#endif
                          (XtPointer) &insert_select,
...
#endif /* _NO_PROTO */
{
  ...
  /* Delete the selection */
      ...
      if (_XmTextModifyVerify(widget, NULL, &left, &right,
                              &cursorPos, &block, &newblock, &freeBlock)) {
	 if ((*widget->text.source->Replace)(widget, (XEvent *) req_event,
				 &left, &right, &newblock, True) != EditDone) {
            if (freeBlock && newblock.ptr) XtFree(newblock.ptr);
	    return False;
	 } else {
...
/* Losing Primary Selection */
   if (*selection == XA_PRIMARY && _XmStringSourceHasSelection(source)) {
      ...
      cb.event = NULL;
      XtCallCallbackList(w, tw->text.lose_primary_callback, (Opaque) &cb);

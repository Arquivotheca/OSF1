...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Traversal.c,v $ $Revision: 1.1.8.2 $ $Date: 1993/05/06 15:55:49 $"
#endif
#endif
#ifdef  REV_INFO
#ifndef lint
...
#endif /* _NO_PROTO */
{
  return (XmFocusData) XtCalloc( sizeof( XmFocusDataRec), 1) ;
}
...
{   
  ...
  if(    focusData = _XmGetFocusData( new_wid)    )
    {
...
#ifdef VMS
	 &&  (int) _XmGetNavigability( new_wid)    )
#else
      ...
      if(    focus_data->focus_item == NULL    )
	{
	  ...
	  if(    XtIsShell( wid)    )
	      ...
	      if(    focus_data->first_focus == NULL    )
		{
	      ...
	      if(    firstManaged = FindFirstManaged( wid)    )
		{
...
{   
    ...
    while(    ancestor = _XmGetClippingAncestor( ancestor, &focus_rect)    )
    {   
        if(    sw = _XmIsScrollableClipWidget( ancestor, &focus_rect)    )
        {   
...
#endif /* _NO_PROTO */
{
  ...
  if(    focus_data = _XmGetFocusData( wid)    )
    {
      focus_data->focus_item = NULL ;

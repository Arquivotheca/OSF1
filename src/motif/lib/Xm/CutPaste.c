...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: CutPaste.c,v $ $Revision: 1.1.8.5 $ $Date: 1994/01/14 20:16:47 $"
#endif
#endif
/*
...
#define BYTELENGTH( length, format ) \
	((format ==8) ? length : \
	((format == 16) ? length * sizeof(short) : \
	length * sizeof(Cardinal))) 
#endif

#ifdef ALPHA_BUG_FIX
/*
 * When calculating the number of elements for format == 32, need
 * to adjust for architectures where sizeof(long) > sizeof(Cardinal), 
 * otherwise number of element calculations come out larger than they 
 * should be.  (Analagous to BYTELENGTH fix, above).
 */

#define	FORMAT	(format == 32 ? format * sizeof(long)/sizeof(Cardinal) : format)
...
#if defined(__osf__) && defined(DEC_MOTIF_BUG_FIX)
	{
	    XPointer xp;
	
	    if (!XFindContext (display, (XID) NULL, (XContext) formatitem->displayId, &xp))
		formatitem->displayId = (Display *) xp;
	    if (!XFindContext (display, (XID) NULL, (XContext) formatitem->cutByNameCallback, &xp))
		formatitem->cutByNameCallback = (XmCutPasteProc) xp;
	    if (!XFindContext (display, (XID) NULL, (XContext) formatitem->cutByNameWidget, &xp))
		formatitem->cutByNameWidget = (Widget) xp;
	}
#endif
...
#endif /* _NO_PROTO */
{
    ...
    XtPointer int_ptr;
#ifdef DEC_MOTIF_EXTENSION
    Window selection_owner = (Window) 0;
...
#ifdef DEC_MOTIF_EXTENSION
	root_clipboard_header->ownSelection = selection_owner;
#endif
    }
...
#ifdef DEC_MOTIF_EXTENSION
	/*  This code helps V3 clients deal with the case where a V2 client   */
	/*  has corrupted the clipboard header.  This corruption occurs       */
	/*  because V2 clients assume that the clipboard header record is     */
	/*  60 bytes long, and that the list of clipboard items is stored     */
	/*  directly after these bytes.  V3 clients use a 64 byte clipboard   */
	/*  header, which will be corrupted when accessed by a V2 application */
	/*  because of the previously mentioned assumptions.		      */

	/* check consistency of header */
	if (root_clipboard_header->dataItemList + 
	    root_clipboard_header->currItems * sizeof( itemId ) 
	    != headerlength - add_length )
	{
	    /* it is likely that the clipboard data structures are corrupted, 
	    /* so we should reinitialize them. */
	    ...
	    */
	    ...
	    selection_owner = root_clipboard_header->ownSelection;
	    root_clipboard_header = (ClipboardHeader) NULL;
	}
    }while( root_clipboard_header == (ClipboardHeader) NULL);
#endif
...
#endif /* _NO_PROTO */
{
    ...
    headerlength = root_clipboard_header -> dataItemList + 
    		(root_clipboard_header -> currItems) * sizeof( itemId );
#else
...
#endif /* _NO_PROTO */
{
    ...
    */
    idptr = (itemId*) AddAddresses( (XtPointer) queryitem, queryitem->formatIdList );
    currformatid = *idptr;
...
#endif /* _NO_PROTO */
{
    ...
    */
    ...
    deleteptr = (itemId*)AddAddresses( (XtPointer) datalist, datalist->formatIdList ); 
...
#if defined(__osf__) && defined(DEC_MOTIF_BUG_FIX)
	{
	    ...
	    if (!XFindContext (display, (XID) NULL, (XContext) formatdata->displayId, &xp))
		formatdata->displayId = (Display *) xp;
	    if (!XFindContext (display, (XID) NULL, (XContext) formatdata->cutByNameCallback, &xp))
		formatdata->cutByNameCallback = (XmCutPasteProc) xp;
	    if (!XFindContext (display, (XID) NULL, (XContext) formatdata->cutByNameWidget, &xp))
		formatdata->cutByNameWidget = (Widget) xp;
	}
#endif
...
#endif /* _NO_PROTO */
{
    ...
    */
    listptr = (itemId*)AddAddresses( (XtPointer) header, header->dataItemList ); 
...
#endif /* _NO_PROTO */
{
    ...
    */
    nextIdPtr = (itemId*)AddAddresses((XtPointer) header, header->dataItemList); 
...
#endif /* _NO_PROTO */
{
...
#ifdef DEC_MOTIF_EXTENSION
    /* if the header is an old size header then ignore the startCopyCalled */
    /* field
    */
    if( header->dataItemList == sizeof( ClipboardHeaderRec ))
#endif
    ...
    */
...
#if defined(__osf__) && defined(DEC_MOTIF_BUG_FIX)
    {
	XContext contextid = XUniqueContext();
	
	XSaveContext (display, (XID) NULL, contextid, (XPointer) display);
	itemheader->displayId = (Display *) contextid;
    }
#else
...
#if defined(__osf__) && defined(DEC_MOTIF_BUG_FIX)
	{
	    ...
	    XSaveContext (display, (XID) NULL, contextid, (XPointer) callback);
	    itemheader->cutByNameCallback = (XmCutPasteProc) contextid;
	    contextid = XUniqueContext();
	    XSaveContext (display, (XID) NULL, contextid, (XPointer) widget);
	    itemheader->cutByNameWidget = (Widget) contextid;
	}
#else
...
#endif /* _NO_PROTO */
{
    ...
    */
    newitemaddr = (itemId*)AddAddresses((XtPointer) header, newitemoffset );
...
#ifdef DEC_MOTIF_EXTENSION
    itemheader->permanentItemFlag = 1;
#endif /* DEC_MOTIF_EXTENSION */
#if defined(__osf__) && defined(DEC_MOTIF_BUG_FIX)
	{
	    XPointer xp;
	
	    if (!XFindContext (display, (XID) NULL, (XContext) itemheader->displayId, &xp))
		itemheader->displayId = (Display *) xp;
	    if (!XFindContext (display, (XID) NULL, (XContext) itemheader->cutByNameCallback, &xp))
		itemheader->cutByNameCallback = (XmCutPasteProc) xp;
	    if (!XFindContext (display, (XID) NULL, (XContext) itemheader->cutByNameWidget, &xp))
		itemheader->cutByNameWidget = (Widget) xp;
	}
#endif
...
#ifdef DEC_MOTIF_EXTENSION
    /* make sure the item is temporary
    */
    _XmClipboardFindItem( display,
                           deleteitemid,
                           (XtPointer *) &itemheader,
    ...
    if ( itemheader->permanentItemFlag == 1 )
    {
        /* end copy to clipboard has already been called for this item */
        /* so can't cancel
        */
        return;
    }

    _XmClipboardFreeAlloc( (XtPointer) itemheader );
#endif /* DEC_MOTIF_EXTENSION */
...
#endif /* _NO_PROTO */
{
	...
	*/
	...
	if (matchformat != 0)
	{
		...
		if (!XFindContext (display, (XID) NULL, (XContext) matchformat->displayId, &xp))
		    matchformat->displayId = (Display *) xp;
		if (!XFindContext (display, (XID) NULL, (XContext) matchformat->cutByNameCallback, &xp))
		    matchformat->cutByNameCallback = (XmCutPasteProc) xp;
		if (!XFindContext (display, (XID) NULL, (XContext) matchformat->cutByNameWidget, &xp))
		    matchformat->cutByNameWidget = (Widget) xp;
	    }
#endif
...
#ifdef DEC_MOTIF_BUG_FIX
/*
** The documentation says that the length
** argument will be zero if there is no data 
** on the clipboard. So this fix is put in
** to agree with the documentation, whether
** it's right or wrong.
**
** susan 1-14-94
*/
...
#endif /* _NO_PROTO */
{
    ...
    */
    ...
    id_ptr = (itemId*)AddAddresses((XtPointer) header, header->dataItemList);
...
#ifdef DEC_MOTIF_EXTENSION
      if (
        _XmRegIfMatch( display, format_name, "DDIF",            XM_TEXT )
         )  return ClipboardSuccess;
#endif
    }

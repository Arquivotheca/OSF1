...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ToggleBG.c,v $ $Revision: 1.1.8.3 $ $Date: 1993/07/15 17:25:14 $"
#endif
#endif
/*
...
externaldef(xmtogglebuttongcacheobjclassrec) XmToggleButtonGCacheObjClassRec xmToggleButtonGCacheObjClassRec =
#else
...
{
  {
      ...
      /* set values almost  */    (XtProc)NULL,
      /* get values hook    */    (XtArgsProc)NULL,
      /* accept_focus       */    (XtProc)NULL,
      /* version            */    XtVersion,
      /* callback offsetlst */    NULL,
      /* default trans      */    NULL,
      /* query geo proc     */    (XtProc)NULL,
      /* display accelerator*/    (XtProc)NULL,
...
externaldef(xmtogglebuttongadgetclassrec)
	XmToggleButtonGadgetClassRec xmToggleButtonGadgetClassRec = {
   {
    ...
    /* display_accelerator */   (XtProc)NULL,
...
#endif
{
  ...
  if ( (! tb -> rectangle.sensitive) && tb -> label.pixmap_insen )
 ...
 { _XmLeaveGadget( (Widget) w, event, NULL, NULL);
   ...
   if ((TBG_IndType(w)) == XmN_OF_MANY)
   {
    ...
    { 
       ...
       /*****************************************************************/
       /*                                                               */
       /*  If we're leaving, the visual_set flag should be              */
       /*  what the toggle button's current state is.  Toggling         */
       /*  the visual_set flag doesn't work if the server               */
       /*  generates a Press event before an enter event.               */
       /*  And this is what the Sun Xnews server does.                  */
       /*                                                               */
       /*****************************************************************/
        IsOn(w) = TBG_Set(w);
#else
   ...
   {  _XmEnterGadget( (Widget) w, event, NULL, NULL);  
	...
	edge = Min((int)TBG_IndicatorDim(w), 
	 ...
	 { 
           ...
           /*************************************************************/
           /*                                                           */
           /*  If we're entering, the visual_set flag should be         */
           /*  the opposite of what the current state it.  Toggling     */
           /*  the visual_set flag doesn't work if the server           */
           /*  generates a Press event before an enter event.           */
           /*  And this is what the Sun Xnews server does.              */
           /*                                                           */
           /*************************************************************/
            IsOn(w) = (TBG_Set(w) == TRUE) ? FALSE : TRUE;
#else
...
#endif /* _NO_PROTO */
{
  ...
  /**********************************************************************/
  /*                                                                    */
  /*    If the release occurs before a leave, and we're outside the     */
  /*    button, we should redraw the toggle button.  This was added     */
  /*    because the Sun Xnews server does just this.                    */
  /*                                                                    */
  /**********************************************************************/
  else
  {
...
/* END OSF Fix pir 2480 */
    ...
    if (TBG_IndOn(new_w))
    {
        ...
        {
           if ((LabG_MarginRight(new_w)) < (TBG_IndicatorDim(new_w) +
                                         TBG_Spacing(new_w))) {
               LabG_MarginRight(new_w) =  TBG_IndicatorDim(new_w) +
                                        TBG_Spacing(new_w);
           }
        }
        else
 ...
 */
       ...
       if (TBG_IndOn(newcbox))
	    ...
	    if (LayoutIsRtoLG(newcbox))
	        if ((LabG_MarginRight(newcbox) <
			(TBG_IndicatorDim(newcbox) + TBG_Spacing(newcbox))) ||
		       TBG_Spacing(newcbox) != TBG_Spacing(curcbox))
                  LabG_MarginRight(newcbox) = TBG_IndicatorDim(newcbox) +
                                                 TBG_Spacing(newcbox);
            else
            {
#endif /* DEC_MOTIF_RTOL */
...
SetToggleSize(
...
#endif /* _NO_PROTO */
 {
   ...
   /* We know it's a pixmap so find out how how big it is */
    if ((newtbg->rectangle.sensitive) &&
        (newtbg->rectangle.ancestor_sensitive))
      {

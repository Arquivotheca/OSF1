...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: BulletinB.c,v $ $Revision: 1.1.6.4 $ $Date: 1993/10/18 15:26:38 $"
#endif
#endif
/*
...
#ifdef DEC_MOTIF_EXTENSION
        {       DXmNautoUnrealize,
                DXmCAutoUnrealize, XmRBoolean, sizeof (Boolean),
                XtOffset (XmBulletinBoardWidget,
                          bulletin_board.dxm_auto_unrealize),
                XmRImmediate, (XtPointer) False
        },
...
#ifdef DEC_MOTIF_EXTENSION
    /* Initialize accelerators if not specified by user or application.
    */
    if(    !new_w->core.accelerators    )
    {
        new_w->core.accelerators = defaultAcceleratorsParsed ;
        }
#endif
...
/****************/
    ...
    if(    (new_w->bulletin_board.shadow_type
        ...
        && !XmRepTypeValidValue( XmRID_SHADOW_TYPE,
			new_w->bulletin_board.shadow_type, (Widget) new_w)    )
    {   new_w->bulletin_board.shadow_type = current->bulletin_board.shadow_type ;
        } 
 ...
 */
    ...
    */
    ...
    {   
        /* Do not consider re-drawing shadow if this is a subclass,
        *   since such behavior could not be overriden and may be
        *   redundant or undesired in a subclass.
        */
        if( ( (new_w->bulletin_board.shadow_type
             != current->bulletin_board.shadow_type)
         ...
         && (XtHeight( new_w) == XtHeight( current))    ) {   
            /* Since width and height are not changing, no resize will be
            *   initiated by the intrinsics.  The shadow must be re-drawn
            *   here if any changes have been made to its thickness or type.
            *   We do not simply want to return true from SetValues, since
            *   an entire redisplay would be overkill.
            */
            _XmClearBorder(XtDisplay((Widget) new_w), 
			   XtWindow((Widget) new_w),
			   0, 0,
			   XtWidth(current), XtHeight(current),
			   current->manager.shadow_thickness);
            _XmDrawShadows(XtDisplay((Widget) new_w), 
			    XtWindow((Widget) new_w),
			    new_w->manager.top_shadow_GC, 
			    new_w->manager.bottom_shadow_GC,
			    0, 0,
     ...
     /* do the margin enforcement only for pure BB, not subclasses,
         they might become sick with the child move */

    /* If the new margins are bigger, re-enforce them using setvalue
       instead of movewidget, so that the normal geometry management
       will occur */
      if (((new_w->bulletin_board.margin_width > 
          ...
          _XmGMEnforceMargin((XmManagerWidget)new_w,
                             ...
                             new_w->bulletin_board.margin_height,
                             True); /* use setvalue, not movewidget */
    ...
    if ( ! l_string)
      {
        ...
        /* then no label has been specified, so go off to the I18n layer
         * and build a label from the ASCII 'name' parameter
         */

/* I18N START */
        /*
        ** Note: at this point we have no information about the context of
        ** the name.  Assume that most buttons are verbs.
        */
        l_string = (XmString) XmGetLocaleString ((I18nContext) NULL, name,
                                                I18NVERB | I18NBUTTON );
...
/* I18N END */
    ...
    XtSetArg( al[ac], DXmNlayoutDirection, LayoutM(bb)); ac++ ;
...
/* I18N START */
        /*
        ** Note: at this point we have no information about the context of
        ** the name.  Assume that most labels are nouns.
        */
        l_string = (XmString) XmGetLocaleString ((I18nContext) NULL, name,
                                                I18NNOUN | I18NLABEL );
/* I18N END */
...
#ifdef DEC_MOTIF_EXTENSION
         || strcmp( "DXmCSText",
                    (child)->core.widget_class->core_class.class_name )==0
#endif
        || XmIsTextField (child)    )
    {   

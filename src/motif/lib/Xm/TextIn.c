...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextIn.c,v $ $Revision: 1.1.6.4 $ $Date: 1993/10/18 15:38:29 $"
#endif
#endif
/*
...
static Boolean 
#ifdef _NO_PROTO
NeedsPendingDeleteDisjoint( tw, left, right )
        XmTextWidget tw ;
	XmTextPosition *left;
	XmTextPosition *right;
#else
...
#endif /* _NO_PROTO */
{
  ...
  XChangeProperty(XtDisplay(w), XtWindow(w), XA_WM_NAME, XA_STRING,
...
#endif /* _NO_PROTO */
{
    ...
    if (_XmTextModifyVerify(tw, event, &from, &to,
       ...
       if ((*tw->text.source->Replace)(tw, event, &from,
				       &to, &newblock, True) != EditDone) {
	   _XmTextEnableRedisplay(tw);
...
#endif /* _NO_PROTO */
{
    ...
    if (_XmTextModifyVerify(tw, event, &from_pos, &to_pos,
                             &cursorPos, &block, &newblock, &freeBlock)) {
       if ((*tw->text.source->Replace)(tw, event, &from_pos,
					&to_pos, &newblock, True) != EditDone) {
	   RingBell((Widget)tw, (XEvent *) NULL, (String *) NULL, (Cardinal) 0);
       } else {
 ...
 /* If the user has input more data than we can handle, bail out */
	...
	(*tw->text.output->DrawInsertionPoint)(tw,
        ...
        if (o_data->use_fontset) {
           if (XmbTextEscapement((XFontSet)font, str, n) == 0 && 
                                                          strchr(str, '\t')==0){
	      (*tw->text.output->DrawInsertionPoint)(tw,
...
/* Just because the character doesn't appear in the font doesn't mean
** we shouldn't insert it - tabs for instance
*/
#else
	...
	if (NeedsPendingDeleteDisjoint(tw, &left, &right)) {
	   beginPos = left;
	   nextPos = right;
           pending_delete = True;
        } else {
	       ...
	       lastPos = (*(tw->text.source->Scan))(tw->text.source,
			     beginPos, XmSELECT_LINE, XmsdRight, 1, TRUE);
               if (nextPos == lastPos) nextPos--;
            }
        }
	block.ptr = str;
	block.length = n;
	block.format = XmFMT_8_BIT;
        if (_XmTextModifyVerify(tw, event, &beginPos, &nextPos,
           ...
           if (pending_delete)
              (*tw->text.source->SetSelection)(tw->text.source, cursorPos,
					         cursorPos, event->xkey.time);
	   if ((*tw->text.source->Replace)(tw, event, &beginPos, &nextPos,
					   &newblock, True) != EditDone) {
	      RingBell(w, event, params, num_params);
	   } else {
...
#endif /* _NO_PROTO */
{
    ...
    if (*num_params == 0) {
      pos = (*tw->text.output->XYToPos)(tw, event->xbutton.x, event->xbutton.y);
      desired_line = _XmTextPosToLine(tw, pos);
    } else {
      sscanf(*params, "%d", &percentage);
      desired_line = ((data->number_lines - 1) * percentage) /100;
    }
      cur_line = _XmTextPosToLine(tw, tw->text.cursor_position);
...
#endif /* _NO_PROTO */
{
    ...
    if (_XmTextModifyVerify(tw, event, &beginPos, &nextPos,
        ...
        if ((*tw->text.source->Replace)(tw, event, &beginPos,
				    &nextPos, &newblock, True) != EditDone) {
...
#endif /* _NO_PROTO */
{
    ...
    if(left != (*tw->text.source->Scan)(tw->text.source, left,
        ...
        while(left < right){
            ...
            if (_XmTextModifyVerify(tw, event, &from_pos, &to_pos,
			 &newCursorPos, &block, &newblock, &freeBlock)) {
	       if ((*tw->text.source->Replace)(tw, event, &from_pos, &to_pos,
					       &newblock, True) != EditDone) {
	           RingBell(w, event, params, num_params);
	           if (freeBlock && newblock.ptr) XtFree(newblock.ptr);
	           break;
...
#endif /* _NO_PROTO */
{
    ...
    cb.event  = event;
    XtCallCallbackList(w, tw->text.activate_callback, (Opaque) &cb);
	  ...
	  max_length = _XmStringSourceGetMaxLength(source);
       ...
       if (_XmTextModifyVerify(tw, NULL, &replace_from, &replace_to,
          ...
          prim_select->num_chars = _XmTextCountCharacters(newblock.ptr,
							  newblock.length);
	  if ((*tw->text.source->Replace)(tw, (XEvent *) NULL,
   ...
   if (event->property == None) {
     ...
     if (data->selectionMove) {
        ...
        if (_XmTextModifyVerify(tw, ev, &left, &right,
                                &cursorPos, &block, &newblock, &freeBlock)) {
	   if ((*tw->text.source->Replace)(tw, ev, &left, &right, 
					   &newblock, True) != EditDone) {
	      RingBell(w, (XEvent *) NULL, (String *) NULL, (Cardinal) 0);
	   } else {
              int count;
...
#endif /* _NO_PROTO */
{
    ...
    cbdata.text = NULL;
    XtCallCallbackList(w, tw->text.losing_focus_callback, (Opaque) &cbdata);
    return(cbdata.doit);
}
...
#ifdef DEC_MOTIF_BUG_FIX
/* Make psect name match variable name, CS 31-August 1993 */
externaldef(_xmdefaulttextactionstable) XtPointer _XmdefaultTextActionsTable =
...
externaldef(_xmdefaulttextactionstablesize) Cardinal _XmdefaultTextActionsTableSize = XtNumber(ZdefaultTextActionsTable);
...
#endif /* _NO_PROTO */
{
    ...
    XtSetSubvalues((XtPointer) tw->text.input->data,
		   input_resources, XtNumber(input_resources), args, *num_args);
}
...
#endif /* _NO_PROTO */
{
   ...
   /* When type = NULL, we are assuming a DELETE request has been requested */
    ...
    if (_XmTextModifyVerify(tw, NULL, &insertPosLeft, &insertPosRight,
                                   &cursorPos, &block, &newblock, &freeBlock)) {
       if ((*tw->text.source->Replace)(tw, (XEvent *) NULL, &insertPosLeft,
				 &insertPosRight, &newblock, True) != EditDone){
	  if (tw->text.verify_bell) XBell(XtDisplay(tw), 0);
	  tw->text.pendingoff = pendingoff;
       } else {

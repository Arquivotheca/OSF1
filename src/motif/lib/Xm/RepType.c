...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: RepType.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/07/15 17:04:58 $"
#endif
#endif
/*
...
{   
    ...
    {   
        ...
        {   
            ...
            *NextString++ = 'X' ;
            *NextString++ = 'm' ;
            while(    *NextString++ = (islower(*SrcPtr)) ?
				toupper( *SrcPtr++) : *SrcPtr++   ){ } 
            ++Index ;
            } 
        } 
    else
...
#endif /* _NO_PROTO */
{
    ...
    {   if(    XmREP_TYPE_MAPPED( rep_type_id)    )
        ...
        {   
            ...
            sprintf( msg, "illegal value (%d) for rep type XmR%s", test_value,
...
{   
  ...
  */
    if (isupper(in_str[0]))
        first = tolower(in_str[0]);
    else
        first = in_str[0];
    if (isupper(in_str[1]))
        second = tolower(in_str[1]);
    else
        second = in_str[1];
...
{   
    ...
    sprintf( WarnMsg, "Cannot convert %s value %d to type String", 
                              Record->rep_type_name, (unsigned int) in_value) ;
    XtAppWarningMsg( XtDisplayToApplicationContext( disp), "conversionError",
                 Record->rep_type_name, "XtToolkitError", WarnMsg, NULL, NULL);
    return( FALSE) ;
    }

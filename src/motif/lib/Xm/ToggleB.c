...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ToggleB.c,v $ $Revision: 1.1.8.3 $ $Date: 1993/07/15 17:24:18 $"
#endif
#endif
/*
   ...
   else
   {
	...
	y = (w->core.height - w->toggle.indicator_dim)/2;
	 ...
	 { 
            ...
            /*                                                          */
            /************************************************************/
            IsOn(w) = (w->toggle.set == TRUE) ? FALSE : TRUE;
#else
...
/* END OSF Fix pir 2480 */
    ...
    if (new_w->toggle.ind_on)
    {
       ...
       {
          if ((new_w->label.margin_right) < (new_w->toggle.indicator_dim +
                                       new_w->toggle.spacing))
              new_w->label.margin_right = (new_w->toggle.indicator_dim +
                                     new_w->toggle.spacing);
       }
       else
       {
 ...
 */
       ...
       if (newcbox->toggle.ind_on)
       {
          ...
          {
            if ((newcbox->label.margin_right < (newcbox->toggle.indicator_dim +
					  newcbox->toggle.spacing)) ||
		 newcbox->toggle.spacing != curcbox->toggle.spacing)
                 newcbox->label.margin_right = newcbox->toggle.indicator_dim +
                                           newcbox->toggle.spacing;
          }
          else
          {
...
#endif /* _NO_PROTO */
{
    ...
    XFillRectangle (XtDisplay(tb), XtWindow(tb), (IsOn(tb) ?
                      tb->toggle.select_GC : tb->toggle.background_gc),
                      fx, fy, fw, fh);

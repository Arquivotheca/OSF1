 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: Screen.c,v $ $Revision: 1.1.4.2 $ (DEC) $Date: 1993/05/06 15:47:37 $";
#endif
/* 
...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Screen.c,v $ $Revision: 1.1.4.2 $ $Date: 1993/05/06 15:47:37 $"
#endif
#endif
/*
...
#ifdef DEC_MOTIF_BUG_FIX
externaldef(_xminvalidcursoriconquark) XrmQuark	_XmInvalidCursorIconQuark ;
externaldef(_xmvalidcursoriconquark) XrmQuark	_XmValidCursorIconQuark ;
externaldef(_xmnonecursoriconquark) XrmQuark	_XmNoneCursorIconQuark ;
externaldef(_xmdefaultdragiconquark) XrmQuark	_XmDefaultDragIconQuark ;
externaldef(_xmmovecursoriconquark) XrmQuark	_XmMoveCursorIconQuark ;
externaldef(_xmcopycursoriconquark) XrmQuark	_XmCopyCursorIconQuark ;
externaldef(_xmlinkcursoriconquark) XrmQuark	_XmLinkCursorIconQuark ;
#else
...
#endif /* _NO_PROTO */
{
    ...
    register XmDragCursorCache  cache = xmScreen->screen.cursorCache;
...
#endif /* _NO_PROTO */
{
     ...
     * XCreatePixmap?
     *
     * For now, we'll take just about any match that will work.
     */
    int curDiff = (unsigned int) (-1);
	    ...
	    (ptr->depth == depth))
	{
	    ...
	    else if ((ptr->width >= width) &&
		(ptr->height >= height))
	    {
		/* Go for the best fit */
		int newDiff = ((ptr->width) * (ptr->height))
	 	    - (width * height);

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: TextOut.c,v $ $Revision: 1.1.6.4 $ $Date: 1994/01/14 18:06:34 $"
#endif
#endif
/*
...
static int 
#ifdef _NO_PROTO
_FontStructFindWidth( data, x, block, from, to )
        OutputData data ;
        int x ;
        XmTextBlock block ;
        int from ;
...
/*
** From X Window System, section 6.5 "Font Metrics":
...
**
**  So, if the per_char pointer is not null, and the character is between the
**  min and max, use the per_char data to get the width; otherwise if the
**  per_char pointer is not null, but the character is outside the min or max,
**  AND there is a default_char, use the per_char data of the default_char to
**  get the width.  But if the per_char pointer IS null, just use the
**  min_bounds width.
*/
            if (font->per_char) {
...
#endif /* _NO_PROTO */
{
    ...
    if (!data->use_fontset)
        return _FontStructFindWidth(data, x, block, from, to);
...
#endif /* _NO_PROTO */
{
    ...
    if (!widget->text.top_character)
      ...
      if (widget->text.top_line < 0) widget->text.top_line = 0;
    }
    ...
    if (data->resizeheight && widget->text.total_lines > data->number_lines) {
       ...
       for (linestart = widget->text.top_character;
	    i + 1 < widget->text.total_lines; i++) {
           text_width = 0;
	   position = widget->text.line_table[i + 1].start_pos;
	   while (linestart < position) {
...
#define no_needs_count 2
    widget->text.vsbar_scrolling &= ~no_needs_count;
#endif
...
#endif /* _NO_PROTO */
{
	...
	if ((int)widget->text.char_size == 1) num_bytes = 1;
	else {
		  ...
		  length += num_bytes;
            ...
            if (newx > rightedge) {
               ...
               if ((int)widget->text.char_size == 1){
                  ...
                  for (i=0, num_bytes = mblen(&block.ptr[i], 
					      (int)widget->text.char_size); 
                       i < length && newx <= rightedge ; i += num_bytes,
                       num_bytes = mblen(&block.ptr[i], 
					 (int)widget->text.char_size)) {
	  ...
	  XFillRectangle(XtDisplay((Widget)widget), XtWindow((Widget)widget),
       ...
       if ((data->cursor_on >= 0) && (data->blinkstate == on)) {
          ...
          XCopyArea(XtDisplay((Widget)widget), data->ibeam_off, 
		    XtWindow((Widget)widget), data->imagegc, 0, 0,
    ...
    if (XtIsRealized(widget)) {
	   ...
	   if (width > 0 && innerheight > 0) {
		  ...
		  XClearArea(XtDisplay(widget), XtWindow(widget),
	       ...
	       /* clear right margin */
#ifdef DEC_MOTIF_BUG_FIX
            /* This fixes a problem caused when margin_width = 0 for */
            /* the text widget.  In that case, the width passed into */
            /* the clear area call would be zero, which has the      */
            /* undesirable effect of clearing a rectangle with a     */
            /* width equal to the width of the window minus the      */
            /* shadow and highlight thicknesses (i.e. the whole      */
            /* right side of the text window was being cleared       */
            /* instead of the right margin.                          */
            /*                                  Rich        2/14/91  */
            if (data->rightmargin - (widget->primitive.shadow_thickness +
...
#endif /* _NO_PROTO */
{
    ...
    *width = (Dimension) ((data->columns_set * data->averagecharwidth) +
                           data->leftmargin + data->rightmargin);
    *height = (Dimension) ((data->rows_set * data->lineheight) +
                            data->topmargin + data->bottommargin);
}
...
/* Create area in which to save text located underneath I beam */
   MakeIBeamOffArea(widget, data->cursorwidth, data->cursorheight);
  ...
  /* Create a new add_mode cursor */
   ...
   XmSetNormGC(widget, data->imagegc, False, False);
}
...
#endif /* _NO_PROTO */
{
    ...
    OutputData data = widget->text.output->data;
#ifdef DEC_MOTIF_BUG_FIX
/* NOTE: this has been reported to OSF */
    if (!XtIsRealized(widget))  /* Do nothing if not still realized */
    {
        data->timerid = NULL;
        data->hasfocus = FALSE;
        return;
    ...
    switch (event->type) {
      case FocusIn:
        if (event->xfocus.send_event && !(newhasfocus)) {
            ...
            cb.event = event;
            XtCallCallbackList (w, widget->text.focus_callback, (Opaque) &cb);
            newhasfocus = TRUE;
    ...
    for (i = y ; i < y + height + data->lineheight ; i += data->lineheight) {
       ...
       last = (*widget->text.source->Scan)(widget->text.source, last,
					   XmSELECT_POSITION,
                              		   XmsdRight, 1, TRUE);
#ifdef DEC_MOTIF_BUG_FIX
        /* if first == last == 0, force it to redraw the cursor
        */
      ...
      
   {
      ...
      if (data->scrollvertical) {
          ...
          XtAddEventHandler(data->vbar, (EventMask)ButtonReleaseMask,
                            FALSE, HandleVBarButtonRelease, (Opaque)widget);
      } else data->vbar = NULL;
...
#endif /* _NO_PROTO */
{
   ...
   if (!XtIsRealized((Widget)tw)) return;
    ...
    XChangeGC(XtDisplay(widget), data->imagegc, GCStipple, &values);
}

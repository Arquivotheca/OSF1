...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DropSMgr.c,v $ $Revision: 1.1.4.7 $ $Date: 1993/11/10 21:57:49 $"
#endif
#endif
/*
...
{ 
    ...
    register int idx, rehash;
    register XtPointer entry;
    register XtPointer info = DSMWidgetToInfo(dsm, widget);
...
#endif /* _NO_PROTO */
{
	 ...
	 * Destroy the clipper
	 */
	DSMUnregisterInfo(dsm, GetDSWidget(clipper));
...
static void 
#ifdef _NO_PROTO
HandleLeave( dsm, motionData, callback, info, style )
        XmDropSiteManagerObject dsm ;
        XmDragMotionClientData motionData ;
        XmDragMotionCallbackStruct *callback ;
        XmDSInfo info ;
...
#endif /* _NO_PROTO */
{
	...
	if (dsm->dropManager.notifyProc)
	{
		XmDropSiteLeaveCallbackStruct	outCB;
...
#endif /* _NO_PROTO */
{
	...
	else if (XmIsManager(w))
	{
		...
		case XmDRAG_UNDER_SHADOW_OUT:
		{
			...
			if (!GetDSHasRegion(dsInfo))
				info->animation_data.borderWidth = mw->core.border_width;
...
#endif /* _NO_PROTO */
{
	...
	/*
	** Need to round allocation to a fullword boundary for systems that
	** align structures.  CS, 10-Sep-1993
	*/
	size0 = ROUND_FULLWORD(sizeof(XmDSStatusRec));

	if (iccInfo.header.dropType & XmDSM_DS_LEAF)
		size1 = ROUND_FULLWORD(sizeof(XmDSRemoteLeafRec));
	else
		size1 = ROUND_FULLWORD(sizeof(XmDSRemoteNodeRec));
#else
	...
	else if (dsm->dropManager.dsRoot)/* XmCR_TOP_LEVEL_LEAVE */
	{
		if (dsm->dropManager.curInfo != NULL)
		{
			...
			HandleLeave(dsm, &cdRec, &cbRec,
				(XmDSInfo) dsm->dropManager.curInfo, style);
...
#endif /* _NO_PROTO */
{
	...
	{
		_XmWarning(widget, "Creating an active drop site with no drop proc.");
	}
	...
	{
		_XmWarning(widget, "Can\'t set rectangles or num rectangles of composite dropsites.");
		fullInfoRec.rectangles = NULL;
		fullInfoRec.num_rectangles = 1;
	}
	...
	*/
	...
	if (fullInfoRec.type == XmDROP_SITE_COMPOSITE)
		size1 = ROUND_FULLWORD(sizeof(XmDSLocalNodeRec));
	else
		size1 = ROUND_FULLWORD(sizeof(XmDSLocalLeafRec));
#else	
	size0 = sizeof(XmDSStatusRec);
...
#endif /* _NO_PROTO */
{
	...
	/* 
	 * Set up the rectangle list stuff.  We'll assume that info 
	 * is a variant which doesn't carry that stuff around.
	 */
	_XmRegionGetRectangles(full_info->region, &rects,
		&num_rects);
	full_info->rectangles = rects;
	full_info->num_rectangles = (Cardinal) num_rects;
...
#endif /* _NO_PROTO */
{
  ...
  XmDisplay xmDisplay = (XmDisplay) XmGetXmDisplay(XtDisplayOfObject(refWidget));
  Boolean found = False;
  Boolean clean;
  Widget shell;
  XmDSInfo shellInfo;

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DragC.c,v $ $Revision: 1.1.4.4 $ $Date: 1993/10/18 15:28:46 $"
#endif
#endif
/*
...
#ifdef DEBUG
#define Warning(str) XtWarning(str)
#else
#define Warning(str) ;
#endif
...
externaldef(xmdragcontextclassrec) XmDragContextClassRec xmDragContextClassRec = {
#else
...
externaldef(xmdragContextclass) WidgetClass 
...
#endif /* _NO_PROTO */
{
	...
	for (i = nchildren - 1; i >= 0; i--) {
	    if (inf = GetClientWindow(dpy, children[i], atom))
	      return inf;
	}
    }
    return 0;
...
#endif /* _NO_PROTO */
{
    ...
    switch(callbackRec.any.reason = reason) {
      case XmCR_TOP_LEVEL_ENTER:
	if (callbackList = dc->drag.topLevelEnterCallback) {
	    XmTopLevelEnterCallback	callback =
	      (XmTopLevelEnterCallback)&callbackRec;
	...
	break;
      case XmCR_TOP_LEVEL_LEAVE:
	if (callbackList = dc->drag.topLevelLeaveCallback) {
	    XmTopLevelLeaveCallback	callback =
	      (XmTopLevelLeaveCallback)&callbackRec;
	...
	break;
      case XmCR_OPERATION_CHANGED:
	if (callbackList = dc->drag.operationChangedCallback) {
	    XmOperationChangedCallback	callback =
	      (XmOperationChangedCallback)&callbackRec;
	...
	{
	    _XmWarning((Widget)dc,
			"GenerateCallback:we shouldn't be here\n");
	}
	break;
...
{   
    ...
    if(!(dc = (XmDragContext) _XmGetDragContextFromHandle( w,
                                           *selection))    )
    {   _XmWarning(w, "invalid selection in DropConvertCallback") ;
        }
    if(    dc->drag.dropFinishTime == 0    ) 
    {   
        _XmWarning(w, "we lost the drop selection") ;
        }
} 
...
#endif /* _NO_PROTO */
{
	...
	break;
      default:
	_XmWarning((Widget)dc, "trying to echo unacceptable message\n");
	break;
    }
}
...
{
    XmDragContext	dc = (XmDragContext)client;
    XmDropSiteLeaveCallbackStruct  *cb = (XmDropSiteLeaveCallbackStruct *)call;
    
	dc->drag.inDropSite = False;

    if (dc->drag.siteLeaveCallback) {
	XtCallCallbackList((Widget) dc, dc->drag.siteLeaveCallback, cb);
    }
...
#endif /* _NO_PROTO */
{
	...
	dc->drag.receiverInfos = (XmDragReceiverInfo)
	  XtCalloc(sizeof(XmDragReceiverInfoStruct), dc->drag.maxReceiverInfos);
	...
	if (startInfo) {
	  ...
	  XtWarning("we don't have startInfo when we should\n");
#endif /* DEBUG */	
	for (i = 1; i < dc->drag.numReceiverInfos; i++) {
...
#endif /* _NO_PROTO */
{
    ...
    Arg			args[4];
    Cardinal		i = 0;
    Window		destination = dc->drag.currReceiverInfo->window;
    ...
    if (dc->drag.currReceiverInfo->window) {
	SendDragMessage(dc, dc->drag.currReceiverInfo->window, XmTOP_LEVEL_LEAVE);
	GenerateClientCallback(dc, XmCR_TOP_LEVEL_LEAVE);
	...
	if ((dc->drag.activeProtocolStyle != XmDRAG_NONE) &&
	    ...
	    dc->drag.dragTimerId = 
	      XtAppAddTimeOut(appContext,
			      ...
			      DropStartTimeout,
			      (XtPointer)dc);
	    SendDragMessage(dc, destination, XmDROP_START);
	}
	else {
	    dc->drag.dragDropCompletionStatus = XmDROP_FAILURE;
	    dc->drag.dropFinishTime = dc->drag.dragFinishTime;
...
#endif /* _NO_PROTO */
{
	...
	_XmWarning(w, "XmDragStart must be called as a result of a button press");
	return NULL;
    }
    /*
...
#ifdef DEBUG
	XtWarning("can't drag; menu system active\n");
#endif	
	return NULL;
    }

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ArrowB.c,v $ $Revision: 1.1.6.2 $ $Date: 1993/05/06 15:27:12 $"
#endif
#endif
/*
...
/* this macro uses shadowthickness hardcoded 2 for 1.1 compatibility */

#define DRAWARROW(t_gc, b_gc, c_gc)    \
    _XmDrawArrow(XtDisplay ((Widget) aw),\
              XtWindow ((Widget) aw),\
              t_gc, b_gc, c_gc,\
              ...
              aw -> primitive.highlight_thickness +\
                 aw -> primitive.shadow_thickness,\
              aw -> core.width\
                 - 2 * aw -> primitive.shadow_thickness\
                 - 2 * aw -> primitive.highlight_thickness,\
              aw -> core.height\
                 - 2 * aw -> primitive.shadow_thickness\
                 - 2 * aw -> primitive.highlight_thickness,\
              2,\
...
#endif /* _NO_PROTO */
{
   ...
   if ((iwidth > 0) && (iheight > 0))
   {
     ...
     DRAWARROW(((aw->arrowbutton.selected && aw->core.sensitive)?
             aw -> primitive.bottom_shadow_GC:
             aw -> primitive.top_shadow_GC),
            ((aw->arrowbutton.selected && aw->core.sensitive)?
             aw -> primitive.top_shadow_GC :
             aw -> primitive.bottom_shadow_GC),
            (((aw->core.sensitive) &&
              (aw->core.ancestor_sensitive)) ?
               aw -> arrowbutton.arrow_GC :
               aw-> arrowbutton.insensitive_GC));
   }
...
#endif /* _NO_PROTO */
{
   ...
   DRAWARROW(aw -> primitive.bottom_shadow_GC,
           aw -> primitive.top_shadow_GC,
           NULL);
...
#endif /* _NO_PROTO */
{
   ...
   DRAWARROW(aw -> primitive.top_shadow_GC,
           aw -> primitive.bottom_shadow_GC,
           NULL);

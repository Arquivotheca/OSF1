...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Form.c,v $ $Revision: 1.1.7.3 $ $Date: 1993/07/15 16:29:57 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
    ...
    if (size_req && c->resizable && ChangeIfNeeded (fw, w, desired)) {
	reply = XtGeometryYes;
	if (!(desired->request_mode & XtCWQueryOnly)) {
	    /* Reposition the widget only if not QueryOnly */
	    PlaceChildren (fw, w, desired);

	   /* We have to take care of the situation where ChangeIfNeeded
	      returned True with a desired size of the Form denied
	      but actually smaller than it's current size.
	      In this case, the current size of the form hasn't changed,
	      but the desire of the child might have been denied by
	      PlaceChidren. If so, we need to return No and backup the 
	      original geometry of the child.
	    */
...
#endif /* _NO_PROTO */
{
	...
	if (height <= 0) height = 1;
        ...
        if (LayoutIsRtoLM(fw))
        {
            ...
            left_x = XtWidth (fw) - c->att[RIGHT].value;
            if ((left_x != ((RectObj) child)->rectangle.x) ||
...
#endif /* _NO_PROTO */
{
    ...
    /* find out the desired size of the form */
	...
	if (g.request_mode) {
	    if (desired && (desired->request_mode & XtCWQueryOnly))
		g.request_mode |= XtCWQueryOnly;
	    if (XtMakeGeometryRequest((Widget)fw, &g, NULL) != XtGeometryYes) {
		/* Momma said no or almost, but maybe we can accomodate 
		   the change anyway it it fits.*/
...
#endif /* _NO_PROTO */
{
        ...
        XmFormWidget req = (XmFormWidget) rw ;
	...
	new_w->core.width = req->core.width;
	new_w->core.height = req->core.height;

#ifdef DEC_MOTIF_RTOL
        /* See if layout direction changed */
        if (LayoutM(old) != LayoutM(new_w))
        {
            CalcFormSize (new_w, &w, &h);
            if (w > XtWidth(new_w)) XtWidth(new_w) = w; /*Only allow to grow here*/
            if (h > XtHeight(new_w)) XtHeight(new_w) = h;
        }
#endif /* DEC_MOTIF_RTOL */
	...
	if (new_w->manager.shadow_thickness != 
	         old->manager.shadow_thickness ||
	    new_w->bulletin_board.shadow_type != 
		old->bulletin_board.shadow_type)
	    returnFlag = TRUE;
#ifdef DEC_MOTIF_RTOL
        if (LayoutM(old) != LayoutM(new_w))
        {
...
#ifdef DEC_MOTIF_RTOL
                                            if (LayoutIsRtoLM(fw))
                                               /* switch the meanings ofleft and right attachements */
                                                a->percent = fw->form.fraction_base -
                                                             (((w->core.x +w->core.width
                                                                + 2 *w->core.border_width)
...
#ifdef DEC_MOTIF_RTOL
                                            if (LayoutIsRtoLM(fw))
                                                /* switch the meaningsof left and right attachements */
                                                a->percent = fw->form.fraction_base
                                                             - ((w->core.x *fw->form.fraction_base) / wid);
                                            else
#endif /* DEC_MOTIF_RTOL */

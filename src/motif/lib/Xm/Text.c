...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Text.c,v $ $Revision: 1.1.6.5 $ $Date: 1993/12/17 21:19:33 $"
#endif
#endif
/*
...
#endif /* _NO_PROTO */
{
   ...
   /*
    * The OSF code below is wrong, the text widget shouldn't
    * be resolving the inheritance of primitive class resources
    * itself, this will cause problems with widgets that try to
    * subclass off of XmText.  OSF fixed this for Label.c and
    * several other widgets in 1.2.2, but they seem to have
    * missed XmText
    */
#else
...
#endif /* _NO_PROTO */
{
  ...
  /* set cursor_position to a known exceptable value (0 is always acceptable) */
   ...
   /* OSF says:  if XmNvalueWcs set, it overrides all else */
	...
	need_new_cursorPos = True;
        ...
        _XmStringSourceSetValue(newtw, newtw->text.value);
        newtw->text.value = NULL;
	need_new_cursorPos = True;
    }
  ...
  /* return cursor_position to it's original changed value */
    ...
    } else if (newtw->text.cursor_position > newtw->text.source->data->length) {
           _XmTextSetCursorPosition(new_w, newtw->text.source->data->length);
    }
 ...
 * Otherwise, use mbtowc to calculate. */
...
#endif /* _NO_PROTO */
{
   ...
   for (bptr = str; num_count_bytes > 0; count++, bptr+= char_size){
      char_size = mbtowc((wchar_t*)NULL, bptr, MB_CUR_MAX);
...
#endif /* _NO_PROTO */
{
    ...
    if (_XmTextModifyVerify(tw, NULL, &frompos, &topos,
       ...
       (*source->Replace)(tw, NULL, &frompos, &topos, &newblock, True);
   ...
   if (XmIsTextField(widget))
	 ...
	 *total_tmp_value = '\0';
     ...
     if (_XmTextModifyVerify(tw, NULL, &paste_pos_left, &paste_pos_right,
                             &cursorPos, &block, &newblock, &freeBlock)) {
	if ((*source->Replace)(tw, NULL, &paste_pos_left,
			       &paste_pos_right, &newblock, True) != EditDone) {
...
#ifdef DEC_MOTIF_BUG_FIX
    /* Need to reset total_lines field */
...
#endif /* _NO_PROTO */
{
   ...
   if (!XmIsTextField(w)) {
      ...
      (void) wcstombs(string, wc_string,
                      (num_chars + 1) * (int)tw->text.char_size);

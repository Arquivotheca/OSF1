...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ScrolledW.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 17:10:42 $"
#endif
#endif
/*
...
static XtTranslations ClipWindowXlations2 = NULL;
#endif
...
#define SWMessage12 "Invalid Placement policy for Right to Left layout."
...
#ifdef DEC_MOTIF_RTOL
static void ScrollBarPlacementDefault (Widget w, int offset, XrmValue *value);
#endif
...
#endif /* _NO_PROTO */
{
    ...
    {
#ifdef DEC_MOTIF_RTOL
    sb = (XmScrollBarWidget) sw->swindow.hScrollBar;
#endif /* DEC_MOTIF_RTOL */

#ifdef DEC_MOTIF_RTOL
       if (LayoutIsRtoLM(sw))
       {
          /* For reversal processing reverse the scrollbar returned value */
          value = sb->scrollBar.maximum + sb->scrollBar.minimum
                - (int )call_data->value - sb->scrollBar.slider_size;
...
/*    XtSetArg (cwArgs[0], XmNtranslations, (XtArgVal) ClipWindowXlations);
    ...
    XtOverrideTranslations (w, ClipWindowXlations);
}
...
static void ScrollBarPlacementDefault (widget, offset, value)
Widget widget;
int      offset;
XrmValue *value;
{
        ...
        value->addr = (XtPointer) &placement;
 ...
 ****************/
    ...
    {
       if (new_w->swindow.Placement != XmBOTTOM_LEFT)
       {
          /* Force bottom left placement if layout is RtoL */
          new_w->swindow.Placement = XmBOTTOM_LEFT;
          _XmWarning((Widget) new_w, SWMessage12);
       }
    }
    else
#endif /* DEC_MOTIF_RTOL */
...
#ifdef DEC_MOTIF_EXTENSION
                ClipWindowXlations2 = XtParseTranslationTable(ClipWindowXlatString);
#else
...
/*    XtSetArg(hSBArgs[j], XmNtraversalOn, (XtArgVal) TRUE); j++;
    ...
    XtSetArg(hSBArgs[j], DXmNlayoutDirection, LayoutM(new_w)); j++;
#endif /* DEC_MOTIF_RTOL */
 ...
 ****************/
	...
	{
	    if (mom->swindow.WorkWindow != NULL) 
	       XtRemoveCallback(mom->swindow.WorkWindow, XmNdestroyCallback,
				KidKilled, NULL);
...
#endif /* _NO_PROTO */
{
   ...
   if (new_w->swindow.ScrollPolicy == XmAPPLICATION_DEFINED)
    ...
    {
       if (LayoutIsRtoLM(new_w) && new_w->swindow.Placement != XmBOTTOM_LEFT)
       {
         /* Force bottom left placement if layout is RtoL
         */
          _XmWarning((Widget) new_w, SWMessage12);

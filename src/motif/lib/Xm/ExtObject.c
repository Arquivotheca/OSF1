...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: ExtObject.c,v $ $Revision: 1.1.6.2 $ $Date: 1993/05/06 15:37:07 $"
#endif
#endif
/*
...
static XmExtCache extarray[XmNUM_ELEMENTS];
...
externaldef(xmextclassrec) XmExtClassRec xmExtClassRec = {
#else
...
#endif /* _NO_PROTO */
{
  ...
  if (size < 100)
  {
    for (i=0; ((i<XmNUM_ELEMENTS) && (extarray[i].inuse == TRUE)); i++);
    if (i== XmNUM_ELEMENTS)
    {
    ...
    {
       extarray[i].inuse = TRUE;
       return (&extarray[i].data[0]);
    }
  }
  else
     return (XtMalloc(size));
...
#endif /* _NO_PROTO */
{
 ...
 for (i=0; ((i<XmNUM_ELEMENTS) && (&extarray[i].data[0] != element)); i++);
 if (i<XmNUM_ELEMENTS)
  extarray[i].inuse = FALSE;
 else
   XtFree((char *) element);

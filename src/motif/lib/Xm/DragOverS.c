...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DragOverS.c,v $ $Revision: 1.1.4.3 $ $Date: 1993/07/15 16:20:55 $"
#endif
#endif
/*
...
externaldef(xmdragovershellclassrec) XmDragOverShellClassRec xmDragOverShellClassRec = {
#else
...
#endif /* _NO_PROTO */
{
    ...
    XSetForeground (XtDisplay((Widget)dos), draw_gc, 1);
    XSetFunction (XtDisplay((Widget)dos), draw_gc, GXxor);
	...
	XSetFunction (XtDisplay((Widget)dos), draw_gc, GXcopy);
	XSetForeground (XtDisplay((Widget)dos), draw_gc, 
			dos->drag.cursorForeground);
...
#endif /* _NO_PROTO */
{
    switch ((int) icon->drag.attachment) {
		...
		_XmWarning ((Widget) icon, MESSAGE2); /* cast ok here */
			...
			XtSetArg (al[0], XmNsourceWidget, &sourceWidget);
			XtGetValues ((Widget) dc, al, 1);
...
#endif /* _NO_PROTO */
{
	...
	XSetClipMask (display, pixmapGC, None);
	...
	if (icon->drag.mask != XmUNSPECIFIED_PIXMAP) {
	    ...
	    XSetFunction (display, maskGC, GXor);
	    XCopyArea (display, icon->drag.mask,
		       mixedIcon->drag.mask, maskGC,
                       sourceX, sourceY,
		       destWidth, destHeight,
                       destX, destY);
	    XSetClipOrigin (display, pixmapGC, iconX, iconY);
	    XSetClipMask (display, pixmapGC, icon->drag.mask);
	}
        else if (mixedIcon->drag.mask != XmUNSPECIFIED_PIXMAP){

	    XSetFunction (display, maskGC, GXset);
...
#endif /* _NO_PROTO */
{
    ...
    if (mixedIcon &&
	(mixedIcon->drag.depth != depth ||
	 mixedIcon->drag.width != width ||
	 mixedIcon->drag.height != height)) {

	DestroyMixedIcon (dos, mixedIcon);
	mixedIcon = blendPtr->mixedIcon = NULL;
    }
	...
	if (need_mask) {
	    mask = _XmAllocScratchPixmap (xmScreen, 1, width, height);
	}	
	else {
	...
	blendPtr->gc = XCreateGC (display, pixmap, vmask, &v);
        ...
        XSetClipMask (display, blendPtr->gc, None);
    }

    XSetFunction (display, blendPtr->gc, GXset);
	...
	else if (cursorBlend->gc != blendPtr->gc) {
            XSetClipMask (display, cursorBlend->gc, None);
        }

        XSetFunction (display, cursorBlend->gc, GXclear);
...
#endif /* _NO_PROTO */
{
    ...
    XtSetArg (al[ac], XmNcursorBackground, &bg); ac++;
    switch ((int) dos->drag.cursorState) {

	case XmVALID_DROP_SITE:
	    XtSetArg (al[ac], XmNvalidCursorForeground, &fg);
	    break;

	case XmINVALID_DROP_SITE:
	    XtSetArg (al[ac], XmNinvalidCursorForeground, &fg);
	    break;

	default:
	    _XmWarning ((Widget) dos, MESSAGE3);
	case XmNO_DROP_SITE:
	    XtSetArg (al[ac], XmNnoneCursorForeground, &fg);
	    break;
    }
    ac++;
    XtGetValues ((Widget) dc, al, ac);
	...
	doChange = True;
	dos->drag.cursorBackground = bg;
	dos->drag.cursorForeground = fg;
	XSetForeground (display, dos->drag.rootBlend.gc, fg);
	XSetBackground (display, dos->drag.rootBlend.gc, bg);
    }
    return (doChange);
}
...
#endif /* _NO_PROTO */
{
    ...
    XtSetArg(al[0], XmNblendModel, &blendModel);
    XtGetValues ((Widget) dc, al, 1);
    ...
    if (oldDos->drag.mode != dos->drag.mode && blendModel != XmBLEND_NONE) {
...
#endif /* _NO_PROTO */
{
    ...
    XSetFunction(XtDisplay(dos), draw_gc, GXcopy);

    if (icon->drag.mask != XmUNSPECIFIED_PIXMAP) {
	XSetClipMask (XtDisplay((Widget)dos),
		      draw_gc, icon->drag.mask);
	XSetClipOrigin (XtDisplay((Widget)dos), draw_gc, x, y);
	clipped = True;
    }
    else {
        XSetClipMask (XtDisplay((Widget)dos), draw_gc, None);
	...
	XCopyPlane(XtDisplay((Widget)dos),
...
#endif /* _NO_PROTO */
{
	...
	if (mixedIcon->drag.mask != XmUNSPECIFIED_PIXMAP) {
	    ...
	    XSetClipMask (display, draw_gc, root_mask);
	    XSetClipOrigin (display, draw_gc, BackingX(dos), BackingY(dos));
...
#endif /* _NO_PROTO */
{
    ...
    if (dos->drag.mode == XmPIXMAP) {
	XtSetArg(al[0], XmNsourcePixmapIcon, &sourceIcon);
	XtGetValues ((Widget) dc, al, 1);
    } 
	...
	XtSetArg(al[0], XmNsourceCursorIcon, &sourceIcon);
	XtGetValues ((Widget) dc, al, 1);
	...
	case XmBLEND_ALL:
	    ...
	    XtSetArg(al[0], XmNoperationCursorIcon, &opIcon);
	    XtGetValues ((Widget) dc, al, 1);
	...
	case XmBLEND_STATE_SOURCE:
	    ...
	    XtSetArg(al[0], XmNstateCursorIcon, &stateIcon);
	    XtGetValues ((Widget) dc, al, 1);

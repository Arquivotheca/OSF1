...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: DrawingA.c,v $ $Revision: 1.1.6.2 $ $Date: 1993/05/06 15:35:32 $"
#endif
#endif
/*
...
/****************/
    ...
    if(    (new_w->drawing_area.resize_policy != XmRESIZE_SWINDOW)
           && !XmRepTypeValidValue( XmRID_RESIZE_POLICY,
		new_w->drawing_area.resize_policy, (Widget) new_w)  )
    {   new_w->drawing_area.resize_policy = current->drawing_area.resize_policy ;
        } 

    if  (XtClass( new_w) == xmDrawingAreaWidgetClass) {
      /* do the margin enforcement only for pure DA, not subclasses,
      ...
      /* If the new margins are bigger, re-enforce them using setvalue
         ...
         will occur (for all sub-classes as well) */
      if (((new_w->drawing_area.margin_width > 
           ...
           (new_w->drawing_area.margin_height >
            ...
            /* one might also want to deny the change in margin
             if the change in setvalue on the child failed, well
             maybe later... */
            }
        } 
    return( False) ;

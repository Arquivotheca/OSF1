...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Manager.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:47:59 $"
#endif
#endif
/*
...
static XtResource resources[] =
{
   ...
   ,{
     ...
     DXmRLayoutDirection,
     sizeof(unsigned char),
     XtOffset (XmManagerWidget, manager.dxm_layout_direction),
     XmRImmediate,
     (XtPointer) DXmLAYOUT_RIGHT_DOWN
   },
...
#endif /* _NO_PROTO */
{
	...
	*mext = (XmManagerClassExt) XtCalloc(sizeof(XmManagerClassExtRec), 
					     1) ;
...
<Key>osfCancel:BulletinBoardCancel()";
	...
	/* Make sure that we check to see if this is a CSText widget, and NOT 
         * override the translations for the widget, it needs to behave just 
	 * like a XmText widget.
         */
	  if ( strcmp( "DXmCSText",
                    (wid)->core.widget_class->core_class.class_name )==0)
	  return;
#endif
...
#endif /* _NO_PROTO */
{
	...
	if (!_XmIsEventUnique(event))
	   return;
        ...
        {
	    if (gadget = (XmGadget) _XmInputInGadget( (Widget) mw, 
						     event->xkey.x,
						     event->xkey.y))
               _XmDispatchGadgetInput( (Widget) gadget, event, XmHELP_EVENT);
          else

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: PanedW.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:52:41 $"
#endif
#endif
/*
 ...
 ************************************************************/
  ...
  for (i=0; i<*num_args; i++)
    {
        if (strcmp (args[i].name, XmNsashIndent) == 0)
        {
           sash_init_flag = FALSE;
           break;
        }
    }
  if (sash_init_flag)
  {
     /* set default sash indent */
     if (LayoutIsRtoLM(pw))
        pw->paned_window.sash_indent = (Position) DEFAULT_SASH_INDENT_RTOL;
     else
        pw->paned_window.sash_indent = (Position) DEFAULT_SASH_INDENT_LTOR;
  }
#endif /* DEC_MOTIF_RTOL */
...
#endif /* _NO_PROTO */
{
	...
	if (new_major = NeedsAdjusting(pw)) {
...
#ifdef DEC_MOTIF_RTOL
#define __SashIndent(x)  (x->paned_window.sash_indent)

   if (__SashIndent (oldpw) == __SashIndent (newpw))
   {
      if (LayoutM(oldpw) != LayoutM(newpw))
      {
         /* check if default sash indent, if yes switch it
          * according the layout
          */

         if (__SashIndent (oldpw) == (Position) DEFAULT_SASH_INDENT_LTOR ||
             __SashIndent (oldpw) == (Position) DEFAULT_SASH_INDENT_RTOL)
         {
            if (LayoutIsRtoLM(newpw))
               __SashIndent (newpw) = (Position) DEFAULT_SASH_INDENT_RTOL;
            else
               __SashIndent (newpw) = (Position) DEFAULT_SASH_INDENT_LTOR;
         }
      }
    }
#endif /* DEC_MOTIF_RTOL */

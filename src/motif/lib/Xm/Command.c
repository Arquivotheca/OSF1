...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Command.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 16:09:55 $"
#endif
#endif
/*
...
****************/
    ...
    {   
        ...
        {   
            ...
            */
            ...
            DXmCSTextSetString( cmdWid->selection_box.text, (XmString) NULL );
#else
    ...
    {   /* Update the text widget to relect the latest list selection.
        */
        ...
        if(    text_value = _XmStringGetTextConcat( listCB->item)    )
        {   
#ifndef USE_TEXT_IN_DIALOGS
...
#else
#ifdef I18N_EXTENSION
    cb.value = DXmCSTextGetString ( client_data->selection_box.text );
#else
...
#ifdef I18N_EXTENSION
    {
        XmString nullXmString;
        nullXmString = XmStringCreateSimple("");
        tmpString = NULL;
        tmpXmString = DXmCSTextGetString ( w->selection_box.text );
        if ( ( tmpXmString == NULL ) ||
                ( XmStringCompare ( tmpXmString, nullXmString ) == True ) ){
            if ( tmpXmString ) XtFree( tmpXmString );
            XtFree ( nullXmString );
            return;
        }
        XtFree ( nullXmString );
...
#else
#ifdef I18N_EXTENSION
    DXmCSTextSetString ( w->selection_box.text, (XmString)NULL );
#else
...
#ifdef DEC_MOTIF_RTOL
   /*
   ** If the direction layout changed, and the prompt did not change,
   ** then check to see if the prompt is the default prompt, and if so,
   ** flip it.  Use > for left-to-right, < for right-to-left.
   */
   if ( (   LayoutM(old) != LayoutM(new_w)  ) &&
        (   ((XmLabelGadget)(old->selection_box.selection_label))->label._label
         == ((XmLabelGadget)(new_w->selection_box.selection_label))->label._label
      ) )
   {
        XmString            defaultPrompt;
        XmString            currentPrompt;
        XmStringDirection   direction;

        /* Get current prompt */
        XtSetArg( argv[0], XmNpromptString, &currentPrompt);
        XtGetValues( (Widget) old, argv, 1) ;

        /* Setup default prompt for current direction */
        if (LayoutIsRtoLM(old)) {
            defaultPrompt = XmStringLtoRCreate("<", XmFONTLIST_DEFAULT_TAG);
        } else {
            defaultPrompt = XmStringLtoRCreate(">", XmFONTLIST_DEFAULT_TAG);
        }

        if (XmStringCompare(currentPrompt, defaultPrompt))
        {
            /* current prompt is same as default, so flip it */
            XmStringFree(defaultPrompt);
            if (LayoutIsRtoLM(new_w)) {
                defaultPrompt = XmStringLtoRCreate("<",
                                   XmFONTLIST_DEFAULT_TAG);
            ...
            XtSetArg(argv[0], XmNstringDirection, direction);
            XtSetArg(argv[1], XmNlabelString, defaultPrompt);
            XtSetValues(new_w->selection_box.selection_label, argv, 2);
        }
        XmStringFree(currentPrompt);
        XmStringFree(defaultPrompt);
     }
...
/****************/
#ifdef I18N_EXTENSION
    DXmCSTextSetString ( w->selection_box.text, value );
#else
...
#ifdef I18N_EXTENSION
    {
        XmString tmpXmStr, concatXmStr;
        tmpXmStr = DXmCSTextGetString ( w->selection_box.text );
        concatXmStr = XmStringConcat ( tmpXmStr, value );
        DXmCSTextSetString ( w->selection_box.text, concatXmStr );
        XtFree ( tmpXmStr );
        XtFree ( concatXmStr );
    }
#else
...
/****************/
	...
	*/
	com = XtCreateWidget (name, xmCommandWidgetClass, ds, NULL, 0);

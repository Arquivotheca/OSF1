...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: VendorS.c,v $ $Revision: 1.1.4.6 $ $Date: 1993/07/27 18:29:24 $"
#endif
#endif
/*
...
#if defined(OSF1_SHARED) || defined(VMS_SHARED)
externaldef(vendorshellclassrec) VendorShellClassRec _Xm_vendorShellClassRec = {
#else
...
externaldef(vendorshellwidgetclass) WidgetClass
  _Xm_vendorShellWidgetClass = (WidgetClass) (&_Xm_vendorShellClassRec);
#else
externaldef(vendorshellwidgetclass) WidgetClass 
...
#endif /* _NO_PROTO */
{
    ...
    if (secondaryCreate = (*cePtr)->secondaryObjectCreate)
...
#ifdef DEC_MOTIF_EXTENSION
    XtAddEventHandler (new_w, (EventMask) 0, TRUE, _XEditResCheckMessages,
		       (XtPointer) NULL);
#endif
	...
	if (pixmap_name = _XmGetIconPixmapName())
	  {
	    ...
	    vw->wm.wm_hints.icon_pixmap =
	      XmGetPixmap(screen, pixmap_name,
...
#endif /* _NO_PROTO */
{
    ...
    if (ext = _XmGetWidgetExtData(new_w, XmSHELL_EXTENSION)) {
...
#endif /* _NO_PROTO */
{
	...
	ove->focus_policy)
      {
	  int tmp_int = nve->focus_policy ;
	  XtPointer tmp = (XtPointer) tmp_int ;
	  _XmFocusModelChanged( nv->ext.logicalParent, NULL, tmp);
      }
 ...
 *      called during shared library initialization (LIB$INITIALIZE
 *      on VMS and -init on OSF/1).
 *
 *	On OSF/1, this also sets the global i18n_motif_113 flag
 *	declared in Xt/I18N_EN.x to tell the I18n layer that this is
 *	NOT the Motif 1.1.3 shared library.
 ************************************************************************/
#ifdef _NO_PROTO
int _XmOverwriteXtVendorShell(init_coroutine,cli_coroutine,image_info)
        void *  init_coroutine();
        void *  cli_coroutine();
        int *   image_info;
#else
...
{
#ifdef OSF1_SHARED
    extern Boolean i18n_motif_113;

    i18n_motif_113 = FALSE;   /* Tell the I18n layer in Xt that this is not Motif 1.1.3. */
#endif

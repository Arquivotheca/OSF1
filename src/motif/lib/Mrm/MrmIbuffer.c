...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: MrmIbuffer.c,v $ $Revision: 1.1.6.2 $ $Date: 1993/05/06 13:45:19 $"
#endif
#endif
...
/*
 *  If the entire file is mapped as a global section, return the address
 *  of the wanted record.
 */
IDBLowLevelFilePtr      low_level_file_id ; /* low level data structure */
int                     temp ;
static IDBRecordBuffer  localbuf ;
struct  RAB     *rab;                   /* RAB Block pointer            */

low_level_file_id = file_id->lowlevel_id;
rab = (struct RAB *) low_level_file_id->rab;
if (rab == NULL)
{
    temp =      ((record - 1) * IDBRecordSize);
    *buffer_return = &localbuf ;
    localbuf.validation = IDBRecordBufferValid;
    localbuf.IDB_record = low_level_file_id->mapadr.range_start + temp;
    if ( localbuf.IDB_record > low_level_file_id->mapadr.range_end)
         return Urm__UT_Error
             ("Idb__BM_GetRecord", "Invalid record number",

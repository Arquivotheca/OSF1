...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmwcrw.c,v $ $Revision: 1.1.6.6 $ $Date: 1993/12/17 21:19:17 $"
#endif
#endif
 ...
 */
	        ...
	        *val = (int) cstg;
                ...
                /* fix memory leak */
	    	UrmPlistAppendPointer (ftllist, (XtPointer)cvttype);
#ifdef ALPHA_BUG_FIX
		UrmPlistAppendPointer (ftllist, (long)*val);
#else
...
#ifdef DEC_EXTENSION
                /*
                 * Call the instrinsic routine for a font fallback strategy
                 */
                font = (XFontStruct *) _Xt_LoadQueryFont (display, fontstg);
#else
...
#ifdef DEC_MOTIF_BUG_FIX
	  case MrmRtypeFont:
	    /* Do not free the font.
	       Xm does not currently free fonts in general. Fix for CLD
               fetching and destroying widgets. */
	    break;
#endif
	case MrmRtypeFontList:
...
*vec_count = 0;
#ifdef ALPHA_BUG_FIX
bufptr = (XtPointer) UrmRCBuffer (context_id) ;
#else

...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmhier.c,v $ $Revision: 1.1.6.3 $ $Date: 1993/07/15 18:07:09 $"
#endif
#endif
...
/*
 * VMS simply relies on the RMS services with a default file spec, and
 * assumes the language logicals are already set up. The OS extension of
 * IDBOpenFile is used.
 */
char			*uid_spec = ".UID";
char			*dflt_user_spec = "DECW$USER_DEFAULTS:.UID";
char			*dflt_sys_spec = "DECW$SYSTEM_DEFAULTS:.UID";
MrmOsOpenParam		dflt_osext;	/* to set up default */

/*
 * Use the OS extension to specify the extension default. Use the user's 
 * extension if supplied, by copying from it into the default.
 */
dflt_osext.version = MrmOsOpenParamVersion;
dflt_osext.default_fname = uid_spec;
dflt_osext.nam_flg.related_nam = 0;
if ( os_ext != NULL )
    {
    dflt_osext.version = os_ext->version;
    if ( os_ext->default_fname != NULL )
	dflt_osext.default_fname = os_ext->default_fname;
    dflt_osext.nam_flg.related_nam = os_ext->nam_flg.related_nam;
    }

/*
 * First, simply try a file open with the user-supplied parameters.
 */
result = UrmIdbOpenFileRead (name, &dflt_osext, file_id_return, dummy) ;
if ( result == MrmSUCCESS)
    return result;

/*
 * No go there, then try the default user specification.
 */
if ( os_ext == NULL )
    {
    dflt_osext.default_fname = dflt_user_spec;
    }
else
    {
    if ( os_ext->default_fname == NULL )
...
/*
 * Open the file, reporting errors
 */
result = UrmIdbOpenFileRead (name, &dflt_osext, file_id_return, dummy) ;
if ( result == MrmSUCCESS)
    return result;
...
/*
 * No go there, then try the default system specification.
 */
if ( os_ext == NULL )
    {
    dflt_osext.default_fname = dflt_sys_spec;
    }
else
    {
    if ( os_ext->default_fname == NULL )
...
if ( result != MrmSUCCESS )
	...
	case MrmNOT_VALID:
	    sprintf (err_stg,
		     "Couldn't open file %s - UID file version mismatch",
		     name) ;
	    break ;
	default:
	    sprintf (err_stg,
    ...
    return Urm__UT_Error
	("I18NOpenFile", err_stg, NULL, NULL, result) ;
    }
else
    return result;
}

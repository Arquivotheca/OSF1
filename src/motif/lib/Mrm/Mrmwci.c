...
#ifdef REV_INFO
#ifndef lint
static char rcsid[] = "$RCSfile: Mrmwci.c,v $ $Revision: 1.1.6.2 $ $Date: 1993/05/06 13:48:30 $"
#endif
#endif
...
#ifdef DEC_MOTIF_EXTENSION
create_name_len = strlen(create_name) + 1;
cldesc = (WCIClassDescPtr) XtMalloc (sizeof(WCIClassDesc) + create_name_len);
#else
...
#ifdef DEC_MOTIF_EXTENSION
UrmBCopy (create_name, cldesc->creator_name, create_name_len);
#else
...
#define _shift 2      
...
#ifdef DEC_MOTIF_EXTENSION
    UrmBCopy(c_value, (char *)al_value, l_length);
#else
    ...
    /* Since this function returns what is essentially an index */
    /* into an array, it is necessary to make sure the value    */
    /* returned is a positive number.                           */

 ...
 * HISTORY
 */
#include <X11/Xdxm/COPY>
/*
* SCCS_data: @(#) Xdxm.c    1.2 92/06/10 06:15:18
*
* Xdxm - Dec Public Utilities for Wcl - Xdxm.c
*
* This module contains registration routine, and convenience
* callbacks for the Dec widget set.
*
* Module_history:
*                                                  
...
*   08/05/92  J.Ludwig  all       modified for DEC support
*
* Design_notes:
*    
*******************************************************************************
...
void XdxmErrorMsgHandler( name, type, class, defaultMsg, params, numParams )
...
/* Replace the original Xt warning message handler with XdxmErrorMsgHandler
*/
void XdxmReplaceErrorHandler( app )
    XtAppContext app;
{
    if ( OriginalHandler != (XtErrorMsgHandler)0 )
	OriginalHandler = XtAppSetWarningMsgHandler( app, XdxmErrorMsgHandler );
}
...
void OriRegisterDec ( app )
    XtAppContext app;
{
    XdxmRegisterDec(app);
}

void XdxmRegisterAll ( app )
    XtAppContext app;
{
    XdxmRegisterDec(app);
}

void XdxmRegisterDec ( app )
    XtAppContext app;
{
    ...
    XdxmReplaceErrorHandler( app );

#ifdef INCLUDEDEMOS
    registerListDemoCBs(app);

    /* -- register all Dec widget constructors */
    ...
    /* -- register all Dec widget classes */

...
/*
* SCCS_data: @(#) MapAg.c 1.5 92/06/10 06:11:06
*
    ...
    MapAg	agent;
{
    ...
    if ( agent->shiftA  != 0L			)	return 0;
    if ( agent->shiftB  != 0L			)	return 0;
    if ( agent->shiftC  != 0L			)	return 0;
    return 1;
}
...
{
    if ( agent->table   == (Map*)0		 )	return 0;
    if ( 0L != (agent->mask & (agent->mask + 1)) )	return 0;
    if ( agent->numMaps <=  0			 )	return 0;
    ...
    MapAg agent;
{
    ...
    agent->shiftA = agent->shiftB = agent->shiftC = 0L;
}
    ...
    MapAg	agent;
{
    ...
    for ( oldBkt = 0L ; oldBkt < oldSize ; oldBkt++ ) 
    {
...
void MapAg_Forget_Normal( agent, a, b, c )
    ...
    char  *a, *b, *c;
{
    long bkt  = MapAg_HashBucket( agent, a, b, c );

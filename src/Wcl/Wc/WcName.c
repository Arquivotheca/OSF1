...
/*
* SCCS_data: @(#) WcName.c 1.9 92/06/10 06:11:16
*
...
*******************************************************************************
    ...
    relative prefix characters: ^ or ~, which means the reference widget will
    be some relative node in the widget tree, such as the parent or the shell
    ...
    char*  name;
{
	...
	*/
	    ...
	    || name[i] == '.')	/* eaten and ignored */
	{
	    ...
	    */
	    if (!XtParent(refWidget) && (name[i] == '^' || name[i] == '~'))
	    {
		return NULL;
	    }
	    else if (name[i] == '^')
	...
	*/
    ...
    if ( name[0] == '*' )
    {
	/* First see if the name is within this widget tree:
	*/

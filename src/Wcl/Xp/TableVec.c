...
/*LINTLIBRARY*/
/*
 * SCCS_data:    @(#) TableVec.c	1.4 92/06/10 06:14:45
 */
...
**========================**
   Each XpTable instance has two XpTableVectors: one describes the
   columns, and the other describes the rows.

   The table vectors are created based on information in the real_layout
   XpTableLoc array, hence they must be created after the real_layout,
...
{
    int			i;
    Boolean		shrink_simple	= tw->table.shrink_simple;
    XpTableLoc		loc		= tw->table.real_layout;
	...
	*/
	if (  do_col && loc->col_span == 1 )
	{
		...
		pref = XpTableLocPreferredWidth( loc, shrink_simple );
	...
	else if ( !do_col && loc->row_span == 1 )
	{
		...
		pref = XpTableLocPreferredHeight( loc, shrink_simple );

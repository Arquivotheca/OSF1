 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ypxfrd_client.c,v $ $Revision: 4.2.3.2 $ (DEC) $Date: 1992/03/26 15:24:24 $";
#endif
/*
 */
...
#include <ndbm.h>
#if (defined (vax) || defined(i386) || defined(mips) || defined(__alpha))
#define DOSWAB 1
#endif
...
#endif
			errno = 0;
			lseek(db->dbm_pagf, (off_t)(PAG->pag_u.ok.blkno * PBLKSIZ), L_SET);
			if (errno != 0) {
				perror("seek");
				exit(-1);
			}
	...
	XDR            *xdrs;
{
	...
	while (1) {
		if (DIR->status == OK) {
			errno = 0;
			lseek(db->dbm_dirf, (off_t)(DIR->dir_u.ok.blkno * DBLKSIZ), L_SET);
			if (errno != 0) {
				perror("seek");
				exit(-1);
			}

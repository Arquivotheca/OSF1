 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ypxfr.c,v $ $Revision: 4.2.3.4 $ (DEC) $Date: 1993/01/05 15:51:02 $";
#endif
/*
 */
...
bool run_secure = FALSE; /* on SunOS, we'd use issecure() */
...
get_master_addr()
{
	...
	h=gethostbyaddr((char *)&(master_addr.s_addr),sizeof(master_addr.s_addr),AF_INET);
	if (h){
	...
	if (master_prog_vers == YPVERS) {
		...
		while (TRUE) {
			if (strncmp(inkey.dptr,yp_prefix,yp_prefix_sz)) {
				if (dbm_store(db, inkey, inval,0) < 0) {
					logprintf(
	...
	int status;
{
	...
	if (status == YPPUSH_SUCC) {
		...
		execlp("/etc/yp/yppush","yppush","-T",map,0);
		perror("yppush");
		}
#endif
		exit(0);
	...
	int *status;
{
	...
	resp.transid = (unsigned int) atoi(tid);
	...
	if ((domb.dom_client = clntudp_create(&(domb.dom_server_addr),
	    (unsigned int) atoi(proto), YPPUSHVERS, 

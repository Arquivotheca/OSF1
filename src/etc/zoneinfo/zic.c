 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: zic.c,v $ $Revision: 1.1.2.4 $ (DEC) $Date: 1992/12/14 17:08:35 $";
#endif
...
char *	argv[];
{
	...
	/* Add "/usr/bin" to PATH environment variable in order to
	 * locate 'yearistype' script    
	 */
	...
	if ((oldpath = getenv ("PATH")) == (char *) NULL) { 
		...
		if ((newpath = 
		    (char *) malloc (strlen(oldpath) + strlen(":/usr/bin") + 1)) == (char *) NULL) { 
			(void) fprintf(stderr, "%s: Unable to change PATH \n",progname);
			exit(1);
		}
		if (sprintf (newpath,"PATH=%s%s",oldpath,":/usr/bin") < 0) {
			(void) fprintf(stderr, "%s: Unable to change PATH \n",progname);
                	exit(1);
        	}
		if (putenv (newpath) != 0) {

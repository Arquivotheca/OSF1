    ...
    int *auth_datalenp;			/* RETURN */
{
    ...
    int saddrlen;
    char *saddr;
    char *lastp, *p, *dnetptr;		/* char pointers */
     ...
     * second colon.
     */
    if (dnetptr = strstr(display_name, "::")) {	    /* then DECnet format */
	dnet = True;
	Xfree(phostname);
	phostname = copystring (lastp, dnetptr - lastp);
	if (!phostname) goto bad;		    /* no memory */
	p = dnetptr + 1;
    }
...
#ifdef SMT
    /*
     * Specifying "local:0" as the display will cause the
     * client to try to use shared memory to communicate
     * with the server.  Because the display structure
     * is not available here, a procedure is called which
     * does nothing more than set a variable private to
     * the SMT extension.  This variable is tested in
     * XOpenDisplay() after the initial protocol exchange
     * (the earliest point shared memory can be activated.
     */
    else if (strcmp (phostname, "local") == 0) {
    connfunc = MakeUNIXSocketConnection;
    _SmtSetTryExtension(TRUE);
    }
#endif /* SMT */
    if (!connfunc)
	goto bad;
    ...
    Display *dpy;
{
...
#ifdef SMT
        if (dpy->pSmt) {
            if (ipSpaceAvailable(&dpy->pSmt->chan) >= dpy->max_request_size) {
#ifdef SMT_SEM
                dpy->pSmt->pPriv->client.status &= ~SMT_NEED_WAKEUP;
#endif SMT_SEM
                return;
            }
        }
...
#ifdef SMT
        /*
         * Don't select for write on SMT connections, since we're not
         * using the socket to transmit requests
         */
        if (dpy->pSmt) {
            CLEARBITS(w_mask);
        } else {
...
#if defined(SMT) && defined(SMT_SEM)
        /* Tell server we're not going to sleep */
        if (dpy->pSmt) {
            dpy->pSmt->pPriv->client.status &= ~SMT_NEED_WAKEUP;
        }
#endif /* SMT && SMT_SEM */
	...
	if (_XANYSET(r_mask)) {
	    char buf[BUFSIZE];
	    int pend_not_register; /* _XBytesReadable expects int */
	    register long pend;
	    register xEvent *ev;

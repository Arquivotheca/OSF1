...
#ifdef SMT
    /*
     * In the shared-memory transport, requests are not made
     * visible to the server until they are complete, and then
     * they are sent immediately by a hook inserted into
     * SyncHandler().  Therefore, this operation becomes a
     * no-op, unless it detects that there is a request in
     * progress (e.g. if _XFlush() were called from _XReply()).
     * If this is the case, then the request is posted
     * to the server.  If the server is asleep, we need to wake
     * it up.
     */
    if (dpy->pSmt) {
...
#ifdef SMT_SEM
    /*
     * I experimented with two ways of preventing client-server
     * deadlock.  The default method is that the client sends
     * a wakeup request whenever this is any possibility the
     * server may be asleep; and requests the server to send
     * a wakeup event if the client is about to go to sleep
     * (and no reply is expected).  The other method, controlled
     * by SMT_SEM is for the client and the server to keep
     * track of their status in shared variables, access to
     * which is controlled by a mutex.  This reduces traffic
     * down the wire, but incurs the overhead of waiting to
     * gain access to the critical section.  Though both
     * synchronization methods work, the mutex approach (which
     * was implemented first) seemed "choppier", so synchronization
     * over the wire was made the default, and has been tested
     * much more thoroughly at this point.
     */
	...
	register Display *dpy;
{
	...
	do {
	    ...
	    if (BytesReadable(dpy->fd, &pend_not_register) < 0)
	    	_XIOError(dpy);
...
#ifdef SMT
    /* Just copy the data into the shared memory buffer, and flush */
        if (dpy->pSmt) {
            Data(dpy, data, size);
            _XFlush(dpy);
            return;
...
#ifdef SMT
    /*
     * The preceding _XFlush will cause the request needing a reply
     * to be sent to the server.  It is safe to sleep as long as
     * the server is awake: the reply will kick us out of the select.
     * If the server needs a wakeup, however, it will be necessary to
     * send a wakeup request over the wire.  This test must be
     * done within a critical section in order to prevent deadlock.
     */
#ifdef SMT_SEM
    if (dpy->pSmt) {
        register smtDisplayPtr pSmt = dpy->pSmt;

    ...
    _State	    *state;
{
	...
	_CSID	ctid;
	    ...
	    goto _Normal_char;
          case CSI:
            /* not supported yet */
#ifdef DEC /* DEC_BUG_FIX */
	    /* eat some recognized sequences */
	    if (*ct_str == (unsigned char)CSI) {
		if ((*(ct_str+1) == 0x5D) && (ct_bytes >= 2)) {
		    /* CSI 05/13 end of string */
		    ct_str+=2, ct_bytes-= 2;
		    continue;
		} else if ((*(ct_str+1) == 0x31) && (*(ct_str+2) == 0x5D) && 
			   (ct_bytes >= 3)) {
		    /* CSI 03/01 05/13 begin left-to-right text */
		    ct_str+=3, ct_bytes-= 3;
		    continue;
		} else if ((*(ct_str+1) == 0x32) && (*(ct_str+2) == 0x5D) && 
			   (ct_bytes >= 3)) {
		    /* CSI 03/02 05/13 begin right-to-left text */
		    /* NOTE: the text may be backwards..... 
		     * this is really not implemented yet...
		     */
		    ct_str+=3, ct_bytes-= 3;
		    continue;
		}
...
_Normal_char:
		...
		char *esc;
		if(!_XcwIdGetEncoding(xlocale, &esc))
		    Return(BadEncoding);
		strcpy(seq, esc);

...
#ifdef SMT
#define ERRMSG \
"SMT-WARNING: shared-memory not attached, using unix-domain socket.\n"
    dpy->pSmt = NULL;
    if (_SmtSetTryExtension(FALSE)) {
    /*
     * Attempt to attach to the server via shared memory.
     * Only attempt using shared memory for requests for now. XXX
     * If the attach succeeds, dpy->pSmt will be non-null.
     * If it fails, the client will continue using the unix socket.
     * Initially allocate a buffer four times the size of the
     * maximum request.  This is necessary to guarantee when
     * a request is started, in can (contiguously) grow to
     * the maximum possible size.  SMT originally reserves
     * dpy->max_request_size words, then returns the unused
     * portion when the request is dispatched.  The oversize
     * buffer allows several requests to be pending without
     * having to wait for the server to catch up.
     */
        if (_SmtInit(dpy)) {
        int status =
            XSmtAttach(dpy, SMT_REQ, dpy->max_request_size<<2, 0);
        if (!status) fprintf(stderr, ERRMSG);
        }
    }
#undef ERRMSG
#endif SMT

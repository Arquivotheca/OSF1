...
#ifdef I18N_BUG_FIX
    grab->keybut = ( keybut == AnyKey ? _AnyKey : (unsigned short) keybut );
#else
    grab->keybut = keybut;
#endif
    if (need_ext) {
    ...
    register XtServerGrabPtr pFirstGrab, pSecondGrab;
{
	...
	second.pMask = NULL;
#ifdef I18N_BUG_FIX
    if (DetailSupersedesSecond(&first, &second, _AnyKey))
#else
...
#ifdef I18N_BUG_FIX
    if (DetailSupersedesSecond(&secondD, &firstD, _AnyKey) &&
#else
...
#ifdef I18N_BUG_FIX
    if (DetailSupersedesSecond(&firstD, &secondD, _AnyKey) &&
#else
    ...
    XtServerGrabPtr 	pMinuendGrab;
{
	    ...
	    (pDisplay(grab) == pDisplay(pMinuendGrab)))
	{
		...
		*next = grab;
		...
		ext = GRABEXT(grab);
#ifdef I18N_BUG_FIX
	    if ((grab->keybut == _AnyKey) && (grab->modifiers != AnyModifier))
#else
...
#ifdef I18N_BUG_FIX
	    } else if ((pMinuendGrab->keybut != _AnyKey) &&
#else
...
#ifdef I18N_BUG_FIX
    tempGrab.keybut = ( keyOrButton == AnyKey ? _AnyKey : keyOrButton ) ;
#else

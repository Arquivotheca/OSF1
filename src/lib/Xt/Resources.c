...
/*ddh*/	else if	(size == sizeof(int))	    u.intval = (int)src;
...
/*ddh*/	    else if (size == sizeof(int))	*dst = (XtArgVal)u.intval;
...
#ifdef DEC_EXTENSION
void _XtPrintUncompiledResourceList(resources, num_resources)
    XtResourceList  resources;	  
    Cardinal	    num_resources;
{
    register Cardinal count;
    XrmQuark quarked_type;

#define xrmres  ((XrmResourceList) resources)

    for (count = 0; count < num_resources; resources++, count++) {
        printf ("Name = %-31s, Class = %-31s, Type = %-31s,\n    Size = %3d, Offset = %3d, Default Type = %-31s, ",
    		xrmres->xrm_name,
    		xrmres->xrm_class,
    		xrmres->xrm_type,
		xrmres->xrm_size,
	...
	quarked_type = XrmStringToQuark((String) xrmres->xrm_default_type);
	if (quarked_type == QImmediate)
	{
	    quarked_type = XrmStringToQuark((String) xrmres->xrm_type);
	    if (quarked_type == QString)
	        printf ("Default = %s\n", &xrmres->xrm_default_addr);
	    else if (xrmres->xrm_size == 4)
	        printf ("Default = %d\n", xrmres->xrm_default_addr);
	    else if (xrmres->xrm_size == 2)
	    {
	        int value = (short)xrmres->xrm_default_addr;
	        printf ("Default = %d\n", value);
	    }
	    else if (xrmres->xrm_size == 1)
	    {
	        int value = (char)xrmres->xrm_default_addr;
	        printf ("Default = %d\n", value);
	    }
	    else
	        printf ("\n");
	}
	else if (quarked_type == QCallProc)
	    printf ("Default = %x\n", xrmres->xrm_default_addr);
	else if (quarked_type == QString)
	{
	    if (xrmres->xrm_default_addr)
	        printf ("Default = %s\n", xrmres->xrm_default_addr);
	}
	else if (quarked_type == QBoolean)
	{
	    int value = (char)xrmres->xrm_default_addr;
	    printf ("Default = %d\n", value);
	}

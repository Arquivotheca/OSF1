...
#endif
#if defined(DEC_EXTENSION) && defined(VMS)
...
#if defined(DEC_EXTENSION) && defined(VMS)
    /*
    **	Unregister asynchronous event handlers for this widget
    */
    _XtUnregisterAsyncHandlers(widget);
#endif /* DEC_EXTENSION */
...
#include <rms.h>
    ...
    result = XtMalloc(NAM$C_MAXRSS + 1);
    esa = XtMalloc(NAM$C_MAXRSS + 1);
    ...
    else
    {
         ...
         *  From the VAX-RMS notes conference...
         *    For 99.99% combine the existance of ".DIR" in file name
         *    with what you will find if you the look in the FAB:
         *        RAT = BLK, RFM = VAR, MRS = 512, ORG = SEQ
         */
        if (fab.fab$v_blk && fab.fab$b_rfm == FAB$C_VAR &&
            fab.fab$w_mrs == 512 && fab.fab$b_org == FAB$C_SEQ &&
            nam.nam$b_type == 4 && (strncmp(nam.nam$l_type, ".DIR", 4) == 0)
           )
            return_value = FALSE;
    }

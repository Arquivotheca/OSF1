	...
	if (oldMask != mask) 
      ...
      if (asyncHandler != NULL) {
          XSelectAsyncInput(
                  ...
                  asyncHandler, (unsigned long) asyncClosure);
      }
#endif /* DEC_EXTENSION */
    }
    ...
    Opaque closure;
{
    ...
    while (mask = XtPending())
    {
        if (mask & XtIMXEvent)
        {
            ...
            else if (display != event.xany.display)
...
extern void _XtUnregisterAsyncHandlers(widget)
    Widget widget;
{
    if (asyncHandler != NULL)
    {
	/*
	**  This uses an undocumented feature of XSelectAsyncInput.
	**  If a NULL async procedure and a -1 event mask is passed,
	**  the async procedure is cancelled for the window for all
	**  events.
	*/
	XSelectAsyncInput(
	    XtDisplay(widget), XtWindow(widget), (EventMask) -1,
	    (XtAsyncHandler) NULL, (unsigned long) asyncClosure);
    }
}
#endif /* DEC_EXTENSION */

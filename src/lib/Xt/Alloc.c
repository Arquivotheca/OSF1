...
#ifdef VMS
extern char *lib$vm_malloc(), *lib$vm_realloc(), *lib$vm_calloc();
extern void lib$vm_free();
#define Xmalloc(size)       lib$vm_malloc(size)
#define Xrealloc(ptr, size) lib$vm_realloc(ptr, size)
#define Xcalloc(num, size)  lib$vm_calloc(num, size)
...
#ifdef DEC_EXTENSION
/*
 *  NOTE WELL:  We have XtFree macros defined in IntrinsicI.h and
 *              XUICompat.h!  If Xfree changes, the macros
...
#else
#ifdef DEC_BUG_FIX
    /* really should be long boundary for all cases, not just Alpha */

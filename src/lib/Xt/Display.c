...
#ifdef VMS
        if (XConnectionNumber(d) + 1 > app->fds.nfds) {
            app->fds.nfds = XConnectionNumber(d) + 1;
#else
...
#include <rms.h>
                    ...
                    /*
                     * Call RMS to parse the image name, and use the file name 
                     * as the application name
                     */

                    struct FAB fab;
                    struct NAM nam;
                    char result[NAM$C_MAXRSS];
                    int status;

                    fab = cc$rms_fab;
                    fab.fab$l_fna = argv[0];
                    fab.fab$b_fns = strlen(argv[0]);
                    fab.fab$l_nam = &nam;
                    nam = cc$rms_nam;
                    ...
                    if (!(status & 1))
                        ...
                        applName =
                            strncpy(XtMalloc(nam.nam$b_name + 1),
                                    nam.nam$l_name,
                                    nam.nam$b_name);

			/* terminate the string */
			applName[nam.nam$b_name] = '\0';
		    }
#else

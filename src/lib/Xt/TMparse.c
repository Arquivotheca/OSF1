...
#ifndef VMS
#define DEC_KEYSYM_FALLBACK	/* also defined in Xm/Xm.h */
#endif

#ifdef DEC_KEYSYM_FALLBACK

/* Included from <Xm/Virtkeys.h> */
...
/* Equate DRemove to _Remove */

#define DRemove		0x1000FF00


KeySym _XtR3StringToKeysym(str)
    String str;
    
/*
 *  Match a fixed set of OSF virtual keys.
 *  to simulate R5 Xlib behavior.
 */
{

typedef struct _vkht {
    char*      vk_name;
    KeySym     keysym;
} vkht;

static vkht  virtual_key_r3_hack_table[] = {
    ...
    for (i = 0; i < XtNumber(virtual_key_r3_hack_table); i++)
    {
	if (strcmp(str, virtual_key_r3_hack_table[i].vk_name) == 0)
	    return(virtual_key_r3_hack_table[i].keysym);
    }	

...
typedef struct _GenericGetEvent {
    ...
    WidgetInfo * widgets;
    unsigned short num_entries;		/* number of set values requests. */
} GenericGetEvent, GetResEvent, GetGeomEvent, GetValuesEvent;
...
#define GETVALUES_PROTOCOL_VERSION 409L
...
static void _XEditResPutValueString8();
...
Boolean *cont;
{
	...
	if (!first_time) {
	    extern int _XtInitStringConverters();
	...
	if ((c_event->data.l[3] != CURRENT_PROTOCOL_VERSION) 
		&& (c_event->data.l[3] != GETVALUES_PROTOCOL_VERSION)) {
 ...
 */
    ...
    XtOwnSelection(w, sel, CurrentTime,
		   ConvertReturnCommand, (XtLoseSelectionProc) NULL, 
		   CommandDone);
}
...
#ifdef __alpha
	/* 
	 * This is a hack.  Fit in the top half of the address of the shell
	 * widget onto the 32 bit window ids we receive from the editres
	 * protocol to form the 64 bit addresses we need.
	 */
	else {
	    info->ids[i] &= 0x00000000FFFFFFFF;
	    info->ids[i] |= (unsigned long)globals.base_address & 0xFFFFFFFF00000000;
	}
#endif
    }
    return(TRUE);
...
Widget w;
{
    ...
    XtGetApplicationResources( w, (caddr_t) &globals, resources,
			      XtNumber(resources), (ArgList) NULL, 
			      (Cardinal) 0);
}
...
/************************************************************
 *
 * Procedures for performing GetValues.
 *
 ************************************************************/

/*	Function Name: DoGetValues
 *	Description: Gets the Resources values associated with
 *                   the widgets passed.
 *	Arguments: w - a widget in the tree.
 *                 event - the event that caused this action.
 *                 stream - the protocol stream to add.
...
ProtocolStream * stream;
{
    unsigned int i;
    char * str;
    GetValuesEvent * res_event = (GetValuesEvent *) event;
	...
	if ((str = VerifyWidget(w, &(res_event->widgets[i]))) != NULL) {
	    ...
	    _XEditResPutBool(stream, False); /* no error occured. */
	    ExecuteGetValues(res_event->widgets[i].real_widget,
				stream);
	}
    }
    return(NULL);
...
/*	Function Name: ExecuteGetValues.
 *	Description: Gets the resources for any individual widget.
 *	Arguments: w - the widget to get resources on.
 *                 stream - the protocol stream.
 *	Returns: none.
...
ProtocolStream * stream;
{
	...
	_XEditResPutString8(stream, norm_list[i].resource_class);
	_XEditResPutString8(stream, norm_list[i].resource_type);
	_XEditResPutValueString8(stream, w, &norm_list[i]);
    }
    XtFree((char *) norm_list);
	...
	for ( i = 0; i < (int) num_cons; i++) {
	    ...
	    _XEditResPutString8(stream, cons_list[i].resource_class);
	    _XEditResPutString8(stream, cons_list[i].resource_type);
	    _XEditResPutValueString8(stream, w, &cons_list[i]);
	}
	XtFree((char *) cons_list);
    }
}

static void _XEditResCvtWarningHandler(name, type, class, def, params, num_params)
String name;
String type;
String class;
String def;
...
static void
_XEditResPutValueString8(stream, w, res)
ProtocolStream * stream;
Widget w;
XtResource * res;
{
  long value;
  int size = res->resource_size;
  char * string = "";
  char	buffer[64];
  Arg args[1];
  XrmValue to, from, to_color;

/* unknown type Type = Callback
 * unknown type Type = Screen
 * unknown type Type = Colormap
 * unknown type Type = TranslationTable
 * unknown type Type = AcceleratorTable
 * 
 * don't know that type Type = Pixmap
 * don't know that type Type = FontStruct
 * don't know that type Type = Justify
 * don't know that type Type = Bitmap
 */
...
#endif
  default:
    fprintf(stderr, "_XEditResPutValueString8: bad size %d\n",
	    res->resource_size);
    string = "bad size";
    goto done;
  }
...
/* *****************************************************
  if (!strcmp(XtRShapeStyle, res->resource_type) ||
     (!strcmp(XtRBoolean, res->resource_type))   ||
      (!strcmp(XtRBool, res->resource_type))     ||
      (!strcmp(XtRPosition, res->resource_type)) ||
      (!strcmp(XtRInt, res->resource_type))      ||
      (!strcmp(XtRCardinal, res->resource_type)) ||
      (!strcmp(XtRPixel, res->resource_type))    ||
      (!strcmp(XtRUnsignedChar, res->resource_type))    ||
      (!strcmp(XtRDimension, res->resource_type))) {
    from.size = res->resource_size;
    from.addr = (caddr_t) &value;
    to.addr = NULL;
    to.size = 0;
    XtConvert(w, res->resource_type, &from, XtRString, &to);
    string = to.addr;
    goto done;
  }
  else if (strcmp(XtRString, res->resource_type) == 0) {
    if (value == NULL)
      string = "(null)";
    else
      string = (char *) value;
...
****************************************************** */
	...
	if (0 == strcmp(res->resource_type, XtRPixel))
	    {
	    if (XtConvertAndStore(w, XtRPixel, &from, XtRColor, &to))
		{
		to_color.addr = NULL;
		to_color.size = 0;
		if (XtConvertAndStore(w, XtRColor, &to, XtRString, &to_color))
		    string = to_color.addr;
		}
	    }
	else if (XtConvertAndStore(w, res->resource_type, &from, XtRString, &to))
	    {
	    string = to.addr;
	    /* goto done; */
	    }
...
#ifdef HPWIDGETS
  else if ((!strcmp(XtRLabelLocation, res->resource_type))) {
    static char * strs[] = { "right", "left" };
    
    string = strs[value];
  }
  else if ((!strcmp(XtRRCMode, res->resource_type))) {
    static char * strs[] = { "one_of_many", "n_of_many" };
    
    string = strs[value];
  }
  else if ((!strcmp(XtRLayout, res->resource_type))) {
    static char * strs[] = { "maximize",
			       "minimize",
			       "ignore" };
    
    string = strs[value];
  }
  else if ((!strcmp(XtRLayoutType, res->resource_type))) {
    static char * strs[] = { "requested_columns",
			       "maximum_columns",
			       "maximum_unaligned" };
  ...
  if (string == NULL) {
    fprintf(stderr, "_XEditResPutValueString8: couldn't convert to string\n");
    fprintf(stderr, "Class = %s Type = %s Name = %s\n", res->resource_type,
	    res->resource_name,res->resource_class);
    string = "";
  }
 done:  
  _XEditResPutString8(stream, string);
#ifdef DEBUG

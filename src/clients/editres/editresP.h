...
#define GETVALUES_PROTOCOL_VERSION 409
...
typedef enum {LocalSendWidgetTree, LocalSetValues, LocalFindChild, 
	      ...
	      LocalGetValues, LocalOldSendWidgetTree} ResCommand;
...
typedef struct _WidgetResourceInfo {
    char * name, * class, *type, *value; /* Name, Class, Type and Value of each resource. */
} WidgetResourceInfo;
...
typedef struct _CurrentClient {
    ...
    int protocol_version;	/* protocol version this client understands */
} CurrentClient;
...
typedef struct _TreeInfo {
    ...
    char * res_name, * res_value; /* name and value of resource to be set */
} TreeInfo;

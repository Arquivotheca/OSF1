...
char ** errors;
{
	...
	values = (char **) XtMalloc(sizeof(char *) *
				   (resources->num_normal + 1));
	for (i = 0 ; i < resources->num_normal ; i++) {
	    values[i] = resources->normal[i].value;
	...
	cons_values = (char **) XtMalloc(sizeof(char *) *
					(resources->num_constraint + 1));
	
	for (i = 0 ; i < resources->num_constraint ; i++) {
	    cons_names[i] = resources->constraint[i].name;
	    cons_values[i] = resources->constraint[i].value;
	    }
	cons_names[i] = NULL;
    }
    else
...
WidgetResourceInfo * resource;
{
    ...
    resource->value = res_info->value;
    res_info->value = NULL;	/* Keeps it from being deallocated. */
}
...
WidgetResources * resources;
{
	...
	for (i = 0; i < resources->num_normal; i++) {
	    ...
	    if (resources->normal[i].value != NULL)
		XtFree(resources->normal[i].value);
	}
	...
	for (i = 0; i < resources->num_constraint; i++) {
	    ...
	    if (resources->constraint[i].value != NULL)
		XtFree(resources->constraint[i].value);
...
ProtocolStream * stream;
{
    ...
    sprintf(buf, 
	    "This version of editres uses protocol version %d or %d.\n%s%d",
	    CURRENT_PROTOCOL_VERSION, GETVALUES_PROTOCOL_VERSION,

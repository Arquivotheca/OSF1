...
char * msg;
{
    ...
    if (msg == NULL) 
	msg = "Click the mouse pointer on any client.";
	...
	if ( (global_client.window = GetClientWindow(w, NULL, NULL)) == None) 
	    return;
	global_client.protocol_version = GETVALUES_PROTOCOL_VERSION;
    }
    ...
    client_event.data.l[3] = global_client.protocol_version;
...
ResCommand command;
{
	...
	break;
    case LocalGetValues:
	str = " asking it to get a widget's resource values";
	break;
    case LocalFindChild:
...
int * format;
{
	...
	if (global_client.protocol_version == CURRENT_PROTOCOL_VERSION)
	    strcat(msg, "\nVersion 4 protocol cannot display current values.");
	SetMessage(global_screen_data.info_label, msg);
	return;
    }
    SetMessage(global_screen_data.info_label, error_str);
    XtFree(error_str);
    if ((error_code == ProtocolMismatch) 
	&& (global_client.command == LocalSendWidgetTree)
	&& (global_client.protocol_version == GETVALUES_PROTOCOL_VERSION))
	{
	global_client.protocol_version = CURRENT_PROTOCOL_VERSION;
	SetCommand(w, LocalOldSendWidgetTree, 
			"Trying again with older protocol");
	}
}
...
ProtocolStream * stream;
{
    ...
    case LocalGetValues:
	    ...
	    res_event->type = GetValues;

	    if (!_XEditResGet16(stream, &(res_event->num_entries)))
		goto done;

	    res_event->info = (GetValuesInfo *) 
		                   XtCalloc(sizeof(GetValuesInfo),
					    res_event->num_entries);

	    for (i = 0; i < (int)res_event->num_entries; i++) {
		GetValuesInfo * res_info = res_event->info + i;
		if (!(_XEditResGetWidgetInfo(stream, &(res_info->widgets)) &&
		      _XEditResGetBoolean(stream, &(res_info->error))))
		{
		    goto done;
		...
		if (res_info->error) {
		    ...
		    res_info->res_info = (ValueInfo *) 
			                  XtCalloc(sizeof(ValueInfo),
						   res_info->num_resources);

		    for (j = 0; j < res_info->num_resources; j++) {
			unsigned char temp;
			ValueInfo * info = res_info->res_info + j;
			if (!(_XEditResGetResType(stream, &(temp)) &&
			      _XEditResGetString8(stream, &(info->name)) &&
			      _XEditResGetString8(stream, &(info->class)) &&
			      _XEditResGetString8(stream, &(info->type)) &&
			      _XEditResGetString8(stream, &(info->value))))
			{
			    goto done;
			}
			else
...
Event * event;
{
    ...
    case GetValues:
        {
	  /* GetResourcesEvent and GetValuesEvent are the same struct type */
		...
		for (i = 0; i < get_event->num_entries; i++, info++) {
			...
			if (res_info != NULL) {
				...
				if (res_info->value != NULL)
				    XtFree(res_info->value);
			    }

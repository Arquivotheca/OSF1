...
XtPointer junk, garbage;
{
    ...
    /*
     * If the client understands the current protocol, use GetValues,
     * else fall back to protocol version 4 and use GetResources.
     */
    if (global_client.protocol_version == GETVALUES_PROTOCOL_VERSION)
	SetCommand(global_tree_info->tree_widget, LocalGetValues, NULL);
    else
...
XtPointer node_ptr, junk;
{
    ...
    res_name = GetResourceName(res_box);
    strcat(buf, res_name);
    ...
    /*
     * Find the value of the resource and use it to initialize the text
     * of the value entry text widget.
     */
    temp = NULL;
    for (i = 0; (i < node->resources->num_normal) && (temp == NULL); i++)
	if (0 == strcmp(res_name, node->resources->normal[i].name))
	    temp = node->resources->normal[i].value;
    for (i = 0; (i < node->resources->num_constraint) && (temp == NULL); i++)
	if (0 == strcmp(res_name, node->resources->constraint[i].name))
	    temp = node->resources->constraint[i].value;

    XtSetArg(args[0], XtNstring, temp);
    XtSetValues(res_box->value_wid, args, ONE);
}
...
XtPointer node_ptr, junk;
{
    ...
    /*
     * store the name and value of the resource to be set in the
     * global_tree_info to be used later to update the resource values list
     * of this widget when we receive confirmation that the value was set.
     */

    if (global_tree_info->res_value != NULL)
	XtFree(global_tree_info->res_value);
    global_tree_info->res_value = value;
    global_tree_info->res_name = info.name;

...
static char * anypc 		= "KEYBOARD_PCSTYLE";
static char * decpc 		= "DECKEYBOARD_PCSTYLE";
static char * declk 		= "DECKEYBOARD_LKX01";
static char * declknoesc 	= "DECKEYBOARD_LKX01_NOESCAPE";

#ifdef DEC
static void _DEC_KeyboardType( display, defs)
    Display *display;
    register char *defs;
{
    Bool decserver;
	...
	if ( decserver ) {
		...
		AddSimpleDef(defs, declk);
	    } else {
		/* PC-style keyboard in standard PC mode */
		AddSimpleDef(defs, decpc);
	    }
	}
    } else {
	if ( decserver ) {
...
#ifdef DEC
    _DEC_KeyboardType(display, defs);
#endif
}

    ...
    XErrorEvent *rep;
{
    ...
    if (rep->request_code == X_SetFontPath)
	{
	fprintf(stderr,"%s:  bad font path.  Possible causes are:\n", progName);
	fprintf(stderr,"    Bad path syntax\n");
	fprintf(stderr,"    Missing fonts.dir in any path component\n");
	fprintf(stderr,"    Path component unreadable by X Server\n");
	}

...
#ifdef __alpha /* This really shouldn't be duplicated here. It's in Xlib.  */
 ...
 */
{
	...
	while (nElements--) {
	    ...
	    fValue = (int)_XcmsGetElement (format, pChar, pCount)
		    / (XcmsFloat)255.0;
	    printf ("\t\t0x%x\t%8.5lf\n", hValue, fValue);
	}
	break;
 ...
 */
{
	...
	for (i = 0; i < 3; i++) {
	    ...
	    for (j = 0; j < 3; j++) {
		printf ("\t%8.5lf", 
			(int)_XcmsGetElement(format, &pChar, &nitems)

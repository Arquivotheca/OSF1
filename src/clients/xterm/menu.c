...
static void do_securekbd(), do_allowsends(), do_visualbell(),
    ...
    do_appcursor(), do_appkeypad(), do_bsisdel(), do_scrollkey(),
    do_scrollttyoutput(), do_allow132(), do_cursesemul(),
    do_marginbell(), do_tekshow(), do_altscreen(), do_softreset(),
    do_hardreset(), do_clearsavedlines(), do_tekmode(), do_vthide(),
    ...
    Cardinal *param_count;      /* 0 or 1 */
{
	...
	FindFontSelection ((char *) NULL, True);
	set_sensitivity (screen->fontMenu,
    ...
    int nentries;
{
    ...
    m = XtCreatePopupShell (name, simpleMenuWidgetClass, toplevelw, 
	                   (ArgList) NULL, 0);
...
void DoSecureKeyboard (time)
    Time time;
{
    do_securekbd (term->screen.mainMenu, (caddr_t) NULL, (caddr_t) NULL);
}
...
static void do_bsisdel (gw, closure, data)
    Widget gw;
    caddr_t closure, data;
{
    register TScreen *screen = &term->screen;

    screen->backspaceIsDelete = !screen->backspaceIsDelete;
    update_bsisdel();
}
...
void HandleBsIsDel(w, event, params, param_count)
    Widget w;
    XEvent *event;
    String *params;
    Cardinal *param_count;
{
    handle_toggle (do_bsisdel, (int) term->screen.backspaceIsDelete,
		   params, *param_count, w, (caddr_t) NULL, (caddr_t) NULL);
}
    ...
    Cardinal *param_count;
{
	...
	switch (params[0][0]) {
	  case 'v': case 'V':
	    if (term->screen.TekEmu) do_vtmode (w, (caddr_t) NULL, (caddr_t) NULL);
	    break;
	  case 't': case 'T':
	    if (!term->screen.TekEmu) do_tekmode (w, (caddr_t) NULL, (caddr_t) NULL);
	    break;
	  default:
	    Bell();
	}
    ...
    Cardinal *param_count;
{
    ...
    if (proc) (*proc) (w, (caddr_t) NULL, (caddr_t) NULL);
    else Bell();
}
...
void HandleTekPage(w, event, params, param_count)
    ...
    Cardinal *param_count;
{
    do_tekpage(w, (caddr_t) NULL, (caddr_t) NULL);
}
...
void HandleTekReset(w, event, params, param_count)
    ...
    Cardinal *param_count;
{
    do_tekreset(w, (caddr_t) NULL, (caddr_t) NULL);
}
...
void HandleTekCopy(w, event, params, param_count)
    ...
    Cardinal *param_count;
{
    do_tekcopy(w, (caddr_t) NULL, (caddr_t) NULL);
}

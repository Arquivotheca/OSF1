...
#define XtNhiliteTrackInhibit "hiliteTrackInhibit"
#define XtNbackspaceIsDelete "backspaceIsDelete"
...
#define XtCHiliteTrackInhibit "HiliteTrackInhibit"
#define XtCBackspaceIsDelete "BackspaceIsDelete"
...
{XtNallowSendEvents, XtCAllowSendEvents, XtRBoolean, sizeof(Boolean),
	...
	XtRBoolean, (caddr_t) &defaultFALSE},
{XtNhiliteTrackInhibit, XtCHiliteTrackInhibit, XtRBoolean, sizeof(Boolean),
	XtOffsetOf(XtermWidgetRec, screen.hiliteTrackInhibit),
	XtRBoolean, (caddr_t) &defaultFALSE},
#ifdef DEC
{XtNbackspaceIsDelete, XtCBackspaceIsDelete, XtRBoolean, sizeof(Boolean),
	XtOffsetOf(XtermWidgetRec, screen.backspaceIsDelete),
	XtRBoolean, (caddr_t) &defaultBSIsDel},
#else
{XtNbackspaceIsDelete, XtCBackspaceIsDelete, XtRBoolean, sizeof(Boolean),
	XtOffsetOf(XtermWidgetRec, screen.backspaceIsDelete),
	XtRBoolean, (caddr_t) &defaultFALSE},
...
static int *parsestate = groundtable;
static int row = 0, col = 0, top = 0, bot =0, scstype = 0;
static unsigned char *cp = NULL;
static void vt_emulate(int c);
...
static void VTparse()
{
	...
	for ( ; ; )
		vt_emulate((int)doinput());
}

static void vt_emulate(c)
int c;
{
...
/* 
 * read from the pty, placing the chars in the input buffer
 */
static int pty_read(fd)
int fd;
{
...
in_put()
{
    ...
    for( ; ; ) {
	if((bcnt>0) && (select_mask & pty_mask))
		break;
...
void flush_input()
{
    ...
    register int i;
    static struct timeval select_timeout;
    /* should have a select here.... */
    pty_read(screen->respond);
    for(;bcnt-->0;)
	vt_emulate(*bptr++);
}
    ...
    void (*func)();
{
	...
	for (i=0; i<nparam; ++i) {
		...
		case 1001:		/* xterm sequence w/hilite tracking */
			if (screen->hiliteTrackInhibit)
			    {
			    fprintf(stderr, 
			    "\n%s: Mouse Hilite Tracking disabled by hiliteTrackInhibit resource.\n", xterm_name);
			    break;
			    }
			if(func == bitset)
   ...
   Cardinal *num_args;
{
   ...
   new->screen.hiliteTrackInhibit = request->screen.hiliteTrackInhibit;
   new->screen.backspaceIsDelete = request->screen.backspaceIsDelete;
    ...
    XSetWindowAttributes *values;
{
	...
	if (!LoadNewFont(screen, term->misc.f_n, term->misc.f_b, False, 0)) {
		...
		(void) LoadNewFont (screen, "fixed", (char *) NULL, False, 0);
    ...
    Cardinal *param_count;
{
    ...
    XtGetSubresources( w, (XtPointer)&keymap, mapName, mapClass,
		       key_resources, (Cardinal)1, (ArgList) NULL, (Cardinal)0 );
    if (keymap != NULL)
    ...
    int *format;
{
	...
	if (len > 1000  ||  strchr(val, '\n'))
	    return;
	if (!LoadNewFont (&term->screen, val, (char *) NULL, True, 
		fontMenu_fontsel))
	    Bell();
    }
}
    ...
    Bool justprobe;
{
	...
	*(pAtom = &atoms[atomCount++]) = XmuMakeAtom(atom_name);
	...
	XtGetSelectionValue((Widget)term, target, XA_STRING,
			    DoSetSelectedFont, (XtPointer) NULL,

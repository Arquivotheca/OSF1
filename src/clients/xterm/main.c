...
#ifdef USE_SYSV_UTMP
#if (!defined(SVR4) && !defined(__osf__)) /* otherwise declared in utmp.h */
...
#if ! ( defined(SYSV386) || defined(__osf__) )	/* could remove paragraph unconditionally? */
extern struct passwd *getpwent();	/* not used in main.c ! */
extern struct passwd *getpwuid();
extern struct passwd *getpwnam();	/* not used in main.c ! */
extern void setpwent();
extern void endpwent();	/* not used in main.c ! */
#endif
...
{"-hi",		"*hiliteTrackInhibit",	XrmoptionNoArg,	(caddr_t) "on"},
{"+hi",		"*hiliteTrackInhibit",	XrmoptionNoArg,	(caddr_t) "off"},
{"-bs",		"*backspaceIsDelete",	XrmoptionNoArg,	(caddr_t) "on"},
{"+bs",		"*backspaceIsDelete",	XrmoptionNoArg,	(caddr_t) "off"},
...
{ "-/+hi",                 "turn on/off hilite tracking inhibit" },
...
char **argv;
{
	...
	*/
...
#ifdef DEC
	/*
	 * Determine an appropriate default value for the BackspaceIsDelete
	 * resource.
	 */
	display = XtDisplay(toplevel);
	defaultBSIsDel = False;
	...
	if (((strncmp(ServerVendor(display), "DECWINDOWS", 10) == 0) ||
	   ...
	   && (( XKeysymToKeycode(display, XK_Scroll_Lock) != NoSymbol )
	     ...
	     * things).
	     */
	    && (  ( XKeysymToKeycode(display, XK_Find) == NoSymbol )
	...
	XtRegisterGrabAction (HandlePopupMenu, True,
	    ...
	    "vt100", xtermWidgetClass, toplevel, (ArgList) NULL, 0);
...
/*
 * Disable utmp logging if not the login shell.
 */
	if (!term->misc.login_shell)
	    {
	    resource.utmpInhibit = TRUE;
	    }
/*
...
/*
 *  This was moved before get_terminal() and spawn() and is modified to
 *  return a status value so that an error message can be displayed if
 *  opening the log fails.
 *
 * #ifdef ALLOWLOGGING
 *	if (term->misc.log_on) {
 *		logstat = StartLog(screen);
...
#define PTYCHARLEN 2
		...
		signal(SIGHUP, SIG_DFL);
#ifdef ALLOWLOGGING
		if ((term->misc.log_on) && (logstat == -1))
		    fprintf(stdout, "%s: Can't open log file %s\n",

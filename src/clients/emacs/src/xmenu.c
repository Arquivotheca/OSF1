...
/* $Source: /usr/sde/x11/rcs/x11/src/./clients/emacs/src/xmenu.c,v $
 * $Author: devrcs $
 * $Locker:  $
 * $Header: /usr/sde/x11/rcs/x11/src/./clients/emacs/src/xmenu.c,v 1.2 92/08/25 17:36:57 devrcs Exp $
 *
 */

#ifndef lint
static char *rcsid_GXMenu_c = "$Header: /usr/sde/x11/rcs/x11/src/./clients/emacs/src/xmenu.c,v 1.2 92/08/25 17:36:57 devrcs Exp $";
#endif	lint
...
#ifdef XDEBUG
       printf ("list_of_panes check tail, i=%d\n", i);
#endif
       item1 = Fcar(Fcar(tail));
       CHECK_STRING (item1, 1);
#ifdef XDEBUG
       printf ("list_of_panes check pane, i=%d%s\n", i,
		XSTRING(item1)->data);
#endif
  ...
  for (i=0,tail = pane; !NULL (tail); tail = Fcdr (tail), i++)
    {
       ...
       if (XTYPE (item) != Lisp_Cons) (void) wrong_type_argument(Qlistp, item);
#ifdef XDEBUG
       printf ("list_of_items check tail, i=%d\n", i);
#endif
       (*vector)[i] =  Fcdr (item);
       item1 = Fcar(item);
       CHECK_STRING (item1, 1);
#ifdef XDEBUG
       printf ("list_of_items check item, i=%d%s\n", i,
		XSTRING(item1)->data);
#endif

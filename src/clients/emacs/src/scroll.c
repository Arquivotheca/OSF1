...
void
scrolling_1 (int window_size, int unchanged_at_top, int unchanged_at_bottom,
	     int *draw_cost, int *old_hash, int *new_hash, int free_at_end)
...
/* matrix is of size window_size + 1 on each side.  */
void
calculate_scrolling (struct matrix_elt *matrix, int window_size,
		     int lines_below,
...
#else /* _NO_EMACS_PROTO */ 
int
scrolling_max_lines_saved ( int start, int end, int *oldhash, 
				int *newhash, int *cost)
#endif /* _NO_EMACS_PROTO */
{
...
#else /* _NO_EMACS_PROTO */ 
void
CalcIDCosts (char *ins_line_string, char *multi_ins_string,
...
#else /* _NO_EMACS_PROTO */ 
void
CalcIDCosts1 (char *one_line_string, char *multi_string,
	      char *setup_string, char *cleanup_string,
	      int *costvec, int *ncostvec, int extra)
#endif /* _NO_EMACS_PROTO */
{

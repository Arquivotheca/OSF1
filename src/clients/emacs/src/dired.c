...
#else /* _NO_EMACS_PROTO */
Lisp_Object
file_name_completion (Lisp_Object file, Lisp_Object dirname, 
  ...
  for (passcount = !!all_flag; NULL (bestmatch) && passcount < 2; passcount++)
      ...
      while (1)
	{
	  ...
	  if (!dp->d_ino
	      ...
	      || 0 <= scmp (dp->d_name, (char *)XSTRING (file)->data,
			    XSTRING (file)->size))
	    continue;
	  ...
	  if (passcount || !CONSP (tem))
	      ...
	      else
		{
		  ...
		  matchsize = scmp((char *)p1, (char *)p2, compare);
		  if (matchsize < 0)
		    matchsize = compare;

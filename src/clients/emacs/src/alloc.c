...
/* changed size in make-list from int to lisp_object  PDS */
...
#else
long pure[PURESIZE / sizeof (long)] = {0,};   /* Force it into data space! */
#endif	/* NOT VMS */
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  cons_block->next = (struct cons_block *)0;
  bzero (cons_block->conses, sizeof cons_block->conses);
  cons_block_index = 0;
  cons_free_list = (struct Lisp_Cons *)0;
}
...
#endif /* _NO_EMACS_PROTO */
{
  XFASTINT (ptr->car) = (long) cons_free_list;
  cons_free_list = ptr;
}
#ifdef _NO_EMACS_PROTO
...
/* Strings reside inside of string_blocks.  The entire data of the string,
 both the size and the contents, live in part of the `chars' component of
 a string_block.
...
(((SIZE) + 2 * sizeof (long)) & ~(sizeof (long) - 1))
...
init_strings ()
{
  ...
  current_string_block->next = (struct string_block *)0;
  current_string_block->prev = (struct string_block *)0;
  current_string_block->pos = 0;
  large_string_blocks = (struct string_block *)0;
}
...
#else /* _NO_EMACS_PROTO */
Lisp_Object
make_string ( char *contents, int length )
#endif /* _NO_EMACS_PROTO */
{
  register Lisp_Object val;
...
#else /* _NO_EMACS_PROTO */
Lisp_Object
make_pure_string ( char *data, long length )
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (pureptr + size > PURESIZE)
    error ("MAKE_PURE_STRING : Pure Lisp storage exhausted");
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (pureptr + sizeof (struct Lisp_Cons) > PURESIZE)
    error ("Lisp_Object : Pure Lisp storage exhausted");
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (pureptr + size > PURESIZE)
    error ("MAKE_PURE_VECTOR : Pure Lisp storage exhausted");
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  switch (XTYPE (obj))
    ...
    case Lisp_String:
      return make_pure_string ((char*)XSTRING (obj)->data, XSTRING (obj)->size);
...
#endif /* _NO_EMACS_PROTO */
{
 ...
 loop:
  ...
  switch (XGCTYPE (obj))
	...
	if (ptr->size & MARKBIT)
	...
	else
	  {
		...
		XMARK (*objptr);
	      ...
	      XFASTINT (*objptr) = ptr->size;
	    if ((long)objptr & 1) abort ();
	    ptr->size = (long) objptr & ~MARKBIT;
	    if ((long) objptr & MARKBIT)
	      ptr->size ++;
	  }
      }
      break;
	...
	mark_object ((Lisp_Object *)&ptr->function);
	mark_object ((Lisp_Object *)&ptr->plist);
	ptr = ptr->next;
	if (ptr)
	  {
...
gc_sweep ()
{
  ...
  /* Put all unmarked conses on free list */
  {
	...
	for (i = 0; i < lim; i++)
	  if (!XMARKBIT (cblk->conses[i].car))
	    {
	      XFASTINT (cblk->conses[i].car) = (long) cons_free_list;
	      num_free++;
  ...
  /* Put all unmarked symbols on free list */
  {
    ...
    symbol_free_list = (struct Lisp_Symbol *)0;
	...
	for (i = 0; i < lim; i++)
	  if (!XMARKBIT (sblk->symbols[i].plist))
	    {
	      XFASTINT (sblk->symbols[i].value) = (long) symbol_free_list;
	  ...
	  if (!XMARKBIT (mblk->markers[i].chain))
	    {
	      ...
	      if (tem1->buffer)
		unchain_marker (tem);
	      XFASTINT (mblk->markers[i].chain) = (long) marker_free_list;
	  ...
	  if ((unsigned long) size > STRING_BLOCK_SIZE)
		     ...
		     > max (to_sb->pos, STRING_BLOCK_SIZE))
		{
		 ...
		 and update TO_POS for the space being used.  */
	      ...
	      if (nextstr != newaddr)
		bcopy (nextstr, newaddr, size + 1 + sizeof (long));

...
#else /* _NO_EMACS_PROTO */
void
describe_syntax (Lisp_Object value)
#endif /* _NO_EMACS_PROTO */
{
  register enum syntaxcode code;
  ...
  if (start1)
    insert ((unsigned char *)"1", 1);
  if (start2)
    insert ((unsigned char *)"2", 1);

  if (end1)
    insert ((unsigned char *)"3", 1);
  if (end2)
    insert ((unsigned char *)"4", 1);
...
#else /* _NO_EMACS_PROTO */
struct lisp_parse_state *
scan_sexps_forward (register int from, int end, int targetdepth, 
		    int stopbefore, Lisp_Object oldstate)
#endif /* _NO_EMACS_PROTO */
{
...
DEFUN ("parse-partial-sexp", Fparse_partial_sexp, Sparse_partial_sexp, 2, 5, 0,
  ...
  (Lisp_Object from, Lisp_Object to, Lisp_Object targetdepth, 
   Lisp_Object stopbefore, Lisp_Object oldstate)
#endif /* _NO_EMACS_PROTO */
{
  struct lisp_parse_state state;
  int target;

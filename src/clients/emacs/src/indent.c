...
#endif /* _NO_EMACS_PROTO */
{
  ...
  while (fromcol < col)
    {
      insert ((unsigned char *)"        ", min (8, col - fromcol));
...
compute_motion (from, fromvpos, fromhpos, to, tovpos, tohpos, width, hscroll, tab_offset)
     ...
     Lisp_Object to, tovpos, tohpos;
     register Lisp_Object width;
     Lisp_Object hscroll;
     int  tab_offset;
...
compute_motion (
     Lisp_Object from,
     int fromvpos, int fromhpos,
     Lisp_Object to, Lisp_Object tovpos, Lisp_Object tohpos,
     register Lisp_Object width,
     Lisp_Object hscroll,
     int  tab_offset )
#endif /* _NO_EMACS_PROTO */
{
  ...
  register int c;
  register Lisp_Object tab_width = XFASTINT (current_buffer->tab_width);
  register int ctl_arrow = !NULL (current_buffer->ctl_arrow);
  int selective
...
struct position *
vmotion (
     register int from, register int vtarget, register int width,
     int hscroll,
     Lisp_Object window )
#endif /* _NO_EMACS_PROTO */
{

...
#endif /* _NO_EMACS_PROTO */
{
  ...
  create_process (proc, (char **)new_argv);
...
Fourth arg SERVICE is name of the service desired, or an integer\n\
 specifying a port number to connect to.")
   (Lisp_Object name, Lisp_Object buffer, Lisp_Object host, 
    Lisp_Object service)
#endif /* _NO_EMACS_PROTO */
{
  Lisp_Object proc;
...
{
  if (NULL (proc))
    wait_reading_process_input (-1L, 0L, 0);
  else
    {
      proc = get_process (proc);
      wait_reading_process_input (0L, (long)XPROCESS (proc), 0);
    }
  return Qnil;
}
...
#else /* _NO_EMACS_PROTO */
void
wait_reading_process_input (long time_limit, long read_kbd, int do_display)
#endif /* _NO_EMACS_PROTO */
{
  ...
  while (1)
      ...
      if (update_tick != process_tick && do_display)
	{
	  ...
	  timeout = 0;
#endif /* not HAVE_TIMEVAL */
	  if (select (MAXDESC, (fd_set *)&Atemp, 0, 0, &timeout) <= 0)
	    status_notify ();
	}
      ...
      else
	{
	 ...
	 and cause input available signals to zero out timeout */
      if (read_kbd < 0)
	set_waiting_for_input ((long *)&timeout);
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (!NULL (p->buffer) && !NULL (XBUFFER (p->buffer)->name))
	...
	opoint += nchars;
      ...
      insert ((unsigned char *)chars, nchars);
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  start1 = XINT (start);
  send_process (proc, (char *)&FETCH_CHAR (start1), XINT (end) - XINT (start));
...
process_send_signal (Lisp_Object process, int signo, 
		     Lisp_Object current_group, int nomsg)
#endif /* _NO_EMACS_PROTO */
{
  Lisp_Object proc;
  register struct Lisp_Process *p;
  ...
  if (!NULL (current_group))
	...
	{
	case SIGINT:
	  if (ioctl (XFASTINT (p->infd), TIOCGETC, &c) < 0)
		  break;
...
#ifdef SIGTSTP
	case SIGTSTP:
	  if (ioctl (XFASTINT (p->infd), TIOCGLTC, &lc) < 0)
		  break;
	...
	{
	case SIGINT:
	  if (ioctl (XFASTINT (p->infd), TCGETA, &t) < 0)
		  break;

...
Lisp_Object read_minibuf_unwind ();
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  val = make_string ((char *)BEG_ADDR, Z - BEG);
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  while (1)
      ...
      else
	{
	  ...
	  0 > scmp ((char *)XSTRING (eltstring)->data, (char *)XSTRING (string)->data,
		    XSTRING (string)->size))
	{
	  /* Yes. */
...
scmp (s1, s2, len)
     ...
     int len;
#else /* _NO_EMACS_PROTO */  
scmp (register char *s1, register char *s2, int len)
#endif /* _NO_EMACS_PROTO */
{
  register int l = len;
...
If INITIAL-INPUT is non-nil, insert it in the minibuffer initially.\n\
Case is ignored if ambient value of  completion-ignore-case  is non-nil.")
  (Lisp_Object prompt, Lisp_Object table, Lisp_Object pred, 
	Lisp_Object require_match, Lisp_Object init)
#endif /* _NO_EMACS_PROTO */
{
  Lisp_Object val;
  int count = specpdl_ptr - specpdl;
  ...
  ()
{
...
#else /* Rewritten code */
  {
    ...
    if (i > 0 ||
	0 <= scmp ((char *)buffer_string, (char *)completion_string, 
		   buffer_length))
      {
	/* Set buffer to longest match of buffer tail and completion head. */
	if (i <= 0) i = 1;
	buffer_string += i;
	buffer_length -= i;
	while (0 <= scmp ((char *)buffer_string++, (char *)completion_string, 
			  buffer_length--))
	  i++;
	del_range (1, i + 1);
	SET_PT (ZV);

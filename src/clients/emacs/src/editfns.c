...
init_editfns ()
{
  ...
  Vuser_full_name = make_string ((char *)p, q ? q - p : strlen (p));
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  for (argnum = 0; argnum < nargs; argnum++)
	...
	{
	  str[0] = XINT (tem);
	  insert_before_markers ((unsigned char *)str, 1);
	}
      else if (XTYPE (tem) == Lisp_String)
	{
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (beg < GPT && end > GPT)
    move_gap (beg);
  return make_string ((char *)&FETCH_CHAR (beg), end - beg);
}
  ...
  ()
{
  if (BEGV < GPT && ZV > GPT)
    move_gap (BEGV);
  return make_string ((char *)BEGV_ADDR, ZV - BEGV);
}
...
If optional arg NOUNDO is non-nil, don't record this change for undo\n\
and don't mark the buffer as really changed.")
  (Lisp_Object start, Lisp_Object end, Lisp_Object fromchar,
   Lisp_Object tochar, Lisp_Object noundo )
#endif /* _NO_EMACS_PROTO */
{
  ...
  while (1)
    {
      ...
      doprnt (buf, total + 1, (char *)strings[0], nargs, (char **)(strings + 1));
      if (buf[total - 1] == 0)
	break;
     ...
     char *string1;
{
  ...
  args[3] = arg3;
  args[4] = arg4;
  doprnt (buf, sizeof buf, string1, 5, (char **)args);
#else

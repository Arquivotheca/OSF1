...
If optional fourth arg NOSUFFIX is non-nil, don't try adding\n\
 suffixes .elc or .el to the specified name FILE.\n\
Return t if file exists.")
  (Lisp_Object str, Lisp_Object noerror,
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  *ptr = stream;
  XSET (lispstream, Lisp_Internal_Stream, (long) ptr);
...
#else /* _NO_EMACS_PROTO */
int
openp (Lisp_Object path, Lisp_Object str, char *suffix, 
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  tem = oblookup (obarray, (char *)XSTRING (str)->data, XSTRING (str)->size);
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  hash = hash_string ((unsigned char *)ptr, size);
  hash %= obsize;
...
#else /* _NO_EMACS_PROTO */
void
map_obarray (Lisp_Object obarray, void (*fn) (), Lisp_Object arg)
#endif /* _NO_EMACS_PROTO */
{
  register int i;
  register Lisp_Object tail;

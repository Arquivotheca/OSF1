...
long recent_keys_index;	/* Index for storing next element into recent_keys */
...

/* PDS Lisp_Object recursive_edit_unwind (), command_loop (); */
...
/* Note SIGIO has been undef'd if FIONREAD is missing.  */
	...
	{
#ifdef subprocesses
	  wait_reading_process_input (0, -1L, 1);
#else
/* Note SIGIO has been undef'd if FIONREAD is missing.  */
#ifdef SIGIO
	  if (interrupt_input)
...
#else /* _NO_EMACS_PROTO */
int
read_key_sequence (char * keybuf, int bufsize, 
		   unsigned char *prompt, int nodisplay)
#endif /* _NO_EMACS_PROTO */
{
  register int i;
...
DEFUN ("this-command-keys", Fthis_command_keys, Sthis_command_keys, 0, 0, 0,
  ...
  ()
{
  return make_string ((char *)this_command_keys, this_command_key_count);
}
...
#else /* _NO_EMACS_PROTO */
void
set_waiting_for_input ( long *word_to_clear )
#endif /* _NO_EMACS_PROTO */
{

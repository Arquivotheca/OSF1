...
#else /* _NO_EMACS_PROTO */
int
search_buffer (Lisp_Object string, int pos, int lim, int n, int RE, 
	       register unsigned char *trt)
#endif /* _NO_EMACS_PROTO */
{
  int len = XSTRING (string)->size;
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  val = make_string ((char *)p, len - punct_count + 5 * (word_count - 1) + 4);
...
Optional fourth argument is repeat count--search for successive occurrences.\n\
See also the functions match-beginning and match-end and replace-match.")
  (Lisp_Object string, Lisp_Object bound, Lisp_Object noerror, 
   Lisp_Object count)
#endif /* _NO_EMACS_PROTO */
{

...
/* prototype above char *decode_mode_spec (); */

...
void
message (char *m, long a1, long a2, long a3)
#endif /* _NO_EMACS_PROTO */
{
  if (noninteractive)
  ...
  else if (FROM_KBD)
    {
      ...
      doprnt (message_buf, screen_width, m, 3, (char **)a);
#else
  ...
  if (echo_area_contents || minibuf_level == 0)
    {
      ...
      display_string (XWINDOW (minibuf_window), vpos,
		      echo_area_contents ? (unsigned char *)echo_area_contents : (unsigned char *)"",
		      0, 0, 0L, screen_width);
	...
	for (i = vpos + 1; i < vpos + XWINDOW (minibuf_window)->height; i++)
	  {
	    ...
	    display_string (XWINDOW (minibuf_window), vpos,
			    (unsigned char *)"", 0, 0, 0, screen_width);
	  }
      }
    }
...
redisplay ()
{
  ...
  int must_finish = 0;
  int all_windows;
  register Lisp_Object tlbufpos, tlendpos; /* because zv could be 64 bits - pds */
  struct position pos;
  ...
  if (!all_windows && tlbufpos > 0 && NULL (w->update_mode_line)
      ...
      else
	{
	  pos = *compute_motion (tlbufpos, 0,
				XINT (w->hscroll) ? 1L - XINT (w->hscroll) : 0L,
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  register Lisp_Object width = XFASTINT (w->width) - 1L
    - (XFASTINT (w->width) + XFASTINT (w->left) != screen_width);
  register int startp;
  register Lisp_Object hscroll = XINT (w->hscroll);
  struct position pos;
  int opoint;
  int tem;
      ...
      if (point_vpos < 0)
	{
	  ...
	  pos = *compute_motion (startp, 0,
				((EQ (window, minibuf_window) && startp == 1)
				 ? minibuf_prompt_width : 0L)
				+
				(hscroll ? 1L - hscroll : 0L),
				ZV, height / 2,
				- (1 << (SHORTBITS - 1)),
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  register struct window *w = XWINDOW (window);
  register int height = XFASTINT (w->height) - !EQ (window, minibuf_window);
  Lisp_Object top = XFASTINT (w->top);
  Lisp_Object start = marker_position (w->start);
...
#else /* _NO_EMACS_PROTO */
struct position *
display_text_line ( struct window *w, long start, long vpos, 
     long hpos, int taboffset )
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (w == XWINDOW (minibuf_window) && start == 1
    ...
    {
      if (minibuf_prompt)
	hpos = display_string (w, vpos, (unsigned char *)minibuf_prompt, hpos,
			       !truncate ? '\\' : '$', -1, -1);
...
#endif /* _NO_EMACS_PROTO */
{
 tail_recurse:
  ...
  switch (XTYPE (elt))
	...
	while (hpos < maxendcol && *this)
	  {
		...
		if (c == 'M')
		  ...
		  hpos = display_string (w, vpos,
					 	(unsigned char *)(decode_mode_spec (w, c,
							   spec_width - hpos)),
    ...
    default:
    invalid:
      return (display_string (w, vpos, (unsigned char *)"*invalid*", hpos, 0,
			      minendcol, maxendcol));
    }

 end:
  if (minendcol > hpos)
    hpos = display_string (w, vpos, (unsigned char *)"", hpos, 0, minendcol, -1);
  return hpos;
}

...
display_string (
     ...
     int hpos,
     char truncate,
     Lisp_Object mincol, Lisp_Object maxcol )
#endif /* _NO_EMACS_PROTO */
{
  register int c;
  register unsigned char *p1;
  ...
  {
    register long len = p1 - new_screen->contents[vpos]; 

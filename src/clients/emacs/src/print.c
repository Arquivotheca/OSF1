...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (EQ (printcharfun, Qnil))
    {
      insert ((unsigned char *)ptr, size >= 0 ? size : strlen (ptr));
#ifdef MAX_PRINT_CHARS
      if (max_print)
...
#else /* _NO_EMACS_PROTO */
Lisp_Object
internal_with_output_to_temp_buffer (char * bufname, 
...
#else /* _NO_EMACS_PROTO */
static void
print (Lisp_Object obj, Lisp_Object printcharfun, int escapeflag)
#endif /* _NO_EMACS_PROTO */
{
  char buf[30];
  ...
  switch (XTYPE (obj))
    ...
    case Lisp_String:
      if (!escapeflag)
	strout ((char *)XSTRING (obj)->data, (int)XSTRING (obj)->size, printcharfun);
      else
	{
	  register int i;
      ...
      if (escapeflag)
	{
	  ...
	  strout ((char *)XSTRING (XPROCESS (obj)->name)->data, -1, 
		  printcharfun);
	  PRINTCHAR ('>');
	}
      else
      ...
      else
	{
	  ...
	  strout ((char *)XSTRING (XMARKER (obj)->buffer->name)->data, -1, printcharfun);
	}
      PRINTCHAR ('>');
      break;
#endif /* standalone */

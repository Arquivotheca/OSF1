...
#else /* _NO_EMACS_PROTO */
void
barf_or_query_if_file_exists (Lisp_Object absname, char *querystring,
			      int interactive)
#endif /* _NO_EMACS_PROTO */
{
...
Fourth arg non-nil means give the new file the same last-modified time\n\
that the old one has.  (This works on only some systems.)")
  (Lisp_Object filename, Lisp_Object newname, 
 ...
 */
  ...
  if (XINT (start) != XINT (end))
      ...
      if (XINT (start) < GPT)
	{
	  ...
	  tem = XINT (start);
	  failure = 0 > e_write (desc, (char *)&FETCH_CHAR (tem),
				 min (GPT, end1) - tem);
	  save_errno = errno;
	}
      ...
      if (XINT (end) > GPT && !failure)
	{
	  tem = XINT (start);
	  tem = max (tem, GPT);
	  failure = 0 > e_write (desc, (char *)&FETCH_CHAR (tem), XINT (end) - tem);
	  save_errno = errno;
	}
    }
...
#else /* _NO_EMACS_PROTO */
int
e_write (int desc, register char *addr, register int len)
#endif /* _NO_EMACS_PROTO */
{
  char buf[16 * 1024];
  register char *p, *end;
...
Fourth arg MUSTMATCH non-nil means require existing file's name.\n\
 Non-nil and non-t means also require confirmation after completion.\n\
DIR defaults to current buffer's directory default.")
  (Lisp_Object prompt, Lisp_Object dir, 
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (homedir != 0
    ...
    {
      dir = make_string ((char *)XSTRING (dir)->data + strlen (homedir) - 1,

...
int
scroll_screen_lines (
     long from, long end, long amount )
#endif /* _NO_EMACS_PROTO */
{
  register int i;
  ...
  if (amount > 0)
	...
	ins_del_lines (end, -amount);
      ...
      rotate_vector ((char*)(current_screen->contents + from),
  ...
  if (amount < 0)
	...
	ins_del_lines (end + amount, -amount);
      ...
      rotate_vector ((char *)(current_screen->contents + from + amount),
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  output_chars ((char *)&current_screen->contents[vpos][hpos], 1);
  fflush (stdout);
  ++cursor_hpos;
...
#endif /* _NO_EMACS_PROTO */
{
  ...
  if (!olen)
      ...
      if (nlen > nsp)
	{
	  ...
	  output_chars ((char *)(nbody + nsp), nlen - nsp);
	}
  ...
  if (nlen != tem || olen != tem)
      ...
      if (!endmatch || nlen == olen)
	{
	  ...
	  if (nlen == screen_width)
	    olen = 0;
	  output_chars ((char *)(nbody + nsp + begmatch), nlen - tem);
#ifdef obsolete
/* the following code loses disastrously if tem == nlen.
   ...
   solution found above.  */
	...
	{
	  output_chars ((char *)(nbody + nsp + begmatch), olen - tem);
	  insert_chars ((char *)(nbody + nsp + begmatch + olen - tem),
			nlen - olen);
	  olen = nlen;
	}
      else if (olen > nlen)
...
#endif /* _NO_EMACS_PROTO */
{
  register char *p1 = (char *)str1;
  register char *p2 = (char *)str2;
  while (*p1++ == *p2++);
  return (p1 - (char *)str1 - 1);
}

#ifdef _NO_EMACS_PROTO
...
#else /* _NO_EMACS_PROTO */
void
change_screen_size ( register int newlength,  register int newwidth, 
     register int pretend, register int delayed,  register int force )
#endif /* _NO_EMACS_PROTO */
{
...
#else /* _NO_EMACS_PROTO */
void
change_screen_size_1 ( register int newlength,  register int newwidth, 
     register int pretend, register int force )
#endif /* _NO_EMACS_PROTO */
{
  if ((newlength == 0 || newlength == screen_height)
      && (newwidth == 0 || newwidth == screen_width))
...
#ifdef subprocesses
  wait_reading_process_input (t, 0L, 0);
  ...
  if (XINT (n) > 0)
    {
      ...
      gobble_input ();
#endif				/* SIGIO */
      wait_reading_process_input (XINT (n), 1L, 1);

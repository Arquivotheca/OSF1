...
char	**argv;
{
    ...
    if (debugLevel >= 10)
	nofork_session = 1;
    /* We had to move BecomeDaemon. It looks as though:
     * init called rc3 script
     * rc3 script called S90xdm script
     * S90xdm script called xdm
     * xdm forked
     * xdm continued and so did S90xdm script
     * init finished running rc3 script
     * sh cleaned up all children, killing xdm since xdm had not yet
     *    had time to do store pid and get to the setpgrp in BecomeDaemon
     * So, we move BecomeDaemon here.
     */
    if (debugLevel == 0 && daemonMode)
	BecomeDaemon ();
    /* SUPPRESS 560 */
    ...
    /* Here was BecomeDaemon... */

    /* Clean up any old Authorization files */
    sprintf(cmdbuf, "rm %s/A*", authDir);
    system(cmdbuf);
...
struct display	*d;
{
    ...
    char lang[4];
    char keyboard[8];
    unsigned int lang_keyboard;
    char default_keymap[PATH_MAX];
    char lang_keymap[PATH_MAX];
...
#define KB_LK443        4
	...
	/* Set the default keymap based on the console's language environment variable.
	 */
	lang[0] = '\0';
	if (-1 == getsysinfo(GSI_PROM_ENV, lang, sizeof(lang), 0, "language"))
	    lang[0] = '\0';
	lang_keyboard = lang[0];

	keyboard[0] = '\0';
	if (-1 == getsysinfo(GSI_KEYBOARD, keyboard, sizeof(keyboard), 0, NULL))
	    keyboard[0] = '\0';
	else
	    {
	    if (0 == strcmp("PCXAL", keyboard))
		lang_keyboard |= KB_PCXAL << 8;
	    else if (0 == strcmp("LK201", keyboard))
		lang_keyboard |= KB_LK201 << 8;
	    else if (0 == strcmp("LK401", keyboard))
		lang_keyboard |= KB_LK401 << 8;
	    else if (0 == strcmp("LK421", keyboard))
		lang_keyboard |= KB_LK421 << 8;
	    else if (0 == strcmp("LK443", keyboard))
		lang_keyboard |= KB_LK443 << 8;
	    }

	if (ParseKeymaps(d, lang_keyboard, default_keymap, lang_keymap))
	    if (0 != access(lang_keymap, R_OK))
		LogError("Can't read keymap %s\n", lang_keymap);
	    else
		{
		unlink(default_keymap);
		if (0 != symlink(lang_keymap, default_keymap))
		    LogError("Can't create keymap link %s -> %s\n", default_keymap,
				lang_keymap);
		}
#endif
    ...
    int		    forceReserver;
{
    ...
    {
	if (d->serverPid != -1)
	    TerminateProcess (d->serverPid, SIGHUP);
	d->status = notRunning;
    }
}

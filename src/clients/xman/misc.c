...
/*
 * In OSF/1, man and catman use a different nroff device type (-Tlp) than
 * xman so xman cannot use pre-formatted manpages.
 */
...
/*
 * If we are always formatting manpages on the fly, there is no point
 * in annoying the user with a popup window saying "please stand by".
 * On Alpha/osf the formatting is so fast this is really not necessary;
 * the longest manpages are formatted in under 3 seconds.
 */
#ifdef SAVE_FORMATTED
...
#ifndef SAVE_FORMATTED
     /*
      * delete the temporary file and make no attempt to save it.
      * In OSF/1, man and catman use a different nroff device type (-Tlp)
      * than xman so xman cannot use pre-formatted manpages.
      */

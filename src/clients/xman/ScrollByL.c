 ...
 * $Header: /usr/sde/osf1/rcs/x11/src/clients/xman/ScrollByL.c,v 1.2.6.3 1993/12/21 20:56:07 Peter_Derr Exp $
 *
 ...
 */
    ...
    case BACKSPACE:		/* Backspacing for nroff bolding */
      if ((c[-1] == c[1] && c[1] != BACKSPACE)
      	 && !(c[1] == 'o' && c[2] == BACKSPACE && c[3] == '+'))
	 {	/* overstriking one char */
	...
	*bufp++ = c[1];
	...
	if ((c[-1] == 'o' && c[1] == '+')          /* Nroff bullet */
	    ...
	    || (c[-1] == 'o' && c[1] == 'o' && c[2] == BACKSPACE 
		&& c[3] == '+' && c[4] == BACKSPACE && c[5] == '+'))
	       /* Nroff bullet on osf/1 is o\bo\b+\b+ */
	    {	   /* Nroff bullet on osf/1 */
	  ...
	  h_col += bufp - buf;
	  bufp = buf;
	  *bufp = (char)183; /* bullet */
	  x_loc = DumpText(w, x_loc, y_loc, buf, 1, SYMBOL);
	  h_col++;
	  if (c[-1] == 'o' && c[1] == 'o' && c[2] == BACKSPACE 
		&& c[3] == '+' && c[4] == BACKSPACE && c[5] == '+') 
	    c += 5;
	  else
	    c++;
 ...
 */
  ...
  for (sp_pointer = sp, count = 0; count < length; sp_pointer++,count++) 
    if ( !isupper(*sp_pointer) && !isspace(*sp_pointer) )
      return(0);
/*
 * Could bold if every word in the line is capitalized.
 *
 * for (sp_pointer = sp, count = 0; count < length; sp_pointer++,count++) 
 *   if ( !isupper(*sp_pointer) && !isspace(*sp_pointer) 
 *	&& (count == 0 || isspace(*(sp_pointer-1))))
 *      return(0);
 */
  return(1);
}

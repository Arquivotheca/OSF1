...
*/
/* For Alpha, all the field sizes can be represented in normal C,
 * but the byte order is little endian.
 */

typedef struct raw_mo_header_t {
          unsigned int		rmoh_magic;    /* magic number = 0xdeafbead */
	  ...
	  unsigned short	rmoh_byte_order;
          ...
          unsigned int		rmoh_cpu_type;
          unsigned int		rmoh_cpu_subtype;
	  unsigned int		rmoh_vendor_type;
	  unsigned int		rmoh_flags;
	  unsigned int		rmoh_load_map_cmd_off; 
	  unsigned int		rmoh_first_cmd_off;
	  unsigned int		rmoh_sizeofcmds; /* size in bytes of cmds+map */
	  unsigned int		rmoh_n_load_cmds;
	  unsigned int		rmoh_reserved [2]; /* = 0, for future use */
	} raw_mo_header_t;

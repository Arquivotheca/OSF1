 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: build_delete.c,v $ $Revision: 1.1.2.3 $ (DEC) $Date: 1993/07/13 17:20:47 $";
#endif
/*
**++
**  FACILITY:  [[facility]]
...

{
  /*-insert-code-external-routines-*/
    ...
    if (type)
	reply_routine = moss_send_action_reply;
    else 
	reply_routine = moss_send_delete_reply; 
    ...
    if (status == MAN_C_SUCCESS)
        ...
        return (_error_exit("\n*** Error in delete, class not supported\n", status));
    ...
    status = moi_directive(mom_handle,
			   ...
			   access_control,
			   NULL,			/* Directive OID */
			   action_information,		/* Directive AVL */
			   perform_delete,		/* Directive routine */
			   reply_routine,		/* Reply routine */
			   invoke_id,
			   return_routine,
			   action_type);
...
**      moi_delete_instance
**
**	This routine calls delete_instance with the DELETE flag set.
**
**  FORMAL PARAMETERS:
**

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: build_perform_get.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/04/26 20:58:25 $";
#endif
/*
**++
**  FACILITY:  [[facility]]
...
**	see which attributes (or attribute groups) need to be returned.
**	If any errors occur, a AVL containing the attribute and error is
**	is returned.
**
**  FORMAL PARAMETERS:
**
...

{
    ...
    status = moss_compare_partial_oid( attr_oid, &UNIV_ATTR_DNA, 0, 0, ATTRIBUTES_LENGTH);
    if (status == MAN_C_EQUAL)
        {
	/*
	 * Yes, we want an entire group.
	 */
	status = moss_compare_oid( attr_oid, &UNIV_ALL_STATUS_ATTR_DNA );
    	if (status == MAN_C_EQUAL) 
        {
	    /*-insert-code-list-status-*/
	} else {
	status = moss_compare_oid( attr_oid, &UNIV_ALL_COUNTER_ATTR_DNA );
        if (status == MAN_C_EQUAL)
    	{
    	    /*-insert-code-list-counters-*/
    	} else {
	status = moss_compare_oid( attr_oid, &UNIV_ALL_ID_ATTR_DNA );
	if (status == MAN_C_EQUAL)
	{
            /*-insert-code-list-id-*/
	} else {
	status = moss_compare_oid( attr_oid, &UNIV_ALL_CHAR_ATTR_DNA );
        if (status == MAN_C_EQUAL)
	{
	    /*-insert-code-list-char-*/
	} else {
	status = moss_compare_oid( attr_oid, &UNIV_ALL_ATTR_DNA );
        if (status == MAN_C_EQUAL)
	{
	    status = [[class_prefix]]get_all_attributes(instance, reply_attribute_list);
	} else {
	       status = (man_status) _list_error( 
					attr_oid, 
				    	reply_attribute_list, 
    ...
    {
        status = moss_compare_partial_oid( attr_oid, &[[class_prefix]]attr_prefix_DNA, 0, 0, ATTRIBUTES_LENGTH);
...
**	if all attributes are requested or if a specific attribute or attribute
**	group is requested.
**
**  FORMAL PARAMETERS:
**

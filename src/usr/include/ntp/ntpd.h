 ...
 */
/*
 *	@(#)$RCSfile: ntpd.h,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/05/04 17:41:49 $
 */
/*
 */

/*
 *  	Previous History		ntpd.h,v $
 * Revision 4.1  91/09/19  01:43:47  devrcs
 * Start of AG pool
 * 
 * Revision 3.1  91/06/24  15:07:13  devbld
 * terry>>>Added ntpd.h for NTP
 * 
...
#define	PEER_FL_SELECTED	0x8000	/* actually used by query routine */
	...
	u_int	reach;			/* receive, transmit, clear */
	u_int	valid;			/* packet, transmit, clear */
	u_int	timer;			/* receive, transmit, poll update */
	int	stopwatch;		/* <<local>> for timing */
	/*
	 * first order offsets
	 */
	struct	filter {
	...
	u_int	pkt_sent;		/* <<local>> */
	u_int 	pkt_rcvd;		/* <<local>> */
	u_int	pkt_dropped;		/* <<local>> */
};
...
struct clockinfo {
	...
	int	estdisp;			/* scaled by 1000 */
	int	estdelay;			/* in milliseconds */
	int	estoffset;			/* in milliseconds */
	u_int refid;
	struct l_fixedpt reftime;
	struct info_filter {
		short index;
		short filler;
		int offset[PEER_SHIFT];	/* in milliseconds */
		int delay[PEER_SHIFT];		/* in milliseconds */
	} info_filter;
};

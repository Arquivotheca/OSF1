...
# HISTORY
# 
# @(#)$RCSfile: ld.sh,v $ $Revision: 4.2.8.2 $ (DEC) $Date: 1993/06/09 20:30:09 $ 
#
...
LDINITFL="-init __sys5init_"

for i in $*
do
	if test $i = "-non_shared"
	then
		cmd="$LD $LDINITFL -u _habitat_id -L$SVID2PATH/usr/lib"
		static=1
	fi
done
...
if [ "$static" -ne 1 ]
then
	cmd="$LD -u _habitat_id -L$SVID2PATH/usr/shlib -L$SVID2PATH/usr/lib -L/usr/shlib"
fi

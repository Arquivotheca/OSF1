 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: killall.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 06:58:03 $";
#endif 
/*
 */
...
/* for SVID-2 compatibility, made - in "killall [-]n" and "killall - [-]n"
 * truly optional
 */
...
char *argv[];
{
	...
	if (*argv[1] == '-') {
		...
		else if (isdigit(argv[1][1])){		/* added for SVID-2 */
		signo = atoi(argv[1]+1);
			if (signo < 0 || signo > NSIG) {
			printf(MSGSTR(OUT_OF_RANGE,
			"killall: %s: number out of range\n"), argv[1]);
			exit(1);
			}
		} 

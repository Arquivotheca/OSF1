 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: diff3prog.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 05:21:33 $";
#endif 
/*
 */
/*
...
/*
 *                                                                   
 * diff3 [-exEX3] f1 f2 f3 
 * diff3prog is called by diff3 shell script.
 *
 *  -E,-X	These are similar to -e and -x respectively, but treat 
 *              overlapping chages differently.  The overlapping lines 
 *		from both files will be inserted by the edit script, 
 *		bracketted by "<<<<<<" and ">>>>>>" lines.
 ...
 *              ====.
 *  -3          Produces an edit script to incorporate only changes flagged 
 *              ====3. 
 *
...
#include "diff3_msg.h"
nl_catd	catd;
#define MSGSTR(Num, Str) NLcatgets(catd, MS_DIFF3, Num, Str)
#else
...
char **argv;
{
		...
		case 'x':
			eflag = 1;
                ...
                case 'X':
                        ...
                        break;
		default:
			fprintf(stderr, MSGSTR(UNKFLG, "Unknown flag: '%c'\n"), argv[1][1]);
			exit(1);
		}
		argv++;
		argc--;
	...
	if(argc<6) {
		fprintf(stderr, MSGSTR(ARGCNT, "diff3: arg count\n"));
		exit(1);
	}
        if (oflag) { 
	...
	for(i=0;i<=2;i++)
		if((fp[i] = fopen(argv[i+3],"r")) == NULL) {
			printf(MSGSTR(CANTOPEN, "diff3: can't open %s\n"),
								argv[i+3]);
			exit(1);
		}
	merge(m,n);
...
/* 
 * NAME: getline:
 * FUNCTION: gets the next line of input from file b.
 */
getline(b)
FILE *b;
{
...
/* 
 * NAME: merge
 * FUNCTION: merges the changes in m1 and m2 together
 */
merge(m1,m2)
{
...
trouble()
{
	fprintf(stderr, MSGSTR(LOGERR, "diff3: logic error\n"));
	abort();
}
...
edscript(n)
{
	...
	for(n=n;n>0;n--) {
		...
		for(k=de[n].new.to-de[n].new.from;k>0;k-= j) {
			j = k>BUFSIZ?BUFSIZ:k;
			if(fread((void *)block,(size_t)1,(size_t)j,fp[2])!=j)
				trouble();
			(void)fwrite((void *)block, (size_t)1, (size_t)j, stdout);
		}
                if (!oflag || !overlap[n]) 
                        printf(".\n");
                else {

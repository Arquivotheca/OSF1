 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: init.c,v $ $Revision: 4.2.5.5 $ (DEC) $Date: 1994/01/04 16:19:31 $";
#endif 
/*
 */
/*
...
#include "gated_msg.h" 
extern nl_catd catd;
#define MSGSTR(n,s) NLcatgets(catd,MS_INIT,n,s) 
#else
#define MSGSTR(n,s) s
#endif
 ...
 */
  ...
  for (;;) {
    ...
    if (ifreq_buf == 0) {
      syslog(LOG_ERR, MSGSTR(INIT_1,"init_if: malloc: out of memory\n"));
      quit();
    }
    ...
    if (bufsize > 40 * sizeof(struct ifreq)) {
      syslog(LOG_ERR, MSGSTR(INIT_3,"init_if: more than 39 interfaces\n"));
      quit();
    }
    bufsize <<= 1;
  }
  ...
  for (i = 0; i < n_intf; i++) {
    ...
    if (ifp == 0) {
      syslog(LOG_ERR, MSGSTR(INIT_4,"init_if: malloc: out of memory\n"));
      quit();
    }
    ...
    if (doing_rip && (ifp->int_flags & IFF_POINTOPOINT) && (rip_supplier < 0)) {
      rip_supplier = TRUE;
      TRACE_INT(MSGSTR(INIT_14,"init_if: PointoPoint RIP supplier to: %s\n"), ifp->int_name);
    }
    if (doing_hello && (ifp->int_flags & IFF_POINTOPOINT) && (hello_supplier < 0)) {
      hello_supplier = TRUE;
      TRACE_INT(MSGSTR(INIT_15,"init_if: PointoPoint HELLO supplier to: %s\n"), ifp->int_name);
    }
    /*
     ...
     * Also allow aliases if on same interface.
     */
    for (ifp = ifnet->int_next; ifp != 0; ifp = ifp->int_next) {
	 ...
	 if (strcmp(ifnet->int_name, ifp->int_name) == 0 ) {
             syslog(LOG_ERR, MSGSTR(INIT_112, "init_if: alias found on interface %s\n"),
                 ifnet->int_name);
         }
         else {
             syslog(LOG_ERR, MSGSTR(INIT_16,"init_if: Exit as multiple non PTP"));
             syslog(LOG_ERR, MSGSTR(INIT_17,"interfaces to net of interface %s\n"),
  ...
  if (n_interfaces == 0) {
    syslog(LOG_ERR, MSGSTR(INIT_18,"if_init: Exit no interfaces\n"));
    quit();
  }
  if (doing_rip && (n_interfaces > 1) && (rip_supplier < 0)) {
    rip_supplier = TRUE;
    syslog(LOG_NOTICE, MSGSTR(INIT_19,"if_init: Acting as RIP supplier to our direct nets"));
    TRACE_TRC(MSGSTR(INIT_20,"if_init: Acting as RIP supplier to our direct nets\n"));
  }
  if (doing_hello && (n_interfaces > 1) && (hello_supplier < 0)) {
    hello_supplier = TRUE;
    syslog(LOG_NOTICE, MSGSTR(INIT_21,"if_init: Acting as HELLO supplier to our direct nets"));
    TRACE_TRC(MSGSTR(INIT_22,"if_init: Acting as HELLO supplier to our direct nets\n"));
  }
  free((char *)ifreq_buf);
  (void) close(init_ifs);
}
	...
	FILE *fp;
{
	...
	if (egpngh == NULL) {
        ...
        if (nneigh > 1) {
          if (gd_inet_netof(ngp->ng_addr) != gd_inet_netof(egpngh->ng_addr)) {
            syslog(LOG_ERR, MSGSTR(INIT_32,"init_egpngh: EGP neighbors on different nets"));
            error = TRUE;
          }
        }
#endif SAMENET
  ...
  if (mysystem == 0) {
    syslog(LOG_ERR, MSGSTR(INIT_33,"init_egpngh: autonomous system # 0\n"));
    error = TRUE;
  }
  if ((maxacq == 0) && (nneigh > 0)) {
    maxacq = nneigh;
    TRACE_INT(MSGSTR(INIT_34,"init_egpngh: egpmaxacquire not specified, %d assumed\n"),
              maxacq);
    syslog(LOG_WARNING, MSGSTR(INIT_35,"init_egpngh: egpmaxacquire not specified, %d assumed"),
              maxacq);
  }
  if (maxacq > nneigh) {
    syslog(LOG_ERR, MSGSTR(INIT_36,"init_egpngh: egpmaxacquire = %d > # neighbors = %d\n"),
                         maxacq, nneigh);
    error = TRUE;
  }
  if (nneigh == 0) {
    syslog(LOG_ERR, MSGSTR(INIT_37,"init_egpngh: no EGP neighbors.\n"));
    error = TRUE;
  }
  if (error) {
    syslog(LOG_ERR, MSGSTR(INIT_38,"init_egpngh: %s: initialization error\n"), EGPINITFILE);
    quit();
  } else {
	...
	FILE *fp;
{
	...
	if ((fgets(buf, sizeof(buf), fp) == NULL) || ((option = cap(buf)) == NULL)) {
		syslog(LOG_ERR, MSGSTR(INIT_67,"parse_egpneighbor: missing neighbor name or address"));
		return((struct egpngh *)0);
	}
	if (!getnetorhostname("host", option, &egpnghaddr)) {
		syslog(LOG_ERR, MSGSTR(INIT_69,"parse_egpneighbor: invalid neighbor name or address %s\n"), option);
		return((struct egpngh *)0);
	}
	while ( (option = cap(buf)) != NULL && (*option != '#') ) {
		...
		if ( kp->kw_flags == 0 ) {
			syslog(LOG_ERR,	MSGSTR(INIT_70,"parse_egpneighbor: invalid parameter: %s"),
				option);
			return((struct egpngh *)0);
		}
		flags |= kp->kw_flags;
		switch (kp->kw_flags) {
			...
			case NG_INTF:
				if (((option = cap(buf)) == NULL) || (*option == '#') ) {
					syslog(LOG_ERR, MSGSTR(INIT_77,"parse_egpneighbor: missing interface name or address"));
					return((struct egpngh *)0);
				}
				if (!getnetorhostname("host", option, &intf_addr)) {
					syslog(LOG_ERR, MSGSTR(INIT_79,"parse_egpneighbor: invalid interface name or address %s\n"), option);
					return((struct egpngh *)0);
				}
				if (if_ifwithaddr((struct sockaddr *)&intf_addr) <= (struct interface *)0) {
					syslog(LOG_ERR, MSGSTR(INIT_80,"parse_egpneighbor: invalid interface %s\n"), inet_ntoa(intf_addr.sin_addr));
					TRACE_TRC(MSGSTR(INIT_80,"parse_egpneighbor: invalid interface %s\n"), inet_ntoa(intf_addr.sin_addr));
					return((struct egpngh *)0);
				}
				break;
			case NG_SADDR:
				if (((option = cap(buf)) == NULL) || (*option == '#') ) {
					syslog(LOG_ERR, MSGSTR(INIT_82,"parse_egpneighbor: missing source net name or address"));
					return((struct egpngh *)0);
				}
				if (!getnetorhostname("net", option, &source_net)) {
					syslog(LOG_ERR, MSGSTR(INIT_84,"parse_egpneighbor: invalid source net name or address %s\n"), option);
					return((struct egpngh *)0);
				}
				break;
			case NG_GATEWAY:
				if (((option = cap(buf)) == NULL) || (*option == '#') ) {
					syslog(LOG_ERR, MSGSTR(INIT_85,"parse_egpneighbor: missing gateway name or address"));
					return((struct egpngh *)0);
				}
				if (!getnetorhostname("host", option, &gateway_addr)) {
					syslog(LOG_ERR, MSGSTR(INIT_87,"parse_egpneighbor: invalid gateway name or address %s\n"), option);
					return((struct egpngh *)0);
				}
				if (if_withdst(&gateway_addr) <= (struct interface *)0) {
					syslog(LOG_ERR, MSGSTR(INIT_88,"parse_egpneighbor: invalid gateway %s\n"), inet_ntoa(gateway_addr.sin_addr));
					TRACE_TRC(MSGSTR(INIT_88,"parse_egpneighbor: invalid gateway %s\n"), inet_ntoa(gateway_addr.sin_addr));
					return((struct egpngh *)0);
				}
				break;
		}
	...
	if (ngp == NULL) {
		syslog(LOG_ERR, MSGSTR(INIT_90,"parse_egpneighbor: malloc: out of memory\n"));
		quit();
	}
	ngp->ng_next = NULL;
...
int upper, missing;
{
  ...
  if ( (option = cap(buf)) == NULL ) {
    if (missing) {
      syslog(LOG_ERR, MSGSTR(INIT_91,"%s: missing value for parameter %s"), caller, keyword);
      TRACE_TRC(MSGSTR(INIT_92,"%s: missing value for parameter %s\n"), caller, keyword);
    }
    return(-1);
  } else if ( (sscanf(option, "%d", &temp) != 1) || ( temp < 0 ) || ( temp > upper) ) {
    syslog(LOG_ERR, MSGSTR(INIT_94,"%s: invalid value (%s) for parameter %s"), caller, option, keyword);
    TRACE_TRC(MSGSTR(INIT_95,"%s: invalid value (%s) for parameter %s\n"), caller, option, keyword);
    return(-1);
  } 
  return(temp);
}
	...
	struct sockaddr_in *sin;
{
  if (strcasecmp(type, "net") == 0) {
    ...
    if ((n = gd_inet_isnetwork(name)) == (u_int) -1) {
  ...
  if (strcasecmp(type, "host") == 0) {
    ...
    if ((sin->sin_addr.s_addr = inet_addr(name)) == (u_int) -1) {
...
init_egp()
{
  ...
  for (ngp = egpngh; ngp != NULL; ngp = ngp->ng_next) {
	...
	if ((ifp = if_withdst(&dst) ) != NULL) {
	  ngp->ng_myaddr = sock_inaddr(&ifp->int_addr);
	} else {
	  TRACE_INT(MSGSTR(INIT_98,"init_egp: no direct net to neighbor %s\n"),
	...
	if ((ifp = if_withdst(&dst) ) != NULL) {
	  ...
	  TRACE_INT(MSGSTR(INIT_99,"init_egp: no direct net to neighbor %s's gateway\n"),
			       inet_ntoa(ngp->ng_addr));
	  quit();
	}
      }
...
rip_init()
{
  ...
  if (sp == NULL) {
    syslog(LOG_ERR,MSGSTR(INIT_102,"No service for router available\n"));
    quit();
  }
...
\*/
  ...
  addr.sin_port = ripport = sp->s_port;
...
#endif SO_RCVBUF
    TRACE_TRC(MSGSTR(INIT_105,"get_rip_socket: RIP receive buffer size set to %dK\n"), on/1024);
  }
	...
	int domain, type, protocol;
{
  ...
  if (setsockopt(egpsocks, SOL_SOCKET, SO_RCVBUF, (char *)&value, sizeof(value)) < 0) {
    ...
    TRACE_TRC(MSGSTR(INIT_109,"get_egp_socket: EGP receive buffer size set to %dK\n"), value/1024);
  }
  value = 4*EGPMAXPACKETSIZE;
  if (setsockopt(egpsocks, SOL_SOCKET, SO_SNDBUF, (char *)&value, sizeof(value)) < 0) {
    p_error("get_egp_socket: setsockopt SO_SNDBUF");
  } else {
    TRACE_TRC(MSGSTR(INIT_111,"get_egp_socket: EGP send buffer size set to %dK\n"), value/1024);
#endif SO_RCVBUF
  }
  return (egpsocks);
}

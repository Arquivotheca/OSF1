 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: egp.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/04/20 15:55:49 $";
#endif 
/*
 */
/*
...
#include "gated_msg.h" 
extern nl_catd catd;
#define MSGSTR(n,s) NLcatgets(catd,MS_EGP,n,s) 
#else
#define MSGSTR(n,s) s
#endif
...
egpjob()
{
        ...
        if (gatedtime >= ngp->ng_htime) {
          ...
          if (ngp->ng_reach & NG_DOWN) {
            if (nresponses > NRESPONSES) {      /* down -> up */
             syslog(LOG_WARNING, MSGSTR(EGP_1,"egpjob: EGP neighbor %s state change DOWN -> UP, %d/%d responses received"),
                    inet_ntoa(ngp->ng_addr), nresponses, NCOMMANDS);
             TRACE_EXT(MSGSTR(EGP_2,"egpjob: EGP neighbor %s state change DOWN -> UP, %d/%d responses received at %s"),
            ...
            if (nresponses < NRESPONSES) {  /* up -> down */
             syslog(LOG_WARNING, MSGSTR(EGP_3,"egpjob: EGP neighbor %s state change UP -> DOWN, %d/%d responses received"),
                    inet_ntoa(ngp->ng_addr), nresponses, NCOMMANDS);
             TRACE_EXT(MSGSTR(EGP_4,"egpjob: EGP neighbor %s state change UP -> DOWN, %d/%d responses received at %s"),
                    inet_ntoa(ngp->ng_addr), nresponses, NCOMMANDS, strtime);
              ngp->ng_reach |= NG_DOWN;
#ifndef NSS
              if (maxacq < nneigh ) {
          ...
          if (ngp->ng_reach == BOTH_UP) {
            if (ngp->ng_snpoll < NPOLL) {
              ...
              if (ngp->ng_noupdate <= MAXNOUPDATE)
                ...
                TRACE_EXT(MSGSTR(EGP_5,"egpjob: no valid NR update"));
                TRACE_EXT(MSGSTR(EGP_6," for %d successive new poll"), MAXNOUPDATE);
                TRACE_EXT(MSGSTR(EGP_7," id's\n"));
                egpcease(ngp, UNSPEC);
              }
              break;
            }
  ...
  if (gatedtime >= chkcmdtime) {
    if (chkcmdtime)                 /* ignore initialization */
      egpchkcmd((time_t)gatedtime - chkcmdtime);
...
{
  reg egpng *ngp;
  time_t  cmdtime;                /* time since last check # commands recvd */
  u_int  nmaxcmd;                /* max # commands allowed */
  ...
  for (ngp = egpngh; ngp != NULL; ngp = ngp->ng_next) {
    if (ngp->ng_rcmd > nmaxcmd) {
      ngp->ng_flags |= NG_BAD;
      TRACE_EXT(MSGSTR(EGP_8,"egpjob: recv too many commands (%d) in "), ngp->ng_rcmd);
      TRACE_EXT(MSGSTR(EGP_9,"%d sec from %s\n"), cmdtime, inet_ntoa(ngp->ng_addr));
        ...
        char *pkt;
{
  ...
  if (ip->ip_off & ~IP_DF) {
    TRACE_EXT(MSGSTR(EGP_10,"egpin: recv fragmented pkt from %s\n"), inet_ntoa(ip->ip_src));
    egp_stats.inerrors++;
    return;
  }
  iphlen = ip->ip_hl;
   ...
   */
  ifp = if_withdst(&gateway);
  if (ifp == NULL) {
    TRACE_EXT(MSGSTR(EGP_11,"egpin: gw %s no interface?\n"), inet_ntoa(ip->ip_src));
    syslog(LOG_ERR, MSGSTR(EGP_12,"egpin: gw %s no interface?"), inet_ntoa(ip->ip_src));
    egp_stats.inerrors++;
    return;
  }
  /*
  ...
  if (egp_cksum((char *)egp, egplen) != 0) {
    TRACE_EXT(MSGSTR(EGP_13,"egpin: bad EGP checksum from %s at %s"), inet_ntoa(ip->ip_src), strtime);
    syslog(LOG_WARNING, MSGSTR(EGP_14,"egpin: bad EGP checksum from %s"), inet_ntoa(ip->ip_src));
    egp_stats.inerrors++;
    return;
  }
  if (egplen < EGPLEN) {
    syslog(LOG_WARNING, MSGSTR(EGP_15,"egpin: bad pkt length %d from %s"), egplen, inet_ntoa(ip->ip_src));
    TRACE_EXT(MSGSTR(EGP_16,"egpin: bad pkt length %d from %s at %s"), egplen, inet_ntoa( ip->ip_src), strtime);
    egp_stats.inerrors++;
    return;
  }
  ...
  for (ngp=egpngh; ngp != NULL; ngp = ngp->ng_next) {    /* all legit neigh */
    ...
    if (ngp->ng_addr.s_addr == ip->ip_src.s_addr) {      /* legit neigh */
      if (ngp->ng_myaddr.s_addr != ip->ip_dst.s_addr && egp->egp_type != EGPERR) {
        egpserr(ip->ip_src, ip->ip_dst, egp, egplen, EUNSPEC, (egpng *)0, MSGSTR(EGP_17,"invalid destination address"));
        egp_stats.inerrors++;
        egp_stats.inmsgs--;
        return;
      }
      if (egp->egp_ver != EGPVER) {
        ...
        if (reason != EUVERSION) {
          egpserr(ip->ip_src, ip->ip_dst, egp, egplen, EUVERSION, ngp, MSGSTR(EGP_18,"unsupported Version"));
          egp_stats.inerrors++;
          egp_stats.inmsgs--;
        }
        ngp->ng_flags |= NG_BAD;
      ...
      switch (egp->egp_type) {
	    ...
	    u_int reason = ntohs(((struct egperr *) egp)->ee_rsn);
          syslog(LOG_WARNING,
          MSGSTR(EGP_19,"egpin: error packet (reason %s) from EGP neighbor %s"),
                         reason <= EUMAX ? egp_reasons[reason] : "BAD REASON!",
                                inet_ntoa(ngp->ng_addr));
            TRACE_EXT(MSGSTR(EGP_20,
            "egpin: error packet (reason %) from EGP neighbor %s at %s"),
                        reason <= EUMAX ? egp_reasons[reason] : "BAD REASON!",
                                inet_ntoa(ngp->ng_addr),
                                strtime);
            if (egp->egp_status == DOWN) {
      syslog(LOG_WARNING, MSGSTR(EGP_21,
      "egpin: EGP neighbor %s thinks we are down"), inet_ntoa(ngp->ng_addr));
      TRACE_EXT(MSGSTR(EGP_22,
      "egpin: EGP neighbor %s thinks we are down at %s"),
       inet_ntoa(ngp->ng_addr), strtime);
          ...
          break;
        default:
          egpserr(ip->ip_src, ip->ip_dst, egp, egplen, EBADHEAD, (egpng *)0, MSGSTR(EGP_23,"invalid Type field"));
          return;
      }                               /* end switch type */
      break;
    }                                 /* end if legit */
  ...
  if (egp->egp_ver != EGPVER) {
    ...
    if (reason != EUVERSION) {
      egpserr(ip->ip_src, ip->ip_dst, egp, egplen, EUVERSION, (egpng *)0, MSGSTR(EGP_24,"unsupported Version"));
      egp_stats.inerrors++;
      egp_stats.inmsgs--;
    }
    return;
  ...
  switch (egp->egp_type) {            /* May be legit. neighbor */
    case EGPERR:
      if (tracing & TR_EXT) {
        if (tracing & TR_EGP) {
          printf(MSGSTR(EGP_25,"egpin: recv above error packet\n"));
        } else {
          traceegp(MSGSTR(EGP_26,"egpin: recv error pkt: "), ip->ip_src,ip->ip_dst,egp,egplen);
        }
      }
      break;
    case EGPACQ:        /* Not legit. neighbor */
      TRACE_EXT(MSGSTR(EGP_27,"egpin: ACQUIRE packet from illegit neighbor %s\n"), inet_ntoa(ip->ip_src));
      syslog(LOG_ERR, MSGSTR(EGP_28,"egpin: ACQUIRE packet from illegit neighbor %s at %s"), inet_ntoa(ip->ip_src), strtime);
      egp_stats.inerrors++;
      egp_stats.inmsgs--;
      switch (egp->egp_code) {
        case NAREQ:
        ...
        int     egplen;
{
  ...
  switch (egp->egp_code) {
    case NAREQ:                     /* Neighbor acquisition request */
      ...
      if ( (ngp->ng_flags & NG_ASIN) && (ngp->ng_asin != htons(egp->egp_system)) ) {
        TRACE_EXT(MSGSTR(EGP_29,"egpacq: neighbor %s specified AS %d, we expected %d at %s"),
          inet_ntoa(ngp->ng_addr), htons(egp->egp_system), ngp->ng_asin, strtime);
        syslog(LOG_ERR, MSGSTR(EGP_30,"egpacq: neighbor %s specified AS %d, we expected %d"),
    ...
    case NAREFUS:                   /* Neighbor acq. refuse */
      if (ngp->ng_state == NGS_DOWN && egprid_h == ngp->ng_sid) {
        syslog(LOG_WARNING, MSGSTR(EGP_33,"egpacq: Neighbor acquisition refuse from EGP neighbor %s"), inet_ntoa(ngp->ng_addr));
        TRACE_EXT(MSGSTR(EGP_34,"egpacq: Neighbor acquisition refuse from EGP neighbor %s at %s"), inet_ntoa(ngp->ng_addr), strtime);
        egpstunacq(ngp);
      } else {
        error = 1;
      }
    ...
    case NACEASE:                   /* Neighbor acq. cease */
      ngp->ng_rcmd++;
      if (ngp->ng_state == NGS_UP) {
        syslog(LOG_WARNING, MSGSTR(EGP_35,"egpacq cease from EGP neighbor %s, status %s"), inet_ntoa(ngp->ng_addr), egp_acq_status[egp->egp_status]);
        TRACE_EXT(MSGSTR(EGP_36,"egpacq: cease from EGP neighbor %s, status %s at %s"), inet_ntoa(ngp->ng_addr), egp_acq_status[egp->egp_status], strtime);
      }
      egpstunacq(ngp);
    ...
    case NACACK:                    /* Neighbor acq. cease ack */
      if (ngp->ng_state == NGS_CEASE && egprid_h == ngp->ng_sid) {
        syslog(LOG_WARNING, MSGSTR(EGP_37,"egpacq cease-ack from EGP neighbor %s"), inet_ntoa(ngp->ng_addr));
        TRACE_EXT(MSGSTR(EGP_38,"egpacq: cease-ack from EGP neighbor %s at %s"), inet_ntoa(ngp->ng_addr), strtime);
        egpstunacq(ngp);
      } else {
        /*
  ...
  if (error && tracing & TR_EXT) {
    if (!(tracing & TR_EGP))
      traceegp(MSGSTR(EGP_39,"egpacq: recv bad pkt: "),
                        ngp->ng_addr, ngp->ng_myaddr, egp, egplen);
    TRACE_EXT(MSGSTR(EGP_40,"egpacq: last recvd pkt bad, recvd in state %d\n"), ngp->ng_state);
  }
  return;
}
        ...
        register     struct  egppkt  *egppkt;
{
  ...
  if (helloint > MAXHELLOINT || pollint > MAXPOLLINT) {
    TRACE_EXT(MSGSTR(EGP_41,"egpsetint: Hello interval = %d or poll interval = %d too big from %s, code %d at %s"),
                        helloint, pollint, inet_ntoa(ngp->ng_addr), egpa->ea_pkt.egp_code, strtime);
    syslog(LOG_WARNING, MSGSTR(EGP_42,"egpsetint: Hello interval = %d or poll interval = %d too big from %s, code %d"),
                        helloint, pollint, inet_ntoa(ngp->ng_addr), egpa->ea_pkt.egp_code);
    return(1);
  }
  if ((helloint != MINHELLOINT) || (pollint != MINPOLLINT))
    syslog(LOG_WARNING, MSGSTR(EGP_43,"egpsetint: EGP neighbor %s specified hello/poll intervals %d/%d, we specified %d/%d"),
           inet_ntoa(ngp->ng_addr), helloint, pollint, MINHELLOINT, MINPOLLINT);
    TRACE_EXT(MSGSTR(EGP_44,"egpsetint: EGP neighbor %s specified hello/poll intervals %d/%d, we specified %d/%d at %s"),
  ...
  syslog(LOG_WARNING, MSGSTR(EGP_45,"egpsetint: Using actual intervals %d/%d with EGP neighbor %s"),
         helloint, pollint, inet_ntoa(ngp->ng_addr));
  TRACE_EXT(MSGSTR(EGP_46,"egpsetint: Using actual intervals %d/%d with EGP neighbor %s at %s\n"),
  ...
  if (helloint < egpsleep)
      egpsleep = helloint;
#ifdef debug
  printf(MSGSTR(EGP_47,"egpsetint: intervals hello %d, poll %d, egpsleep %d\n"),
        ...
        register     egpkt   *egp;
{
  ...
  if (ngp->ng_state != NGS_UP) {
    n_acquired++;
    TRACE_EXT(MSGSTR(EGP_48,"egpstngh: acquired EGP neighbor %s at %s"), inet_ntoa(ngp->ng_addr), strtime);
    syslog(LOG_WARNING,MSGSTR(EGP_49,"egpstngh: acquired EGP neighbor %s"), inet_ntoa(ngp->ng_addr));
  }
  ngp->ng_state = NGS_UP;
  ...
  rt_area_add(htons(egp->egp_system), ngp->ng_myaddr);  /* Add to directly connAS */
...
egpstime()
{
  ...
  if (egpsleep < MINHELLOINT) {
    TRACE_EXT(MSGSTR(EGP_50,"egpstime: bad sleep time %d\n"), egpsleep);
        ...
        register     egpkt   *egp;
{
  ...
  if (ngp->ng_state != NGS_UP) {
    ...
    if (egp->egp_status == DOWN) {
      syslog(LOG_WARNING, MSGSTR(EGP_51,"egphello: EGP neighbor %s thinks we are down"), inet_ntoa(ngp->ng_addr));
      TRACE_EXT(MSGSTR(EGP_52,"egphello: EGP neighbor %s thinks we are down at %s"), inet_ntoa(ngp->ng_addr), strtime);
      ngp->ng_reach |= ME_DOWN;
    } else
      ngp->ng_reach &= ~ME_DOWN;
  ...
  if (error && tracing & TR_EXT) {
    if (!(tracing & TR_EGP))
      traceegp(MSGSTR(EGP_53,"egphello: recv bad pkt: "), ngp->ng_addr,
                        ngp->ng_myaddr, egp, sizeof(struct egppkt));
    printf(MSGSTR(EGP_54,"egphello: last recvd pkt bad, recvd in state %d\n"), ngp->ng_state);
  }
  return;
}
        ...
        register     struct  egppkt  *egp;
{
  ...
  if (ngp->ng_state != NGS_UP) {
    ...
    error = 100;
      ...
      error = EBADHEAD;
      msg = MSGSTR(EGP_55,"invalid Code field in NR Poll");
      ...
      error = ENOREACH;
      msg = MSGSTR(EGP_56,"IP Net Address field of NR Poll not expected");
#if     !SAMENET
    ...
    error = ENOREACH;
    msg = MSGSTR(EGP_58,"IP Net Address field of NR Poll not a directly connected network");
  } else if (ngp->ng_reach & NG_DOWN) {
      error = EUNSPEC;
      msg = MSGSTR(EGP_59,"NR Poll received while neighbor substate is down");
  } else {
    if (gatedtime < ngp->ng_rtime) {        /* my NR may be lost*/
      if (egprid_h != ngp->ng_rid || ++ngp->ng_rnpoll > NPOLL)
        error = EXSPOLL;
        msg = MSGSTR(EGP_60,"too many NR Polls received");
    } else {
  ...
  if (error == NOERROR) {
    ...
    if (error < 100) {
      ...
      if (tracing & TR_EXT) {
        if (!(tracing & TR_EGP)) {
          traceegp(MSGSTR(EGP_61,"egppoll: recv bad pkt: "), ngp->ng_addr, ngp->ng_myaddr, egp, sizeof(struct egppoll));
        }
        printf(MSGSTR(EGP_62,"egppoll: last recvd pkt bad, recvd in state %d\n"), ngp->ng_state);
      }
    }
  }
  return;
        ...
        int     egplen;
{
  ...
  if (ngp->ng_state != NGS_UP) {
    ...
    if ((error = rt_NRupdate(ngp, egp, egplen)) > NOERROR) {
      switch (error) {
        case EBADDATA:
          msg = MSGSTR(EGP_66,"invalid NR Response message format");
          break;
        default:
          msg = MSGSTR(EGP_67,"internal error parsing NR Response");
      }
    } else {
      ngp->ng_noupdate = 0;
    }
  ...
  if (error != NOERROR) {
    if (error < 100) {
      ...
      if (tracing & TR_EXT) {
        if (!(tracing & TR_EGP)) {
          traceegp(MSGSTR(EGP_68,"egpnr: recv bad pkt: "), ngp->ng_addr, ngp->ng_myaddr, egp, egplen);
        }
        printf(MSGSTR(EGP_69,"egpnr: last recvd pkt bad, recvd in state %d\n"), ngp->ng_state);
      }
    }
  }
  return;
        ...
        char    *msg;
{
  ...
  if (ngp) {
    TRACE_EXT(MSGSTR(EGP_70,"egpserr: error packet to neighbor %s: %s at %s"), inet_ntoa(ngp->ng_addr), msg, strtime);
    syslog(LOG_WARNING, MSGSTR(EGP_71,"egpserr: error packet to neighbor %s: %s"), inet_ntoa(ngp->ng_addr), msg);
  } else {
    TRACE_EXT(MSGSTR(EGP_72,"egpserr: error packet to non-neighbor %s: %s at %s"), inet_ntoa(dst), msg, strtime);
    syslog(LOG_WARNING, MSGSTR(EGP_73,"egpserr: error packet to non-neighbor %s: %s"), inet_ntoa(dst), msg);
  }
  if (tracing & TR_EXT) {
    if (tracing & TR_EGP) {
      printf(MSGSTR(EGP_74,"egpserr: send error pkt:\n"));
    } else {
      traceegp(MSGSTR(EGP_75,"egpserr: send error pkt "), src, dst, (struct egppkt *)&errpkt, sizeof(errpkt));
    }
  }
        ...
        u_short id;
{
  ...
  if (nrp == NULL) {
    syslog(LOG_WARNING, MSGSTR(EGP_76,"egpsnr: malloc: out of memory\n"));
    TRACE_TRC(MSGSTR(EGP_77,"egpsnr: malloc: out of memory at %s"), strtime);
    return;
  }
  ...
  if (length != ERROR) {
    ...
    if (length > EGPMAXPACKETSIZE) {
      syslog(LOG_WARNING, MSGSTR(EGP_78,"egpsnr: NR message size (%d) larger than EGPMAXPACKETSIZE (%d)"),
             length, EGPMAXPACKETSIZE);
      TRACE_TRC(MSGSTR(EGP_79,"egpsnr: NR message size (%d) larger than EGPMAXPACKETSIZE (%d) at %s"),
                length, EGPMAXPACKETSIZE, strtime);
    }
    egp_send( ngp->ng_addr, ngp->ng_myaddr, (struct egppkt *)nrp, length);
  } else {
    syslog(LOG_WARNING, MSGSTR(EGP_80,"egpsnr: NR message not sent"));
    TRACE_TRC(MSGSTR(EGP_81,"egpsnr: NR message not sent at %s"), strtime);
  }
  free((char *)nrp);
  return;
}
 ...
 
{
  ...
  ifp = if_ifwithaddr((struct sockaddr *)&egpsaddr);
  if (ifp == NULL) {
    syslog(LOG_ERR, MSGSTR(EGP_82,"egp_send: no interface for source address %s"), inet_ntoa(src));
    TRACE_INT(MSGSTR(EGP_83,"egp_send: no interface for source address %s at %s"), inet_ntoa(src), strtime);
    error = TRUE;
  } else {
    ...
    if (!dontroute && (!(rt = rt_find(&egpsaddr)) || rt->rt_ifp != ifp)) {
      char *string = MSGSTR(EGP_84,"egp_send: improper route sending %d bytes to %s\n");
...
#endif /* NSS */
    {
      char *string = MSGSTR(EGP_85,"egp_send: sendto error sending %d bytes to %s: %s\n");
...
SIGTYPE egpallcease()
{
  ...
  syslog(LOG_WARNING, MSGSTR(EGP_86,"egpallcease: Terminate signal received"));
  TRACE_TRC(MSGSTR(EGP_87,"egpallcease: Terminate signal received at %s\n"), strtime);
  /*
  ...
  if (ceasesent) {
    terminate = TRUE;
    syslog(LOG_WARNING, MSGSTR(EGP_88,"egpallcease: Shutting down EGP"));
    TRACE_TRC(MSGSTR(EGP_89,"egpallcease: Shutting down EGP at %s\n"), strtime);
  } else {
    quit();
  }
}
...
struct egpngh *down_ngp;
{
  ...
  TRACE_EXT(MSGSTR(EGP_90,"egpcheckas: lost all neighbors to AS %d\n"), down_ngp->ng_asin);
#ifdef  NSS
...
struct egpngh *ngp;
{
  ...
  if (changes) {
    ...
    egp2esrec();
#endif  /* NSS */
  TRACE_RT(MSGSTR(EGP_91,"rt_NRupdate: above changes due to loss of neighbor %s at %s"),
                      inet_ntoa(ngp->ng_addr), strtime);
  }
}

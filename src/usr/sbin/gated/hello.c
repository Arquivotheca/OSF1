 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: hello.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/04/20 15:56:24 $";
#endif 
/*
 */
/*
...
#include "gated_msg.h" 
extern nl_catd catd;
#define MSGSTR(n,s) NLcatgets(catd,MS_HELLO,n,s) 
#else
#define MSGSTR(n,s) s
#endif
...
/*** for backward compat ***/
  ...
  if ((ifp = if_withdst(&gateway)) == NULL) {
    TRACE_EXT(MSGSTR(HELLO_1,"helloin: gw %s no shared net?\n"), inet_ntoa(ip->ip_src));
    return;
  }
  ...
  if (cksum == hi->h_cksum) {
    ...
    while (dsize > 0) {
      ...
      switch (mh->mh_type) {
        case 0:
          index += mh->mh_len * sizeof (struct type0pair);
          dsize -= mh->mh_len * sizeof (struct type0pair);
          /* not interested in type 0 messages */
          break;
	case 1:
          t1 = (struct type1pair *)&hellobuf[index];
          for (i = 0; i < mh->mh_len; i++, t1++) {
	 ...
	 */
            ...
            if (tracing & TR_UPDATE) {
              TRACE_HEL(MSGSTR(HELLO_2,"HELLO: %s time delay %d %s\n"),
				inet_ntoa(dst.sin_addr), delay,
				latched ? MSGSTR(HELLO_3,"latched") : "");
            }
            ...
            if (rt == NULL) {	/* new route */
              ...
              if (is_martian(dst.sin_addr)) {
                ...
                TRACE_EXT(MSGSTR(HELLO_5,"helloin: ignoring invalid net %s from %s at %s"), inet_ntoa(dst.sin_addr), badgate, strtime);
                continue;
              }
              /* 
		...
		if (latched)
          ...
          break;
        default:
          syslog(LOG_ERR, MSGSTR(HELLO_6,"helloin: invalid type %d\n"), mh->mh_type);
      } /* switch (mh->mh_type) */
    } /* while dsize */
  }	/* checksum */	
    
  if ( (change && (tracing & TR_RT)) || (tracing & TR_HELLO) ) {
    printf(MSGSTR(HELLO_7,"hello_update: above %d routes from %s updated %s\n"),
                   mh->mh_len,inet_ntoa(ip->ip_src), strtime);
  }
} 
...
hellojob()
{
  ...
  if (!(hello_pointopoint)) {
    for (ifp = ifnet; ifp != 0; ifp = ifp->int_next) {
      TRACE_JOB(MSGSTR(HELLO_8,"hellojob: Checking interface %s\n"), ifp->int_name);
      if (ifp->int_flags & IFF_PASSIVE) {
        TRACE_JOB(MSGSTR(HELLO_9,"hellojob: No HELLO - %s is passive\n"), ifp->int_name);
        continue;
      }
      if ( !(ifp->int_flags & IFF_UP) ) {
        TRACE_JOB(MSGSTR(HELLO_10,"hellojob: No HELLO - %s is down\n"), ifp->int_name);
        continue;
      }
      if (ifp->int_flags & IFF_NOHELLOOUT) {
        TRACE_JOB(MSGSTR(HELLO_11,"hellojob: HELLO not allowed out %s\n"), ifp->int_name);
        continue;
      }
      ...
      TRACE_JOB(MSGSTR(HELLO_12,"hellojob: Sending HELLO packet to %s, interface %s\n"),
  ...
  for (ad = srchellolist; ad; ad = ad->next) {
    ...
    if ((ifp = if_withdst(&tmpdst)) <= (struct interface *)0) {
      TRACE_INT(MSGSTR(HELLO_13,"Source HELLO gateway %s not on same net\n"),
    ...
    if (((ifp->int_flags & IFF_UP) == 0) ||
      ...
      continue;
    sin = (struct sockaddr_in *)&ifp->int_addr;
    TRACE_JOB(MSGSTR(HELLO_14,"hellojob: Sending HELLO packet to %s, interface %s\n"),
    ...
    for (rt = rh->rt_forw; rt != (struct rt_entry *)rh; rt = rt->rt_forw ) {
      ...
      switch (rt->rt_proto) {
        ...
        case RTPROTO_EGP:
          ...
          break;
        default:
          syslog(LOG_ERR, MSGSTR(HELLO_15,"supply: Unknown protocol %d for net %s"),
                 rt->rt_proto, inet_ntoa(sock_inaddr(&rt->rt_dst)));
          TRACE_TRC(MSGSTR(HELLO_16,"supply: Unknown protocol %d for net %s\n"),
                 rt->rt_proto, inet_ntoa(sock_inaddr(&rt->rt_dst)));
      }
      if (split_horizon) {
        delay = split_horizon;
      ...
      if ( index + sizeof(struct type1pair) >= HELLOMAXPACKETSIZE ) {
        hello_send(dst, src, (u_char *)hpkt, index, (int) mh->mh_len);
     ...
     int	n_nets;		/* number of nets in this update for debugging */
{
   ...
   */
  hi->h_time = htonl((time_t)((gmt->tm_sec + gmt->tm_min * 60 + gmt->tm_hour
  ...
  ifp = if_ifwithaddr((struct sockaddr *)&hellosaddr);
  if (ifp == NULL) {
    syslog(LOG_ERR, MSGSTR(HELLO_18,"hello_send: no interface for source address %s\n"),
              inet_ntoa( src));
    error = TRUE;
  } else {
    ...
    if (sendhello) {
      ...
      if (sendto(hello_socket, (char *)hello, length, flags,
        ...
        (void) sprintf(err_message, MSGSTR(HELLO_19,"hello_send: sendto() error sending %d bytes to %s"),

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: rt_control.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/04/20 15:58:14 $";
#endif 
/*
 */
/*
...
*/
/*
 * COMPONENT_NAME: TCPIP rt_control.c
 *
 * FUNCTIONS: procname1
 *
...
#include "gated_msg.h" 
extern nl_catd catd;
#define MSGSTR(n,s) NLcatgets(catd,MS_RT_CONTROL,n,s) 
#else
#define MSGSTR(n,s) s
#endif
	...
	struct restrictlist *control_net;
{
  ...
  if (rt == 0) {
    syslog(LOG_ERR, MSGSTR(RT_CONTROL_1," control_add: malloc: out of memory\n"));
    return;
  }
  rt->rhash = hash;
	...
	FILE *fp;
{
  ...
  while (fgets(buf, sizeof(buf), fp) != NULL) {
    ...
    if (((strcasecmp(keyword, "announce") == 0) ||
      ...
      if ((option = cap(buf)) == (char *)NULL) {
        printf(MSGSTR(RT_CONTROL_10,"rt_resinit: unexpected EOLN at line %d\n"),line);
        error = TRUE;
        continue;
      }
      ...
      if (getnetorhostname(nethost, option, &addr_in) <= 0) {
        printf(MSGSTR(RT_CONTROL_17,"rt_resinit: invalid %s %s at line %d\n"),
                    nethost, option, line);
        error = TRUE;
        continue;
      }
      ...
      if (((strcasecmp(savekeyword, "listenhost") == 0) ||
        ...
        printf(MSGSTR(RT_CONTROL_22,"rt_resinit: 'gateway' expected at line %d\n"),line);
        error = TRUE;
        continue;
      } else {
        if ((strcasecmp(savekeyword, "listenhost") != 0) &&
          ...
          if (strcasecmp(option,"intf") != 0) {
            printf(MSGSTR(RT_CONTROL_26,"rt_resinit: 'intf' expected at line %d\n"),line);
            error = TRUE;
            continue;
          }
        }
      ...
      while ((option = cap(buf)) != (char *)NULL) {
        if (strcasecmp(option, "proto") == 0) {
          if (intfc == 0) {
	    printf(MSGSTR(RT_CONTROL_28,"rt_resinit: No interface list at line %d\n"),line);
            error = TRUE;
          }
          break;
        }
        if (strcasecmp(option, "all") == 0) {
          if (intfc != 0) {
            printf(MSGSTR(RT_CONTROL_30,"rt_resinit: 'all' unexpected at line %d\n"),line);
            error = TRUE;
          } else {
            if ((option = cap(buf)) == (char *)NULL) {
              printf(MSGSTR(RT_CONTROL_10,"rt_resinit: unexpected EOLN at line %d\n"),line);
              ...
              if (strcasecmp(option, "proto") != 0) {
                printf(MSGSTR(RT_CONTROL_33,"rt_resinit: 'proto' expected at line %d\n"),line);
                error = TRUE;
              }
            }
          }
        ...
        if (!getnetorhostname("host",option,&addr_in)) {
          printf(MSGSTR(RT_CONTROL_35,"rt_resinit: invalid host %s at line %d\n"),
                          option, line);
          error = TRUE;
          break;
        }
        if ((strcasecmp(savekeyword, "listenhost") != 0) &&
          ...
          if ((tifp = if_ifwithaddr((struct sockaddr *)&addr_in)) <= (struct interface *)0) {
            printf(MSGSTR(RT_CONTROL_38,"rt_resinit: invalid interface %s at line %d\n"),
                          option, line);
            error = TRUE;
            break;
          }
        ...
        if (intfc >= MAXINTERFACE) {
          printf(MSGSTR(RT_CONTROL_39,"rt_resinit: max number of interfaces at line %d\n"),line);
          error = TRUE;
          break;
        }
        ...
        if ((strcasecmp(savekeyword, "noannounce") == 0) ||
          ...
          if (tifp->int_flags & IFF_ANNOUNCE) {
            printf(MSGSTR(RT_CONTROL_42,"rt_resinit: announce and noannounce specified "));
            printf(MSGSTR(RT_CONTROL_43,"for interface %s at line %d\n"), option, line);
            error = TRUE;
	    fatalerror = TRUE;
            break;
          } else {
      ...
      while ((option = cap(buf)) != (char *)NULL) {
        if (strcasecmp(option, "egpmetric") == 0) {
          if (protocols == 0) {
            printf(MSGSTR(RT_CONTROL_50,"rt_resinit: No protocol list at line %d\n"),line);
            error = TRUE;
          }
          break;
        }
        if (strcasecmp(option, "all") == 0) {
          if (protocols != 0) {
            printf(MSGSTR(RT_CONTROL_52,"rt_resinit: 'all' unexpected at line %d\n"),line);
            error = TRUE;
          } else {
            if ((option = cap(buf)) == (char *)NULL) {
              printf(MSGSTR(RT_CONTROL_10,"rt_resinit: unexpected EOLN at line %d\n"),line);
              ...
              if (strcasecmp(option, "egpmetric") != 0) {
                printf(MSGSTR(RT_CONTROL_55,"rt_resinit: 'egpmetric' expected at line %d\n"),line);
                error = TRUE;
              }
            }
          }
        ...
        if (strcasecmp(option, "rip") == 0)
          ...
          printf(MSGSTR(RT_CONTROL_62,"rt_resinit: unknown protocol %s at line %d\n"),
                     option, line);
          error = TRUE;
          break;
        }
      ...
      if (res_net.rproto & RTPROTO_EGP) {
        ...
        if ((res_net.regpmetric >= 255) || (res_net.regpmetric < 0)) {
          printf(MSGSTR(RT_CONTROL_66,"rt_resinit: invalid EGP metric %d at line %d\n"),
  ...
  if (((glob_announcethesenets != 0) && ((glob_donotannounce !=0) ||
       ...
       (announcethesenets != 0))) {
    printf(MSGSTR(RT_CONTROL_82,"The \"Do not announce\" list and \"announce\" list both\n"));
    printf(MSGSTR(RT_CONTROL_83,"initialized globally.  You can only have one or the other\n"));
    printf(MSGSTR(RT_CONTROL_84,"applied globally.  You can have both on different interfaces.\n"));
    fatalerror = TRUE;
  }
  if (fatalerror == TRUE) {
    printf(MSGSTR(RT_CONTROL_85,"rt_resinit: %s: initialization error\n"), EGPINITFILE);
    quit();
  }
}
	...
	int init_flag;
{
  ...
  while (fgets(buf, sizeof(buf), fp) != NULL) {
    ...
    if ((strcasecmp(keyword, "announcetoAS") == 0) ||
      ...
      if ( (tmp_list = (struct as_list *) calloc(1, sizeof(struct as_list))) == NULL ) {
        TRACE_TRC(MSGSTR(RT_CONTROL_89,"rt_ASinit: out of memory\n"));
        syslog(LOG_EMERG, MSGSTR(RT_CONTROL_90,"rt_ASinit: out of memory"));
        fatalerror = TRUE;
        break;
      }
      ...
      for ( ptr_list = my_sendas; ptr_list; ptr_list = ptr_list->next) {
        if (ptr_list->as == tmp_list->as) {
          TRACE_EXT(MSGSTR(RT_CONTROL_92,"rt_ASinit: duplicate %sannouncetoAS in line %d, %sannouncetoAS already specified for AS %d\n"),
                    tmp_list->flags & AS_DONOTSEND ? "no" : "",
                    line,
                    ptr_list->flags & AS_DONOTSEND ? "no" : "",
                    tmp_list->as);
          syslog(LOG_ERR,MSGSTR(RT_CONTROL_97,"rt_ASinit: duplicate %sannouncetoAS in line %d, %sannouncetoAS already specified for AS %d"),
                    tmp_list->flags & AS_DONOTSEND ? "no" : "",
                    line,
                    ptr_list->flags & AS_DONOTSEND ? "no" : "",
                    tmp_list->as);
      ...
      if ( ( (keyword = cap(buf)) != (char *)NULL ) && 
        ...
        syslog(LOG_ERR, MSGSTR(RT_CONTROL_105,"rt_ASinit: invalid keyword: %s"), keyword);
        TRACE_TRC(MSGSTR(RT_CONTROL_106,"rt_ASinit: invalid keyword: %s\n"), keyword);
        free((char *)tmp_list);
        error = TRUE;
        continue;
      }
      ...
      my_sendas = tmp_list;
      TRACE_EXT(MSGSTR(RT_CONTROL_114,"rt_ASinit: %sannouncetoAS %d %srestrict ASlist"),
      ...
      if ((keyword = cap(buf)) == (char *)NULL) {
        TRACE_TRC(MSGSTR(RT_CONTROL_124,"rt_ASinit: unexpected EOLN at line %d\n"),line);
        syslog(LOG_ERR,MSGSTR(RT_CONTROL_124,"rt_ASinit: unexpected EOLN at line %d\n"),line);
        free((char *)tmp_valid);
        error = TRUE;
        continue;
      }
      if (getnetorhostname("net", keyword, &addr_in) <= 0) {
        TRACE_TRC(MSGSTR(RT_CONTROL_127,"rt_ASinit: invalid net %s at line %d\n"), keyword, line);
        syslog(LOG_ERR, MSGSTR(RT_CONTROL_127,"rt_ASinit: invalid net %s at line %d\n"), keyword, line);
        free((char *)tmp_valid);
        error = TRUE;
        continue;
      }
  ...
  if (!fatalerror) {
    ...
    if (init_flag) {
      printf(MSGSTR(RT_CONTROL_140,"rt_ASinit: %s: initialization error\n"), EGPINITFILE);
      quit();
    } else {
      printf(MSGSTR(RT_CONTROL_141,"rt_ASinit: %s: reinitialization error - changes ignored\n"), EGPINITFILE);
    }
...
martian_init()
{
	...
	for ( ptr = martian_nets; *ptr; ptr++ ) {
		if ((mptr = (struct advlist *) calloc(1, sizeof(struct advlist))) == NULL) {
			syslog(LOG_EMERG, MSGSTR(RT_CONTROL_142,"martian_init: out of memory"));
			quit();
		}
		if (!getnetorhostname("host", *ptr, &net)) {
			syslog(LOG_ERR, MSGSTR(RT_CONTROL_144,"is_martian: invalid net %s in martian table\n"), *ptr);
			continue;
		}

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: ripquery.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/04/20 15:58:03 $";
#endif 
/*
 */
/*
	...
	int cmd;
{
	...
	if (hp == 0) {
		...
		if (router.sin_addr.s_addr == (u_int)-1) {
			printf("%s: unknown\n", host);
			exit(1);
		}
	} else {
	...
	router.sin_port = ripport;
	msg->rip_cmd = sp->s_port;
	msg->rip_vers = RIPVERSION;
	msg->rip_nets[0].rip_dst.sa_family = htons(AF_UNSPEC);
	msg->rip_nets[0].rip_metric = htonl((u_int)HOPCNT_INFINITY);

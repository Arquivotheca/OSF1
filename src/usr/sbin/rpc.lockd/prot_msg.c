 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: prot_msg.c,v $ $Revision: 1.1.5.2 $ (DEC) $Date: 1993/08/17 20:04:53 $";
#endif
#ifndef lint
static char sccsid[] = "@(#)prot_msg.c	1.3 90/11/09 NFSSRC4.1 Copyr 1990 Sun Micro";
#endif
		...
		if (msgp->reply == NULL) { /* check for retransimssion */
			if (msgp->proc != KLM_LOCK) {
				...
				if (msgp->t.exp == msgp->t.curr) {
					...
					(void) nlm_call(msgp->proc, msgp->req,
							msgp);
					msgp->t.curr = 0;

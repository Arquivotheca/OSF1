 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: prot_alloc.c,v $ $Revision: 1.1.3.4 $ (DEC) $Date: 1992/12/11 16:53:15 $";
#endif
#ifndef lint
static char sccsid[] = "@(#)prot_alloc.c	1.4 90/11/09 NFSSRC4.1 Copyr 1990 Sun Micro";
#endif
	...
	reclock *a;
{
	...
	if (a->lck.svr == a->lck.caller_name) {
		if (a->lck.svr == a->lck.clnt_name) {
			xfree(&a->lck.svr);
			a->lck.clnt_name= a->lck.caller_name= NULL;
		} else {
			xfree(&a->lck.svr);

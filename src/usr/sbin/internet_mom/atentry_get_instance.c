 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: atentry_get_instance.c,v $ $Revision: 1.1.2.3 $ (DEC) $Date: 1993/07/13 19:19:27 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
**--
*/
static man_status atEntry_build_instance_avl( object_instance, 
...

{
  ...
  /*
   * Move down the avl to the atEntry's name, backup one octet,
...
}  /* End of atEntry_build_instance_avl */

/*
**++
**  FUNCTIONAL DESCRIPTION:
**
**      atEntry_get_instance
**
...
**	      are needed in the atEntry_get_next_match routine.
**
**  FORMAL PARAMETERS:
**
...
man_status atEntry_get_instance( object_class,
				  	 object_instance,
				  	 iso_scope,
				  	 specific_instance,
				  	 specific_object_instance,
...
int			build_avl;
{
	...
	*return_context = context;
	...
	status = (man_status) refresh_atEntry_list(new_atEntry_header); if ERROR_CONDITION(status) return status;
        context->atEntry_PTR = new_atEntry_header;
	...
	if (context->atEntry_PTR == new_atEntry_header)
	    status = MAN_C_FALSE;
	else
	    status = MAN_C_TRUE;
    }
	...
	*more_instances = FALSE;
    ...
    if (build_avl != TRUE)
    {
        ...
        *specific_instance = context->atEntry_PTR->next; 
	*object_uid = &(*specific_instance)->instance_uid;
    ...
    {
        status = (man_status) atEntry_find_matching_instance ( object_instance,
	                             context);
        if (status == MAN_C_FALSE)
        {
           *return_context = NULL;
        ...
        *specific_instance = context->atEntry_PTR; 
    ...
    return MAN_C_SUCCESS;
} /* End of atEntry_get_instance */


man_status atEntry_find_matching_instance (
                                   instance_avl,
                                   context
                                  )
...
/** Multiple identifier OIDs found. Verify the correct OID **/
    ...
    /* First identifier is atIfIndex.  Byte copy to input_oid[0] */

    bcopy(instance_octet->string,&input_oid[0],sizeof(int));

    /* Second identifier is atDummy. Byte copy to input_oid[1] */
    status = moss_avl_point(instance_avl,NULL,NULL,NULL,&instance_octet,NULL);
    if (status != MAN_C_SUCCESS)
        return(MAN_C_NO_SUCH_OBJECT_INSTANCE);
    if ((instance_octet->length == 0) || (*instance_octet->string) <= 0)
        return(MAN_C_MISSING_ATTRIBUTE_VALUE);

    bcopy(instance_octet->string,&input_oid[1],sizeof(int));

    /* Next identifier is the atNetAddress.  Byte copy at input_oid[2] */
    ...
    */
    ...
    context->atEntry_PTR = context->atEntry_PTR->next;

    tmpoid.count = atEntry_INDEX_SIZE;
    while (context->atEntry_PTR != new_atEntry_header) {

	 ch = (char *)context->atEntry_PTR->instance_name;
         bcopy(ch, &tmp_oid[0], 2*sizeof(int));    /* Move atIfIndex and atDummy */
         for (i = 0; i < 2*sizeof(int); i++)
	      ch++;
         for (i = 2; i < atEntry_INDEX_SIZE; i++){
         tmp_oid[i] = *ch & 0xff;		   /* Move atNetAddress */
         ch++;
         }
...
} /* End of atEntry_find_matching_instance */

/* End of atEntry_get_instance.c */

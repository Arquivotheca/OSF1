 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: tcpconnentry_perform_getnext.c,v $ $Revision: 1.1.2.3 $ (DEC) $Date: 1993/07/13 19:23:40 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
**  MODULE DESCRIPTION:
**
**      tcpConnEntry_PERFORM_TABLE_GETNEXT.C
**
...
**  FUNCTIONAL DESCRIPTION:
**
**      tcpConnEntry_construct_table_instance
**
...
avl         *instance;
tcpConnEntry_DEF *instance_elem;
...
/** Multiple identifier OIDs found. Verify the correct OID **/
        ...
        if (status != MAN_C_SUCCESS)
         ...
         /*
          | Back up to first index (atIfIndex), and then remove the four,
          | viz., tcpConnLocalAddress , tcpConnLocalPort, tcpConnRemAddress ,
	  | and, tcpConnRemPort.
          */
    ...
    instance_octet.string = (char *)(instance_elem->tcpConnLocalAddress);
    ...
    instance_octet.string = (char *)&(instance_elem->tcpConnLocalPort);
    ...
    instance_octet.length = sizeof (int);
    instance_octet.data_type = INET_C_SMI_IP_ADDRESS;
    instance_octet.string = (char *)(instance_elem->tcpConnRemAddress);
    ...
    instance_octet.length = sizeof (int); 
    instance_octet.data_type = ASN1_C_INTEGER;
    instance_octet.string = (char *)&(instance_elem->tcpConnRemPort);
...
} /* End of tcpConnEntry_construct_table_instance() */

...
**  FUNCTIONAL DESCRIPTION:
**
**      tcpConnEntry_find_next_instance
**
...
**      elements relating to the instance identifier, viz. tcpConnLocalAddress,
...
**      doubly-linked in the new_tcpConnEntry_header structure.
**
**  FORMAL PARAMETERS:
**
...
/** note to developers: this routine assumes that instance is an integer
    ...
    inputoid.count = 10;  /* Size of four identifiers, viz., 1+4+1+4, for 
                             tcpConnLocalAddress, tcpConnLocalPort,
			     tcpConnLocalAddress, tcpConnRemPort */

    /* First identifer is tcpConnLocalAddress  */
    ch = (char *)inst_octet->string;
    for (i = 0; i < 4; i++){
         input_oid[i] = *ch & 0xff;
         ch++;
    }
    /* Second identifier is tcpConnLocalPort */
    ...
    /* Third identifier is tcpConnRemAddress */
    status = moss_avl_point(instance_avl,NULL,NULL,NULL,&inst_octet,NULL);
    if (status != MAN_C_SUCCESS)
        return(MAN_C_NO_SUCH_OBJECT_INSTANCE);
    if (inst_octet->length != sizeof(struct in_addr))
    ...
    /* Fourth identifier is tcpConnRemPort */
    status = moss_avl_point(instance_avl,NULL,NULL,NULL,&inst_octet,NULL);
    if (status != MAN_C_SUCCESS)
        return(MAN_C_NO_SUCH_OBJECT_INSTANCE);
    if ((inst_octet->length == 0))
        return(MAN_C_NO_SUCH_OBJECT_INSTANCE);
    bcopy(inst_octet->string,&input_oid[9],4);
   inputoid.value = &input_oid[0];

    *instance = new_tcpConnEntry_header;/*point to top of class struct list*/
    ...
    */
    ...
    header_p = new_tcpConnEntry_header;

    while(header_p->next != new_tcpConnEntry_header) {
...
} /* End of tcpConnEntry_find_next_instance() */

...
**  FUNCTIONAL DESCRIPTION:
**
**      tcpConnEntry_find_first_instance
**
...
**      available instances doubly-linked in the new_tcpConnEntry_header
...
/** note to developers: this routine assumes that instance is an integer
    ...
    tcpConnEntry_DEF    *current_instance = NULL;
    int                 i,j;
    char               	*instance_string, *ch;
    tcpConnEntry_DEF    *header_p = NULL;
	...
	{
         tmpoid.count = header_p->next->instance_name_length;
         bcopy(header_p->next->instance_name,tmp_oid,tmpoid.count);
         tmpoid.value = tmp_oid;
         curroid.count = header_p->next->instance_name_length;
    ...
    *instance = current_instance;
    return MAN_C_SUCCESS;
} /* End of tcpConnEntry_find_first_instance() */

...
status = (man_status) refresh_tcpConnEntry_list(new_tcpConnEntry_header); if ERROR_CONDITION(status) return status;
    ...
    if (attr_status == MAN_C_NO_ELEMENT) {
       ...
       ret_oid = &tcpConnEntry_ATTR_tcpConnState_SNP;
...
/** Multiple identifier OIDs found. Verify the correct OID **/
	    ...
	    if (status != MAN_C_SUCCESS) {
            ...
            if (attr_status == MAN_C_NO_ELEMENT) {
                status = tcpConnEntry_find_first_instance(&specific_instance);
                if (ERROR_CONDITION(status)) {
                    moss_avl_free( &return_attribute, TRUE );
                    return MAN_C_NO_SUCH_ATTRIBUTE_ID;
                }
	    ...
	    /* find next instance */
            ...
            {
                status = tcpConnEntry_find_next_instance(*instance, instance_octet, &specific_instance);
	        if (status != MAN_C_SUCCESS)			     /* if no next, return to first */
                {
                   if (status == MAN_C_NO_SUCH_OBJECT_INSTANCE)
                   {
		      status = tcpConnEntry_next_oid( in_oid, &ret_oid );	/* but make sure there are more attributes */
		      if (status != MAN_C_SUCCESS)
                      {
                        moss_avl_free( &return_attribute, TRUE );
                        return MAN_C_NO_SUCH_ATTRIBUTE_ID;
	    ...
	    status = tcpConnEntry_construct_table_instance(*instance,specific_instance);
            if ERROR_CONDITION (status) {
                moss_avl_free( &return_attribute, TRUE );
                return status;
            }
	...
	status = tcpConnEntry_perform_get(
					    pad_oid,
					    return_attribute,
					    specific_instance,
					    attribute_value,
...
} /* End of tcpConnEntry_perform_getnext() */

/* End of tcpConnEntry_perform_getnext.c */

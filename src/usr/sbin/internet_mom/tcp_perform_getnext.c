 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: tcp_perform_getnext.c,v $ $Revision: 1.1.2.5 $ (DEC) $Date: 1993/09/07 21:26:16 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
**  MODULE DESCRIPTION:
**
**      tcp_PERFORM_NONTABLE_GETNEXT.C
**
...
man_status  status;
    ...
    if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpRtoAlgorithm_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpRtoMin_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpRtoMin_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpRtoMax_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpRtoMax_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpMaxConn_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpMaxConn_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpActiveOpens_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpActiveOpens_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpPassiveOpens_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpPassiveOpens_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpAttemptFails_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpAttemptFails_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpEstabResets_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpEstabResets_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpCurrEstab_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpCurrEstab_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpInSegs_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpInSegs_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpOutSegs_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpOutSegs_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpRetransSegs_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpRetransSegs_SNP)) == MAN_C_EQUAL) 
        status = MAN_C_NO_SUCH_ATTRIBUTE_ID;
/*
        *ret_oid = &tcp_ATTR_tcpInErrs_SNP; NO_SUCH, next is tcpConnEntry MIA
 */
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpInErrs_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &tcp_ATTR_tcpOutRsts_SNP;
    else if ((status = moss_compare_oid( attr_oid, &tcp_ATTR_tcpOutRsts_SNP)) == MAN_C_EQUAL) 
        status = MAN_C_NO_SUCH_ATTRIBUTE_ID;
...
status = (man_status) refresh_tcp_list(new_tcp_header); if ERROR_CONDITION(status) return status;
    ...
    if (attr_status == MAN_C_NO_ELEMENT) {
       ...
       ret_oid = &tcp_ATTR_tcpRtoAlgorithm_SNP;
	...
	if ( ( instance_octet->length == 0 ) && ( instance_value == NULL) ) {
	    ...
	    status = tcp_next_oid( in_oid, &ret_oid );	/* use next attribute */
	    if (status != MAN_C_SUCCESS) {
	        if (new_oid != NULL) moss_free_oid( new_oid );
	        return status;
	    }
	...
	if (ERROR_CONDITION(status)) {
	    in_oid = ret_oid;
	    inloop_status = tcp_next_oid( in_oid, &ret_oid );
...
} /* End of tcp_perform_getnext() */

/* End of tcp_perform_getnext.c */

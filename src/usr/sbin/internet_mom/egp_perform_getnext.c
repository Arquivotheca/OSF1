 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: egp_perform_getnext.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/09/03 19:31:17 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
**  MODULE DESCRIPTION:
**
**      egp_PERFORM_NONTABLE_GETNEXT.C
**
...
**	 MOM Generator - version V1.1
**
**  CREATION DATE:  01-Sep-1993
**
...
man_status  status;
    ...
    if ((status = moss_compare_oid( attr_oid, &egp_ATTR_egpInMsgs_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &egp_ATTR_egpInErrors_SNP;
    else if ((status = moss_compare_oid( attr_oid, &egp_ATTR_egpInErrors_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &egp_ATTR_egpOutMsgs_SNP;
    else if ((status = moss_compare_oid( attr_oid, &egp_ATTR_egpOutMsgs_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &egp_ATTR_egpOutErrors_SNP;
    else if ((status = moss_compare_oid( attr_oid, &egp_ATTR_egpOutErrors_SNP)) == MAN_C_EQUAL) 
        status = MAN_C_NO_SUCH_ATTRIBUTE_ID;
/*        *ret_oid = &egp_ATTR_egpAs_SNP; NO_ATTR_ID *** DEM */
    else if ((status = moss_compare_oid( attr_oid, &egp_ATTR_egpAs_SNP)) == MAN_C_EQUAL) 
        status = MAN_C_NO_SUCH_ATTRIBUTE_ID;
...
status = (man_status) refresh_egp_list(new_egp_header); if ERROR_CONDITION(status) return status;
	...
	/* set attribute to first attribute for class */
       ...
       ret_oid = &egp_ATTR_egpInMsgs_SNP;
	...
	if ( ( instance_octet->length == 0 ) && ( instance_value == NULL) ) {
	    ...
	    status = egp_next_oid( in_oid, &ret_oid );	/* use next attribute */
	    if (status != MAN_C_SUCCESS) {
	        if (new_oid != NULL) moss_free_oid( new_oid );
	        return status;
	    }
	...
	/*
	 | Now that we have the specific instance of interest, call 
         | egp_perform_get routine to obtain the requested
	...
	if (ERROR_CONDITION(status)) {
	    in_oid = ret_oid;
	    inloop_status = egp_next_oid( in_oid, &ret_oid );
    ...
    return status;
} /* End of egp_perform_getnext() */

/* End of egp_perform_getnext.c */

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: ipaddrentry_perform_getnext.c,v $ $Revision: 1.1.2.3 $ (DEC) $Date: 1993/07/13 19:21:18 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
**  MODULE DESCRIPTION:
**
**      ipAddrEntry_PERFORM_TABLE_GETNEXT.C
**
...
**  FUNCTIONAL DESCRIPTION:
**
**      ipAddrEntry_construct_table_instance
**
...
} /* End of ipAddrEntry_construct_table_instance() */

...
**  FUNCTIONAL DESCRIPTION:
**
**      ipAddrEntry_find_next_instance
**
...
**      elements relating to the instance identifier, ipAdEntAddress.  It 
**      then compares this with the tmp_oid
**      built the same way from each of the available instances
**      doubly-linked in the new_ipAddrEntry_header structure.
**
**  FORMAL PARAMETERS:
**
...
ipAddrEntry_DEF *return_instance = NULL;
...
unsigned int        ret_array[ 4 ] ;
    ...
    /* Get ipAdEntAddr (4 bytes) into the array */
    ...
    *instance = new_ipAddrEntry_header;	/* point to top of class structure list */

    candidate_octet.data_type = INET_C_SMI_IP_ADDRESS; 

    while ((*instance)->next != new_ipAddrEntry_header) {
...
} /* End of ipAddrEntry_find_next_instance() */

...
**  FUNCTIONAL DESCRIPTION:
**
**      ipAddrEntry_find_first_instance
**
...
**      available instances doubly-linked in the new_ipAddrEntry_header
...
ipAddrEntry_DEF *current_instance = NULL;
...
unsigned int        cand_array[ 4 ] ;
    ...
    *instance = current_instance;
    return MAN_C_SUCCESS;
} /* End of ipAddrEntry_find_first_instance() */

...
ipAddrEntry_DEF		*specific_instance;
octet_string	*instance_octet = NULL;
unsigned int    itag, imod;
avl		*return_attribute = (avl *)NULL;

status = (man_status) refresh_ipAddrEntry_list(new_ipAddrEntry_header); if ERROR_CONDITION(status) return status;
    ...
    if (attr_status == MAN_C_NO_ELEMENT) {
       ...
       ret_oid = &ipAddrEntry_ATTR_ipAdEntAddr_SNP;
 ...
 */
    ...
    do {
	    ...
	    if (status != MAN_C_SUCCESS) {
            ...
            if (attr_status == MAN_C_NO_ELEMENT) {
                status = ipAddrEntry_find_first_instance(&specific_instance);
                if (ERROR_CONDITION(status)) {
                    moss_avl_free( &return_attribute, TRUE );
                    return MAN_C_NO_SUCH_ATTRIBUTE_ID;
                }
	    ...
	    /* find next instance */
            ...
            {
                status = ipAddrEntry_find_next_instance(instance_octet, &specific_instance);
	        if (status != MAN_C_SUCCESS)			     /* if no next, return to first */
                {
                   if (status == MAN_C_NO_SUCH_OBJECT_INSTANCE)
                   {
		      status = ipAddrEntry_next_oid( in_oid, &ret_oid );	/* but make sure there are more attributes */
		      if (status != MAN_C_SUCCESS)
                      {
                        moss_avl_free( &return_attribute, TRUE );
                        return MAN_C_NO_SUCH_ATTRIBUTE_ID;
	    ...
	    status = ipAddrEntry_construct_table_instance(*instance,specific_instance);
            if ERROR_CONDITION (status) {
                moss_avl_free( &return_attribute, TRUE );
                return status;
            }
	...
	status = ipAddrEntry_perform_get(
					    pad_oid,
					    return_attribute,
					    specific_instance,
					    attribute_value,
...
} /* End of ipAddrEntry_perform_getnext() */

/* End of ipAddrEntry_perform_getnext.c */

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: ipaddrentry_get_instance.c,v $ $Revision: 1.1.2.3 $ (DEC) $Date: 1993/07/13 19:21:12 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
**
**      ipAddrEntry_build_instance_avl 
**
...
**--
*/
static man_status ipAddrEntry_build_instance_avl( object_instance, 
...

{
  ...
  /*
   * Move down the avl to the ipAddrEntry's name, backup one octet,
...
} /* End of ipAddrEntry_build_instance_avl */

/*
**++
**  FUNCTIONAL DESCRIPTION:
**
**      ipAddrEntry_get_instance
**
...
**	      are needed in the ipAddrEntry_get_next_match routine.
**
**  FORMAL PARAMETERS:
**
...
man_status ipAddrEntry_get_instance( object_class,
				  	 object_instance,
				  	 iso_scope,
				  	 specific_instance,
				  	 specific_object_instance,
...
int			build_avl;
{
	...
	*return_context = context;
	...
	status = (man_status) refresh_ipAddrEntry_list(new_ipAddrEntry_header); if ERROR_CONDITION(status) return status;
        context->ipAddrEntry_PTR = new_ipAddrEntry_header;

	/*
	 * Find the first matching name, if any.
	 */
        if ( build_avl == TRUE )
	   status = (man_status) ipAddrEntry_get_next_match(object_instance,
							     iso_scope,
							     context);
        else
          status = MAN_C_TRUE;
	...
	if (context->ipAddrEntry_PTR == new_ipAddrEntry_header)
	    status = MAN_C_FALSE;
	else
	    status = MAN_C_TRUE;
    }
	...
	*more_instances = FALSE;
    ...
    if (build_avl != TRUE)
        *specific_instance = context->ipAddrEntry_PTR->next; 
    else
        *specific_instance = context->ipAddrEntry_PTR; 
    ...
    *object_uid = &(*specific_instance)->instance_uid;
	...
	{
        status = ipAddrEntry_build_instance_avl( object_instance, 
						     specific_object_instance, 
						     *specific_instance );
        if ERROR_CONDITION( status )
            return status;
	...
	*more_instances = FALSE;
    ...
    return MAN_C_SUCCESS;
} /* End of ipAddrEntry_get_instance */

/* End of ipAddrEntry_get_instance.c */

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: ipnettomediaentry_perform_getnext.c,v $ $Revision: 1.1.2.5 $ (DEC) $Date: 1993/12/22 16:00:05 $";
#endif
/*
**++
**  FACILITY:  ZKO3-3
...
man_status  status;
    ...
    if ((status = moss_compare_oid( attr_oid, &ipNetToMediaEntry_ATTR_ipNetToMediaIfIndex_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &ipNetToMediaEntry_ATTR_ipNetToMediaPhysAddress_SNP;
    else if ((status = moss_compare_oid( attr_oid, &ipNetToMediaEntry_ATTR_ipNetToMediaPhysAddress_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &ipNetToMediaEntry_ATTR_ipNetToMediaNetAddress_SNP;
    else if ((status = moss_compare_oid( attr_oid, &ipNetToMediaEntry_ATTR_ipNetToMediaNetAddress_SNP)) == MAN_C_EQUAL) 
        *ret_oid = &ipNetToMediaEntry_ATTR_ipNetToMediaType_SNP;
...
} /* End of ipNetToMediaEntry_next_oid() */
...
**  FUNCTIONAL DESCRIPTION:
**
**      ipNetToMediaEntry_construct_table_instance
**
...
avl         *instance;
ipNetToMediaEntry_DEF *instance_elem;
...
/** Multiple identifier OIDs found. Verify the correct OID **/
         ...
         /*
          | Back up to first index (ipNetToMediaIfIndex), and then remove 
          | the two, viz., ipNetToMediaIfIndex and ipNetToMediaNetAddress
          */
    ...
    instance_octet.string = (char *)&(instance_elem->ipNetToMediaIfIndex);
...
/** Multiple identifier OIDs found. Verify the correct OID **/
    ...
    instance_octet.string = (char *)(instance_elem->ipNetToMediaNetAddress);
...
} /* End of ipNetToMediaEntry_construct_table_instance() */

...
**  FUNCTIONAL DESCRIPTION:
**
**      ipNetToMediaEntry_find_next_instance
**
...
**      elements relating to the instance identifier, viz. ipNetToMediaIfIndex, 
**      and ipNetToMediaNetAddress.  It then compares this with the tmp_oid
**      built the same way from each of the available instances
**      doubly-linked in the new_ipNetToMediaEntry_header structure.
**
**  FORMAL PARAMETERS:
**
...
/** note to developers: this routine assumes that instance is an integer
    ...
    man_status          status;
    ipNetToMediaEntry_DEF *return_instance = NULL;
    ...
    /* First identifier is ipNetToMediaIfIndex (passed to the routine by
default) */
    if ((inst_octet->length == 0) || (*inst_octet->string) <= 0)
        return MAN_C_MISSING_ATTRIBUTE_VALUE;

    inst_index = *inst_octet->string;

    /* Second identifier is ipNetToMediaNetAddress */
    ...
    *instance = new_ipNetToMediaEntry_header;/*point to top of class struct list */
    /*
     *
    */
    inputoid.count = 5 ;
    input_oid[0] = inst_index;
    ...
    */
    ...
    header_p = new_ipNetToMediaEntry_header;

    while(header_p->next != new_ipNetToMediaEntry_header) {
...
} /* End of ipNetToMediaEntry_find_next_instance() */

...
**  FUNCTIONAL DESCRIPTION:
**
**      ipNetToMediaEntry_find_first_instance
**
...
ipNetToMediaEntry_DEF *current_instance = NULL;
    int                 i,j;
    char                *instance_string, *ch;
    ipNetToMediaEntry_DEF *header_p = NULL;
    ...
    while(header_p->next != new_ipNetToMediaEntry_header) {
        ...
        {
         tmpoid.count = 10;
         bcopy(header_p->next->instance_name,tmp_oid,10);
         tmpoid.value = tmp_oid;
         curroid.count = 10;
         bcopy(current_instance->instance_name,curr_oid,10);
    ...
    *instance = current_instance;
    return MAN_C_SUCCESS;
} /* End of ipNetToMediaEntry_find_first_instance() */

...
status = (man_status) refresh_ipNetToMediaEntry_list(new_ipNetToMediaEntry_header); if ERROR_CONDITION(status) return status;
    ...
    if (attr_status == MAN_C_NO_ELEMENT) {
       ...
       ret_oid = &ipNetToMediaEntry_ATTR_ipNetToMediaIfIndex_SNP;
...
/** Multiple identifier OIDs found. Verify the correct OID **/
	    ...
	    if (status != MAN_C_SUCCESS) {
            ...
            if (attr_status == MAN_C_NO_ELEMENT) {
                status = ipNetToMediaEntry_find_first_instance(&specific_instance);
                if (ERROR_CONDITION(status)) {
                    moss_avl_free( &return_attribute, TRUE );
                    return MAN_C_NO_SUCH_ATTRIBUTE_ID;
                }
	    ...
	    /* find next instance */
            ...
            {
                status = ipNetToMediaEntry_find_next_instance( *instance, instance_octet, &specific_instance);
	        if (status != MAN_C_SUCCESS)			     /* if no next, return to first */
                {
                   if (status == MAN_C_NO_SUCH_OBJECT_INSTANCE)
                   {
		      status = ipNetToMediaEntry_next_oid( in_oid, &ret_oid );	/* but make sure there are more attributes */
		      if (status != MAN_C_SUCCESS)
                      {
                        moss_avl_free( &return_attribute, TRUE );
                        return MAN_C_NO_SUCH_ATTRIBUTE_ID;
	    ...
	    status = ipNetToMediaEntry_construct_table_instance(*instance,specific_instance);
            if ERROR_CONDITION (status) {
                moss_avl_free( &return_attribute, TRUE );
                return status;
            }
	...
	status = ipNetToMediaEntry_perform_get(
					    pad_oid,
					    return_attribute,
					    specific_instance,
					    attribute_value,
...
} /* End of ipNetToMediaEntry_perform_getnext() */

/* End of ipNetToMediaEntry_perform_getnext.c */

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: pcnfsd_print.c,v $ $Revision: 1.1.8.2 $ (DEC) $Date: 1993/05/26 17:46:47 $";
#endif
...
#ifdef ACCEPT_BSD_PR_NAMES
        if(alt_printers == NULL)
                return(0);      /* No alt list  */
	...
	while(fgets(buff, 255, p) != NULL) {
		...
		if(!cp)
			continue;
                        ...
                        curr->cm = my_strdup("(Incomplete configuration detected)");
                        curr->pr_next = NULL;
                        goto yo_lo_tengo;
                }
                if(!strcmp(cp, "device"))

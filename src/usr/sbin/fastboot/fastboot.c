 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: fastboot.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1993/10/14 20:50:34 $";
#endif 
/*
 */
...
nl_catd catd;
#define MSGSTR(n,s) catgets(catd,MS_FASTBOOT,n,s)
#ifdef SEC_BASE
#define MSGSTR_SEC(n,s) catgets(catd,MS_FASTBOOT_SEC,n,s)
#endif
...
static char command[MAXFILE+1] = "/sbin/reboot " ; /* "start-stop" like file. */
	...
	char **argv;
{
	...
	catd = catopen(MF_FASTBOOT,NL_CAT_LOCALE);
 ...
 if (geteuid()) {
	...
	if(argc && (!strcmp(*argv,"-n") || !strcmp(*argv,"-q") || !strcmp(*argv, "-l")))
		;
	else {
		 fprintf(stderr, MSGSTR(USAGE, "usage: fastboot [-q][-l][-n]\n"));
                        exit(1);
                }

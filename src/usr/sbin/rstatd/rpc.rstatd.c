 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: rpc.rstatd.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/05/26 17:47:38 $";
#endif
...
int kmem;
unsigned long firstifnet, numintfs;	/* chain of ethernet interfaces */
...
#ifdef DEBUG
	rtn = (statsvar *) result;
	putline(rtn->curtime.tv_sec, rtn->boottime, rtn->avenrun);
	if (result == NULL)
...
#ifdef DEBUG
	_msgout("returning  stats_service");
#endif
	return(&ret);
}
...
rstatproc_stats_4()
{
...
#endif
	sincelastreq = 0;
	if (ret.cp_time.cp_time_val ==  NULL) {
	...
	printf("v_pgpgin: %d cp_time_val: %d dk_xfer_val: %d \n",
                    ret.v_pgpgin, ret.cp_time.cp_time_val, 
		    ret.dk_xfer.dk_xfer_val);
	putline(ret.curtime.tv_sec, ret.boottime, ret.avenrun);
#endif
...
updatestat()
{
	...
	sincelastreq++;
	if (lseek(kmem, nl[X_HZ].n_value, 0) == -1) {
	...
	if (lseek(kmem, nl[X_CPTIME].n_value, 0) == -1) {
		_msgout("can't seek in kmem");
		do_exit(1);
	}
	...
	if (lseek(kmem, nl[X_AVENRUN].n_value, 0) ==-1) {
		_msgout("can't seek in kmem");
		do_exit(1);
	}
#ifdef vax
...
#endif
	if (lseek(kmem, (unsigned long)nl[X_BOOTTIME].n_value, 0) == -1) {
		_msgout("can't seek in kmem");
		do_exit(1);
	}
	...
	if (lseek(kmem, (unsigned long)nl[X_SUM].n_value, 0) ==-1) {
		_msgout("can't seek in kmem");
		do_exit(1);
	}
 	if (read(kmem, &sum, sizeof sum) != -1) {
	...
	if (lseek(kmem, (unsigned long)nl[X_DKXFER].n_value, 0) == -1) {
		_msgout("can't seek in kmem");
		do_exit(1);
	}
	...
	for (off = firstifnet, i = 0; off && i < numintfs; i++) {
		...
		if (read(kmem, &ifnet, sizeof ifnet) == -1) {
		...
		off = (unsigned long) ifnet.if_next;
	}
...
setup()
{
	...
	if (read(kmem, &firstifnet, sizeof( long)) == -1 ) {

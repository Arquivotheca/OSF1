 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: sm_statd.c,v $ $Revision: 1.1.6.2 $ (DEC) $Date: 1993/09/23 15:57:35 $";
#endif
#ifndef lint
static char sccsid[] = 	"@(#)sm_statd.c	1.2 90/07/23 4.1NFSSRC Copyr 1988 Sun Micro";
#endif
...
statd_init()
{
	...
	struct dirent *dirp;
	DIR 	*dp;
	char from[NAME_MAX], to[NAME_MAX], path[NAME_MAX];
	FILE *fp, *fopen();

	if (debug)
		printf("enter statd_init\n");
	if (bindhostname)
		strcpy(hostname, bindhostname);
	else
		(void) gethostname(hostname, NAME_MAX);
	if ((fd = open(STATE, O_RDWR|O_CREAT, 0644)) == -1) {
		perror("rpc.statd cannot open state file");
		exit(1);
	}
...
xdr_notify(xdrs, ntfp)
	...
	stat_chge *ntfp;
{
	if (!xdr_string(xdrs, &ntfp->name, NAME_MAX+1)) {
		return(FALSE);
	}

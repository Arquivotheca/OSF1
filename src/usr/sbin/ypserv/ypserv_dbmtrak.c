 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ypserv_dbmtrak.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/04/01 16:50:35 $";
#endif
/*
 */
...
check_db()
{
	...
	if (ok == 0) {
		if (gotdb->db)
			dbm_close(gotdb->db);
		gotdb->db = dbm_open(gotdb->name,0,0);

 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ypserv_proc.c,v $ $Revision: 4.2.2.6 $ (DEC) $Date: 1992/12/22 10:42:10 $";
#endif
/*
 */
...
extern char    *environ;
#ifndef YPXFR_PROC
#define YPXFR_PROC "/var/yp/ypxfr"
#endif
static char     ypxfr_proc[] = YPXFR_PROC;
#ifndef YPPUSH_PROC
#define YPPUSH_PROC "/var/yp/yppush"
#endif
...
extern int      debuginterdomain;
extern int      dbinterdomain;
	...
	for (prev = children, chl = children; chl;) {
		...
		if ((chl->pid == 0) && (chl->enqtime.tv_sec + secs) < now.tv_sec) {
			...
			if (0 == strcmp(map, chl->map))
				if (chl->key.dptr) {
					if (keydat.dptr[keydat.dsize - 1] == NULL)

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: readfile.c,v $ $Revision: 4.2.4.3 $ (DEC) $Date: 1994/01/24 20:23:47 $";
#endif
/*
 */
...
PRIVATE int modtime = 0;	/* Last modification time of bootptab */
...
char *src;
{
	...
	switch (retval) {
		...
		if (!(hp->flags.subnet_mask)) {
			...
			case 0:
			case 1:
			    hp->subnet_mask.s_addr = htonl(0xFF000000);
			    break;
			case 2:
			    hp->subnet_mask.s_addr = htonl(0xFFFF0000);
			    break;
			case 3:
			    hp->subnet_mask.s_addr = htonl(0xFFFFFF00);
			    break;
		    }
...
struct host *hp;
{
	...
	case SYM_HTYPE:
	    switch (optype) {
		case OP_ADDITION:
		    value = 0;			/* Assume an illegal value */
		    eat_whitespace(symbol);
		    if (isdigit(**symbol)) {
			value = get_u_long(symbol);
		    } else {
...
{
    /*
     * up to first '.' is the hostname - allow '_' in hostname
     */
    if (!isalpha(*hostname++)) {	/* First character must be a letter */
        return FALSE;
    }
    while (isalnum(*hostname) || (*hostname == '-') || (*hostname == '_')) {
	hostname++;		   /* Alphanumeric or a hyphen or a underbar */
    }
 ...
 * to point to the next data position.   "result" points to an unsigned int
 ...
 * int pointed to by "result".  For malformed addresses, -1 is returned,
 * (*src) points to the first illegal character, and the unsigned int pointed
 ...
 * returned as an unsigned int and the character pointer is updated to
 * point to the first illegal character.
 */

PRIVATE u_int get_u_long(src)
char **src;
{
    register u_int value, base;
    char c;

 ...
 */
#ifndef lint
static char *sccsid = "@(#)$RCSfile: portmap.c,v $ $Revision: 4.3.3.5 $ (DEC) $Date: 1993/02/15 15:25:03 $";
#endif
/*
 */
/*
 ...
 * Copyright (c) 1984,1990 by Sun Microsystems, Inc. ALL RIGHTS RESERVED.
 */
	...
	register struct rmtcallargs *cap;
{
	...
	if (xdr_u_int(xdrs, &(cap->rmt_prog)) &&
	    xdr_u_int(xdrs, &(cap->rmt_vers)) &&
	    xdr_u_int(xdrs, &(cap->rmt_proc))) {
...
xdr_rmtcall_result(xdrs, cap)
	...
	register struct rmtcallargs *cap;
{
	if (xdr_u_int(xdrs, &(cap->rmt_port)))
		return (xdr_encap_parms(xdrs, &(cap->rmt_args)));
	return (FALSE);
}
	...
	SVCXPRT		*xprt;
{
	...
	if (!xdr_u_int(&outxdr, &(a.rmt_proc)))
		return;
...
handle_reply()
{
	...
	a.rmt_port = (u_int)ntohs(from.sin_port);
	if (forward_find(reply_msg.rm_xid, ntohs(from.sin_port), &to, &xto)) {
		if (chklocal(ntohl(from.sin_addr)) == FALSE) {
			if (verbose)
				fprintf(stderr,
"portmap: attempt to forward reply from host %s port %d to host %s port%d\n",
		...
		if (debugging)
			printf(
"portmap: lookup ok xid=%d port=%d to=%s.%d xto=%d\n",
				reply_msg.rm_xid, ntohs(from.sin_port),
				inet_ntoa(to.sin_addr),

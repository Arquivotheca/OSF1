 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: doname.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1992/11/20 17:24:45 $";
#endif

/* static char	*sccsid = "@(#)doname.c	4.2	(ULTRIX)	10/15/90"; */
 ...
 */
	...
	for(lp = p->linep ; lp!=0 ; lp = lp->nextline)
	{
		...
		for(q = lp->depp ; q!=0 ; q=q->nextdep)
		{
			...
			{
			    blprt(reclevel);
			    printf("TIME2(%s)=%s", q->depname->namep, ctime(&td1));
			}
			td = max(td1,td);
			if(ptime < td1)
...
searchdir:
			...
			for(suffp1=lp1->depp;suffp1!=0;suffp1 = suffp1->nextdep)
			{
				...
				{
					sourcename[strlen(sourcename) - 1] =
									CNULL;
					if(!sdot(sourcename))
						trysccs(sourcename);
				}
 ...
 */
				...
				if(IS_ON(DBUG))
				{
					TIMETYPE foo= exists(p);
					
					blprt(reclevel);
					printf("TIME3(%s)=%s",p->namep,ctime(&foo));
					if(td > foo) {
						blprt(reclevel);
						printf("Building %s using inference rule %s, because it is out of date relative to %s\n", p->namep, concsuff, p2->namep);
					}
				}
				tdep = max(tdep, td);
				setvar("*", prefix);
 ...
 */
			pos = rindex(prefix, '/');
			if(pos == NULL) setvar("*", copys(prefix));
			else setvar("*", copys(pos + 1));
			if(onetime == 1)
				goto endloop;
		}
	}

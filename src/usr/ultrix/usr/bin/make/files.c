 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: files.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1992/11/20 17:25:07 $";
#endif

/* static char	*sccsid = "@(#)files.c	4.1	(ULTRIX)	8/17/88"; */
...
/* | consent of MIPS.                                               | */
/* ------------------------------------------------------------------ */
/* $Header: /usr/sde/alpha/rcs/alpha/src/./usr/ultrix/usr/bin/make/files.c,v 1.1.2.2 1992/11/20 17:25:07 Robin_Miller Exp $ */
/*
...
char *name;
{
	...
	*/
	...
	else	/* ar_type == ARport */
	{
		...
		*/
		if ((offs = calloc(num_symbols,sizeof(long))) == NULL)
		{
			fatal1("Cannot calloc offsets table for archive %s",
				archname);
		}
		if (fread(offs, sizeof(long), num_symbols, arfd) != num_symbols)
			goto read_error;
		strtablen = sym_size - ((num_symbols + 1) * sizeof(long));
		if ((syms = calloc(1,strtablen)) == NULL)
		{
			fatal1("Cannot calloc string table for archive %s",
				archname);
		}

...
/* static char	*sccsid = "@(#)gram.y	4.2	(ULTRIX)	12/14/88"; */

%{
#ifndef lint
static char *rcsid = "@(#)$RCSfile: gram.y,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1992/11/20 17:25:16 $";
#endif
#include "defs"
%}
 ...
 */
	...
	{
		if(linesptr[0])
			while(linesptr[1] && *linesptr[1] == NEWLINE)
			{
				yylineno++;
				linesptr++;
			}
		...
		for(p=yytext ; ; ++p)
		{
			...
			else if (kc == NEWLINE)
			{
				...
				if(p==yytext || p[-1]!=BACKSLASH)
					break;
				if(sysvmode && (incom || yytext[0] == TAB))
					*p++ = NEWLINE;
				else
					p[-1] = BLANK;
				nflg = YES;

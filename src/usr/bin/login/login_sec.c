 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: login_sec.c,v $ $Revision: 4.2.7.2 $ (DEC) $Date: 1992/03/19 12:39:06 $";
#endif
/*
...
#include "login_msg.h" 
nl_catd catd;
#define MSGSTR_SEC(n,s) catgets(catd,MS_LOGIN_SEC,n,s) 
#else
...
#endif
static int login_type = ES_LOGIN_LOCAL;
	...
	register int *pnopassword;
{
	...
	printf(MSGSTR_SEC(LOGIN_SEC_1, "You don't have a password.\n"));
	(void) fflush(stdout);
	...
	if (pr == (struct pr_passwd *) 0)  {
		audit_login(pr, (struct passwd *) 0, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_2,
			"protected password entry vanished after write"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_3,
	...
	if (passwd_status < 0) {
		audit_login(pr, (struct passwd *) 0, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_4,
			"cannot execute passwd program"), ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_5,
		...
		audit_login(pr, (struct passwd *) 0, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_6,
			"unsuccessful password change"), ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_7,
	...
	register struct pr_term *prtc;
{
	...
	if (expiration_time && now > expiration_time)  {
		audit_auth_entry(pr->ufld.fd_name, OT_PRPWD,
                        MSGSTR_SEC(LOGIN_SEC_8, "password expired"), ET_LOGIN);
		printf (MSGSTR_SEC(LOGIN_SEC_9, "Your password has expired.\n"));
		(void) fflush(stdout);
		...
		if (pr == (struct pr_passwd *) 0)  {
			audit_login(pr, (struct passwd *) 0,
				prtc->ufld.fd_devname, MSGSTR_SEC(LOGIN_SEC_2,
				"protected password entry vanished after write"),
				ES_LOGIN_FAILED);
			fprintf (stderr, MSGSTR_SEC(LOGIN_SEC_3,
			"Protected Password information suddenly vanished\n"));
		...
		if (passwd_status < 0) {
			audit_login(pr, (struct passwd *) 0,
				prtc->ufld.fd_devname, MSGSTR_SEC(LOGIN_SEC_4,
				"cannot execute passwd program"),
				ES_LOGIN_FAILED);
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_5,
				"Cannot execute passwd program\n"));
			...
			audit_login(pr, (struct passwd *) 0,
				prtc->ufld.fd_devname,
		MSGSTR_SEC(LOGIN_SEC_6,	"unsuccessful password change"),
				ES_LOGIN_FAILED);
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_10,
		...
		printf(MSGSTR_SEC(LOGIN_SEC_11, "\nYour password will expire on %s\n"),
	...
	struct pr_term *prtc;
{
	...
	if (strcmp(pr->ufld.fd_name, NO_ENTRY) == 0)  {
		...
		if ((prtc != (struct pr_term *) 0) && prtc->uflg.fg_devname)
			audit_login(pr, (struct passwd *) 0,
				prtc->ufld.fd_devname,
	MSGSTR_SEC(LOGIN_SEC_13, "login name doesn't exist"), ES_LOGIN_FAILED);
		else
			audit_login(pr, (struct passwd *) 0, (char *) 0,
	MSGSTR_SEC(LOGIN_SEC_13, "login name doesn't exist"), ES_LOGIN_FAILED);
 ...
 */
	...
	if (!putprpwnam(pr->ufld.fd_name, pr))  {
		audit_login(pr, (struct passwd *) 0, prtc->ufld.fd_devname,
	MSGSTR_SEC(LOGIN_SEC_14, "can't rewrite protected password entry"),
			ES_LOGIN_FAILED);
		fprintf (stderr, MSGSTR_SEC(LOGIN_SEC_15,
 ...
 */
	...
	if (pr == (struct pr_passwd *) 0)  {
		audit_login(pr, (struct passwd *) 0, prtc->ufld.fd_devname,
MSGSTR_SEC(LOGIN_SEC_2, "protected password entry vanished after write"),
			ES_LOGIN_FAILED);
		fprintf (stderr, MSGSTR_SEC(LOGIN_SEC_16,
	...
	if ((prtc != (struct pr_term *) 0) && prtc->uflg.fg_devname)
		...
		audit_login(pr, (struct passwd *) 0, (char *) 0, (char *) 0,
			ES_LOGIN_FAILED);
	...
	register struct passwd *p;
{
	...
	if (locked_out(pr))  {
		if (su_on_console)
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_17,
	"Account is disabled but console login is allowed.\n"));
		else  {
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_18,
	...
	if (!auth_for_terminal(pr,prtc))  {
		if (su_on_console)
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_19,
	"Not authorized for terminal access but console login is allowed.\n"));
		else  {
#if SEC_NET_TTY
                        if (is_network)
                                fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_20,
"Not authorized for access from your host -- see System Administrator.\n"));
			else  
#endif
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_21,
	"Not authorized for terminal access -- see System Administrator.\n"));
			audit_login(pr, p, prtc->ufld.fd_devname,
	 MSGSTR_SEC(LOGIN_SEC_22, "user not authorized for terminal/host"),
				 ES_LOGIN_FAILED);
			cancel_process(1, pr, prtc);
		}
	}
	...
	if (pr->uflg.fg_retired && pr->ufld.fd_retired) {
		audit_login(pr, p, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_23, "account has been retired"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_24,
	...
	if (time_lock(pr))  {
		if (su_on_console)
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_25,
			"Wrong time period but console login is allowed.\n"));
		else  {
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_26,
	...
	if (save_tty_path == (char *) 0 ||
	    ...
	    chmod(save_tty_path, 0600) == -1)  {
		audit_login(pr, p, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_27, "cannot set terminal mode"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_28,
	...
	if (!pr->uflg.fg_uid || (setluid(pr->ufld.fd_uid) != 0))  {
		audit_login(pr, p, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_29, "bad login user id"),
                        ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_30, "Bad login user id.\n"));
		cancel_process(1, pr, prtc);
	}
	...
	if (!get_sec_level(pr, prtc))  {
		 ...
		 */
		audit_login(pr, p, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_31, "bad session security level"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_32,
...
#if SEC_NCAV
	if (!pr->uflg.fg_nat_caveats || !get_caveat_set(pr, prtc))  {
		/*
		 * Don't distinguish between a set not dominated by the user's
		 * maximum and an invalid set, so that the user cannot
		 * determine names of caveat sets that he may not be
		 * allowed to know of.
		 */
		audit_login(pr, p, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_33, "bad caveat set"),
                        ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_34,
			"Bad maximum, citizenship, or caveat set.\n"));
		(void) fflush(stderr);
...
#if SEC_NET_TTY
		if (is_network)
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_35,
				"Can't set host security level.\n"));
                else
#endif
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_36,
			"Can't set terminal security level.\n"));
		audit_login(pr, p, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_37,
	...
	fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_39, "Last   successful login for %s: %s"),
	...
	if (pr->uflg.fg_suctty) {
		...
		if (hp == (struct hostent *) 0)
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_40,
				" from (unknown)"));
		else
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_41,
				" from %s"), hp->h_name);
		endhostent();
            }
            else
#endif
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_42,
	...
	fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_43,
	...
	char *reason;
{
	...
	if (delay_between_attempts == 0)
		return;
	printf(MSGSTR_SEC(LOGIN_SEC_44, "\nWait for login %s: "), reason);
	(void) fflush(stdout);
...
#ifdef DEBUG
printf (MSGSTR_SEC(LOGIN_SEC_45,
	"Login_fillin_user: Before getprpwnam %s ppr 0x%x\n"), user, ppr);
#endif
	*ppr = getprpwnam(user);
#ifdef DEBUG
printf (MSGSTR_SEC(LOGIN_SEC_46, "After getprpwnam ppr 0x%x\n"), *ppr);
#endif
	...
	int *pnopassword;
{
	...
	if (*pprpwd == (struct pr_passwd *) 0) {
		audit_login((struct pr_passwd *) 0, (struct passwd *) 0,
                        (*pprtc)->ufld.fd_devname, MSGSTR_SEC(LOGIN_SEC_48,
	...
	if (*pprtc == (struct pr_term *) 0 &&
		...
		audit_login(*pprpwd, (struct passwd *) 0, (char *) 0,
MSGSTR_SEC(LOGIN_SEC_49, "no entry for terminal in terminal control database"),
	...
	if ((*pprpwd)->uflg.fg_pwchanger) {
		...
		printf(MSGSTR_SEC(LOGIN_SEC_51,
	...
	if ((*pprpwd)->ufld.fd_encrypt[0] == '\0')
		...
		if(strcmp(login_crypt(MSGSTR_SEC(LOGIN_SEC_52, "Password:"),
				(*pprpwd)->ufld.fd_encrypt),
			...
			printf(MSGSTR_SEC(LOGIN_SEC_53, "Login incorrect\n"));
			if ((*pprtc != (struct pr_term *) 0) &&
			    (*pprtc)->uflg.fg_devname)
				audit_login(*pprpwd, (struct passwd *) 0,
					(*pprtc)->ufld.fd_devname, 
		MSGSTR_SEC(LOGIN_SEC_54, "login incorrect"), ES_LOGIN_FAILED);
			else
				audit_login(*pprpwd, (struct passwd *) 0,
		(char *) 0, MSGSTR_SEC(LOGIN_SEC_54, "login incorrect"),
					ES_LOGIN_FAILED);
...
#ifdef DEBUG
printf (MSGSTR_SEC(LOGIN_SEC_56, "login_term_params: start tty_path %s tty_name %s\n"),
tty_path, tty_name);
#endif
...
#ifdef DEBUG
printf (MSGSTR_SEC(LOGIN_SEC_57, "login_term_params: after check_devasg tty_path, pr 0x%x\n"), pr);
#endif
	if (pr == (struct pr_term *) 0)  {
		audit_login((struct pr_passwd *) 0, (struct passwd *) 0,
                        (char *) 0, MSGSTR_SEC(LOGIN_SEC_58,
			"current terminal not in terminal control database"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_59,
	...
	if ((fstat(fileno(stdin), &in_buf) != 0) ||
		...
		audit_login((struct pr_passwd *) 0, (struct passwd *) 0,
	(char *) 0, MSGSTR_SEC(LOGIN_SEC_61, "stdin/out/err are not the same"),
		ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_62,
	...
	{
		audit_login((struct pr_passwd *) 0, (struct passwd *) 0,
			(char *) 0, MSGSTR_SEC(LOGIN_SEC_63,
			"could not get terminal settings"),
                        ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_64,
	...
	current_prtc = pr;
#ifdef DEBUG
printf (MSGSTR_SEC(LOGIN_SEC_65, "login_term_params: exit pr 0x%x\n"), pr);
#endif
	return pr;
}
        ...
        char *hostname;
{
	...
	login_type = ES_LOGIN_REMOTE;
		...
		audit_auth_entry(hostname, 0, MSGSTR_SEC(LOGIN_SEC_66,
			"current host is not in \'/etc/hosts\'"), ET_LOGIN);
                fprintf(MSGSTR_SEC(LOGIN_SEC_67,
		...
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_69,
	...
	char *envinit[];
{
	...
	printf(MSGSTR_SEC(LOGIN_SEC_70, "No login program on root\n"));
}
#endif
 ...
 */
	...
	if (!putprtcnam(pr->ufld.fd_devname, pr))  {
		fprintf (stderr, MSGSTR_SEC(LOGIN_SEC_71,
 ...
 */
	...
	current_prtc = pr;
	if (pr == (struct pr_term *) 0)  {
		audit_login(prpw, (struct passwd *) 0, pr->ufld.fd_devname,
MSGSTR_SEC(LOGIN_SEC_72, "terminal control information vanished after write"),
                        ES_LOGIN_FAILED);
		fprintf (stderr,
 ...
 */
	...
	if (!putprtcnam(pr->ufld.fd_devname, pr))  {
		audit_login(prpw, (struct passwd *) 0, pr->ufld.fd_devname,
MSGSTR_SEC(LOGIN_SEC_74, "can't rewrite updated terminal control entry"),
                        ES_LOGIN_FAILED);
		fprintf (stderr, MSGSTR_SEC(LOGIN_SEC_71,
			"Can't rewrite terminal control entry for %s\n"),
			 pr->ufld.fd_devname);
 ...
 */
	...
	if (pr == (struct pr_term *) 0)  {
		audit_login(prpw, (struct passwd *) 0, pr->ufld.fd_devname,
MSGSTR_SEC(LOGIN_SEC_75, "terminal control entry vanished after write"),
                        ES_LOGIN_FAILED);
		fprintf (stderr,
MSGSTR_SEC(LOGIN_SEC_73, "Terminal Control information suddenly vanished\n"));
 ...
 */
	...
	if (!putprpwnam(prpwd->ufld.fd_name, prpwd))  {
		audit_login(prpwd, pwd, prtc->ufld.fd_devname,
                        MSGSTR_SEC(LOGIN_SEC_76,
...
MSGSTR_SEC(LOGIN_SEC_3, "Protected Password information suddenly vanished\n"));
	...
	if (prpwd->uflg.fg_nice || prpwd->sflg.fg_nice)  {
		...
		if (errno != 0)  {
			audit_login(prpwd, pwd, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_77, "bad priority setting"),
			ES_LOGIN_FAILED);
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_78,
	...
	if (setgid(pwd->pw_gid) != 0) {
		audit_login(prpwd, pwd, prtc->ufld.fd_devname, MSGSTR_SEC(LOGIN_SEC_79, "bad group id"),
                        ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_80, "Bad group id.\n"));
		login_delay("exit");
		exit(1);
	}
#if SEC_GROUPS
	...
	if (setuid(pwd->pw_uid) != 0) {
		audit_login(prpwd, pwd, prtc->ufld.fd_devname, MSGSTR_SEC(LOGIN_SEC_81, "bad user id"),
                        ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_82, "Bad user id.\n"));
		login_delay("exit");
		exit(1);
	}
	...
	if (setsysauths(sysprivs) != 0) {
		audit_login(prpwd, pwd, prtc->ufld.fd_devname,
			MSGSTR_SEC(LOGIN_SEC_83,
			"unable to set kernel authorizations"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_84,
			"Unable to set kernel authorizations.\n"));
		login_delay("exit");
		exit(1);
	}
	if (setbaseprivs(baseprivs) != 0)  {
		audit_login(prpwd, pwd, prtc->ufld.fd_devname,
		MSGSTR_SEC(LOGIN_SEC_85, "unable to set base privileges"),
		ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_86,
	...
	/*
         ...
         */
        audit_login(prpwd, pwd, prtc->ufld.fd_devname, (char *) 0, login_type);
...
timeout()
{
	...
	fflush(stdout);
	audit_login(current_prpwd, (struct passwd *) 0,
		current_prtc->ufld.fd_devname,
		MSGSTR_SEC(LOGIN_SEC_87, "login timed out"),
		ES_LOGIN_FAILED);
	fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_88, "\nLogin timed out\n"));
	fflush(stderr);
	timeout_occurred = 1 ;
	...
	register struct pr_passwd *prpw;
{
	...
	if (locked || ((max_tries != 0) && (attempts >= max_tries)))  {
		 audit_lock(pr->ufld.fd_devname, ES_TERM_LOCK, attempts,
                        ET_LOGIN);
		if (su_on_console)
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_89,
			"Terminal is disabled but root login is allowed.\n"));
		else  {
#if SEC_NET_TTY
			if (is_network)
                                fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_90,
		"Host is disabled -- see Account Administrator. \n"));
                        else 
#endif
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_91,
...
struct pr_term *prtc;
{
	...
	fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_92,
		"\nThe security databases are corrupt.\n"));
	fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_93,
	...
	fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_94,
	...
	register struct pr_term *prtc;
{
	...
	tty = fopen("/dev/tty", "r+");
	if (tty == (FILE *) 0)  {
		audit_login(prpwd, (struct passwd *) 0, prtc->ufld.fd_devname,
		MSGSTR_SEC(LOGIN_SEC_95, "cannot request sensitivity level"),
		ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_96,
			"Cannot request sensitivity level.\n"));
		cancel_process(1, prpwd, prtc);
	}

	if ((er_buffer = malloc(BUFSIZ)) == (char *) 0) {
		audit_login(prpwd, (struct passwd *) 0, prtc->ufld.fd_devname,
		MSGSTR_SEC(LOGIN_SEC_95, "cannot request sensitivity level"),
		ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_96,
	...
	if (valid_sec_level)  {
		...
		if (sec_level_er != (char *) 0) {
			...
			if (er_buffer) {
				...
				printf(MSGSTR_SEC(LOGIN_SEC_98,
					"Sensitivity level for process: "));
				printbuf(sec_level_er,32,",/ ");
				printf("\n");
				(void) fflush(stdout);
			} else {
				fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_99,
					"Sensitivity label is too long\n"));
				valid_sec_level = 0;
			}
		}
		else {
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_100,
	...
	if (is_network) {
                ...
                if (ttynm == NULL) {
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_101,
...
#endif
	{
		...
		if((ttynm == NULL) || ((ttynm = strrchr(ttynm,'/')) == NULL)) {
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_102,
	...
	if((prdevasg = getdvagnam(ttynm)) == (struct dev_asg *) 0) {
		fprintf(stderr,MSGSTR_SEC(LOGIN_SEC_103,
...
register struct pr_term *prtc;
{
	...
	if (tty == (FILE *) 0)  {
		audit_login(prpwd, (struct passwd *) 0, prtc->ufld.fd_devname,
                        MSGSTR_SEC(LOGIN_SEC_104,
			"error in getting security level from /dev/tty"),
                        ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_105,
	...
	if ((ncav_buffer = malloc(BUFSIZ)) == (char *) 0) {
		audit_login(prpwd, (struct passwd *) 0, prtc->ufld.fd_devname,
                        MSGSTR_SEC(LOGIN_SEC_106, "cannot request caveat set"),
			ES_LOGIN_FAILED);
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_105,
			"Cannot request caveat set.\n"));
		cancel_process(1, prpwd, prtc);
	...
	if (ncav_er != (char *) 0) {
		...
		if (ncav_buffer) {
			...
			printf(MSGSTR_SEC(LOGIN_SEC_109,
				"Caveat set for process: "));
			printbuf(ncav_er,24,",");
			printf("\n");
			(void) fflush(stdout);
		} else {
			fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_110,
				"Caveat set is too long\n"));
			return(0);
		}
	}
	else {
		fprintf(stderr,MSGSTR_SEC(LOGIN_SEC_111,
	...
	if((prdevasg = getdvagnam(ttynm)) == (struct dev_asg *) 0) {
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_112,
...
#endif
{
	...
	if((dev = getdvagnam(prtc->ufld.fd_devname)) == (struct dev_asg *) 0) {
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_113,
	...
	if(stat(newdev,&statbuf) == -1) {
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_114,
	...
	if (is_network) {
                ...
                if(link(newdev,remdev) == -1) {
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_118,
	...
	if (is_network) {
                if(chslabel(remdev,mand_ir) == -1) {
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_119,
	...
	if (is_network) {
                if(chncav(remdev,ncav_ir) == -1) {
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_120,
	...
	if (is_network) {
                if((fd = open(remdev,O_RDWR)) == -1) {
                        fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_121,
	...
	if (ioctl(0, TIOCSCTTY, 0) < 0)
		fprintf(stderr, MSGSTR_SEC(LOGIN_SEC_122,

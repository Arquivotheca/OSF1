 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: checknr.c,v $ $Revision: 4.2.6.2 $ (DEC) $Date: 1993/10/11 16:00:37 $";
#endif 
/*
 */
/*
 ...
 *
 * checknr.c	1.2  com/bsd.d/checknr.d,3.1,9021 11/18/89 17:53:31
 */
...
/*
 * checknr: check an nroff/troff input file for matching macro calls. we also
 * attempt to match size and font changes, but only the embedded kind.  These
 * must end in \s0 and \fP resp.  Maybe more sophistication later but for now
 * think of these restrictions as contributions to structured typesetting. 
 */
#include <stdio.h>
#include <ctype.h>
#include <locale.h>
...
#include <NLchar.h>
/* NOTE:  \sxx commands expect the xx number to be in ascii */

#include "checknr_msg.h"
#include "nl_types.h"
nl_catd         catd;
#define MSGSTR(n,s) catgets(catd,MS_CHECKNR,n,s)
...
/*
 * All commands known to nroff, plus macro packages. Used so we can complain
 * about unrecognized commands. 
 */
    ...
    char          **argv;
{
    ...
    setlocale (LC_ALL, "");
    catd = catopen (MF_CHECKNR, NL_CAT_LOCALE);
...
usage ()
{
    printf (MSGSTR (USAGE, "Usage: checknr [-s] [-f] [-a.x1.y1.x2.y2....xn.yn] [-c.x1.x2.x3....xn] [file...]\n"));	/* MSG */
    exit (1);
}
    ...
    FILE           *f;
{
	...
	if (line[0] == '.')
	{
	    ...
	    {
		pe (lineno);
		printf (MSGSTR (EMPTY, "Specify a parameter with the -c or -a flag.\n"));	/* MSG */
	    } else
	    if (isspace (mac[1]))
	    {
		mac[1] = 0;
	    ...
	    {
		pe (lineno);
		printf (MSGSTR (TOOLONG, "Command too long\n"));	/* MSG */
	    }
	    /*
	...
	/*
	 * At this point we process the line looking for \s and \f. 
	 */
	j = 0;
	for (i = 0; line[i]; i += j = NCchrlen (line[i]))
	{
	    c = line[i];
	    c2 = line[i - 1];
	    if (NCisshift (c))
		...
		if (!sflag && c == (NLchar) 's')
		{
			...
			{
			    pe (lineno);
			    printf (MSGSTR (SUNMATCHED, "unmatched \\s0\n"));	/* MSG */
		...
		if (!fflag && line[i] == 'f')
		{
			...
			{
			    pe (lineno);
			    printf (MSGSTR (FUNMATCHED, "unmatched \\fP\n"));	/* MSG */
			}
			/* unfolding */
		    } else
		    {
    ...
    /*
     * We've hit the end and look at all this stuff that hasn't been matched
     * yet!  Complain, complain. 
     */
    for (i = stktop; i >= 0; i--)
    {
	complain (i);
...
{
    pe (stk[i].lno);
    printf (MSGSTR (UNMATCHED, "Unmatched "));	/* MSG */
    prop (i);
    printf ("\n");
}
...
prop (i)
{
	...
	case FT:
	    ...
	    break;
	default:
	    printf (MSGSTR (BUG, "Bug: stk[%d].opno = %d = .%s, .%s"),	/* MSG */
    ...
    char           *mac;
{
	...
	for (i = 0; br[i].opbr; i++)
	{
	    ...
	    /*
	     * Maybe it's an unmatched closer. NOTE: this depends on the fact
	     * that none of the closers can be openers too. 
	     */
	    if (eq (mac, br[i].clbr))
	    {
		nomatch (mac);
    ...
    char           *mac;
{
    ...
    /*
     * Look for a match further down on stack If we find one, it suggests
     * that the stuff in between is supposed to match itself. 
     */
    for (j = stktop; j >= 0; j--)
	if (eq (mac, br[stk[j].opno].clbr))
	{
	    ...
	    {
		/*
		 * Check for special case \fx..\fR and don't complain. 
		 */
		...
		/*
		 * We have two unmatched frobs.  Chances are they were
		 * intended to match, so we mention them together. 
		 */
		pe (stk[j + 1].lno);
		prop (j + 1);
		printf (MSGSTR (DOESNOTMATCH, " does not match %d: "), stk[j + 2].lno);	/* MSG */
		prop (j + 2);
		printf ("\n");
	    } else
    ...
    pe (lineno);
    printf (MSGSTR (MACUNMATCHED, "Unmatched .%s\n"), mac);	/* MSG */
}
    ...
    char           *mac;
{
    ...
    pe (lineno);
    printf (MSGSTR (UNKCOM, "unknown command: .%s\n"), mac);	/* MSG */
}
    ...
    char           *line;
{
    ...
    {
	pe (lineno);
	printf (MSGSTR (ILLEGAL, "illegal define: %s\n"), line);	/* MSG */
	return;
    }
    mac[2] = 0;
    ...
    if (ncmds >= MAXCMDS)
    {
	printf (MSGSTR (COMALLOW, "Only %d known commands allowed\n"), MAXCMDS);	/* MSG */
	exit (1);
    }
    addmac (mac);
}
...
/*
 * Add mac to the list.  We should really have some kind of tree structure
 * here but this is a quick-and-dirty job and I just don't have time to mess
 * with it.  (I wonder if this will come back to haunt me someday?)  Anyway,
 * I claim that .de is fairly rare in user nroff programs, and the register
 * loop below is pretty fast. 
 */
addmac (mac)
    char           *mac;
{
...
/*
 * Do a binary search in knowncmds for mac. If found, return the index.  If
 * not, return -1. 
 */
binsrch (mac)
    char           *mac;
{
    ...
    register int    top,
                    bot;	/* boundaries of bin search, inclusive */

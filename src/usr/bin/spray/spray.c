 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: spray.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/05/26 17:44:55 $";
#endif
	...
	char **argv;
{
	...
	double	delta;
	extern  unsigned int   ntohl(), htonl();
	...
	if (host == NULL)
		usage();
	if (isinetaddr(host)) {
		adr.s_addr = (unsigned int) htonl(inet_addr(host));
		host = adrtostr(adr);
	} else {
	...
	if (cumul.counter < cnt) {
		printf("\t%d packets (%.2f%%) dropped by %s\n",
			cnt - cumul.counter,
		...
		printf("\tno packets dropped by %s\n", host);
		psec = (1000000.0 * cumul.counter) / delta;
		bsec = (lnth * 1000000.0 * cumul.counter) / delta;

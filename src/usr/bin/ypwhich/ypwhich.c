 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ypwhich.c,v $ $Revision: 4.2.2.4 $ (DEC) $Date: 1992/04/01 14:46:25 $";
#endif
/*
 */
...
{		
	...
	host_addr.s_addr = *(u_int *)hp->h_addr;
}
	...
	struct in_addr *server;
{
	...
	if (server->s_addr == myaddr.sin_addr.s_addr) {
		...
		/** hack because vax yp code doesn't do htonl() with addresses **/
		if((hp = gethostbyaddr(server, sizeof(*server), AF_INET)) == NULL){
			server->s_addr = htonl(server->s_addr);
			hp = gethostbyaddr(server, sizeof(*server), AF_INET); 
		}

			if (!hp)
		        server->s_addr = ntohl(server->s_addr);

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: io.c,v $ $Revision: 1.1.4.2 $ (DEC) $Date: 1993/05/25 20:08:22 $";
#endif
 ...
 *
 * January 21, 1993 by Robin Miller.
 *	Modified logic associated with handling of too many open files
 *	so reaching the system limit (ENFILE) is treated as an error.
 *	Only close and try again if the process limit is reached.
 */
...
#if defined(atarist) || defined(_OSF_SOURCE) || defined(ultrix)
			/*
			 * On DEC systems, we only close and continue if
			 * the process limit is reached.  If the system
			 * limit is reached (ENFILE), we want to abort
			 * with an error, since no other processes are
			 * able to open any files (the system is hosed).
			 */
			if (errno == EMFILE)
#else

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: main.c,v $ $Revision: 1.1.6.2 $ (DEC) $Date: 1993/10/01 01:38:02 $";
#endif
	...
	for (i = 1; i < argc; i++) {
            ...
            if (!(strcmp(argv[i],"-v"))) 
			  ...
			  pre_assign(argv[i]);
                          ...
                          for (vint = i; vint < argc - 1; vint++)  
                              argv[vint] = argv[vint+1];
                          argc--; 
                          i--;

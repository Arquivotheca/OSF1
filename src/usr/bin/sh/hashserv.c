...
static char rcsid[] = "@(#)$RCSfile: hashserv.c,v $ $Revision: 4.2.5.2 $ (DEC) $Date: 1993/06/10 15:38:19 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 *	1.13  com/cmd/sh/sh/hashserv.c, bos320, 9142320i 10/16/91 09:54:00
 */
	...
	register uchar_t *name;
{
		...
		switch (hashtype(h->data))
		{
			...
			case COMMAND: {
				...
				sprintf ((char *)msgbuf, temp,
					(char *)name, (char *)rtn_path(name, hashval));
				prs_buff(msgbuf);
				return;
			}
		}
	...
	if ((cnt = findpath(name, 0)) > 0)
	{
		...
		sprintf ((char *)msgbuf, temp, 
				(char *)name, (char *)rtn_path(name, cnt));
		prs_buff(msgbuf);
	}
	else
	{

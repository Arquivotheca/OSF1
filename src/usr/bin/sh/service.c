...
static char rcsid[] = "@(#)$RCSfile: service.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/01 19:18:40 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 *	1.22  com/cmd/sh/sh/service.c, cmdsh, bos320, 9138320 9/9/91 15:16:02
 */
        ...
        i;    /* pid to wait-for */
{
        ...
        while (pwc)
        {
                 ...
                 */
                if ( trapcom [SIGCHLD] && strlen ((char *)trapcom [SIGCHLD]))
                {
                        p = trap_waitrc;
                        w = trap_status;
                }
...
uchar_t *s, *t; 
{
  	uchar_t *sdec = (uchar_t *) malloc(strlen((char *)s));
    	uchar_t *tdec = (uchar_t *) malloc(strlen((char *)t));
    	int r;

    	NLSdecode1(s, sdec);
    	NLSdecode1(t, tdec);
    	r = strcmp((char *)sdec, (char *)tdec);
    	alloc_free(sdec);
    	alloc_free(tdec);
   	return (r);
}
	...
	if (shaccton)
	{
		...
		if (!getrusage(RUSAGE_CHILDREN, &ru))
		{
			...
			sabuf.ac_rw    = compress(ru.ru_inblock+ru.ru_oublock,
						0);
			sabuf.ac_mem   = compress(ru.ru_ixrss+ru.ru_idrss,0);
		}

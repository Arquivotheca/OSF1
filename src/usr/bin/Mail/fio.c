...
static char rcsid[] = "@(#)$RCSfile: fio.c,v $ $Revision: 4.2.10.3 $ (DEC) $Date: 1993/09/07 18:16:18 $";
#endif
/*
 * HISTORY
 */
 ...
 */
/* 
 * COMPONENT_NAME: CMDMAILX fio.c
 * 
 * FUNCTIONS: Fdopen, MSGSTR, append, edstop, expand, fsize, getfold, 
 *            holdsigs, makemessage, opentemp, putline, readline, 
 *            relsesigs, remove, setinput, setptr 
 *
 * ORIGINS: 10  26  27 
 *
	...
	register struct message *mp;
{
	...
	if (fseek(itf, off, 0) < 0) {
		perror("fseek");
		panic(MSGSTR(SEEK, "temporary file seek")); /*MSG*/
	}
	return(itf);
}
...
makemessage(f)
{
	...
	if (mp == NULLSTR) {
		printf(MSGSTR(NOMEM, "Insufficient memory for %d messages\n"), msgCount); /*MSG*/
		exit(1);
	}
...
edstop()
{
	...
	for (mp = &message[0], gotcha = 0; mp < &message[msgCount]; mp++) {
		...
		if (Tflag != NULLSTR && (mp->m_flag & (MREAD|MDELETED)) != 0) {
			if ((id = hfield("article-id", mp)) != NULLSTR)
				fprintf(readstat, "%s\n", id);
		}
	}
	if (Tflag != NULLSTR)
		fclose(readstat);
	if (!gotcha || Tflag != NULLSTR)
		goto done;
	ibuf = NULL;
	if (stat(editfile, &statb) >= 0 && statb.st_size > mailsize) {
		sprintf(tempname, "%s/mboxXXXXXX", gettmpdir()); /*GAG*/
		mktemp(tempname);
	...
	fclose(obuf);
	/* mod for SVID-2, inplement keep environment variable */
	if (gotcha && value("keep") == NULLSTR) {
		remove(editfile);
		printf(MSGSTR(GONE, "removed\n")); /*MSG*/
	}
	else
		printf(MSGSTR(DONE, "complete\n")); /*MSG*/
	fflush(stdout);
	...
	char name[];
{
	...
	sprintf(cmdbuf, MSGSTR(ECHOM, "echo %s"), name); /*MSG*/
	if ((pid = vfork()) == 0) {
		sigchild();
		Shell = value("SHELL");
		if (Shell == NULLSTR)
	...
	s &= 0377;
	if (s != 0 && s != SIGPIPE) {
		fprintf(stderr, MSGSTR(NOECHO, "\"Echo\" failed\n")); /*MSG*/
		goto err;
	}
	if (l < 0) {
		perror("read");
	...
	if (l == 0) {
		fprintf(stderr, MSGSTR(NOPE, "\"%s\": No match\n"), name); /*MSG*/
		goto err;
	}
	if (l == BUFSIZ) {
		fprintf(stderr, MSGSTR(EXPBUF, "Buffer overflow expanding \"%s\"\n"), name); /*MSG*/
		goto err;
	}
	xname[l] = 0;
	for (cp = &xname[l-1]; *cp == '\n' && cp > xname; cp--)
	...
	*++cp = '\0';
#ifdef ASIAN_I18N
	if (mb_any(" ", xname, &mb) && stat(xname, &sbuf) < 0) {
#else
	if (any(' ', xname) && stat(xname, &sbuf) < 0) {
#endif
		fprintf(stderr, MSGSTR(AMB, "\"%s\": Ambiguous\n"), name); /*MSG*/
		goto err;
	}
	...
	char *name;
{
	...
	if ((folder = value("folder")) == NULLSTR)
		return(-1);
	if (*folder == '/')
		strcpy(name, folder);
	else

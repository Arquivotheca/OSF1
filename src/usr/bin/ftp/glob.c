 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: glob.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:34:47 $";
#endif 
/*
 */
/*
	...
	if (*cs == '~' && gpathp == gpath) {
		...
		if (!*cs || *cs == '/') {
			if (gpathp != gpath + 1) {
				*gpathp = 0;
				if (gethdir(gpath + 1))
					globerr = MSGSTR(UNKNOWN_USER, "Unknown user name after ~");
...
patherr1:
	closedir(dirp);
patherr2:
	globerr = MSGSTR(BAD_DIR, "Bad directory components");
}
	...
	register char *s1, *s2;
{
	...
	if (len >= gnleft || gargc >= GAVSIZ - 1)
		globerr = MSGSTR(ARG_TOO_LONG, "Arguments too long");
	else {
		gargc++;
		gnleft -= len;
		gargv[gargc] = 0;
	...
	char c;
{
	...
	if (gpathp >= lastgpathp)
		globerr = MSGSTR(PATH_TOO_LNG, "Pathname too long");
	else {
		*gpathp++ = c;
		*gpathp = 0;
	}
	...
	register char *cp, *dp;
{
	...
	if (ep == (char *)0)
		fatal(MSGSTR(FATAL_NO_MEM, "Out of memory"));
	(void) strcpy(ep, cp);
	(void) strcat(ep, dp);
	return (ep);
}

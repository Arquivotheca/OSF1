...
static char rcsid[] = "@(#)$RCSfile: list.c,v $ $Revision: 1.1.5.3 $ (DEC) $Date: 1993/09/03 03:16:48 $";
#endif
/*
 * HISTORY
 */
...
/*
 * getrdate - get the recent date in the format specified by the locale.
 *
 * DESCRIPTION
 *
 *	Construct the recent date according to format specified in the
 *	locale. The following construction rules are applied in order
 *	to find an applicable one.
 *
 *	1. The function call nl_langinfo(_M_D_RECENT), if applicable, is
 *	   use to retrieve the correct month-day format.
 *	2. The default format "%b %e %H%:M" is used.
 */
static char *getrdate(struct tm *atm)
{
	static char  date[64]		;
	char	    *dateform = NULL	;

#ifdef	_M_D_RECENT
	dateform = nl_langinfo(_M_D_RECENT) ;
#endif	/* _M_D_RECENT */
	if ((dateform == NULL) || (*dateform == '\0'))
		dateform = "%b %e %H:%M" ; /* Default format: "Jan 15 12:00" */
	date[0] = '\0' ;
	(void)strftime(date, sizeof(date), dateform, atm);
	return(date) ;
}

/*
 * getodate - get the old date in the format specified by the locale.
 *
 * DESCRIPTION
 *
 *	Construct the old date according to format specified in the
 *	locale. The following construction rules are applied in order
 *	to find an applicable one.
 *
 *	1. The function call nl_langinfo(_M_D_OLD), if applicable, is
 *	   use to retrieve the correct month-day format.
 *	2. The default format "%b %e  %Y" is used.
 */
static char *getodate(struct tm *atm)
{
	static char  date[64]		;
	char	    *dateform = NULL	;

#ifdef	_M_D_OLD
	dateform = nl_langinfo(_M_D_OLD) ;
#endif	/* _M_D_OLD */
	if ((dateform == NULL) || (*dateform == '\0'))
		dateform = "%b %e  %Y" ; /* Default format: "Jan 15 1993" */
	date[0] = '\0' ;
	(void)strftime(date, sizeof(date), dateform, atm);
	return(date) ;
}
...

{
	...
	fprintf(msgfile,"%7ld  %s %4d  ",
	               asb->sb_size, getrdate(atm),
		       atm->tm_year + 1900);
    }
...

{
	...
	if (f_verbose) {
	    ...
	    fprintf(msgfile," %d/%d %6d %s %4d %s",
		    asb->sb_uid, asb->sb_gid, asb->sb_size,
		    getrdate(atm), atm->tm_year + 1900, name);
	} else {
	    fprintf(msgfile, "%s", name);
	}
	switch (mode) {
...

{
	...
	if ((asb->sb_mtime < six_months_ago) || (asb->sb_mtime > now)) {
	    fprintf(msgfile," %s ", getodate(atm)) ;
	} else {
	    fprintf(msgfile," %s ", getrdate(atm)) ;
	}
	fprintf(msgfile, "%s", name);
	if ((asb->sb_nlink > 1) && (from = islink(name, asb))) {

...
static char rcsid[] = "@(#)$RCSfile: tar.c,v $ $Revision: 1.1.5.2 $ (DEC) $Date: 1993/06/10 20:37:26 $";
#endif
/*
 * HISTORY
 */
...

{
	...
	if (f_extract || f_list) {
	    ...
	    fprintf(stderr, MSGSTR(TAR_BSIZE, "Tar: blocksize = %d\n"), blocking);
	} else {
	    blocking = 1;
	}
#else /* !USG */
...

{
	...
	fprintf(stderr, MSGSTR(TAR_OPT, "%s: unknown option %c\n"), argv[0], c);
	return ('?');
    }
    place++;
    if (*place == ':') {
	if (optind < argc) {
	    ...
	    fprintf(stderr, MSGSTR(TAR_ARG, "%s: %c argument missing\n"),
		    argv[0], c);
	    return ('?');
	}
    }
...
{
    fprintf(stderr, MSGSTR(TAR_USAGE1,
		"Usage: %s -c[bfvw] device block filename..\n"), myname);
    fprintf(stderr, MSGSTR(TAR_USAGE2, 
		"       %s -r[bvw] device block [filename...]\n"), myname);
    fprintf(stderr, MSGSTR(TAR_USAGE3, 
		"       %s -t[vf] device\n"), myname);
    fprintf(stderr, MSGSTR(TAR_USAGE4, 
		"       %s -u[bvw] device block [filename...]\n"), myname);
    fprintf(stderr, MSGSTR(TAR_USAGE5, 

...
static char rcsid[] = "@(#)$RCSfile: link.c,v $ $Revision: 1.1.5.2 $ (DEC) $Date: 1993/06/10 19:50:09 $";
#endif
/*
 * HISTORY
 */
 ...
 * RETURNS
 *
 *  Returns true if a match was found in the link chain, else returns false.
 *
 */

static int linkmatch(Link *linkptr, Stat *asb)
{
	if ((ar_format == TAR) && (f_extract || f_list)) {
		if (asb->linkname == NULL)
			return(0);
		else
			return((strcmp(asb->linkname, linkptr->l_name) == 0));
	} else
		return((linkptr->l_ino == asb->sb_ino) && (linkptr->l_dev == asb->sb_dev));
}
...

{
	...
	if (linkmatch(linkp, asb)) {
	    if ((strcmp(name, linkp->l_name) == 0) ||
		(path = (Path *) mem_get(sizeof(Path))) == (Path *)NULL || 
		(path->p_name = mem_rpl_name(name)) == (char *)NULL) {
		return((Link *)NULL);
	    }
	    /*
	     * Insert this path at the head of the pathlist for this link
	     */
...

{
	...
	for (lp = *base; lp; lp = lp->l_forw) {
	    if (lp->l_nlink) {
		warn(lp->l_path->p_name, MSGSTR(LK_UNSEEN, "Unseen link(s)"));
	    }
	}
    }
}

...
static char rcsid[] = "@(#)$RCSfile: diff.c,v $ $Revision: 4.2.8.3 $ (DEC) $Date: 1993/10/11 16:11:48 $";
#endif
/*
 * HISTORY
 */
        ...
        char **argv;
{
	...
	badopt = 0;
			 ...
			 */
	opterr = 0;
	while ((c = getopt(argc,argv,"bcC:D:efhilnrsS:tw")) != EOF) {
		ucc = (unsigned char)c;
		switch (ucc) {
		case 'b':
			bflag = 1;
		...
		case 'C':
			...
			if ( optarg != NULL
				...
				fprintf(stderr,MSGSTR(EBADCNT2,
					"diff: -%c: bad count\n"),ucc);
				status=2;
				badopt++;
			}
	...
	if (badopt) {
		fprintf(stderr,MSGSTR(USAGE, "Usage: diff [-bitw] [-c |-C Lines|-D String|-e|-f|-h|-n] File1 File2\n       diff [-bilrstw] [-c |-C Lines|-e|-f|-h|-n] [-S File] Directory1 Directory2\n"
));
		exit(2);
	}
	...
	if ( (S_ISCHR(stb1.st_mode)) || (S_ISCHR(stb2.st_mode)) ) {
		fprintf(stderr,MSGSTR(ISCHR,
			"diff: Cannot compare character special file.\n"));
		status = 2;
		done();
	} else if ( (S_ISBLK(stb1.st_mode)) || (S_ISBLK(stb2.st_mode)) ) {
		fprintf(stderr,MSGSTR(ISBLK,
			"diff: Cannot compare block special file.\n"));
		status = 2;
		done();
	} else if ( (S_ISFIFO(stb1.st_mode)) || (S_ISFIFO(stb2.st_mode)) ) {
		fprintf(stderr,MSGSTR(ISFIFO,
			"diff: Cannot compare FIFO special file.\n"));
		status = 2;
		done();
	} else if ((stb1.st_mode & S_IFMT) == S_IFDIR && 

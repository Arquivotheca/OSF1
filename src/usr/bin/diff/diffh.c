...
static char rcsid[] = "@(#)$RCSfile: diffh.c,v $ $Revision: 4.2.11.3 $ (DEC) $Date: 1993/10/11 16:11:51 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * diffh.c	1.8  com/cmd/files/diff/diffh.c, bos320 4/28/91 20:35:20";
 */
/*
 *  diffh is diff -h
...
char *f1,*f2;
{
	...
	if(stat(f1,&statbuf)==-1) {
		error(MSGSTR(EACCESS2,"diffh: Can't access %s\n"),f1);
		exit(2);
	}
	...
	f = fopen(f1,"r");
	if(f==NULL) {
		error(MSGSTR(EOPEN3,"diffh: Can't open %s\n"),f1); 
		exit(2);
	}
	return(f);
}

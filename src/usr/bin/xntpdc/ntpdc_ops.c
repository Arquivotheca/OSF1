 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ntpdc_ops.c,v $ $Revision: 4.3.3.3 $ (DEC) $Date: 1992/07/07 16:11:55 $";
#endif
/*
 */
 ...
 */
void peerlist(), peers(), dmpeers();
void showpeer(), peerstats();
void loopinfo(), sysinfo(), sysstats();
void memstats(), iostats(), timerstats();
void traps(), addtrap(), clrtrap();
void ctlstats(), leapinfo();
void clockstat(), clkbug();

#ifdef DES_OK
void addpeer(), addserver(), broadcast();
void unconfig(), set(), clear();
void restrict(), unrestrict(), delrestrict();
void monitor();
void reset(), preset();
void dodirty(), dontdirty();
void readkeys();
void trustkey(), untrustkey();
void authinfo();
void requestkey(), controlkey();
void fudge(), maxskew();
void reslist(), monlist();
void setprecision(), setselect();
#endif /* DES_OK */
...
struct xcmd opcmds[] = {
	...
	{ "clear",	clear,		{ STR, OPT|STR, OPT|STR, OPT|STR },
					{ "bclient|auth", "...", "...", "..." },
...
extern char *humandate();
extern char *uglydate();
 ...
 */
int checkitemsize(itemsize, expected)
	int itemsize;
	int expected;
{
	...
	int dmstyle;
{
	...
	while (items > 0) {
		...
		poll = 1<<max(min3(plist->ppoll, plist->hpoll, NTP_MAXPOLL),
		    NTP_MINPOLL);
		(void) fprintf(fp,
		    "%c%-15.15s %-15.15s %2d %4d  %3o  %6.6s %9.9s %6.6s\n",
		    c, nntohost(plist->srcadr),
		    numtoa(plist->dstadr),
		    plist->stratum, poll, plist->reach,
		    ufptoa(NTOHS_FP(plist->delay), 4),
		    lfptoa(&tempts, 6),
	...
	FILE *fp;
{
	...
	(void) fprintf(fp,
	    "leap %c%c, refid [%s], distance %s, dispersion %s\n",
	    pp->leap & 0x2 ? '1' : '0',
	    pp->leap & 0x1 ? '1' : '0',
	    str, ufptoa(HTONS_FP(pp->distance), 4),
	...
	for (i = 0; i < PEER_SHIFT; i++) {
		(void) fprintf(fp, " %-8.8s", ufptoa(HTONS_FP(pp->delay[i]),4));
		if (i == (PEER_SHIFT>>1)-1)
			(void) fprintf(fp, "\n              ");
	}
	(void) fprintf(fp, "\n");
	...
	(void) fprintf(fp, "estdelay %s, estbdelay %s\n",
	    ufptoa(HTONS_FP(pp->estdelay), 4),
	    mfptoa(0, ntohl(pp->estbdelay), 4));

	HTONL_FP(&pp->estoffset, &tempts);
	(void) fprintf(fp, "estoffset %s, estdisp %s\n",
	    lfptoa(&tempts, 6),
	    ufptoa(HTONS_FP(pp->estdisp), 4));
}
	...
	FILE *fp;
{
	...
	if (oneline) {
		l_fp temp2ts, temp3ts;

		HTONL_FP(&il->last_offset, &tempts);
		HTONL_FP(&il->drift_comp, &temp2ts);
		HTONL_FP(&il->compliance, &temp3ts);

		(void) fprintf(fp,
		    "offset %s, drift %s, compliance %s, timer %d seconds\n",
		    lfptoa(&tempts, 7),
		    lfptoa(&temp2ts, 7),
		    lfptoa(&temp3ts, 7),
		...
		(void) fprintf(fp, "frequency:  %s seconds\n",
		    lfptoa(&tempts, 7));
		HTONL_FP(&il->compliance, &tempts);
		(void) fprintf(fp, "compliance: %s seconds\n",
		    lfptoa(&tempts, 7));
	...
	FILE *fp;
{
	...
	(void) fprintf(fp, "sync distance:    %s\n",
	    ufptoa(NTOHS_FP(is->distance), 4));
	(void) fprintf(fp, "sync dispersion:  %s\n",
	    ufptoa(NTOHS_FP(is->dispersion), 4));
	if (is->stratum <= 1) {
		junk[4] = 0;
	...
	(void) fprintf(fp, "system hold time: %d\n", ntohl(is->holdtime));
	...
	FILE *fp;
{
    ...
    "================================================================\n");
	while (items > 0) {
		(void) fprintf(fp, "%-15.15s %5d %9d %4d   %3d %9u %9u\n",
		    numtoa(ml->addr),
		    ntohs(ml->port),
		    ntohl(ml->count),
		    ml->mode, ml->version,
	...
	int req;
{
	...
	res = doquery(IMPL_XNTPD, req, 1, ritems, sizeof(u_int),
	...
	FILE *fp;
{
	...
	(void) fprintf(fp, "time since reset:  %d seconds\n",
	    ntohl(ia->timereset));
	...
	int req;
{
	...
	res = doquery(IMPL_XNTPD, req, 1, 1, sizeof(u_int),
	...
	FILE *fp;
{
	...
	(void) fprintf(fp, "leap.indicator: %s\n",
	    leapbittab[il->leap_indicator & INFO_LEAP_MASK]);
	(void) fprintf(fp, "leap.warning:   %s\n",
	    leapbittab[il->leap_warning & INFO_LEAP_MASK]);
...
struct clktype {
	...
	{ REFCLK_LOCALCLOCK,	"local clock synchronization (1)" },
	{ REFCLK_WWV_HEATH,	"Heathkit WWV clock (2)" },
	{ REFCLK_WWV_PST,	"Traconex (PSTI) WWV clock (3)" },
	{ REFCLK_WWVB_SPECTRACOM, "Spectracom WWVB clock (4)" },
	{ REFCLK_GOES_TRUETIME,	"True Time GOES clock (5)" },
	{ REFCLK_GOES_TRAK,	"TRAK Microwave GOES clock (6)" },
	{ REFCLK_CHU,		"Direct synced to CHU (7)" },
	{ -1,			"" }
};
	...
	FILE *fp;
{
	...
	while (items-- > 0) {
		...
		(void) fprintf(fp, "bad data responses:   %u\n",
		    ntohl(cl->baddata));
		(void) fprintf(fp, "running time:         %u seconds\n",
		...
		(void) fprintf(fp, "fudge value 1:        %d\n",
		    ntohl(cl->fudgeval1));
		(void) fprintf(fp, "fudge value 2:        %d\n",
		    ntohl(cl->fudgeval2));
	...
	FILE *fp;
{
	...
	maxskew = HTONS_FP(LFPTOFP(&tmp));

	res = doquery(IMPL_XNTPD, REQ_SET_MAXSKEW, 1, 1, sizeof(u_fp),
	    (char *)&maxskew, &items, &itemsize, &dummy);
	...
	FILE *fp;
{
	...
	res = doquery(IMPL_XNTPD, REQ_SET_PRECISION, 1, 1, sizeof(int),
	...
	FILE *fp;
{
	...
	res = doquery(IMPL_XNTPD, REQ_SET_SELECT_CODE, 1, 1, sizeof(u_int),

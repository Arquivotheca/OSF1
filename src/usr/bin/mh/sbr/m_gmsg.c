...
/* PDW: For X.400 mail we need to check real and effective uid. */
/* PJS: Access() uses the real_uid to check, hence when really root but
 * effectively the user, the access() fails across NFS, and the folder is
 * marked as READONLY...
 *
    if ((st.st_uid != getuid() && st.st_uid != geteuid()) || access (name, 02) == NOTOK)
 */

...
static normalize();
static char *address_break_point();
...
#else DONTSPLIT
            while (len > wid) {
                if ((sp = address_break_point (lp, len, wid)) == NULLCP)
/* no break points found */
                        break;
                while (sp > lp && isspace (*sp))
                        sp--;
                len -= sp - lp + 1;
                while (cp < ep && lp <= sp)
                    *cp++ = *lp++;
...
/*
 * break the address (lp) at a comma or, failing that, a space.  If there is no space or comma, DO NOT
 * break the address.  It would be possible to break the address at a carefully selected subset of
 * address meta-characters (eg angle-brackets, comment delimters, quoted string delimeters), but
 * somewhat difficult and not really worth the effort.
 *
 * We do not use a space or comma as a break point if it is between angle-brackets or parentheses or,
 * in the case of a space, if it is adjacent to a quote or has a neighbouring space adjancent to a
 * quote.
 */
static char *address_break_point (lp, len, wid)
char *lp;
int len, wid;
{
	register char *sp;

	char in_brackets = 0;	/* set to the matching bracket symbol */
	char in_quotes = 0;	/* a boolean */
	int late_break = 0;
	char *last_comma = NULLCP, *last_space = NULLCP;
		
		
	sp = lp - 1;
	for (;;) {
		while (++sp < lp+len || (late_break && sp < lp+wid)) {
			if (sp >= lp+wid && (last_comma != NULLCP || last_space != NULLCP))
				break;
            if (in_brackets != '\0') {
                if (*sp == '\\')
                    sp++;
                else if (*sp == in_brackets)
                    in_brackets = 0;
            }
            else {
                switch (*sp) {
                ...
                case '>':
                    ...
                    break;
                case ',':
                    if (!late_break || sp >= lp+wid)
                        last_comma = sp;
                    break;
                case ' ':
                case '\t':
                    if ((!late_break || sp >= lp+wid) && isspace (*sp)) {
                        char *ptr;
                        last_space = sp;
/* PDW: UMC version : 							 */
/* 		for (ptr = sp; last_space != NULLCP && ptr >= lp; ptr--) */

                        for (ptr = sp; last_space != NULLCP && ptr >= lp; )
                            if (*ptr-- == '"')
                                last_space = NULLCP;
                        for (ptr = sp; last_space != NULLCP && ptr <= lp+len; )
                            if (*ptr++ == '"')
                                last_space = NULLCP;
                    }
                    break;
        ...
        if (last_comma != NULLCP)
            return last_comma;
        else if (last_space != NULLCP)
            return last_space;
        else {
            if (late_break)
                return NULLCP;    /* tried to find a late break point and failed */
            else {
                sp = lp - 1;
                late_break = 1;
            }

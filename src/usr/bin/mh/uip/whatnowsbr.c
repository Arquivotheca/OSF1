...
#ifndef	lint
static char ident[] = "@(#)$RCSfile: whatnowsbr.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 01:39:41 $ devrcs Exp Locker: devbld $";
#endif	lint
	...
	default: 
		...
		if (((status & 0xff00) != 0xff00)
		    ...
		    if (!use && (status & 0xff00) &&
			    (mhrename (file, m_backup (file)) != NOTOK)) {
...
#ifdef	WP
    if (mhrename (file, cp = m_backup (file)) == NOTOK) {
	advise (cp, "unable to rename %s to", file);
	(void) unlink (draft);
	return 1;
    }
    if (mhrename (draft, file) == NOTOK) {

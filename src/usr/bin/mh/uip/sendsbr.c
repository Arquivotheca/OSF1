...
#ifndef	lint
static char ident[] = "@(#)$RCSfile: sendsbr.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 01:39:00 $ devrcs Exp Locker: devbld $";
#endif	lint
...
register struct stat *st;
{
    ...
    if (pushsw && unique) {
	if (mhrename (drft, strcpy (file, m_scratch (drft, invo_name)))
		== NOTOK)
	...
	default: 		/* wait for it */
		...
		if (annotext && fd2 != NOTOK)
		    anno (fd2, st);
		if (mhrename (drft, strcpy (buf, m_backup (drft))) == NOTOK)
...
int     out;
{
	...
	case OK: 		/* send it */
		...
		if ((in = open (file, 0)) == NOTOK)
		    ...
		    (void) strcpy (buf, "\n------- End of Unsent Draft\n");
		    (void) write (out, buf, strlen (buf));
		    if (mhrename (file, strcpy (buf, m_backup (file))) == NOTOK)

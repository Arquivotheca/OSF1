...
#ifndef	lint
static char ident[] = "@(#)$RCSfile: dropsbr.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 01:37:26 $ devrcs Exp Locker: devbld $";
#endif	lint
...
char   *text;
{
	...
	case UUCP: 		/* I hate this... */
	    ...
	    start = lseek (md, 0L, 1);
/* PJS: If we are not at the start of the file into which we are dropping
 * this UUCP-style mail, then prepend by a NEWLINE.
 */
	    if (tell(md) > 0) {
		write(md, "\n", 1);
		++size;

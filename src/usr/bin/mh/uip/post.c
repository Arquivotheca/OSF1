...
#ifndef	lint
static char ident[] = "@(#)$RCSfile: post.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 01:38:10 $ devrcs Exp Locker: devbld $";
#endif	lint
...
#define HXMTS   0x0800          /* X-Mts: header */
#define HRMTS   0x1000          /* Resent-X-Mts: header */
...
#define MMTS    0x0020          /* we've seen X-Mts: */
};
...
#define X400MTS  1
#define X4MTS    2
#define DEFMTS SMTPMTS
#endif X400

static char *mts_vals[] = {
    "smtp",
#ifdef X400
    "x400",
    "x.400"
...
#ifdef  X400
static int  msgxmts = NONEMTS;  /* holds value of mts from draft */
static int  resent_mts = NONEMTS; /* holds value of resent mts */
static int  argxmts = NONEMTS;  /* holds value of mts from command line */
static int  mts_arg = DEFMTS;   /* holds value of mts being used, SMTP */
                                /* being the default. */
static int  note = 0;
#endif  X400
static int  mtsval  = NONEMTS;  /* used in checking if valid mts */
...
static	anno(), insert_fcc(), make_bcc_file(), verify_all_addresses();
		...
		case SERVSW:
		    ...
		    continue;
                ...
                case MTSSW:
                    if (!(mts = *argp++) || *mts == '-')
                        adios (NULLCP, "missing argument to %s", argp[-2]);
                    argxmts = get_mts_val (mts);
                    if (argxmts == NONEMTS)
                        adios (NULLCP, "unknown mts");
                    mts_arg = argxmts;
                    continue;
                case NOTESW:
                    note++;
...
#ifdef X400
/* PJS: Munge the Message-ID header so that it is permissible to include
 * one in a notification request.
 * Also, switch off any request for message-id generation.
 */
    if (note > 0) {
    int i;
	for (i=0; i < sizeof(NHeaders) / sizeof(NHeaders[0]); i++)
	    if (uleq(NHeaders[i].value, "Message-ID"))
		NHeaders[i].flags = HNIL;
	msgid = 0;
    }
#endif X400
    start_headers ();
...
#ifdef X400
    if (hdr -> flags & HXMTS) {       /* If X-Mts field set, make sure it's  */
        mtsval = get_mts_val (str);   /* valid. Store the value in msgxmts   */
        if (mtsval != NONEMTS && resent_mts == NONEMTS) {  /* if there isn't */
            msgxmts = mtsval;                              /* Resent-XMts.   */
            mts_arg = msgxmts;
        }
        return;
    }
    if (hdr -> flags & HRMTS) {      /* If Resent-XMts field set, make sure   */
        mtsval = get_mts_val (str);  /* it's valid. Store value in resent_mts */
        if (mtsval != NONEMTS) {     /* and mts_arg.                          */
            resent_mts = mtsval;
            mts_arg = resent_mts;
        }
        return;
...
/*  */
	...
	if (mp -> m_nohost) {	/* also used to test (hdr -> flags & HTRY) */
	    ...
	    np = mp;
/* PJS:
 * If this address did not expand into an alias, then we should still be
 * using the m_text part of the address, rather than the m_mbox.
 */
	    if (strcmp(pp, mp -> m_mbox) == 0)
		pp = mp->m_text;
...
static  make_bcc_file () {
    ...
    fprintf (out, "From: %s\n", signature);
#ifdef X400
    mtsval = msgxmts;           /* If draft file has valid X-Mts, use it */
    if (mtsval == NONEMTS)      /* else if command line has valid mts    */
        mtsval = argxmts;       /* use it, else use the default.         */
    if (mtsval == NONEMTS)
        mtsval = DEFMTS;
    fprintf (out, "X-Mts: %s\n", mts_vals[mtsval]);
#endif X400
...
int     talk;
{
    ...
    if (mts_arg == X400MTS) {
        if (!whomsw ||checksw)
            if (rp_isbad (retval = x400s_init (clientsw, serversw, 0, 0, snoop))
                    || rp_isbad (retval = x400s_winit (smtpmode, from)))
        ...
        talk;
{
    ...
    if (verbose)
	if (msgflags & MINV)
            printf (" -- Posting for %s %s Recipients --\n",
                    bccque ? "Blind" : "Sighted", mts_vals[mtsval]);
	else
            printf (" -- Posting for All %s Recipients --\n",mts_vals[mtsval]);
    sigon ();
...
#ifdef  X400
    if (mts_arg == X400MTS) {
        if (rp_isbad (retval = x400s_init (clientsw, serversw, watch, verbose,
                snoop)) || rp_isbad (retval = x400s_winit (smtpmode, from)))
...
#ifdef  X400
    if (mts_arg == X400MTS)
        (void) x400s_end (!(msgflags & MINV) || bccque ? OK : DONE);
    else
#endif  X400
#ifdef	SENDMTS
...
#ifdef  X400
    if (mts_arg == X400MTS) {
        if (rp_isbad (retval = x400s_waend (note)))
...
int     fd;
{
...
#ifdef  X400
    if (mts_arg == X400MTS) {
        if (rp_isbad (retval = x400s_wtxt (buf, state)))
            die (NULLCP, "problem writing text; %s\n", rp_string (retval));
    }
    else
#endif  X400
	...
	retval = sm_wtend ();
#endif SENDMTS
#if defined (X400) || defined (SENDMTS)
    switch (retval) {
	case RP_OK: 
	    break;
...
#endif	defined (X400) || defined (SENDMTS)
}
#endif	not MHMTS
...
#ifdef  X400
    if (mts_arg == X400MTS)
        retval = x400s_wadr (mbox, host,
                           lp -> m_type != UUCPHOST ? lp -> m_path : NULLCP);
    else
#endif  X400
        retval = sm_wadr (mbox, host,
                         lp -> m_type != UUCPHOST ? lp -> m_path : NULLCP);
...
char *cp;
{
	...
	*p = '\0';
    ...
    numents = sizeof (mts_vals) / sizeof (mts_vals[0]);

    for (loop = 0; loop < numents; loop++)
	if (uleq (mts_vals[loop], cp))
	    break;
    if (loop == X4MTS)
	loop = X400MTS;
    return ((loop == numents) ? NONEMTS : loop);
}
#endif X400

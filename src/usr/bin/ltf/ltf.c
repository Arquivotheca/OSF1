 ...
 */
#ifndef lint
static char *sccsid = "@(#)$RCSfile: ltf.c,v $ $Revision: 4.2.4.2 $ (DEC) $Date: 1993/10/11 17:25:57 $";
#endif	lint
 ...
 *
 *	  01.3		20-Mar-86	Suzanne Logcher
 *			Change logic that handles unit numbers
 *
 *	  02.0		17-May-91	Sam Lambert
 *			Modified for use in the OSF/1 environment.
 *
 */
/**/
...
#else
char	unit[3];		/* Unit number of named tape device */
int	unit_int = 0;		/* Integer form of unit[3] */
int	unit_nums = 0;		/* Count of # of unit number digits */
#endif
...
\*------*/
	...
	setlocale(LC_ALL, "");
	catd = catopen(MF_LTF, NL_CAT_LOCALE);
	defmsgs();

/* Copy default tape name from default set in mtio.h */
/* 
 * NOTE: Changed from "#ifndef U11" to "#ifdef ULTRIX" for OSF/1 compatability. 
 * 17-May-91 -- lambert
 */
#ifdef ULTRIX
strcpy(Magtdev, DEFTAPE_RH);
#endif
#ifdef __osf__
strcpy(Magtdev, DEFTAPE);
#endif
...
if (mag == 0 && unit_nums != 0) {
	unit_int = expnum(unit);
	if (error_status) {
		PERROR "\n%s: %s %s\n\n", Progname, INVNF, unit_int);
		exit(FAIL);
	}
	if (unit_int > 31 || unit_int < 0) {
		PERROR "\n%s: %s\n\n", Progname, INVUNIT);
		exit(FAIL);
	}
	Magtdev[8] = unit[0];
	if (unit_nums == 2) {
	    Magtdev[10] = Magtdev[9];
	    Magtdev[9] = unit[1];
	}
}
#endif
...
/* JSD: moved expnum() into filenames.c so can fit for overlay version */
/* JSD: moved usage() into filenames.c so can fit for overlay version */
/* JSD: moved showhelp() into filenames.c so can fit for overlay version */

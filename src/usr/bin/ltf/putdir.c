...
static char rcsid[] = "@(#)$RCSfile: putdir.c,v $ $Revision: 4.2.2.3 $ (DEC) $Date: 1993/06/10 22:24:15 $";
#endif
/*
 * HISTORY
 */
...
#ifdef DEBUG
printf("putdir: longname = %s, shortname = %s\n", longname, shortname);
#endif 
...
#ifdef DEBUG
printf("putdir: relpaths = %s, dir = %s\n", relpaths, dir);
#endif
			if (ret < 0 || ret1 < 0) {
...
printf("putdir: path = %s, dir = %s\n", path, dir);
		...
		if ((Inode.st_mode & S_IFMT) == S_IFDIR) {
			...
			if (longname[j] == '/') {
				...
				strcat(path,"/");
#ifdef DEBUG
printf("putdir: got st_dev = %d, st_ino = %d\n", Inode.st_dev, Inode.st_ino);
#endif
			for (dp = Dhead; dp; dp = dp->dir_next) {
#ifdef DEBUG
printf("putdir: checking dev = %d, inode = %d\n", dp->dev, dp->inode);
#endif
				if (dp->dev == Inode.st_dev &&
				    dp->inode == Inode.st_ino) {
#ifdef DEBUG
printf("putdir: directory exists in list\n");
#endif
					found = TRUE;
					break;
				}
			...
			if (!found && !NMEM4D) {
			     ...
			     */
#ifdef DEBUG
printf("putdir:going to dprocess, path = %s, dir = %s\n", path ,dir);
#endif
			    if (!Nodir)
				dprocess(path, dir);
			}/*T if (!found && !NMEM4D) */

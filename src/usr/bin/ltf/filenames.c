 ...
 */
#ifndef lint
static char *sccsid = "@(#)$RCSfile: filenames.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/10/28 16:07:48 $";
#endif	lint
...
for (fstat = F_head; fstat; fstat = fstat->f_next) {
	...
	if (!mstrcmp(name, fstat->f_src))  {
		if ((i = strlen(name)) == (length = strlen(fstat->f_src))) {
			if (name[i-1] == '/' && fstat->f_src[length-1] == '/') {
	    			isdir = 1;
				return(fstat);
			}
		}
...

/* JSD - expnum() put here from ltf.c */
/*
 *
 * Function:
 *
...
/* JSD -  showhelp() put here from ltf.c module */
/**/
/*
 *
 * Function:
...
/* JSD - usage() function put here from ltf.c */
/**/
/*
 *
 * Function:

...
static char rcsid[] = "@(#)$RCSfile: putfile.c,v $ $Revision: 4.2.2.4 $ (DEC) $Date: 1993/09/03 03:44:52 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 *	  01.4		25-Mar-87	Suzanne Logcher
 *			Removed the declaration of Inbuf from
 *			process() because it was not on a word
 *			boundary needed for some tape drives.
 *			Now is called Outbuf and is global.
 */
 ...
 *	char	*Outbuf	file writing buffer
 *
 * Return values:
 *
 *	none
...
if (Rb - Bb > 0) {
	...
	if (write(fileno(Magtfp), Outbuf, Blocksize) != Blocksize) {
...
long append(name, fp, type, tftype, max, Outbuf)
	char	*name;	/* filename */
	FILE	*fp;
	int	type, tftype, max;
	char	*Outbuf;	/* File writing buffer */
...
if (type == TEXT) {
		...
		while (done == FALSE) {
			...
			*cp = '\0';
		    ...
		    if (Format == VARIABLE) {
			if (&p[length+4] > &Outbuf[Blocksize]) {
		    	    while (p < &Outbuf[Blocksize])
				*p++ = PAD;
			    if (write(fileno(Magtfp), Outbuf, Blocksize) != Blocksize) {
	    			PERROR "\n%s: %s %s\n", Progname, ERRWRF, name);
	    			perror(Magtdev);
			...
			if (&p[length+5] <= &Outbuf[Blocksize]) {
			    if (cp) {
				if (midway == FALSE)
	    ...
	    /* process the remaining chars of the text line, if it is not
	       terminated by \n */
	    if ((Format == VARIABLE) && ((length = strlen(line))>0)) { 
...
if (type == BINARY) {
	while ((length = read(fileno(fp), p=Outbuf, Blocksize)) > 0) {
		if (length < Blocksize) {
			p = &p[length];
			while (p < &Outbuf[Blocksize])
				*p++ = PAD;
...
if (type == COUNTED) {
    while ( fread((char *)&length, sizeof (short), 1, fp)) {
	if ( length == -1 || p+length+4 > &Outbuf[Blocksize]) {
		while ( p < &Outbuf[Blocksize])
			*p++ = PAD;
...
if (type == FUF) {
	 ...
	 * integers.
	 */
	while ((res = read(fileno(fp), (char *)&nbytes, sizeof(long))) > 0) {
		if (nbytes > 0L) {
			bytcnt = nbytes;
		...
		res = read(fileno(fp), (char *)&nbytesl, sizeof(long));
...
/*
 * Outbuf is a character array declared globally 
 * to ensure that it begins on a word boundary.
 */
...
if (!(filter_to_a(dummy,REPORT_ERRORS)))
    if (Warning) {
	PERROR "%s: %s %s", Progname, NONAFN, shortname);
 ...
 */
if ((pw_entry=getpwuid(Inode.st_uid))==NULL) {
...
cp = ctime((long *)&Inode.st_mtime);
if (cp[20] == '2' && cp [21] == '0')
    crecent = '0';
else
    crecent = ' ';
ltime = localtime((long *)&Inode.st_mtime);
Blocks = 0L;
max = 0;
...
if (Ansiv != '4') 
	(void) filter_to_a(Dummy,IGNORE_ERRORS);
...
if ((Blocks = append(longname, fp, type, otype, max, Outbuf)) < 0L)
    if (Blocks == (long)(-1)) {
...
#ifdef DEBUG
printf("putfile: longname = %s, shortname = %s\n", longname, shortname);
#endif
...
#ifdef DEBUG
    printf("putfile: going to putdir, longname = %s, shortname = %s\n", longname, shortname);
#endif
...
if ((Inode.st_mode & S_IFMT) == S_IFDIR) {
	...
	while (dirp = readdir(dirfile)) {
		...
		(void) putfile(sbuf, dirent, iflag, workdir);
		sbuf[savindex] = '\0';

 ...
 */
#ifndef lint
static char *sccsid = "@(#)$RCSfile: scantape.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1992/02/18 12:50:56 $";
#endif	lint
...
int	uid;		/* User id */
int	wildc = NO;
char	Xname[MAXPATHLEN+1]; /* alternate file name from xtract */
long	xtractf();
...
FOREVER {
	...
	if (strcmp(L_labid, "HDR") || L_labno != 1) {
		if (Tape) {
hdr1err:
			if (!strcmp(L_labid, "UVL")) {
		    		PERROR "\n%s: %sUVL%d\n", Progname, INVLFS, L_labno,  BELL); 
				continue;
			}
		    PERROR "\n%s: %s HDR1%c\n", Progname, INVLF, BELL); 
		    wbadlab();
...
/**/
		if (Func == EXTRACT && !linkflag) {
			Xname[0] = 0;
		 	ret = xtractf(pathname, ! Numrecs || isdir ? "" : fstat->f_src, charcnt, Xname);
			if (Xname[0] != 0)
			    strcpy(Name, Xname);
...
(void) filter_to_a(Labelbuf, IGNORE_ERRORS);
PERROR "%s", Labelbuf);
PERROR "\n%s: %s\n", Progname, EINVLD);
exit(FAIL);

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: ul.c,v $ $Revision: 4.2.4.2 $ (DEC) $Date: 1993/10/11 19:26:20 $";
#endif 
/*
 */
/*
 ...
 *
 * ul.c        1.4  com/cmd/tty,3.1,9021 11/14/89 13:57:56";
 */
...
nl_catd catd;
#define MSGSTR(num,str) catgets(catd,MS_UL,num,str)  /*MSG*/
	...
	char **argv;
{
	...
	default:
		fprintf(stderr,MSGSTR(TROUBLE, "%s: trouble reading termcap"),
                  argv[0]);
		/* fall through to ... */
...
FILE *f;
{
	...
	case IESC:
		...
		default:
			fprintf(stderr,
			   MSGSTR(ESCAPE, 
                           "ul: unknown escape sequence in input: %o,%o\n"),
			   IESC,c);
			exit(1);
		}
		continue;
	...
	default:
		...
		if (col >= MAXBUF)		/* added for 001 */
			flushln();   		/* added for 001 */

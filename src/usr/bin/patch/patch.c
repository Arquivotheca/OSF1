...
static char rcsid[] = "@(#)$RCSfile: patch.c,v $ $Revision: 1.1.3.4 $ (DEC) $Date: 1993/10/11 20:04:46 $";
#endif
...
"Unreversed (or previously applied) patch detected!  Ignore -R? [%s] "),
	...
	if (failed) {
		...
		if(!noreverse)
		    ...
		    say4(MSGSTR(REJECT, "%d out of %d hunks ignored\n"),
			 failed, hunk);
	    }
	    else {
	    ...
	    if(!noreverse && move_file(TMPREJNAME, rejname) < 0)
		trejkeep = TRUE;
	}
	set_signals(1);
    }
...
reinitialize_almost_everything()
{
    ...
    if (filec >= 2)
	fatal1(MSGSTR(NMORE,"You may not change to a different patch file.\n"));
}
...
usage:
	    default:
		fatal1(MSGSTR(USAGE, 
"Usage: patch [-NRSbflsv] [-c|-e|-n] [-B prefix] [-d dir] [-D define]\n\

...
static char rcsid[] = "@(#)$RCSfile: pch.c,v $ $Revision: 1.1.3.2 $ (DEC) $Date: 1993/06/11 21:13:47 $";
#endif
...
char *filename;
{
    ...
    if (pfp == Nullfp)
	fatal2(MSGSTR(NOTFOUND, "patch file %s not found\n"), filename);
...
grow_hunkmax()
{
	...
	return;
    if (!using_plan_a)
	fatal1(MSGSTR(GNOMEM, "patch: out of memory (grow_hunkmax)\n"));
...
there_is_another_patch()
{
	...
	if (verbose)
	    say1(MSGSTR(DONE, "done\n"));
	return FALSE;
    }
    if (verbose)
	say1(MSGSTR(HUMM, "Hmm..."));
    diff_type = intuit_diff_type();
    if (!diff_type) {
	if (p_base != 0L) {
	    if (verbose)
		say1(MSGSTR(TRAIL, "  Ignoring the trailing garbage.\ndone\n"));
	}
	else
	    say1(MSGSTR(NOPATCH, "  I can't seem to find a patch in there anywhere.\n"));
	return FALSE;
    }
    if (verbose)
	say1(MSGSTR(messages[diff_type*2 + (p_base != 0L)].mesno, 
		   messages[diff_type*2 + (p_base != 0L)].dflt));
    if (p_indent && verbose)
	if (p_indent == 1)
	    say1(MSGSTR(INDENT1,"(Patch is indented 1 space.)\n"));
	else
	    say2(MSGSTR(INDENT2,"(Patch is indented %d spaces.)\n"), p_indent);
    skip_to(p_start,p_sline);
    while (filearg[0] == Nullch) {
	if (force) {
	    say1(MSGSTR(NOFILE, "No file to patch.  Skipping...\n"));
	    filearg[0] = savestr(bestguess);
	    return TRUE;
	}
	ask1(MSGSTR(ASKFILE, "File to patch: "));
	if (*buf != '\n') {
	    if (bestguess)
		free(bestguess);
	...
	if (filearg[0] == Nullch) {
	    ask2(MSGSTR(SKIPTHIS, 
		"No file found--skip this patch? [%s] "), nl_langinfo(NOSTR));
	    if (rpmatch(buf) != 1) {	/* not yes */
		continue;
	    }
	    if (verbose)
		say1(MSGSTR(SKIPPING, "Skipping patch...\n"));
...
long file_line;
{
	...
	Fseek(pfp, p_base, 0);
	say1(MSGSTR(LEADT, "The text leading up to this was:\n"));
	say1("--------------------------\n");
...
another_hunk()
{
	...
	while (p_end < p_max) {
		...
		if (p_max - p_end < 4)
		    ...
		    fatal1(MSGSTR(EXPEOF,"Unexpected end of file in patch.\n"));
		}
	    }
	    p_end++;
		...
		if (p_end != 0) {
		    ...
		    fatal3(MSGSTR(EXPSTAR, "Unexpected *** at line %ld: %s"), p_input_line, buf);
		}
		context = 0;
...
"Duplicate \"---\" at line %ld--check line numbers at line %ld.\n"),
	...
	"Overdue \"---\" at line %ld--check line numbers at line %ld.\n"),
			...
			fatal4(MSGSTR(TOLARGE, "Hunk too large (%ld lines) at line %ld: %s"),
	...
	if (p_end >=0 && !repl_beginning)
	    fatal2(MSGSTR(NODASH, "No --- found in patch at line %ld\n"), pch_hunk_beg());
...
the new style...)\n"));
	...
	for (i=1; i<=p_ptrn_lines; i++) {
	    ...
	    p_input_line++;
	    if (ret == Nullch)
		fatal2(MSGSTR(EXPEOF2, "Unexpected end of file in patch at line %ld.\n"),
		  p_input_line);
	    if (*buf != '<')
		fatal2(MSGSTR(EXPLESS, "< expected at line %ld of patch.\n"), p_input_line);
	...
	if (hunk_type == 'c') {
		...
		fatal2(MSGSTR(EXPEOF2, "Unexpected end of file in patch at line %ld.\n"),
		    p_input_line);
	    if (*buf != '-')
		fatal2(MSGSTR(EXPDASH, "--- expected at line %ld of patch.\n"), p_input_line);
	}
	Sprintf(buf, "--- %ld,%ld\n", min, max);
	...
	for (i++; i<=p_end; i++) {
		...
		fatal2(MSGSTR(EXPEOF2, "Unexpected end of file in patch at line %ld.\n"),
		    p_input_line);
	    if (*buf != '>')
		fatal2(MSGSTR(EXPGTR, "> expected at line %ld of patch.\n"), p_input_line);
	    p_line[i] = savestr(buf+2);
	    if (out_of_mem) {
		p_end = i-1;
		return FALSE;
	...
	if (!pch_swap())
	    say1(MSGSTR(CANTSWAP, "Not enough memory to swap next hunk!\n"));
#ifdef DEBUGGING
    if (debug & 2) {
	int i;
	char special;
...
malformed:
    fatal3(MSGSTR(MALFORMED, "Malformed patch at line %ld: %s"), p_input_line, buf);
...
do_ed_script()
{
	...
	if (verbose)
	    Sprintf(buf, "%s %s", _PATH_ED, TMPOUTNAME);
	else
	    Sprintf(buf, "%s - %s", _PATH_ED, TMPOUTNAME);
	pipefp = popen(buf, "w");
    }

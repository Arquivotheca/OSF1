...
static char rcsid[] = "@(#)$RCSfile: inp.c,v $ $Revision: 1.1.3.2 $ (DEC) $Date: 1993/06/11 21:08:12 $";
#endif
...
char *filename;
{
	...
	plan_b(filename);
    if (verbose) {
	say3(MSGSTR(PLAN, "Patching file %s using Plan %s...\n"), filename,
	  (using_plan_a ? "A" : "B") );
    }
}
...
char *filename;
{
	...
	if (verbose)
	    say2(MSGSTR(CREAT, "(Creating file %s...)\n"),filename);
	...
	if (stat(buf, &filestat) >= 0 || stat(buf+4, &filestat) >= 0) {
	    ...
	    if (verbose)
		say2(MSGSTR(TRYRCS, "Can't find %s--attempting to check it out from RCS.\n"),
		    filename);
	    if (system(buf) || stat(filename, &filestat))
		fatal2(MSGSTR(NOCHECK, "Can't check out %s.\n"), filename);
	}
	else {
		...
		Sprintf(buf, GET, s);
		if (verbose)
		    say2(MSGSTR(TRYSCCS, "Can't find %s--attempting to get it from SCCS.\n"),
			filename);
		if (system(buf) || stat(filename, &filestat))
		    fatal2(MSGSTR(NOGET, "Can't get %s.\n"), filename);
	    }
	    else
		fatal2(MSGSTR(NOFIND, "Can't find %s.\n"), filename);
	}
    }
    filemode = filestat.st_mode;
    if ((filemode & S_IFMT) & ~S_IFREG)
	fatal2(MSGSTR(NOTNORM, "%s is not a normal file--can't patch.\n"), filename);
	...
	fatal2(MSGSTR(NOOPEN, "Can't open file %s\n"), filename);
#ifndef lint
...
"This file doesn't appear to be the %s version--patch anyway? [%s] "),
	    ...
	    if (rpmatch(buf) != 1)
		fatal1(MSGSTR(ABORTED, "Aborted.\n"));
	    }
	}
	else if (verbose)
	 say2(MSGSTR(VERS3, "Good.  This file appears to be the %s version.\n"),
		revision);
    }
...
"This file doesn't appear to be the %s version--patch anyway? [%s] "),
    ...
    if (tibuf[1] == Nullch)
	fatal1(MSGSTR(NOMEM, "Can't seem to get enough memory.\n"));
    for (i=1; ; i++) {
	if (! (i % lines_per_buf))	/* new block */
	    if (write(tifd, tibuf[0], BUFFERSIZE) < BUFFERSIZE)
		fatal1(MSGSTR(TMPWR, "patch: can't write temp file.\n"));
	...
	fatal2(MSGSTR(NOREOPEN, "Can't reopen file %s\n"), TMPINNAME);
    }
}
...
int whichbuf;				/* ignored when file in memory */
{
	...
	if (tiline[0] == baseline)
		...
		fatal2(MSGSTR(TMPRD,"Error reading tmp file %s.\n"), TMPINNAME);
	}

...
static char rcsid[] = "@(#)$RCSfile: util.c,v $ $Revision: 1.1.3.3 $ (DEC) $Date: 1993/07/29 22:51:27 $";
#endif
...
char *from, *to;
{
	...
	fromfd = open(from, 0);
	if (fromfd < 0)
	    fatal2(MSGSTR(NOREOPEN2, "patch: internal error, can't reopen %s\n"), from);
	while ((i=read(fromfd, buf, sizeof buf)) > 0)
	    if (write(1, buf, i) != 1)
		fatal1(MSGSTR(WRFAIL, "patch: write failed\n"));
	Close(fromfd);
	return 0;
    }
	...
	if (link(to, bakname) < 0) {
	    say3(MSGSTR(NOBACKUP, "patch: can't backup %s, output is in %s\n"),
		to, from);
	    return -1;
	}
	} /* if saveorig */
	...
	tofd = creat(to, 0666);
	if (tofd < 0) {
	    say3(MSGSTR(CANTCR, "patch: can't create %s, output is in %s.\n"),
	      to, from);
	    return -1;
	}
	fromfd = open(from, 0);
...
char *from, *to;
{
    ...
    while ((i=read(fromfd, buf, sizeof buf)) > 0)
	if (write(tofd, buf, i) != i)
	    fatal2(MSGSTR(WRFAIL2, "patch: write (%s) failed\n"), to);
    Close(fromfd);
    Close(tofd);
}
...
Reg1 char *s;
{
	...
	if (using_plan_a)
	    ...
	    fatal1(MSGSTR(SSNOMEM, "patch: out of memory (savestr)\n"));
    }
    else {
	t = rv;
	while (*t++ = *s++);

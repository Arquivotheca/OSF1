 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: prs.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/11 19:06:49 $";
#endif 
/*
 */
/*
 ...
 *  
 * prs.c 1.6 com/cmd/sccs/cmd,3.1,9021 12/20/89 20:17:27" 
 */
...
#define BADOPTION	"Usage: prs [-a] [-c Cutoff] [-d String] [-rSID] file | -\n\n\
\tUsage: prs [-a] [-c Cutoff] [-d String] -rSID [-e | -l] file | -\n"

#define NOFILE	"Specify the file to process. (cm3)\n\
\tUsage: prs [-a] [-c Cutoff] [-d String] [-rSID] file | -\n\n\
\tUsage: prs [-a] [-c Cutoff] [-d String] -rSID [-e | -l] file | -\n"
...
char *argv[];
{
	...
	if ((dataspec = malloc(strlen(p)+1)) == NULL) {
		perror("malloc");
		exit(1);
	}
	...
	*/
	...
	invocation of 'main'.
	*/
	while ((c = getopt(argc, argv, "ac:d:r;el")) != -1) {
...
\tto check the p-file for valid SID numbers. (co8)\n"));  /* MSG */
		...
		case 'c':	/* cutoff date[time] */
			if(date_ab(optarg,&Date_time) != -1)
				break;
			else
				fatal(MSGSTR(INVCUTOFF, "\nSpecify the cutoff date in the following format:\n\
\tyy[mm[dd[hh[mm[ss]]]]] (prs4)\n"));	/* MSG */
		...
		default:
			sprintf(ErrMsg,MSGCM(BADOPT,BADOPTION),c); 
			fatal(ErrMsg);
		}
	...
	if (num_files == 0)
		fatal(MSGCM(NOFLE, NOFILE)); 

	if(HADR && HADC)
		fatal(MSGSTR(CANTSPECDATE, "\nDo not specify both a cutoff date and an SID. (prs5)\n"));  /* MSG */
	if(HADC && (!HADL) && (!HADE))
		fatal(MSGSTR(SPECEORLWITHC, "\nSpecify the -e flag or the -l flag with the -c flag. (prs6)\n"));  /* MSG */
	...
	for(lp = spec; *lp != 0; lp++) {
		if(lp[0] == ':' && lp[1] != 0 && lp[2] == ':') {
			...
			case PAIR('M','R'):	/* MR numbers */
				...
				break;
#ifdef CASSI
                        case PAIR('M','C'):  /* cmf flag yes or no */
                                if(Sflags[CMFFLAG - 'a'])
                                        printf(MSGSTR(YESMSG, "yes"));  /* MSG\
 */
                                else
                                        printf(MSGSTR(NOMSG, "no"));  /* MSG *\
/
                                break;
                        case PAIR('A','C'):  /* cmf application field */
                                if(!(k = Sflags[CMFFLAG - 'a']))
                                        printf(MSGSTR(NONEMSG, "none"));  /* M\
SG */
                                else
                                        printf("%s",k);
                                break;
...
register struct packet *pkt;
{
	...
	while (getline(pkt) != NULL) {
		...
		if (*p++ != CTLCHAR)
			...
			if (iod == END)
				remq(pkt,ser);
			else if (pkt->p_apply && (ap = &pkt->p_apply[ser])->a_code == APPLY)
			{
				if (iod == INS)
					tmpvar = YES;
				else
					tmpvar = NO;
				addq(pkt,ser,tmpvar,iod,ap->a_reason & USER);
			}
			else
			{
				if (iod == INS)
					...
					tmpvar = '\0';
				if ( pkt->p_apply == (struct apply *)NULL )
					addq(pkt,ser,tmpvar,iod,(struct apply *)NULL);
				else
					addq(pkt,ser,tmpvar,iod,ap->a_reason & USER);
			}
		}
...
\tuse local problem reporting procedures. (co20)\n"));  /* MSG */
	if (i == 0) {
		...
		to standard output
		*/
		execlp(Getpgm,Getpgm,"-s","-p",rarg,filearg,(char *)0);

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: sccs.c,v $ $Revision: 4.2.6.3 $ (DEC) $Date: 1993/10/11 19:00:32 $";
#endif 
/*
 */
/*
 ...
 *
 * FUNCTIONS: callprog, clean, command, dodiff, getpfent, gstrbotch,
 *            gstrcat, gstrcpy, gstrncat, isbranch, isdir, lookup,
 *            makefile, nextfield, putpfent, safepath, syserr, tail,
 *            unedit, username, usrerr, main
 *
 * ORIGINS: 3, 10, 26, 27
 *
 ...
 *    
 * sccs.c 1.8 com/cmd/sccs/cmd,3.1,9021 2/14/90 10:08:07";
 */
/*
...
**
**	Compilation Instructions for AIX:
**		cc -O -n -s -I/usr/include/bsd sccs.c -lbsd
**
**	Author:
...
#ifdef __STDC__
#define PROGPATH(name) "/usr/bin/"#name /* for AIX */
#else /* __STDC__ */
#define PROGPATH(name) "/usr/bin/name" /* for AIX */
#endif /* __STDC__ */
...
# include "defines.h"
# include "bsd_msg.h"
#define MSGSTR(Num, Str) catgets(catd, MS_BSD, Num, Str)
nl_catd catd;
...
struct sccsprog SccsProg[] =
{
	...
	"cdc",		PROG,	REALUSER,		PROGPATH(cdc),
...
#define USAGE_STR "Usage: sccs [-r] [-d pathname] [-p pathname] command [flags] [file ...]\n"

#define REQ_ARG   "ERROR:  Flag -%c requires an argument\n\
\tUsage: sccs [-r] [-d pathname] [-p pathname] command [flags] [file ...]\n"
...
# endif /* V6 */
	...
	*/
	...
	if (argc < 2)
	{
		fprintf(stderr, MSGSTR(SCCSUSAGE2, USAGE_STR)); 
		exit(EX_USAGE);
	}
	argv[argc] = NULL;
...
# ifndef SCCSDIR
			  case 'p':		/* path of sccs files */
				SccsPath = optarg;
				if ((cmd = lookup(SccsPath)) == NULL)
					break;
				else {
					fprintf(stderr,
					MSGSTR(SCCSUSAGE2, USAGE_STR));
					exit(EX_USAGE);
				}

			  case 'd':		/* directory to search from */
				SccsDir = optarg;
				if ((cmd = lookup(SccsDir)) == NULL)
					break;
				else {
					fprintf(stderr,
					MSGSTR(SCCSUSAGE2, USAGE_STR));
...
# endif /*DEBUG */
			  case ':':
# ifndef SCCSDIR
				fprintf(stderr,MSGSTR(ARGRQD, REQ_ARG), optopt);
				exit(EX_USAGE);
# endif /* !SCCSDIR */
...
# endif
	...
	*/
	...
	if (cmd == NULL)
	{
		usrerr(MSGSTR(UNKNOWNCMD, "%s is not a recognized SCCS command.\n"), *ap); /*MSG*/
		return (EX_USAGE);
	}
	...
	*/
	...
	switch (cmd->sccsoper)
	{
	  ...
	  case FIX:		/* fix a delta */
		...
		{
			usrerr(MSGSTR(NEEDR4FIX, 
                         "Specify the -r flag with the fix command\n")); /*MSG*/
			rval = EX_USAGE;
			break;
		}
...
# endif
		...
		if (i < 0)
		{
			syserr(MSGSTR(CANNOTFORK, 
			    "Cannot create another process.\n")); /*MSG*/
			exit(EX_OSERR);
		}
		else if (i > 0)
		{
	...
	execv(progpath, argv);
	syserr(MSGSTR(CANTEXEC, "Cannot execute %s.(co50)\n"), progpath); /*MSG*/
	exit(EX_UNAVAILABLE);
	/*NOTREACHED*/
}
	...
	register char *p;
{
	...
	printf(
		MSGSTR(PATHERR, "Do not use .. or full pathnames.\n")); /*MSG*/
	return (FALSE);
}
	...
	char **argv;
{
	...
	*/
		...
		*/
		...
		if (pfp != NULL)
		{
			...
			while ((pf = getpfent(pfp)) != NULL)
			{
				...
				printf(MSGSTR(BEDIT, 
                                     "%s is being edited: "),basefile); /*MSG*/
				putpfent(pf, stdout);
			}
			fclose(pfp);
		}
	...
	if (!gotedit && mode == INFOC)
	{
		...
		if (!nobranch) {
			if (usernm == NULL)
			      printf(MSGSTR(NOEDIT, 
				    "Nothing is being edited.\n"));
			else
			      printf(MSGSTR(NOEDITBY, 
                                    "Nothing is being edited by %s.\n"), usernm);
		} else {
			if (usernm == NULL)
			      printf(MSGSTR(NOEDITTRNK,"No trunk of the SCCS file tree is being edited.\n"));
			else
			      printf(MSGSTR(NOEDITTRNKBY,"No trunk of the SCCS file tree is being edited by %s.\n"), usernm);
		}
		printf("\n");
	}
	if (mode == CHECKC)
	...
	char *fn;
{
	...
	if (q[1] != 's' || q[2] != '.')
	{
		usrerr(MSGSTR(BADFNAME, "File name %s is not in correct SCCS format.\n"), fn); /*MSG*/
		return (FALSE);
	}
	...
	*++q = 'p';
	...
	if (pfp == NULL)
	{
		printf(MSGSTR(NOBEDIT, 
		     "File %s is not being edited.\n\
\tUse the unedit command only on files being edited.\n"), fn); /*MSG*/
		return (FALSE);
	}
	...
	if (tfp == NULL)
	{
		usrerr(MSGSTR(CANTCREATE,"Cannot create %s.\n"), tfn); /*MSG*/
		exit(EX_OSERR);
	}
	...
	*/
	...
	if (others)
	{
		...
		{
			syserr(MSGSTR(CANTREOPEN, 
			    "Cannot open %s again.\n"), tfn); /*MSG*/
			exit(EX_OSERR);
		}
		if (freopen(pfn, "w", pfp) == NULL)
		{
			usrerr(MSGSTR(CANTCREATE, 
			    "Cannot create %s.\n"), tfn); /*MSG*/
			return (FALSE);
		}
	...
	if (delete)
	{
		 ...
		 */
		if (unlink(cp) >= 0)
			printf(MSGSTR(REMOVED, 
			   "Temporary file %s is removed.\n"), cp); /*MSG*/
		return (TRUE);
	}
	else
	{
		printf(MSGSTR(NOEDITBYYOU,"Only the user who is editing the file can unedit the file.\n"), fn); /*MSG*/
		return (FALSE);
	}
}
	...
	char *gfile;
{
	...
	if (pipe(pipev) < 0)
	{
		syserr(MSGSTR(DODIFFPIPE, 
		    "dodiff: The pipe system call failed.\n")); /*MSG*/
		exit(EX_OSERR);
	}
	if ((pid = fork()) < 0)
	{
		syserr(MSGSTR(DODIFFFORK,
                    "dodiff: Cannot create another process.\n")); /*MSG*/
		exit(EX_OSERR);
	}
	else if (pid > 0)
	{
	...
	else
	{
		...
		{
			syserr(MSGSTR(DODIFFMAGIC, "dodiff: The close system call or the dup system call failed..\n")); /*MSG*/
			exit(EX_OSERR);
		}
...
# ifdef UIDUSER
	...
	if (pw == NULL)
	{
		syserr(MSGSTR(UNKNOWNUID, 
		    "%d is not an existing user number.\n"), getuid()); /*MSG*/
		exit(EX_OSERR);
	}
	return (pw->pw_name);
# else
...
gstrbotch(str1, str2)
        char	*str1, *str2;
{
 usrerr(MSGSTR(FNAMSTOOBIG,
    "The file name too large for the buffer: %1$s %2$s\n"), str1, str2); /*MSG*/
}

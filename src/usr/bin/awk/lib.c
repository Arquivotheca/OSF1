 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: lib.c,v $ $Revision: 4.2.8.3 $ (DEC) $Date: 1993/12/20 14:41:07 $";
#endif 
/*
 */
/*
	...
	while (svargc > 0) {
		...
		if (infile == NULL) {	/* have to open a new file */
			...
			*FILENAME = file = *svargv;
			...
			if (*file == '-') {
				if (yyin == stdin)
                                   error(FATAL, MSGSTR(RSTDIN,
                       "standard input already used for reading commands"));
				else
					infile = stdin;
			}
			else if ((infile = fopen(file, "r")) == NULL)
				error(FATAL,
                                      MSGSTR(NOPEN, "can't open %s"), file);
		}
		if ((sep = **RS) == 0)
			sep = '\n';
		for (rr = record; ; ) {
		...
		if (rr > record+RECSIZE)
			error(FATAL, 
                        MSGSTR(TOLONG1, "record `%.20s...' too long"), record);
		*rr = 0;
		if (mustfld)
			fldbld();
...
fldbld()
{
	...
	if ((sep = **FS) == ' ')
		for (i = 0; ; ) {
			...
			if (i >= MAXFLD)
				error(FATAL, 
            MSGSTR(TOMANY, "record `%.20s...' has too many fields"), record);
...
recbld()
{
	...
	*(r-1) = '\0';
	...
	if (r > record+RECSIZE)
		error(FATAL,
                MSGSTR(GIANT, "built giant record `%.20s...'"), record);
	dprintf("recbld = |%s|\n", record, NULL, NULL);
}
...
{
	if (n<0 ||n >= MAXFLD)
		error(FATAL, MSGSTR(ACCESS, "trying to access field %d"), n);
...
yyerror(char *s)
{
	fprintf(stderr, MSGSTR(YYERROR,"awk: %s near line %d\n"), s, lineno );
	errorflag = 2;
}
...
long a1, long a2, long a3, long a4, long a5, long a6, long a7)
{
	...
	fprintf(stderr, "\n");
	if (NR && *NR > 0)
		fprintf(stderr, MSGSTR(RECNUM, " record number %g\n"), *NR);
	if (f)
		exit(2);
}

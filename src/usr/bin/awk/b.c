 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: b.c,v $ $Revision: 4.2.7.2 $ (DEC) $Date: 1993/10/01 01:13:32 $";
#endif 
/*
 */
/*
...
struct fa *makedfa(node *p)	/* returns dfa for tree pointed to by p */
{
	node *p1;
	struct fa *fap;
	p1 = op2((obj *) CAT, op2((obj *) STAR, op2((obj *) DOT, (node *) 0, (node *) 0), (node *) 0), p);
		/* put DOT STAR in front of reg. exp. */
	p1 = op2((obj *) FINAL, p1, (node *) 0);		/* install FINAL node */
...
penter(node *p)	/* set up parent pointers and leaf indices */
{
	switch(type(p)) {
		...
		case OR:
			...
			break;
		default:
	error(FATAL,MSGSTR(UNKNOWN1,"unknown type %d in penter\n"), type(p));
			break;
	}
}
...
freetr(node *p)	/* free parse tree and follow sets */
{
	switch(type(p)) {
		...
		default:
			error(FATAL,
                        MSGSTR(UNKNOWN2,"unknown type %d in freetr\n"),type(p));
			break;
	}
}
...
overflo()
{
	error(FATAL, MSGSTR(TOLONG,"regular expression too long\n"));
}

/* enter follow set of each leaf of vertex v into foll[leaf] */
cfoll(register node *v)
{
	register i;
	int prev;
	int *add();
...
first(register node *p)
{
	...
	error(FATAL, MSGSTR(UNKNOWN4,"unknown type %d in first\n"), type(p));
	return(-1);
}
...
notin(int **array, int n, int *prev)
{
	register i, j;
	int *ptr;
	for (i=0; i<=n; i++) {

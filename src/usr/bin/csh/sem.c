...
static char rcsid[] = "@(#)$RCSfile: sem.c,v $ $Revision: 4.2.11.3 $ (DEC) $Date: 1993/12/17 18:38:01 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 *
 *	1.7  com/cmd/csh/sem.c, bos320 5/10/91 15:38:11";
 */ 
...
printf("debug: entering execute\n"); fflush(stdout);
	...
	case TCOM:
		...
		if (t->t_dcom[0][0] == ALIASCHR)
			strcpy( (char *)t->t_dcom[0], (char *) t->t_dcom[0] + 1);
		if ((t->t_dflg & FREDO) == 0)
			Dfix(t);                /* $ " ' \ */
		if (t->t_dcom[0] == 0)
			return;
...
printf("execute(TPAR): execute\n"); fflush(stdout);
	...
	case TFIL:
		omask = sigblock(sigmask(SIGCHLD)); /* Fix pipeline problem */
...
#endif
			execute(t->t_dcar, wanttty,(int *)0, (int *)0);
			if ((getn(value((uchar_t *)"status")) == 0) != (t->t_dtyp == TAND))
				return;
		}
		if (t->t_dcdr) {
			t->t_dcdr->t_dflg |= t->t_dflg & (FPAR|FINT);
...
printf("doio: close of stdout\n");fflush(stdout);
	...
	if (cp = t->t_drit) {
		...
		if ((flags & FCAT) && open((char *)cp, O_WRONLY) >= 0)
			...
			if(!(flags & FANY) && adrof((uchar_t *)"noclobber")) { 
				if (flags & FCAT)
					Perror_free((char *)cp);
				chkclob(cp);
			}
			if (creat((char *)cp, S_DEFFILEMODE) < 0)
				Perror((char *)cp);
		}
		xfree(cp);
        } else if (flags & FPOU) {

...
static char rcsid[] = "@(#)$RCSfile: exec.c,v $ $Revision: 4.2.11.2 $ (DEC) $Date: 1993/06/10 16:36:27 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTIONS: doexec pexerr texec execash xechoit dohash 
 *             dounhash hash
 *
 *
 ...
 *
 *
 *	1.13  com/cmd/csh/exec.c, bos320 5/10/91 15:37:41";
 * 	sh.exec.c    5.7 (Berkeley) 5/27/90
 */ 
...
for (P=pv; P&&*P&&**P;P++)
	...
	if (havhash) {
		hashval = hashname(*av);
#ifdef DEBUG
csh_printf("doexec: *av=%s, hashname(*av)=%d, xhash[hashname(*av)]=%d\n",
	*av, hashname(*av), xhash[hashname(*av)]);
	...
	do {
#ifdef DEBUG
csh_printf("doexec: slash=%d, havhash=%d, *pv=%s, hashval=%d, i=%d\n",
...
csh_printf("texec: %s\n", f);
	...
	case ENOEXEC:
		 ...
		 */
		v = adrof1((uchar_t *)"shell", &aliases);
		if (v == 0) {
			register int ff = open((char *)f, O_RDONLY);
			char ch;
		
			vp = lastsh;
			vp[0] = adrof((uchar_t *)"shell") ? 
				value((uchar_t *)"shell") : (uchar_t *)_PATH_CSHELL;
			vp[1] = (uchar_t *)NULL;
...
execash(int t, struct command *kp)
{
	...
	exiterr++;
#ifdef DEBUG
csh_printf("debug: execash calls doexec\n");
#endif
	doexec(kp);
	/*NOTREACHED*/
}
...
dohash(void)
{
	...
	register int cnt;
	int i = 0;
	struct varent *v = adrof((uchar_t *)"path");
	uchar_t **pv;
	int hashval;
...
hashstat(void)
{
	...
	if (hits+misses)
		csh_printf(MSGSTR(M_HITMISS,"%d hits, %d misses, %d%%\n"),

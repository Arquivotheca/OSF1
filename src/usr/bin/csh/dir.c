...
static char rcsid[] = "@(#)$RCSfile: dir.c,v $ $Revision: 4.2.10.2 $ (DEC) $Date: 1993/06/10 16:24:20 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTIONS: dinit dodirs dtildepr dochngd dfollow dopushd dfind dopopd 
 *            dfree dcanon dnewcwd
 *
 *
 ...
 *
 *
 *	1.4  com/cmd/csh/dir.c, bos320 5/10/91 15:37:32";
 */ 
...
dtildepr(uchar_t *home, uchar_t *dir)
{
	...
	if (!EQ((char *)home, "/") && prefix(home, dir))
		csh_printf("~%s", dir + strlen((char *)home));
	else
		csh_printf("%s", dir);
}
...
dochngd(uchar_t **v)
{
	...
	printd = 0;
	if (*++v == NOSTR) {
		if ((cp = value((uchar_t *)"home")) == NOSTR || *cp == 0)
...
dfollow(uchar_t *cp)
{
	...
	if (cp[0] != '/' && !prefix((uchar_t *)"./", cp) 
	    && !prefix((uchar_t *)"../", cp)
	   && (c = adrof((uchar_t *)"cdpath"))) {
		uchar_t **cdp;
		register uchar_t *p;

		for (cdp = c->vec; *cdp; cdp++) {
			uchar_t buf[PATH_MAX+1];
			unsigned int len = strlen((char *)cdp) + strlen((char *)cp) +1;
			if (len <= PATH_MAX) {
				strcpy((char *)buf, (char *)*cdp);
				strcat(buf, "/");
				strcat(buf, cp);
				if (chdir(buf) == 0) {
					printd = 1;
...
gotcha:
        if (*cp != '/') {
                ...
                for (p = cp; *p++;)
                        ;
		dp = (uchar_t *)calloc((cwdlen + (p - cp) + 2),1);
...
dcanon(uchar_t *cp, uchar_t *p)
{
	...
	if (*cp != '/')
	{
		...
		if((cp = (uchar_t *)getcwd((char *)NULL,PATH_MAX)) == NULL)
		{
			perror("pwd");
			abort();
		}
...
dnewcwd(struct directory *dp)
{
	...
	dcwd = dp;
	 set((uchar_t *)"cwd", savestr(dcwd->di_name));
	if (printd)
		dodirs(fakev);
}

...
static char rcsid[] = "@(#)$RCSfile: t8.c,v $ $Revision: 4.2.6.2 $ (DEC) $Date: 1993/06/10 18:12:17 $";
#endif
/*
 * HISTORY
 */
/*
static	char	*sccsid = "@(#)t8.c	8.1	(Japanese ULTRIX)  2/19/91";
	...
	for(c=0; c<ncol; c++)
	{
		...
		case '=':
			if (real(table[nl][c].col))
				fprintf(stderr,catgets(catd, 1, 1, "%s: line %d: Data ignored on table line %d\n", ifile, iline-1, i+1));
			makeline(i,c,ct);
			continue;
		default:
			continue;

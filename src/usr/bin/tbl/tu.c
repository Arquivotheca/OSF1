...
static char rcsid[] = "@(#)$RCSfile: tu.c,v $ $Revision: 4.2.6.2 $ (DEC) $Date: 1993/06/10 18:47:25 $";
#endif
/*
 * HISTORY
 */
/*
static	char	*sccsid = "@(#)tu.c	8.1	(Japanese ULTRIX)  2/19/91";
...
drawline(i, cl, cr, lintype, noheight, shortl)
{
	...
	for(ln=oldpos=0; ln<lcount; ln++)
	{
		...
		else
		{
			lnch = (char *) ("\\(ul");
			if (pr1403)
				lnch = (char *) (lintype==2 ? "=" : "\\(ru");
			if (cr+1>=ncol)
				fprintf(tabout, "\\l'|\\n(TWu%s%s'", exhr,lnch);
			else
				fprintf(tabout, "\\l'(|\\n(%du+|\\n(%du)/2u%s%s'", cr+CRIGHT, cr+1+CLEFT, exhr, lnch);
		}
		if (linsize)
			fprintf(tabout, "\\v'\\n(%dp/6u'", LSIZE);
		fprintf(tabout, "\\s0");

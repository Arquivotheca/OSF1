...
static char rcsid[] = "@(#)$RCSfile: bdiff.c,v $ $Revision: 4.2.5.5 $ (DEC) $Date: 1993/11/19 16:31:11 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 1.11  com/cmd/files/bdiff.c, cmdfiles, bos320, 9130320 6/27/91 14:39:09
 *
 */
...
char otmp[PATH_MAX+1], ntmp[PATH_MAX+1];
int linenum;
	...
	*/
	...
	if (argc > 3) {
		if (argv[3][0] == '-' && argv[3][1] == 's') {
			Fflags &= ~FTLMSG;
		   if (argc == 5 && 
		       ((*argv[4] <= '0') ||((seglim = patoi(argv[4])) == -1) ))
	...
	if (equal(argv[1],"-"))
		...
		poldfile = fdopen(xopen(argv[1],0),"r");
	if (equal(argv[2],"-"))
		pnewfile = stdin;
	else
		pnewfile = fdopen(xopen(argv[2],0), "r");
...
char type;
{
	...
	temptr = fdopen(xopen(tempfile,0), "r");
...
char *file;
{
	...
	iop = fdopen(xcreat(mktemp(file), TMP_FILE_MODE), "w");
	...
	*	3. If FTLEXIT was set then  exit(2)  has been called

...
static char rcsid[] = "@(#)$RCSfile: expr.c,v $ $Revision: 4.2.10.4 $ (DEC) $Date: 1993/10/11 16:46:57 $";
#endif
/*
 * HISTORY
 */
 ...
 * 
 * 1.13  com/cmd/sh/expr.c, cmdsh, bos320, 9143320g10/18/91 15:06:19 
 */
...
#define ERR_SYNTAX	3	/* expr syntax error			*/
#define ERR_MALLOC	4	/* malloc error				*/
#define ERR_NUMBER	5	/* invalid number in numeric field	*/
...
nl_catd	catd;			/* msg catalog descriptor	*/
...
char *r2; 		/* operand 2	*/
{
	...
	case DIV: 			/* divide	*/
		if (i2 == 0)
		  yyerror(ERR_SYNTAX, MSGSTR(INVALID, "invalid expression"));
		else
		  i1 = i1 / i2; 
		break;
	case REM: 			/* remainder	*/
...
char *p;
{
	 ...
	 */
	if(reg.re_nsub > 1)
		yyerror(ERR_SYNTAX, MSGSTR(PARANS1,"Too many '\\('s"));
	...
	if ((regexec(&reg, s, 2, off, 0) == 0) && (off[0].rm_so == 0))
	{
		...
		{  /* GZ001. If was subexpression in pattern we have to
		    * return substring that matches to this subexpression.
		    * Because nothing matches empty string (not "0") is returned. 
		    */
		if (reg.re_nsub != 0)
		    return "";
		else

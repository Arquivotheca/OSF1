...
static char rcsid[] = "@(#)$RCSfile: tabs.c,v $ $Revision: 4.2.5.4 $ (DEC) $Date: 1993/10/11 19:12:13 $";
#endif
/*
 * HISTORY
 */
 ...
 * 
 * tabs.c 1.8  com/cmd/tty,3.1,9021 4/6/90 00:27:42";
 */
...
nl_catd catd;
#define MSGSTR(num,str) catgets(catd,MS_TABS,num,str)  /*MSG*/
...
#endif
int isplp = 0;
int tflag = 0;                          /* terminal argument specified */
struct stat statbuf;
char *devtty;
...
int main(int argc, char **argv)
{
	...
	if (*scan == '+' && *(scan+1) == 'm') {
		...
		argc--;
		if(argc <= 0)
		   error(MSGSTR(NOTERM, "Must specify terminal type with -T option\n"));
		terminal = *argv;
	    }
	}
	else
...
struct ttab *termadj()
{
    ...
    if(!t->ttype && tflag)
      error(MSGSTR(TERM, "Unknown terminal type\n"));
    else
      return(termtab);
}

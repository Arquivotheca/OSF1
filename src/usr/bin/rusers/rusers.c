 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: rusers.c,v $ $Revision: 1.1.2.2 $ (DEC) $Date: 1993/05/26 17:44:32 $";
#endif
	...
	struct sockaddr_in *raddrp;
{
	...
	if (!iflag && !hflag && !uflag) {
		if (lflag &&  (cnt > 0) )  
		  	for (i = 0; i < cnt; i++){		       
			  if (strcmp( utmpidlearr.uia_arr[i]->ui_utmp.ut_line, ":0"))
			    putline(host, utmpidlearr.uia_arr[i], vers);
			  else {
			    utmpidlearr.uia_arr[i]->ui_idle = 0;
			    putline(host, utmpidlearr.uia_arr[i], vers);
			  }

 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ntp.c,v $ $Revision: 4.2.2.4 $ (DEC) $Date: 1992/09/29 14:24:22 $";
#endif
/*
 */
 ...
 */
/*
 *  	Previous History		ntp.c,v $
 * Revision 4.1  91/09/18  21:51:14  devrcs
 * Start of AG pool
 * 
 * Revision 3.1  91/06/24  15:08:49  devbld
	...
	char *argv[];
{
	...
	int host, n, retry, s, usage=0, hosts=0;
	fd_set readfds;
	...
	while ((n = getopt(argc, argv, "vsf")) != EOF) {
		...
		case 'f':
			...
			break;
		case '?':
		 	printf("usage: ntp [-v][-s[f]] host1 [host2 [host3 ...]] \n");
			exit(1);
		}
	}
	...
	for (host = optind; host < argc; ++host) {
		...
		dst.sin_addr.s_addr = (u_int) HostAddr;
		if (HostAddr == -1) {
		...
		if (verbose) {
			...
			case 1:
				...
				break;
			default:
				clock_host.s_addr = (u_int) pkt->refid;
		...
		if (verbose) 
			printf("Input Timestamp is     %08x.%08x %s",

...
static char rcsid[] = "@(#)$RCSfile: pk0.c,v $ $Revision: 4.3.5.2 $ (DEC) $Date: 1993/09/07 16:07:24 $";
#endif
/* 
 * COMPONENT_NAME: UUCP pk0.c
 * 
 * FUNCTIONS: chksum, pkaccept, pkclose, pkcntl, pkoutput, pkread, 
 *            pkreset, pksack, pksize, pkwrite, pkzero 
 *
 *
...
/*
1.3  pk0.c, bos320 6/16/90 00:00:21";
*/
/* VERSION( pk0.c	5.2 -  -  ); */
#define USER	1
...
register struct pack *pk;
{
	...
	if ( ! ISCNTL(c) ) {
		fprintf(stderr, MSGSTR(MSG_PK0_1, "not cntl\n"));
		return;
	}
	...
	case INITA:
		if (val==0 && pk->p_state&LIVE) {
			fprintf(stderr, MSGSTR(MSG_PK0_2,
	...
	case RR:
		...
		break;
	case SRJ:
		fprintf(stderr, MSGSTR(MSG_PK0_4, "srj not implemented\n"));
		break;
	case CLOSE:
...
char **bp;
	...
	if (rcheck  != pk->p_rwindow) {
		fprintf(stderr, MSGSTR(MSG_PK0_7, "r short %d want %d\n"),
			rcheck,pk->p_rwindow);
		fprintf(stderr, MSGSTR(MSG_PK0_5, "rcount = %d\n"),
			pk->p_rcount);
		fprintf(stderr, MSGSTR(MSG_PK0_6, "xcount = %d\n")
			,pk->p_xcount);
	}
	free((char *) pk);
}

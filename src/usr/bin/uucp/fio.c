 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: fio.c,v $ $Revision: 1.1.3.4 $ (DEC) $Date: 1992/05/05 18:12:16 $";
#endif
/*
 * OSF/1 Release 1.0
 */
 ...
 */
/* 
 * COMPONENT_NAME: UUCP fio.c
 * 
 * FUNCTIONS: falarm, frdblk, frddata, frdmsg, fturnoff, 
 *            fturnon, fwrdata, fwrmsg , frdbuf, fwrblk 
 *
 *
 * Author:
...
acct:
	time(&t2);
	sprintf(ibuf, ret == SUCCESS ?
		"sent data %d bytes %d secs" :
		"send failed after %d bytes %d secs",
		fbytes, t2 - t1);
	if (retries > 0)
...
#define MAXRETRIES	(fbytes < 10000 ? 2 : 1)
...
acct:
	time(&t2);
	sprintf(ibuf, ret == SUCCESS ?
		"received data %d bytes %d secs" :
		"receive failed after %d bytes %d secs",
		fbytes, t2 - t1);
	DEBUG(1, "%s\n", ibuf);
	syslog(ibuf);
	if (ret == FAIL) {

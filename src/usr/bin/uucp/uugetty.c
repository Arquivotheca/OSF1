...
static char rcsid[] = "@(#)$RCSfile: uugetty.c,v $ $Revision: 4.3.8.4 $ (DEC) $Date: 1993/11/11 20:33:03 $";
#endif
/* 
 * COMPONENT_NAME: UUCP uugetty.c
 * 
 * FUNCTIONS: Muugetty, account, assert, cleanup, control, error, 
 *            fdup, fields, find_def, fioctl, getname, getword, 
 *            hang_up_line, logent, openline, parse, quoted, search, 
 *            setupline, timedout 
 *
 *
 ...
 */
/*
uugetty.c	1.6  com/cmd/uucp,3.1,9013 3/16/90 10:46:20";
*/


/*	/sccs/src/cmd/uucp/s.uugetty.c
	uugetty.c	1.3	9/11/84 11:35:39
*/
#include	"pathnames.h"
#include	"uucp.h"
/* VERSION( uugetty.c	5.3 -  -  ); */
...
/*									*/
/*	Usage:  getty -c uugettydefs_like_file				*/
/*									*/
/*	The "-c" flag is used to have "getty" check a uugettydefs file.	*/
...
/*	Maximum length of line in /etc/uugettydefs file and the maximum	*/
...
/*	If the /etc/uugettydefs file can't be opened, the following	*/
/*	default is used.						*/
...
char	*GETTY_DEFS	=	"/etc/uugettydefs";

int 	check = {
	FALSE
};
char	*checkgdfile;		/* Name of uugettydefs file during
				 * check mode.
				 */
...
/* Check a "uugettydefs" file mode. */
		case 'c':
			signal(SIGINT,SIG_DFL);
			if(--argc == 0) {
				fprintf(stderr,
				    "Check Mode Usage: getty -c uugettydefs-like-file\n");
				exit(1);
			}
			check = TRUE;
			checkgdfile = *++argv;
			D4("checkgdfile=\"%s\"",checkgdfile);
/* Attempt to open the check uugettydefs file. */
			if((fp = fopen(checkgdfile,"r")) == NULL) {
				D1("Cannot open \"%s\".",checkgdfile);
...
/* the protections to only allow uucp to read the line. */
        ...
        if (ttylocked(line)) /* There is a lock file already.  Some
                                process is using the line.  Sit and wait.
                                */
                ttywait(line);

	(void) close(0);
	(void) close(1);
	if ((open(line, O_RDWR)) < 0) {	/* this opens stdin */
	...
	dup(0);
        ...
        /* Set ourselves up as the controlling process. */

        if (setsid() < 0)         /* Set ourselves up as session head */
                D1("setsid() failed, code %d.", errno);
        if (ioctl(0, TIOCSCTTY, 0) < 0)
                D1("TIOCSCTTY failed, code %d.",errno);
	...
	if (ttylock(line)) { /*  There is a lock file already */
...
/* predetermined amount of time, set the timer. */
		if(timeout) {
			signal(SIGALRM,(void(*)(int)) timedout);
			alarm(timeout);
		}
...
/* spaces. */
			...
			if (dbgflag >= 2) 
				for (i=0; largs[i]; i++)
					D2("\targ %2d=\"%s\"",i,largs[i]);
			D2("%s /usr/bin/login started.",dbgtime());
#endif
/* Exec "login". */

			execv(_PATH_LOGIN,largs);
			D1("%s /usr/bin/login not executable.",dbgtime());
			exit(1);
...
/*	"find_def" scans "/etc/uugettydefs" for a string with the		*/
...
char *id;
{
	...
	D5("find_def(\"%s\")",id);
/* Decide whether to read the real /etc/uugettydefs or the supplied */
/* check file. */
	if (check) gdfile = checkgdfile;
	else gdfile = GETTY_DEFS;
	D3("gdfile=\"%s\"",gdfile);
/* Open the "/etc/uugettydefs" file.  Be persistent. */
	for (i=0; i < 3;i++) {
...
/*	"parse" breaks up the user's response into separate arguments	*/

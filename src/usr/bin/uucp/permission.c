...
static char rcsid[] = "@(#)$RCSfile: permission.c,v $ $Revision: 4.3.5.2 $ (DEC) $Date: 1993/09/07 16:07:11 $";
#endif
#endif
/* 
 * COMPONENT_NAME: UUCP permission.c
 * 
 * FUNCTIONS: callBack, chkperm, chkpth, cmdMatch, cmdOK, fillFlds, 
 *            fillList, getuline, listMatch, logFind, mchFind, myName, 
 *            nameMatch, next_token, nextarg, parse_tokens, requestOK, 
 *            switchRole, userFind, validateFind 
 *
 *
...
/*
1.11  com/cmd/uucp/permission.c, 9115320a, bos320 3/27/91 10:59:15";
*/
#ifndef UUCHECK
#include "uucp.h"
/* VERSION( permission.c	5.2 -  -  ); */
#endif

static userFind(), cmdMatch();
static int validateFind();
extern char *strchr();
...
#else
		if (_Pubdir == NULL) {
		    (void) fprintf(stderr, MSGSTR(MSG_PERM_2,
				"malloc() error\n"));
		    exit(1);
		}
#endif
...
#ifndef UUCHECK
		ASSERT(i<NUMFLDS, MSGSTR(MSG_PERM_A1,
			"PERMISSIONS file: BAD OPTION--"), pair.name, NUMFLDS);
#else
		if (i >= NUMFLDS) {
			DEBUG(3, "bad option (%s) in PERMISSIONS\n",pair.name);
			(void) printf("\n*****************************\n");
			(void) printf(MSGSTR(MSG_PERM_1,
			 "**BAD OPTION in PERMISSIONS file: %s\n"), pair.name);
			(void) printf("*****************************\n");
			Uerrors++;
			return(0);
		}

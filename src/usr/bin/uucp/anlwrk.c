...
static char rcsid[] = "@(#)$RCSfile: anlwrk.c,v $ $Revision: 4.3.7.2 $ (DEC) $Date: 1993/09/07 16:03:56 $";
#endif
/* 
 * COMPONENT_NAME: UUCP anlwrk.c
 * 
 ...
 */
/*
anlwrk.c	1.4  com/cmd/uucp,3.1,9013 10/23/89 14:38:40";
*/
...
char *file, **wvec;
{
	...
	if (fp == NULL) {
		...
		if (fp == NULL){ /* can't open C. file! */
			errent(Ct_OPEN,file,errno, rcsid, __FILE__, __LINE__);
	...
	if (p_bookmark == NULL)
	    errent(Ct_OPEN, afile, errno, rcsid, __FILE__, __LINE__);
	else {
	...
	while (gnamef(pdir, filename) ) {
		...
		if ((strlen(filename)-strlen(prefix)) != SUFSIZE) {
			errent(MSGSTR(MSG_ANLWRKe2,"bldflst: Funny filename"), 
			   filename, 0, rcsid, __FILE__, __LINE__);
			continue;
		}
		insert(filename);
	}

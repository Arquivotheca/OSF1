...
static char rcsid[] = "@(#)$RCSfile: pk1.c,v $ $Revision: 4.3.5.2 $ (DEC) $Date: 1993/09/07 16:07:29 $";
#endif
/* 
 * COMPONENT_NAME: UUCP pk1.c
 * 
 * FUNCTIONS: cgalarm, pkcget, pkdata, pkgetpack, pkopen, pkxstart 
 *
 *
...
/*
1.5  pk1.c, bos320 10/18/90 13:16:11";
*/
#include "uucp.h"
/* VERSION( pk1.c	5.2 -  -  ); */
...
register struct pack *ipk;
{
	...
	if (k == 9) {
		if (h->sum + h->cntl == CHECK) {
			...
			*/
			if(Debug >= 7) {
		            sprintf(msgline,MSGSTR(MSG_PK1_19, "state -"));
			    delimc = ' ';
...
register int fn;
{
	...
	(void) signal(SIGALRM, (void(*)(int)) cgalarm);

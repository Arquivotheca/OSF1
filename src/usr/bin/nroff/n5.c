...
static char rcsid[] = "@(#)$RCSfile: n5.c,v $ $Revision: 1.1.4.3 $ (DEC) $Date: 1993/10/14 04:17:32 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 001 21-May-91	gws
 *	this came from usr/local/nosupport/troff n5.c -r3.1
...
casell(){
	...
	ll = i;
#ifdef NROFF /* begin 002-kak */
	/* adding the default vertical horizontal and adjustment   */
	/* factors to reflect those of the default file when using */
	/* the ll macro, otherwise the line spacing of the output  */
	/* file is inaccurate                                      */
	t.Adj = 24;
	t.Hor = 24;
	t.Vert = 40;
...
casewh(){
	...
	if(k == NTRAP){
		prstrfl(catgets(catd, 1, 19, "Cannot plant trap.\n"));
		return;
	}
	mlist[k] = j;
	nroff_nlist[k] = i; /* 001-kak */
}
casech(){
	register i, j, k;
...
int i;
{
	...
	for(k=0; k<NTRAP; k++)
		if((nroff_nlist[k] == i) && (mlist[k] != 0))break; /*001-kak*/
	return(k);
}
casepn(){
	register i;
...
casetm(x) int x;{
	...
	lgf++;
	copyf++;
	if(skip() && x)prstrfl(catgets(catd, 1, 20, "User Abort."));
/* +++ WL001 +++ */
	for(i=0 ; i<NTM-2 ; ) {
		register int	wc   ;
		register int	size ;
		...
		if (IS_ENCODED_ACHAR(wc)) {
			wc = Decode (wc) ;
			if (i > NTM - MB_MAX - 1) {
				ch = wc ; /* Unget the character */
				break   ;
			}
			if ((size = wctomb(&tmbuf[i], wc)) > 0)
				i += size ;
		} else /* Is a non-Asian character */
			tmbuf[i++] = wc ;
	}
/* --- WL001 --- */
	if(i == NTM-2)tmbuf[i++] = '\n';
...
e0:
	...
	if((nxev >= NEV) || (nxev < 0) || (evi >= EVLSZ)){
		prstrfl(catgets(catd, 1, 21, "Cannot do ev.\n"));
...
caseie(){
	if(ifx >= NIF){
		prstr(catgets(catd, 1, 22, "if-else overflow.\n"));
		ifx = 0;
		edone(040);
	}
	caseif(1);
...
rdtty(){
/* +++ WL001 +++ */
	char cbuf[MB_MAX] ; /* Character buffer				 */
	int  wc   = 0	  ; /* The complete Asian or non-Asian character */
	int  size = 0     ; /* Multi-byte string length			 */

	while (size < MB_MAX) {
		if (read(0, &cbuf[size], 1) != 1)
			goto end_input ; /* Read from stdin till no more char */
		size++ ;
		if ((wc = mbtowc(&wc, cbuf, size)) > 0)
			break ;	/* A complete character is present */
		if (size >= MB_MAX)
			size = 0 ; /* Some error has occurred, reset size */
	}
	if (size > 1)	/* Should be Asian character if size > 1 */
		wc = Encode(wc) ;
	if (wc == '\n') tty++ ;
		else tty = 1 ;
	if (tty != 3) return(wc) ;
...
/* +++ WL001 +++ */
		Xlate_Map(i, j) ; /* Map char i to char j */
/* --- WL001 --- */
	}
}
casecu(){

...
static char rcsid[] = "@(#)$RCSfile: n3.c,v $ $Revision: 1.1.4.3 $ (DEC) $Date: 1993/10/14 04:17:25 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 001 21-May-91	gws
 *	this came from usr/local/nosupport/troff n3.c -r3.1
...
int i;
{
	...
	contab[NM].rq=(i&~MMASK);
	for(j=0; i!=(contab[j].rq & ~MMASK); j++);
	if(j==NM)j = -1;
	return(j);
}
clrmn(i)
	...
	if(app && (oldmn >= 0) && (contab[oldmn].rq & MMASK)){
		...
		if((i==NM) ||
		   (nextb = alloc()) == 0){
			...
			prstr(catgets(catd, 1, 11, "Too many string/macro names.\n"));
			edone(04);
			return(offset = 0);
		}
...
copyb()
{
	...
	req = j;
/* +++ WL001 +++ */
#define IS_EQUAL(a, b)	(((a) == (b)) || (IS_ENCODED_ACHAR(a) && \
			(((a) & AMASK) == ((b) & AMASK))))

	k = Get_Second_Of_Pair(j) ; /* Get 2nd char of the character pair */
	j = Get_First_Of_Pair (j) ; /* Get 1st char of the character pair */
/* --- WL001 --- */
	copyf++;
	flushi();
	nlflg = 0;
...
int i;
{
	...
	if(!((++offset) & (BLK-1))){
		...
		if(blist[j = blisti(--offset)] == -1){
			if(alloc() == 0){
				prstr(catgets(catd, 1, 12, "Out of temp file space.\n"));
				done2(01);
			}
...
filep p;
{
	...
	if(!((j = (++p)) & (BLK-1))){
		if((i = blist[blisti(--p)]) == -1){
			prstr(catgets(catd, 1, 13, "Bad storage allocation.\n"));
			done2(-5);
		}
		j = ((filep)i)<<BLKBITS;
	}
...
popi(){
	...
	rchar = p->prchar;
	pendt = p->ppendt;
	ap = (long *) p->pap; /* force type cast 001-kak */
	cp = p->pcp;
	ch0 = p->pch0;
/* +++ WL001 +++ */
	Dec_Stack_Level(p) ;
...
filep newip;
{
	...
	p->prchar = rchar;
	p->ppendt = pendt;
	p->pap = (int *) ap; /* force type cast 001-kak */
	p->pcp = cp;
	p->pch0 = ch0;
	p->pch = ch;
	cp = (int *) ap = 0; /* force type cast 001-kak */
...
int	x;
{
	...
	x += sizeof(long) - 1;
	x &= ~(sizeof(long) - 1);
	if ((u_long)(i = sbrk(x)) == -1) {
		prstrfl(catgets(catd, 1, 14, "Core limit reached.\n"));
		edone(0100);
	} else {
		enda = i + x;
	}
...
#define	CPERMAC	200
#define	APERMAC	9
		memp += APERMAC * sizeof(struct s *);
		memp += CPERMAC * sizeof(caddr_t);
		nxf = (struct s *)memp;
	}
	lim = (char *)nxf;
	argpp = (char **)(savnxf + 1L); /* force type cast 001-kak */
	argppend = &argpp[APERMAC];
	SPACETEST(argppend, sizeof(struct s *));
	strp = (long *)argppend;
	/*
	...
	while ((argpp != argppend) && (!skip())) {
		*argpp++ = (char *)strp;
		...
		while (1) {
			...
			*strp++ = (long)i;
			/* force type cast 001-kak */
			/* will also need to worry about this type */
			/* of pointer arithmetic   */
			if (strflg && (strp >= (long *) lim)) {
#if 0
				fprintf(stderr, "strp=0x%x, lim = 0x%x\n",
					strp, lim);
#endif 0
				prstrfl(catgets(catd, 1, 15, "Macro argument too long.\n"));
				copyf--;
				edone(004);
			}
			SPACETEST(strp, 4 * sizeof(struct s *));
		}
		*strp++ = NULL;
	}
	nxf = savnxf;
	/* nxf->nargs = argpp - (char **)(savnxf + 1L); */
	argtop = strp;
rtn:
	copyf--;
}
...
seta()
{
	...
	/* force type cast for ap = 001-kak */
	if(((i = (getch() & CMASK) - '0') > 0) &&
		(i <= APERMAC) && (i <= frame->nargs))
			ap = (long *) (*((caddr_t *)frame + (long)i- 1L +      
			       (sizeof(struct s)/sizeof(caddr_t *))));
}
caseda(){
	app++;
	casedi();
...
casedi(){
	...
	if(++dilev == NDI){
		--dilev;
		prstr(catgets(catd, 1, 16, "Cannot divert.\n"));
		edone(02);
	}
	if(dip != d)wbt(0);
	diflg++;
...
casepm(){
	...
	for(i = 0; i<NM; i++){
		...
		k = 1;
		/*
		 * 002-kak baa helped too
		 * type cast variables to int so that
		 * the comparison will work
		*/
		while ((int)(j = blist[blisti(j)]) != (int)-1)
		{
			k++;
			j <<= BLKBITS;
		}
...
/* +++ WL001 +++ */
			...
			if (IS_ENCODED_ACHAR(ch)) {
				...
				*p++ = ch & 0177 ;
			p[0] = p[1] = p[2] = p[3] = ' ' ; /* Fill in blanks */
			p = pmline + 5 ;      /* Always start size at col 5 */
/* --- WL001 --- */
			kvt(k,p);
			prstr(pmline);
		}

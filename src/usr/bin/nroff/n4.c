...
static char rcsid[] = "@(#)$RCSfile: n4.c,v $ $Revision: 1.1.4.3 $ (DEC) $Date: 1993/10/14 04:17:29 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 001 21-May-91	gws
 *	this came from usr/local/nosupport/troff n4.c -r3.1
...
setn()
{
	...
	if((i=getsn()) == 0)return;
/* +++ WL001 +++ */
	if (Get_First_Of_Pair(i) == '.') switch (Get_Second_Of_Pair(i)) {
/* --- WL001 --- */
		...
		case 'z': i = dip->curd;
/* +++ WL001 +++ */
			cbuf[0] = Get_First_Of_Pair (i) ;
			cbuf[1] = Get_Second_Of_Pair(i) ;
/* --- WL001 --- */
			cbuf[2] = 0;
			cp = cbuf;
			return;
...
int i;
{
	...
	if(j==NN){
		if(!numerr)prstrfl(catgets(catd, 1, 17, "Too many number registers.\n"));
...
a0:
	...
	switch((ii=getch()) & CMASK){
		...
		case '/':
			...
			if(nonumb)break;
			if(i == 0){
				prstrfl(catgets(catd, 1, 18, "Divide by zero.\n"));
				acc = 0;
			}else acc /= i;
			goto a0;
		case '%':

...
static char rcsid[] = "@(#)$RCSfile: n1.c,v $ $Revision: 1.1.4.4 $ (DEC) $Date: 1993/10/14 04:17:14 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 001 01-Apr-91	Fred Canter
 *	MIPS C 2.20+, changes for -std
 * 002 21-May-91	gws
 *	this came from usr/local/nosupport/troff n1.c -r3.1
...
extern int xflg;
extern int stop;
/* +++ WW001 +++ : Add some space for saving incomplete Asian char */
extern char ibuf[IBUFSZ + MB_MAX] ;
extern char xbuf[IBUFSZ + MB_MAX] ;
int mb_lang;
/* --- WW001 --- */
extern char *ibufp;
extern char *xbufp;
...
options:
	while(--argc > 0 && (++argv)[0][0]=='-')
		...
		case 'm':
			...
			if (access(nextf, 4) < 0) {
				char local[NS];
                                /* changed to /usr/share directory */
                                /* where it should be  002-kak     */
				strcpy(local, "/usr/share/lib/tmac/tmac.");
...
#endif
		default:
#ifdef NROFF /* 001-kak begin */
			fprintf(stderr,"Usage: nroff [-olist] [-nN] [-sN] [-mname] [-raN] [-Tname] [-ehiq] [file ...]\n");
#else
			fprintf(stderr,"Usage: troff [-olist] [-nN] [-sN] [-mname] [-raN] [-Ffontfile] [-abfgiqtwxz] [file ...]\n");
#endif /* NROFF 001-kak end */
			exit(-1);
		}
...
/* +++ WW001 +++ */
	prstrfl(catgets(catd, 1, 1, "Floating Exception.\n"));
/* --- WW001 --- */
	signal(SIGFPE,fpecatch);
}
void
...
/* +++ WW001 +++ */
		prstr(catgets(catd, 1, 2, "Cannot create temp file.\n"));
/* --- WW001 --- */
		exit(-1);
	}
	ibf = open(p, 2);
	...
	if(a != 'a')unlkp = p;
/* +++ WW001 +++ */
	Init_Stack() ;		/* Initialize peek stack */
	InitAsianTables(catd);	/* Initializ Asian char class tables */
/* --- WW001 --- */
}
init2()
{
...
/* +++ WW001 +++ */
			prstr(catgets(catd, 1, 3, "Typesetter busy.\n"));
/* --- WW001 --- */
			done3(-2);
		}
	}
...
*/
	switch(k){
		...
		case '}':	/*RIGHT*/
			...
			goto gx;
		case '"':	/*comment*/
			if (!mb_lang) i=skip_comment();
			else while(((i=getch0()) & CMASK ) != '\n');
			goto g2;
...
/* +++ WW001 +++ */
				if (!(i=getach()) || (mb_lang && IS_ENCODED_ACHAR(i)))
					goto g0 ;
/* --- WW001 --- */
				i = (i<<BYTE) | JREG;
				break;
...
again:
	...
	g2:
/* +++ WW001 +++ */
		if (!mb_lang) {		/* for efficiency */
			i = ((int)*ibufp++) & 0377;
			ioff++;
			if((unsigned)i >= 040)goto g4; else i = ifilt[i];
		}
		else {
			/*
			 * If the last character in ibuf[] is not complete,
			 * we move the remaining characters to the beginning 
			 * of ibuf[] and read in another block of characters
			 * following the original one.
			 */
			j = 0 ;
			/* First check if ibufp is pointing to ibuf[] */
			if ((ibufp >= ibuf) && (ibufp - ibuf < sizeof(ibuf)) &&
			    (eibuf - ibufp < MB_MAX) && 
			    ((j = mbtowc(&i1, ibufp, eibuf - ibufp)) <= 0)) {
				register int idx ;
	
				if (ibufp > ibuf) { /*Move character to front*/
					for (idx=0; idx<eibuf-ibufp; idx++)
						ibuf[idx] = ibufp[idx] ;
					eibuf = ibuf + idx ;
				}
				else
					eibuf = ibuf ;
				nx = 0 ;
				if ((idx=read(ifile,eibuf,IBUFSZ)) <= 0)
					goto g0 ;
				ibufp  = ibuf	;
				eibuf += idx	;    /*Update buffer end ptr*/
				if (ip) goto again;
			}
			if (j <= 0)	/* Get a char */
				j = mbtowc(&i1, ibufp, eibuf - ibufp) ;
			if (j <= 0) {	/* should be 8 bit clean */
				i = *ibufp++ & 0377;	
				ioff++;
			} else {
				ioff  += j ;
				ibufp += j ;
				i = Encode(i1);	/* Encode to internal format */
			}
			if((unsigned)i >= 040)goto g4; else i = ifilt[i];
		}
/* --- WW001 --- */
...
g4:
/* +++ WW001 +++ */
	if((copyf == 0) && ((i & ~BMASK) == 0) && ((i & CMASK) != 0377))
/* --- WW001 --- */
#ifndef NROFF
...
/* --- WW001 --- */
	...
	lgf--;
/* +++ WW001 +++ */
	return (i & BMASK) ;	/* Allow the returning of Asian character */
/* --- WW001 --- */
}
casenx(){
	lgf++;
...
getname(){
	...
	for(k=0; k < (NS-1); k++){
		if(((j=(i=getch()) & CMASK) <= ' ') ||
/* +++ WW001 +++ */
			((j > 0176) && !IS_ENCODED_ACHAR(j)) || 
			 Is_MByte_Space(j)) break;

		if (IS_ENCODED_ACHAR(j)) {
			register int ch_size ;

			j = Decode(j) ; /* Decode to original format */
			if (k >= NS - MB_MAX)
				break ; /* No space for this char */
			ch_size = wctomb(j, &nextf[k]) ;
			k += ch_size - 1 ;
		} else
/* --- WW001 --- */
		nextf[k] = j;
...
/* +++ WW001 +++ */
		prstr(catgets(catd, 1, 5, "can't open file "));
/* --- WW001 --- */
		prstr(nextf);
		prstr("\n");
		done(02);
		...
		d2:
			...
			if(nonumb)goto fini;
				...
				*pnp++ = i | neg;
				...
				if(pnp >= &pnlist[NPN-2]){
/* +++ WW001 +++ */
					prstr(catgets(catd, 1, 6, "Too many page numbers\n"));
/* --- WW001 --- */
					done3(-3);
				}
			}

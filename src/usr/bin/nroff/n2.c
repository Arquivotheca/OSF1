...
static char rcsid[] = "@(#)$RCSfile: n2.c,v $ $Revision: 1.1.4.3 $ (DEC) $Date: 1993/10/14 04:17:21 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 001 21-May-91	gws
 *	this came from usr/local/nosupport/troff n2.c -r3.1
...
int c;
{
	...
	switch(j = i & CMASK){
		...
		default:
/* +++ WL001 +++ */
			if (mb_lang) i = Translate(j) | (i & ~CMASK) ;
			else i = ((unsigned char)trtab[j] & BMASK) | 
				(i & ~CMASK);
/* --- WL001 --- */
	}
	pchar1(i);
...
/* +++ WL001 +++ */
		/* Print out an Asian character */
		if (IS_ENCODED_ACHAR(i)) {
			static char  char_buf[2 * MB_MAX + 1]	;
			static int   size			;

			i    = Decode(i) 	   ;
			size = wctomb(char_buf, i) ;
			if (size > 0) {
				char_buf[size] = '\0' ;
				oputs(char_buf)	      ;
			}
...
/* +++ WL001 +++ */
				k-- ;
				oput(Get_First_Of_Pair (*k)) ;
				oput(Get_Second_Of_Pair(*k)) ;
/* --- WL001 --- */
		}
	}else
#endif
...
/* +++ WW001 +++ */
                        if(++waitf <=2)prstr(catgets(catd, 1, 8, "Waiting for Typesetter.\n"));
			sleep(15);
		}
	}
	if(no_out == 0){
...
casepi(){
	...
	if(toolate || skip() || !getname() || (pipe(id) == -1) ||
	   ((i=fork()) == -1)){
		prstr(catgets(catd, 1, 9, "Pipe not created.\n")) ;
		return;
	}
	ptid = id[1];
	if(i>0){
	...
	close(id[1]);
	execl(nextf,nextf,0);
	prstr(catgets(catd, 1, 10, "Cannot exec: "));
	prstr(nextf);
	prstr("\n");
	exit(-4);
}

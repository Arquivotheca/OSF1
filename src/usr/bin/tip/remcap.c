...
static char rcsid[] = "@(#)$RCSfile: remcap.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/09/30 18:39:42 $";
#endif

/* remcap.c	1.3  com/cmd/tip,3.1,9013 10/15/89 10:43:11"; */
/* 
 * COMPONENT_NAME: UUCP remcap.c
 * 
 * FUNCTIONS: MSGSTR, getent, tdecode, tgetent, tgetflag, tgetnum, 
 *            tgetstr, tnamatch, tnchktc, tskip 
 *
 *
	...
	if (cp == (char *)0 || strcmp(cp, _PATH_REMOTE) == 0) {
		...
		if (rc2 == 1) {
			...
			if (strlen(bp) + strlen(p) > BUFSIZE) {
				write(2, MSGSTR(ENTRYTOOLONG, "Remcap entry too long\n"), 23); /*MSG*/
				return (-1);
			}
			strcat(bp, p);
		}
...
tnchktc()
{
	...
	while (*--p != ':')
		if (p<tbuf) {
			write(2, MSGSTR(BADENTRY, "Bad remcap entry\n"), 18); /*MSG*/
			return (0);
		}
	p++;
	...
	*q = 0;
	if (++hopcount > MAXHOP) {
		write(2, MSGSTR(INFINITE, "Infinite tc= loop\n"), 18); /*MSG*/
		return (0);
	}
...
nextc:
			...
			if (*dp)
				goto nextc;
                        ...
                        /*
                         * pass through \d as a 'd' with the high bit set.
                         * - this will be interpreted by output() in generic.c
                         * as a one second delay.
                         */
                        if (c == 'd') {
                                c = ('d' | 0x80) & 0xff;
                                break;

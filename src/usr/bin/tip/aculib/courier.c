...
static char rcsid[] = "@(#)$RCSfile: courier.c,v $ $Revision: 4.2.7.2 $ (DEC) $Date: 1993/09/07 15:07:40 $";
#endif
/*
courier.c	1.3  com/cmd/tip/aculib,3.1,9013 11/16/89 12:03:09";
 */
/* 
 * COMPONENT_NAME: UUCP courier.c
 * 
...
static	int cour_swallow();
extern	ssize_t cour_write(int, const char *, size_t);
...
ssize_t
cour_write(int fd, const char *cp, size_t n)
{
	struct sgttyb sb;
	if (boolean(value(VERBOSE)))
		write(1, cp, n);
...
#define mask(s) (1<<((s)-1))
#define setvec(vec, a) \
        vec.sv_handler = (void (*)())a; vec.sv_mask = vec.sv_onstack = 0

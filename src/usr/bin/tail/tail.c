...
static char rcsid[] = "@(#)$RCSfile: tail.c,v $ $Revision: 4.2.5.5 $ (DEC) $Date: 1993/12/21 21:14:31 $";
#endif
/*
 * HISTORY
 */
 ...
 * 
 * 
 * 1.12  com/cmd/scan/tail.c, cmdscan, bos320, 9130320 6/15/91 22:21:26
 */
...
#include <stdlib.h>
extern void setcount(), dofollow(), dotail(), dobytes(), dolines();
extern void dorev();

void skipwidechars();	/* for tail +m */
void dowidechars();	/* for tail -m */
void dowidefromstart();
void dowidefromend();
void usage();
...
long    n_tail  = 10L;
struct stat     statb;
char    bin[CHUNK + 4]; /* add 4 bytes so that in the case of	*
...
int argc; char **argv;
{
    ...
    if (reverse)
	if (fromtop || !bylines) {
		fprintf(stderr, MSGSTR(CANNOTR, 
			"tail: reverse whole file or last n lines only.\n"));
		exit(1);
	}

    if (fileNextArg) {
	      if (argc != 1 || inputfile[0] != '\0') {
			usage();
	      } else {
			(void)close(0);
			argv++;
...
void dobytes() {
	...
	if ((b = malloc((size_t)size)) == NULL) {
	    size = n_tail;
	    if ((b = malloc((size_t)size)) == NULL) {
		fprintf(stderr, MSGSTR(TOOMUCH, "tail: too much!\n")); /*MSG*/
		exit(1);
	    }
	}
    }
...
usage()
{
	  char *Usage = "Usage: tail [-f | -r] [-c number | -n number] [file]\n   or: tail [+number | -number][unit] [-f | -r] [file]\n";

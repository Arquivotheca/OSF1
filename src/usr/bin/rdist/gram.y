 ...
 * OSF/1 Release 1.0
 */ 
/* @(#)gram.y	1.5  com/cmd/arch/rdist,3.1,9013 11/1/89 15:45:56 */
/*
...
cmd:		  INSTALL options opt_namelist SM = {
			...
			if ($3 != NULL) {
				...
				if (nl) {
					if (nl->n_next != NULL)
						yyerror(MSGSTR(ONENAME, "only one name allowed\n"));
					$1->sc_name = nl->n_name;
					free((void *)nl);
				} else
					$1->sc_name = NULL;
...
again:
	...
	case '"':  /* STRING */
		...
		for (;;) {
			if (cp1 >= cp2) {
				yyerror(MSGSTR(STRTOO, "command string too long\n"));
				break;
			}
			c = getc(fin);
			if (c == EOF || c == '"')
		...
		if (c != '"')
			yyerror(MSGSTR(MISSQU, "missing closing '\"'\n"));
		*cp1 = '\0';
	...
	for (;;) {
		if (cp1 >= cp2) {
			yyerror(MSGSTR(INPUTL, "input line too long\n"));
			break;
		}
		if (c == '\\') {
			if ((c = getc(fin)) != EOF) {
	...
	struct subcmd *subcmds;
{
	...
	for (h = hosts; h != NULL; free((void *)h), h = h->n_next) {
		/*
	...
	char *s;
{
	...
	nerrs++;
	fflush(stdout);
	fprintf(stderr, MSGSTR(LINEER, "rdist: line %d: %s\n"), yylineno, s);
}

 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ntpdc.c,v $ $Revision: 4.2.2.3 $ (DEC) $Date: 1992/07/07 16:11:14 $";
#endif
/*
 */

/*
 * 	Previous History		ntpdc.c,v $
 * Revision 4.1  91/09/18  21:51:25  devrcs
 * Start of AG pool
 * 
 * Revision 3.1  91/06/24  15:09:10  devbld
 * terry>>>Added for NTP submit
 * 
	...
	char *host;
{
	...
	watcher.sin_addr.s_addr = (u_int) HostAddr;
	if (HostAddr == -1) {
	...
	struct clockinfo *n;
{
	...
	for (i = 0; i < PEER_SHIFT; i++) {
		delay[i] = (double) ((int) (ntohl(n->info_filter.delay[i])/1000.0));
		offset[i] = (double) ((int) (ntohl(n->info_filter.offset[i])/1000.0));
	}
	dsp = (int) ntohl(n->estdisp);		/* leave in milliseconds */
	del = (int) ntohl(n->estdelay);	/* leave in milliseconds */
	off = (int) ntohl(n->estoffset);	/* leave in milliseconds */
	c = ' ';
	flags = ntohs(n->flags);
	...
	printf("%-16.16s %2d %4d  %03o  %8.1f %8.1f %8.1f\n",
	       ...
	       n->stratum, (int)ntohl((u_int)n->timer), 
	...
	struct clockinfo *n;
{
	...
	sin.sin_addr.s_addr = n->net_address;
	for (i = 0; i < PEER_SHIFT; i++) {
		delay[i] = (double) (int) ntohl(n->info_filter.delay[i]);
		offset[i] = (double) (int) ntohl(n->info_filter.offset[i]);
	}
	dsp = (double) ((int) ntohl(n->estdisp));	/* in milliseconds */
	del = (double) ((int) ntohl(n->estdelay));	/* in milliseconds */
	off = (double) ((int) ntohl(n->estoffset));	/* in milliseconds */
	...
	if (n->stratum == 1 || n->stratum == 0) {
		...
		clock_host.s_addr = (u_int) n->refid;

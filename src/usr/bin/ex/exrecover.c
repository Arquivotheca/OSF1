...
static char rcsid[] = "@(#)$RCSfile: exrecover.c,v $ $Revision: 4.2.7.3 $ (DEC) $Date: 1993/10/11 16:46:47 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTION: main, blkio, clrstats, enter, error_exr,
 ...
 *
 * 1.17  com/cmd/edit/vi/exrecover.c, , bos320, 9134320 8/11/91 12:32:38
 * 
 */
...
static void blkio_exr(short, wchar_t *, ssize_t (*)(int, void *, size_t));
 ...
 * Scratch for search.
 */
char	bestnb[BUFFERSIZ];		/* Name of the best one */
...
void searchdir(char *dirname)
{
	struct dirent *dirp;
	register DIR *dir;
	/* char dbuf[BUFFERSIZ]; unused UNIX/370. */

	dir = opendir(dirname);
	if (dir == NULL)
		return;
	/* setbuffer(dir, dbuf, sizeof(dbuf)); this breaks UNIX/370. */
	while ((dirp = readdir(dir)) != NULL) {
		if (dirp->d_name[0] != 'E')
			continue;
		/*
...
getblock_exr(line atl, int iof)
{
	...
	if (iof == READ) {
		if (ichanged)
			blkio_exr(iblock, ibuff, (ssize_t (*)(int, void *, size_t))write);
		ichanged = 0;
		iblock = bno;
	...
	if (oblock >= 0)
		blkio_exr(oblock, obuff, (ssize_t (*)(int, void *, size_t))write);
	oblock = bno;
	return (obuff + off);
}
...
static
void blkio_exr(short b, wchar_t *buf, ssize_t (*iofcn)(int, void *, size_t))
{

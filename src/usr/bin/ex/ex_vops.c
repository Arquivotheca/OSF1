...
static char rcsid[] = "@(#)$RCSfile: ex_vops.c,v $ $Revision: 4.2.5.2 $ (DEC) $Date: 1993/06/10 21:16:20 $";
#endif
/*
 * HISTORY
 */
 ...
 */
/*
 * COMPONENT_NAME: (CMDEDIT) ex_vops.c
 *
 * FUNCTION: setpk, vUndo, vchange, vdelete, vfilter, vmacchng, vmove, vnoapp,
 * voOpen, vrep, vshftop, vshift, vundo, vyankit, xdw
 *
 *
 ...
 *
 * 1.9  com/cmd/edit/vi/ex_vops.c, cmdedit, bos320 6/5/91 23:32:47
 * 
...
void vUndo(void)
{
	...
	CP(vutmp, linebuf);
	vUD1 = linebuf; vUD2 = WCstrend(linebuf);
	putmk1(dot, vUNDsav);
	getDOT();
	vUA1 = linebuf; vUA2 = WCstrend(linebuf);
	vundkind = VCAPU;
	if (state == ONEOPEN || state == HARDOPEN) {
		vjumpto(dot, vUNDcurs, (wchar_t)0);
		return;
	}
	vdirty(vcline, 1);
	vsyncCL();
...
void vundo(short show)
/* show:	if true update the screen */
{
	register int cnt;
	register line *addr;
	register wchar_t *cp;
	...
	case VMCHNG:
		 ...
		 * with dol through unddol-1.  Change screen image to
	...
	case VCAPU:
		...
		vdirty(vcline, 1);
		vsyncCL();
		if (cursor > linebuf && cursor >= WCstrend(linebuf))
			cursor--;
		vfixcurs();
		break;
		...
		if (rubble) {
			...
			*cp = c;
			...
			for (cp = cursor; *cp && cp < wcursor;) {
				register int c = *cp++;
				ex_putchar((c == QUOTE_NUL || c == QUOTE_SP) ? ' ' : c);
			}
		}
	}
	vsetcurs(wcursor);
...
void vdelete(wchar_t c)
{
	...
	vsetcurs(cp);
#ifdef NOTDEF
/* next six lines are first pass fix for tab deletion problem */
	if ((DEL[0] == '\t') && ((lastcmd[0] == 'x') || ((lastcmd[0] == 'd') && (lastcmd[1] == ' ')))) { 
		vclear(cp);
		vdirty(0,vcnt);
		vredraw(WTOP);
		vfixcurs();
	...
	if (wdot) {
		...
		if (cursor && wcursor) {
			...
			*cursor = 0;
			...
			getline(*wdot);
			if (wcslen(genbuf) + wcslen(wcursor) > LBSIZE - 2) {
				getDOT();
				ex_beep();
				return;
			}
			wcscat(genbuf, wcursor);
		...
		/*
		 * Now do something with the screen image coordination.
		 */
...
smallchange:
	/*
	 * The rest of this has to do with changes
...
static int xdw(void)
{
	...
	cnt = wdot - dot + 1;
	if (vreg) {
		vremote(cnt, (void (*)(int))YANKreg, vreg);
/*
		if (notp)
			notpart(vreg);
 */
	...
	vreg = 0;
	if (any(op, "cd")) {
		vremote(cnt, (void (*)(int))YANKreg, '1');
/*
		if (notp)
			notpart('1');
 */
...
void vrep(int cnt)
{
	...
	vcursat(cursor);
	doomed = i - cindent();
	vshowmode(value(TERSE) ? MSGSTR(M_279, "r") : MSGSTR(M_241, "REPLACE 1 CHAR"));
	if (!vglobp) {
		c = getesc();
		if (c == 0) {
			vshowmode("");

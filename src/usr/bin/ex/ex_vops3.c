...
static char rcsid[] = "@(#)$RCSfile: ex_vops3.c,v $ $Revision: 4.2.5.2 $ (DEC) $Date: 1993/06/10 21:21:35 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTION: endPS, endsent, isa, lbrack, lfind, lindent, lmatchp, lnext,
 * lskipa1, lskipatom, lskipbal, lsmatch, ltosol1, ltosolid
 *
 *
 ...
 *
 * 1.6  com/cmd/edit/vi/ex_vops3.c, cmdedit, bos320 6/5/91 23:33:26
 * 
 */
...
int lfind(short pastatom, int cnt, void (*f)(void), line *limit)
{
	...
	if (pastatom >= 2) {
		if (pastatom == 3) {
			...
			if (wordof((wchar_t)which, wcursor+1)) {
			  cnt--;
			}
		}
...
static int ltosol1(char *parens)
{
	register wchar_t *cp;

	if (*parens && !*wcursor && !lnext())
		return (0);
	while (iswspace(*wcursor) || (*wcursor == 0 && *parens))
		if (!lnext())
			return (0);
	if (any(*wcursor, parens) || dir > 0)
		return (1);
	for (cp = wcursor; cp > linebuf; cp--)
		if (any(cp[-1], "!?.") || any(cp[-1], parens))
			break;
	wcursor = cp;
	return (1);
}
...
static int lskipa1(char *parens)
{
	register int c;

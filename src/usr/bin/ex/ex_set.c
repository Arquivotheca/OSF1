...
static char rcsid[] = "@(#)$RCSfile: ex_set.c,v $ $Revision: 4.2.5.2 $ (DEC) $Date: 1993/06/10 20:22:02 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 1.9  com/cmd/edit/vi/ex_set.c, , bos320, 9134320 8/11/91 12:29:41
 * 
 */
...
dontset:
		...
		if (op->oname == 0) {
			if (nseq(cp, "vice")) 
				serror(MSGSTR(M_302, "Cannot set novice interactively - invoke vedit"), DUMMY_CHARP);
			else 
...
printone:
		...
		case NUMERIC:
			...
			if (value(TABSTOP) <= 0){
			    value(TABSTOP) = TABS;
			    serror(MSGSTR(M_514,
				"The %s option must be greater than 0. Default value used."), "tabstop");
			}
			if (value(HARDTABS) <= 0){
			    value(HARDTABS) = TABS;
			    serror(MSGSTR(M_514,
				"The %s option must be greater than 0. Default value used."), "hardtabs");
			}
			if (value(SHIFTWIDTH) <= 0) {
			    value(SHIFTWIDTH) = TABS;
			    serror(MSGSTR(M_514,
				"The %s option must be greater than 0. Default value used."), "shiftwidth");
			}
			if (op == &options[WINDOW]) {
				if (value(WINDOW) >= lines)
					value(WINDOW) = lines-1;
				vsetsiz(value(WINDOW));
			} else if (op == &options[SCROLL]) {
 ...
 */
				if (inopen) 
				    error(MSGSTR(M_147, "Can't change type of terminal from within open/visual"),
				    DUMMY_INT);
				setterm(s_type);
			} else {
				if (op->otype == WRAP)
					...
					default:
						error(MSGSTR(M_290,
						"f(lexible), g(eneral), r(igid), or w(ord) required@after ="),
						DUMMY_INT);
						break;
					}
					...
					/* PARTIALCHAR must be a printable single col char */
                                        if      ((wcwidth(nc) > 1) || !iswprint(nc))
                                                error(MSGSTR(M_289,
                                                "partial character indicator should display in one column"), DUMMY_INT);
                                }
				if (op->otype != CLOSEP) {

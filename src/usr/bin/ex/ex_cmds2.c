...
static char rcsid[] = "@(#)$RCSfile: ex_cmds2.c,v $ $Revision: 4.2.7.2 $ (DEC) $Date: 1993/06/10 19:45:04 $";
#endif
/*
 * HISTORY
 */
 ...
 */
/*
 * COMPONENT_NAME: (CMDEDIT) exc_cmds2.c
 *
 * FUNCTION: cmdreg, donewline, endcmd, eol, erewind, error, error0, error1,
 * exclam, fixol, makargs, next, nomore, quickly, resetflav, serror, setflav,
 * skipend, tail, tail2of, tailprim, tailspec, vcontin, vnfl
 *
 *
 ...
 *
 * 1.10 com/cmd/edit/vi/ex_cmds2.c, , bos320, 9134320 8/11/91 12:28:25
 * 
 ...
 */
void error(register char *str, int i)
{
...
nomore(void)
{
	...
	morargc = argc;
	if (argc == 1)
		error(MSGSTR(M_291, "%d more file to edit"), argc);
	else
		error(MSGSTR(M_294, "%d more files to edit"), argc);
}
 ...
 */
void tail(wchar_t *comm)
{

	tailprim(comm, 1, (short)0);
}

void tail2of(wchar_t *comm)
{
...
void tailprim(register wchar_t *comm, int i, short notinvis)
{
	register wchar_t *cp;
	register int c;

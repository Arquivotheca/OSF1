...
static char rcsid[] = "@(#)$RCSfile: ex_vops2.c,v $ $Revision: 4.2.5.2 $ (DEC) $Date: 1993/06/10 21:19:02 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTION: ateopr, back1, bleep, takeout, vappend, vdcMID, vdoappend,
 ...
 *
 * 1.14  com/cmd/edit/vi/ex_vops2.c, cmdedit, bos320, 9136320a 8/30/91 11:18:11
 * 
 */
...
static wchar_t nl[2] = { '\n', 0 };	/* "\n"  as an NLchar string */
static wchar_t sp[2] = { ' ', 0 };		/* " "  as an NLchar string */
...
void vappend(int ch, int cnt, int indent)
{
	...
	for (;;) {
		if (ch == 'r' && repcnt == 0)
			...
			gcursor = vgetline(repcnt, gcursor, &escape, (wchar_t)ch);
...
vgetline(int cnt, wchar_t *gcursor, short *aescaped, wchar_t commch)
{
	...
	*aescaped = TRUE;
	...
	for (;;) {
		...
		if ((nxtpatrn == FALSE) || (isamap == FALSE))
			c = getkey();
		else
			c = '\033';
		ch = c;
		...
		if (c != ATTN && vglobp == 0 && Peekkey == 0 && commch != 'r')
			if (vmacp == 0 || (value(REMAP) && vmacp != 0)) {
				while ((ch = map((wchar_t)c, immacs)) != c) {
					c = ch;
					if (!value(REMAP))
						break;
					if (++maphopcnt > 256)
			...
			if (value(WRAPMARGIN) &&
				 ...
				 */
					...
					if (c == ESCAPE)			
						goto dontbreak;
				 /*
				  * general and flexible column wrap permits 1 closing punctuation
				  * past the wrap margin
				 */
				...
				*gcursor++ = c;
				if (backsl)
					*gcursor++ = getkey();
				*gcursor = 0;
                                dispcol = outcol + wcwidth(*(gcursor -1));
				/*
...
def:
			...
			if (gcursor > &genbuf[LBSIZE])
				error(MSGSTR(M_246, "An input line cannot be longer than %d characters."), LBSIZE);
			/* *gcursor++ = c & TRIM; */
...
static int vmaxrep(wchar_t ch, int cnt)
{
	...
	len = wcslen(linebuf);
	/* 
	 * Return requested count if we can fit it, or if
	 * the only thing appended was a newline in the LBSIZE
	 * position.
	 */
	if ((len + cnt * replen <= LBSIZE - 2) || replen == 0)
		return (cnt);

...
static char rcsid[] = "@(#)$RCSfile: ex_vput.c,v $ $Revision: 4.2.7.4 $ (DEC) $Date: 1994/01/31 18:49:59 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTION: enddm, endim, fixech, godm, goim, physdc, tfixnl, tracec,
 ...
 *
 * 1.7  com/cmd/edit/vi/ex_vput.c, cmdedit, bos320, 9130320 7/1/91 16:32:00
 * 
 */
...
void vclreol(void)
{
	...
	if (destline < 0 || destline > WECHO)
		error(MSGSTR(M_248, "Internal error: vclreol"), DUMMY_INT);
	i = WCOLS - destcol;
...
#ifdef FDEBUG
	if (trace) {
		fprintf(trace, "Before vgoto MSGSTRx %d, y %d, WCOLS %d\n, ",
			x, y, WCOLS);
	}
#endif
	if (x >= WCOLS) {
...
#endif
	if (y < 0) {
		error(MSGSTR(M_249, "Internal error: vgoto"), DUMMY_INT);
	}
	if (outcol >= WCOLS) {
...
void vmaktop(int p, wchar_t *cp)
{
	...
	if (p < 0 || vtube[p] == cp)
		return;
        temp = (wchar_t *)alloca(TUBECOLS * sizeof(wchar_t)); /* WW-03 */
...
/* wrong !!! test for the quoted char QUOTE_BULLET */
	...
	/* copy inserted character to ttube */
        /* WW-01 */
        ttube=(wchar_t *)alloca((TUBESIZE + LBSIZE) * sizeof(wchar_t)); /* WW-03 */
        tt=ttube;       /* WW-01 */

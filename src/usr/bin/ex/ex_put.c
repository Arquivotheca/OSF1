...
static char rcsid[] = "@(#)$RCSfile: ex_put.c,v $ $Revision: 4.2.8.2 $ (DEC) $Date: 1993/06/10 20:08:03 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 1.11  com/cmd/edit/vi/ex_put.c, cmdedit, bos320 6/5/91 23:30:11
 * 
 */
	...
	while (*lp) {
		...
		default:
			...
			for (;;) {
				...
				/*  check that the char is displayable */
				if (!iswprint(c)) {
					outcol += 2;	 /* M-X */
					destcol += 2;	 /* M-X */
					putch('M');
	...
	while (outcol < destcol) {
		...
		if (j > costRP && parm_right_cursor) {
			...
			if (!inopen || (vtube[outline]==NULL) ||
				...
				(wc=vtube[outline][outcol]) < ' ')
				wc = ' ';
			if (wc == QUOTE_NUL || wc == QUOTE_SP)
				wc = ' ';
			if (cursor_right && (insmode || wc == QUOTE_WCD || j < wcwidth(wc))) {
				tputs(cursor_right, 0, (int (*)(int))plodput);
				outcol++;
			} else {
				plodput(wc);
				outcol += (((len = wcwidth(wc)) == -1) ? 1 : len);
			}
		}
		if (plodcnt < 0)
			goto out;

...
static char rcsid[] = "@(#)$RCSfile: ex_vget.c,v $ $Revision: 4.2.5.2 $ (DEC) $Date: 1993/06/10 21:01:13 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 1.15  com/cmd/edit/vi/ex_vget.c, , bos320, 9134320 8/16/91 16:59:27
 * 
 */
...
again:
	...
	doingread = 1;
        for (count = 0, char_ok = 0; count < MB_CUR_MAX; count++)
        {
                ...
                if(read(slevel == 0 ? 0 : ttyindes, &inbuf[count], 1) != 1)
                {
			if (errno == EINTR)
				goto getATTN;
			error(MSGSTR(M_236, "Input read error"), DUMMY_INT);
			break;
		}
...
int readecho(wchar_t c)
{
	...
	if (c) {
	  ...
	  ignore(vgetline(0, genbuf, &waste, c));
	}
	if (Outchar == termchar)
		ex_putchar('\n');
	vscrap();
...
int map(wchar_t c, struct maps *maps)
{
	register int d;
	register wchar_t *p;
	register wchar_t *q;
...
void macpush(wchar_t *st, int canundo)
{
	...
	if ((vmacp ? wcslen(vmacp) : 0) + wcslen(st) > LBSIZE)
		error(MSGSTR(M_238, "Macro too long@ - maybe recursive?"), DUMMY_INT);
	if (vmacp) {
		wcscpy(tmpbuf, vmacp);
		if (!FIXUNDO)

...
static char rcsid[] = "@(#)$RCSfile: ex_vmain.c,v $ $Revision: 4.2.7.4 $ (DEC) $Date: 1994/01/31 18:47:28 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * 1.15  com/cmd/edit/vi/ex_vmain.c, cmdedit, bos320 6/5/91 23:33:07
 * 
...
void vmain(void)
{
	...
	vch_mac = VC_NOTINMAC;
        ...
        esave=(wchar_t *)alloca(TUBECOLS * sizeof(wchar_t));    /* WW-03 */
	/*
...
reread:
		...
		/* 
		 * isamap is TRUE when we should expect to find a
		 * map associated with this invocation from visual 
		 * mode.
		 */
		isamap = FALSE;
		do {
			 ...
			 */
			c = op;
			if (vmacp == 0 || (value(REMAP) && vmacp != 0)) {
				op = map(c,arrows);
			}
#ifdef MDEBUG
			if (trace)
	...
	if (trace)
		...
		/*
		 * ^R, ^L	Clear screen e.g. after transmission error.
		 */
		case Ctrl('L'):
		case Ctrl('R'):
			vclear();
		...
		case Ctrl('B'):
			...
			if (one + vcline != dot && vcnt > 2) {
				addr = dot - vcline + 2 - (cnt-1)*(basWLINES-2);
				forbid (addr <= zero);
				dot = addr;
				vcnt = vcline = 0;
			}
		...
		case '~':
			{
			  ...
			  for (i=count-1; i>=0; i--) {
				...
				mbuf[0] = 'r';
				mbuf[1] = *(cursor+i);
				mbuf[2] = cursor[i+1]==0 ? 0 : ' ';
				mbuf[3] = 0;
				if (iswalpha(mbuf[1]))
					/* toggle the case */
					mbuf[1] = iswupper(mbuf[1]) ?
						towlower(mbuf[1]) :
						towupper(mbuf[1]);
				macpush(mbuf, 1);
			  }
...
insrt:
		...
		case 'p':
			...
			if (!vreg && DEL[0]) {
				forbid (DEL_OVFLOW);
				vglobp = DEL;
                                pp_insert(c,cnt,i);  /** special insert **/
				continue;
			}
...
gogo:
			...
			/* 
			 * Because of all the goto's that might have
			 * brought us here, better make sure that i is
			 * in-sync.  (It will be out of sync for any
			 * command which has generated a call to
			 * change() (eg, via a putline()).  We don't
			 * want to let i be out-of-sync now because
			 * some of the cases where we came from expect
			 * to be able to regenerate the screen data
			 * from the value of cursor because they know
			 * they can't trust (destline, destcol).  A
			 * little later on, we're gonna check that i
			 * == tchng, and if its not, we set cursor=0.
			 * Examples of where we'd be messed up if we
			 * don't sync here are the ctrl-Z, ctrl-G, and
			 * ctrl-A cases.
			 */
			i = tchng;
			goto doinit;
...
# endif
		...
		case ':':
			...
			i = tchng;
			addr = dot;
			if ((value(PROMPT) ? readecho(c) : readecho(0))) {
                                *esave = 0;             /* WW-01 */
				goto fixup;
			}
			getDOT();
 ...
 * Do a z operation.
 */
static void vzop(short hadcnt, int cnt, wchar_t c)
{
	register line *addr;
...
void pp_insert(int c,int cnt,int i)      /** P p insert - allows macros such as "dwPP" **/
{
wchar_t *save_vmacp;
        save_vmacp=vmacp;

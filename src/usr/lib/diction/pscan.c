 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: pscan.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 05:27:56 $";
#endif 
/*
 */
/*
    ...
    char            ce;
{
	...
	case THAT:
		...
		if (verb == 0 && nextc == NV_PL && ((lastc == NOUN
		...
		if (verb == 0 && (nextc == UNK || nextc == NOUN_VERB) && (lastc == NOUN
						&& sent[i - 1].ic == NV_PL))
	    {
		subty = PLURAL;
		sent[i].cc = PRONP;
	...
	case NOUN_VERB:
	    ...
	    if (nextc == NV_PL && isnoun (sent[i + 2].cc))
	    {
		sent[i].cc = NOUN;
		if (sent[i - 1].cc == NOUN)
		{
...
b:
	...
	{			/* UUU. */
	    if (prep == 0)
		subty = SING;
	    if (sent[t].cc == NV_PL || sent[t].cc == PNOUN)
	    {			/* UUZ. */
		if (sent[t + 1].cc == UNK || sent[t + 1].cc == NOUN_VERB)
		{		/* UUZU */
		    sent[t + 1].cc = VERB;
		    verb = 1;
		    sent[t].cc = NOUN;

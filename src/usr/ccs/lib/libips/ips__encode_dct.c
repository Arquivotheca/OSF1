/*  DEC/CMS REPLACEMENT HISTORY, Element SMIENCODEDCT.C */
/*  *9    18-DEC-1990 17:57:23 SHELLEY "ADD ERROR HANDING (IDS PRIORITY)" */
/*  *8     2-NOV-1990 11:53:54 SHELLEY "fix unreachable return" */
/*  *7     1-NOV-1990 10:51:42 SHELLEY "first pass adding error handling" */
/*  *6    12-OCT-1990 16:52:11 WEBER "IPS integration" */
/*  *5     8-OCT-1990 15:11:31 WEBER "Merge changes" */
/*   3A1   8-OCT-1990 14:28:17 WEBER "Add pipe support" */
/*  *4    28-SEP-1990 12:52:21 SHELLEY "exterminate bugs in IPS additions" */
/*  *3    24-SEP-1990 15:53:48 WEBER "Fix freeing NULL pointers and ROI problem" */
/*  *2    18-SEP-1990 08:09:23 WEBER "Integrate XIE/IPS" */
/*  *1    18-SEP-1990 08:01:31 WEBER "DCT compression/decompression sources" */
/*  DEC/CMS REPLACEMENT HISTORY, Element SMIENCODEDCT.C */

/***********************************************************
...
#endif
/*
**  These entry points are local for IPS
*/
#ifndef NODAS_PROTO
PROTO(static unsigned long SmiEncodeDctSegment, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static EncodeDctStatePtr SmiEncodeDctInitializeState, (UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static void SmiEncodeDctDestroyState, (EncodeDctStatePtr /*state*/));
#endif
#endif
...
#ifndef NODAS_PROTO
PROTO(static int DceNextToken, (EncodeDctStatePtr /*state*/));
PROTO(static int DceLenDat, (EncodeDctStatePtr /*state*/));
PROTO(static int DceLenRSC, (EncodeDctStatePtr /*state*/));
PROTO(static int DceLenSOI, (EncodeDctStatePtr /*state*/));
PROTO(static int DceLenSOF, (EncodeDctStatePtr /*state*/));
PROTO(static int DceLenSOS, (EncodeDctStatePtr /*state*/));
PROTO(static int DceLenEOI, (EncodeDctStatePtr /*state*/));
PROTO(static int DceActDat, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static int DceActRSC, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static int DceActSOI, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static int DceActSOF, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static int DceActSOS, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static int DceActEOI, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/));
PROTO(static int DceEncodeData, (EncodeDctStatePtr /*state*/, UdpPtr /*udplst*/[], int /*udpcnt*/, unsigned char */*outptr*/, int /*outlen*/));
PROTO(static void DcePadData, (unsigned char */*srcptr*/, int /*srcrow*/, int /*srccol*/, int /*srcsst*/, int /*srcpst*/, unsigned char */*dstptr*/, int /*dstrow*/, int /*dstcol*/));
PROTO(static int DctEncode, (short int */*z_coef*/, DcEncodeTablePtr /*dc_tbl*/, AcEncodeTablePtr /*ac_tbl*/, short int */*dc_prev*/, unsigned char */*bufptr*/, int /*buflen*/, int /*bitoff*/));
PROTO(static int DctPadByte, (unsigned char */*bufptr*/, int /*bitoff*/));
PROTO(static int DctPutBits, (int /*data_in*/, int /*number_of_bits*/, unsigned char */*bufptr*/, int /*bitoff*/));
PROTO(static int DctCreateScaledQuant, (EncodeDctStatePtr /*state*/, int /*defidx*/, int /*outidx*/, int /*factor*/));
PROTO(static int DctRstEnable, (EncodeDctStatePtr /*state*/));
PROTO(static void Dce8x8_8U_16S, (unsigned char */*PIXELS*/, short int */*COEFS*/, int /*SCAN_WIDTH*/, int /*PEL_WIDTH*/));
#endif

/*
**  Local MACROS
...
int		*size_ret;
{
    ...
    */
    state = SmiEncodeDctInitializeState( udplst, udpcnt );
    if( state == NULL )
	...
	*cdp = *udplst[0];
	...
	if( cdp->UdpA_Base == NULL )
	{
	    SmiEncodeDctDestroyState(state);
	    return DctX_BadAlloc;
	}
	mem_alloc = TRUE;
    }
    ...
    */
#if	defined(IPS)
    DceCurPtr_(state) = cdp->UdpA_Base + (cdp->UdpL_Pos>>3);
#endif
#if	defined(XIESMI)
    ...
    **	Encode the data.
    */
    status = SmiEncodeDctSegment( state, udplst, udpcnt );
    ...
    */
    if( mem_alloc )	
    {
	cdp->UdpA_Base   = DctReallocBits_(cdp->UdpA_Base, bytcnt*8);
...
unsigned long	*final;
{
    ...
    */
    EncodeDctPipeCtxPtr ctx = (EncodeDctPipeCtxPtr)
			      PipeCreateCtx( pipe, &EncodeDctPipeElement );
    ...
    */
	...
	if ( (1 << i) & cmpmsk)
	{
	    DceSrcDrn_(ctx,i) = RmCreateDrain(srcsnk, 1 << i);
	    if( !IsPointer_(DceSrcDrn_(ctx,i)) )
		return( (int) DceSrcDrn_(ctx,i) );
	    RmSetDType(DceSrcDrn_(ctx,i), UdpK_DTypeBU, DtM_BU);
	    RmSetQuantum(DceSrcDrn_(ctx,i), DctK_BlockSize);
	}
    }
    return( DctX_Success );
...
EncodeDctPipeCtxPtr  ctx;
{
	...
	{
	    status = RmInitializePort(DceSrcDrn_(ctx,i));
    ...
    */
    DceSteBlk_(ctx) = SmiEncodeDctInitializeState(udplst,udpcnt);
    ...
    */
	...
	{
    	DceDstUdp_(ctx)->UdpA_Base = (unsigned char *)
	    DctMallocBits_(DceDstUdp_(ctx)->UdpL_ArSize);
	if (DceDstUdp_(ctx)->UdpA_Base == NULL) return DctX_BadAlloc;
	}
    /*
    **	Initialize buffer pointers and lengths
    */
    DceCurLen_(DceSteBlk_(ctx)) = 
	(DceDstUdp_(ctx)->UdpL_ArSize - DceDstUdp_(ctx)->UdpL_Pos) / 8;
    DceDstUdp_(ctx)->UdpL_ArSize = 0;
    DceCurRem_(DceSteBlk_(ctx)) = DceCurLen_(DceSteBlk_(ctx));
    DceCurPtr_(DceSteBlk_(ctx)) = 
	    DceDstUdp_(ctx)->UdpA_Base + DceDstUdp_(ctx)->UdpL_Pos / 8;
...
EncodeDctPipeCtxPtr  ctx;
{
    ...
    if (DceDstUdp_(ctx)->UdpA_Base == NULL)
	{
	bytes = DceCurRem_(DceSteBlk_(ctx)) + DceDstUdp_(ctx)->UdpL_Pos / 8;
    	DceDstUdp_(ctx)->UdpA_Base = (unsigned char *)DctMalloc_(bytes);
	if( DceDstUdp_(ctx)->UdpA_Base == NULL ) return( DctX_BadAlloc );
	DceCurPtr_(DceSteBlk_(ctx)) = DceDstUdp_(ctx)->UdpA_Base
				    + DceDstUdp_(ctx)->UdpL_Pos / 8;
	}

    for( status = DctX_Success; status == DctX_Success; _EncodeDctAbort(ctx) )
	{
	for( i = 0, udpcnt = 0;  i < XieK_MaxComponents;  i++ )
	    /*
	    ...
	    */
		...
		if( DceSrcDat_(ctx,i) == NULL )
		    {
		    DceSrcDat_(ctx,i) = RmGetData(ctx,DceSrcDrn_(ctx,i));
	...
	*/
	if( !(*DceFinal_(ctx) & DceCmpMsk_(ctx)) )
		...
		**  Encode the data.
		*/
	    status = SmiEncodeDctSegment( DceSteBlk_(ctx), udplst, udpcnt );
	    /*
...
**  FUNCTIONAL DESCRIPTION:
**
**      This routine processes any data remaining in the pipeline before
...
EncodeDctPipeCtxPtr  ctx;
{
    ...
    */
    for( i = 0;  i < XieK_MaxComponents ;  i++ )
	if ( (1 << i) & DceCmpMsk_(ctx))
	    RmSetQuantum(DceSrcDrn_(ctx,i), 0);
...
EncodeDctPipeCtxPtr  ctx;
{
    ...
    */
    for( i = 0;  i < XieK_MaxComponents;  i++ )
	if( IsPointer_(DceSrcDat_(ctx,i)) )
	    DceSrcDat_(ctx,i) = RmDeallocData( DceSrcDat_(ctx,i) );
...
EncodeDctPipeCtxPtr  ctx;
{
    ...
    SmiEncodeDctDestroyState( DceSteBlk_(ctx) );
    DceSteBlk_(ctx) = NULL;

    for( i = 0;  i < XieK_MaxComponents;  i++ )
	if( (1 << i) & DceCmpMsk_(ctx) )
	    DceSrcDrn_(ctx,i) = RmDestroyDrain(DceSrcDrn_(ctx,i));
...
static
#endif
unsigned long	    SmiEncodeDctSegment( state, udplst, udpcnt )
EncodeDctStatePtr   state;
UdpPtr		    udplst[];
int		    udpcnt;
{
...
static
#endif
EncodeDctStatePtr   SmiEncodeDctInitializeState(udplst,udpcnt)
UdpPtr		    udplst[];
int		    udpcnt;
{
    int	    i;
    ...
    */
    ...
    {
        xmin = DceCmpWid_(state,0); ymin = DceCmpHgt_(state,0);
	xmax = DceCmpWid_(state,0); ymax = DceCmpHgt_(state,0);

        for (i = 1;  i < DceCmpCnt_(state);  i++)
        {
            if (DceCmpWid_(state,i) < xmin) xmin = DceCmpWid_(state,i);
            if (DceCmpWid_(state,i) > xmax) xmax = DceCmpWid_(state,i);
            if (DceCmpHgt_(state,i) < ymin) ymin = DceCmpWid_(state,i);
            if (DceCmpHgt_(state,i) > ymin) ymax = DceCmpWid_(state,i);
        }
	/*
...
static
#endif
void		    SmiEncodeDctDestroyState(state)
EncodeDctStatePtr   state;
{
    int	i;

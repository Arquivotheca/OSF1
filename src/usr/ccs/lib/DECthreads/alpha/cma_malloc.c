 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: cma_malloc.c,v $ $Revision: 1.1.4.3 $ (DEC) $Date: 1993/05/11 22:03:52 $";
#endif
/*
 ...
 *  MODIFIED BY:
 *
 *	001	Dave Butenhof	 7 January 1992
 *		Adapt OSF/1 libpthreads malloc() to DECthreads, since we need
 *		a thread-safe malloc, and OSF unaccountably choose to put it
 *		in libpthreads rather than libc_r (where it clearly
 *		belongs).
 *	002	Webb Scales	20 January 1992
 *		Integrate into TIN (DEC OSF/1 V1.0)
 *	003	Dave Butenhof	21 January 1992
 *		Add a typecast for new C compiler.
 *	004	Dave Butenhof	 4 March 1992
 *		Integrate latest OSF/1 source changes
 *	005	Dave Butenhof	26 February 1993
 *		Fix compilation errors.
 *	006	Dave Butenhof	26 April 1993
 *		Conditionalize mach_error() printouts on ifndef NDEBUG, to
 *		avoid spurious library messages when system swap space is
 *		insufficient (for example). Standard libc malloc() merely
 *		returns NULL (as defined), and this should, too.
 *	007	Fix tracing statements & conditionalization.
 */

#include <pthread.h>
#include <cma_defs.h>
...
malloc(register size_t size)
{
	...
	if (size <= sizeof(union header)) {
		...
		cma__trace ((
			cma__c_trc_vm,
			"malloc: bad requested block size (%u)",
			size - sizeof(union header)));
		return(0);
	}
	...
	if (i == NBUCKETS) {
		cma__trace ((
			cma__c_trc_vm,
			"malloc: requested block too large (%u)",
			size - sizeof(union header)));
		return(0);
	}
...
free(any_t base)
{
	...
	if (base == 0) {
		cma__trace ((cma__c_trc_vm, "free: ignoring free(0)"));
		return;
	}
  	...
  	for (i = 0, size = MIN_SIZE, fl = malloc_free_list;
		...
		if (fl->in_use != 0 || fl->head != 0) {
			...
			for (n = 0, h = fl->head; h != 0; h = h->next, n += 1)
				;
			total_free += n * size;
			cma__trace ((cma__c_trc_vm, "%10d %10d %10d %10d",
				size, fl->in_use, n, fl->in_use + n));
		}
		cma__spinunlock(&fl->lock);
  	}
  	cma__trace ((cma__c_trc_vm, " all sizes %10d %10d %10d",
...
/*  VAX/DEC CMS REPLACEMENT HISTORY, Element CMA_MALLOC.C */
/*  *6     7-MAY-1993 07:03:24 BUTENHOF "Fix tracing" */
/*  *5    27-APR-1993 12:12:36 BUTENHOF "Remove error printouts" */
/*  *4    26-FEB-1993 16:15:26 BUTENHOF "Fix compilation errors" */

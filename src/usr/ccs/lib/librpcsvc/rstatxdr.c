 ...
 */
#ifndef lint
static char *sccsid = "@(#)$RCSfile: rstatxdr.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1993/05/26 17:45:23 $";
#endif
/*
 */
/*
	...
	char *host;
{
	...
	if (callrpc(host, RSTATPROG, RSTATVERS_SWTCH, RSTATPROC_HAVEDISK,
			xdr_void, (char *)NULL, xdr_int, (char *)&have) != 0)
		return (-1);
	else
		return (have);
}
	...
	struct statsswtch *statp;
{
	...
	if (xdr_int(xdrs, (int *) &statp->v_swtch) == 0)
		return (0);
	for (i = 0; i < 3; i++)
		if (xdr_int(xdrs, &statp->avenrun[i]) == 0)
			return (0);

...
static char rcsid[] = "@(#)$RCSfile: idln.getst.c,v $ $Revision: 4.2.4.2 $ (DEC) $Date: 1993/06/12 21:36:14 $";
#endif
/*
 * HISTORY
 */
/*
 */ 
/*** "idln.getst.c  1.5  com/lib/curses,3.1,8943 10/16/89 23:21:21"; ***/
/*
 * COMPONENT_NAME: (LIBCURSE) Curses Library
 *
 * FUNCTIONS:   _id_line, _getst
 *
 *
...
struct st {		/* symbol table */
	...
	_SHORT	olno;	/* line # of line in "old" array */
};
 ...
 * FUNCTION:
 *
 *      Routines to deal with calculation of how to do ins/del line
 *      optimally.
 *
 ...
 *          of unchanged lines.
 *
 *      We view the old and new screens' hashed values as the old and new
 *      "files". Since crossing match lines cannot be taken advantage of
 *      with real insert and delete line, we ignore those matches - this is
 *      the one real disadvantage of this algorithm.
 *
...
#define idlcost(n, i) n * \
	...
	for (i=first; i<=last; i++) {
		...
		if (oa[i] != na[i]) {
			...
			if (SP->cur_body[i] == 0 || SP->cur_body[i]->length
								== 0) {
				k += SP->std_body[i]->bodylen;
		...
		if(outf) fprintf(outf,
			"%d      %d      %d      %d      %d",
				i, base, j, k, SP->std_body[i]->bodylen);
		if (n < 0)
			if(outf) fprintf(outf,
				"        %d lines inserted", -n);
		else if (n > 0)
...
#ifdef DEBUG
		if(outf) fprintf(outf,
			"will do %d extra inserts, so del now.\n", ndel);
#endif
...
#ifdef DEBUG
		if (outf) fprintf(outf,
		"InputPending %d, aborted after dellines\n", InputPending);
#endif
		return;
	}
	...
	for (i=first; i<=last; i++) {
		...
		if (na[i] < 0) {
			 ...
			 */
			if (na[j]==i || ndel+i>last &&
						last<SP->des_bot_mgn+1) {
				_pos(i-1, 0);
				n = j - i;
				idl_offset -= n;
				ndel -= n;

...
static char rcsid[] = "@(#)$RCSfile: getch.c,v $ $Revision: 4.2.4.2 $ (DEC) $Date: 1993/06/12 21:30:17 $";
#endif
/*
 * HISTORY
 */
 ...
 * 
 * 1.14  com/lib/curses/getch.c, bos, bos320 9/10/91 23:25:12
 */
...
# endif
	...
	/* be set except here. */ /* paw: QAR 5935 */
	if (win->_nodelay)
	...
	if (i > 0) {
		...
		/* what used to be the non-NLS case
		 * if (!win->_use_meta)
		 * 	inp &= 0177;
		 * else
		 * 	inp &= 0377;
...
gotit:
	...
	if(outf) fprintf(outf, "getch returns %o, pushed %o %o %o\n",
		inp, SP->input_queue[0], SP->input_queue[1],
			SP->input_queue[2]);
#endif
     	/* turn off non-blocking i/o as it causes write problems. */
	/* paw: QAR 5935*/
	if (win->_nodelay)
		_fixdelay(SP->fl_nodelay, FALSE);
	return inp;
 ...
 *      (At 15cps repeat rate, this is 67 ms.  We are using 500ms* to allow

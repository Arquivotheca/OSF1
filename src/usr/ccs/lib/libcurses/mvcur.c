...
static char rcsid[] = "@(#)$RCSfile: mvcur.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1993/06/12 22:00:00 $";
#endif
/*
 * HISTORY
 */
/*** "mvcur.c  1.7  com/lib/curses,3.1,9008 11/11/89 20:58:42"; ***/
/*
...
/* predefined these to remove warnings -- TEJ */
static _loc_right(), _loc_left(), _loc_up(), _loc_down();
 ...
 * FUNCTION:
 *
 *      Cursor motion optimization routine.
 *
 * EXECUTION ENVIRONMENT:
 *
 *      This routine takes as parameters
 *      the screen positions that the cursor is currently at, and the
 *      position you want it to be at, and it will move the cursor there
 *      very efficiently.  It isn't really optimal, since several
 *      approximations are taken in the interests of efficiency and
 *      simplicity.  The code here considers directly addressing the
 *      cursor, and also considers local motions using left, right, up,
 *      down, tabs, backtabs, vertical and horizontal addressing, and
 *      parameterized motions.  It does not consider using home down, or
 *      taking advantage of automatic margins on any of the four
 *      directions.  (Two of these directions, left and right,
...
int oldrow, oldcol, newrow, newcol;
{
	...
	if(outf) fprintf(outf,
		"mvcur(oldrow=%d, oldcol=%d, newrow=%d, newcol=%d)\n",
...
#ifdef DEBUG
if(outf) fprintf(outf,
"home_right %d,cur_right %d,cur_left %d,home_down %d,cur_down %d,cur_up %d\n",
...
#ifdef DEBUG
	if(outf) fprintf(outf,
		"cost_ca %d, cost_hdr %d, cost_cr %d, cost_lm %d\n",
...
#ifdef DEBUG
		if(outf) fprintf(outf,
		"chose carriage return + local motions\n");
#endif
		tputs(carriage_return, 1, _outch);
		if (newrow > oldrow)
...
#define  BLANK_ATTRS    A_STANDOUT | A_UNDERLINE | A_REVERSE

/*
 * NAME:        _loc_right, _loc_left, _loc_up, _loc_down
 *
 * FUNCTION:
 *
 ...
 * EXECUTION ENVIRONMENT:
 *
 *      They return the cost, in characters.  If the third
 *      argument is 1, they actually do the motion.  The row number is
 *      useful if we're going to actually do the motion - that way we can
 *      look in the screen image (if we have it) and just output the
 *      characters that are already on the screen - this usually saves 50%
 *      over cursor_right.
 */
...
_loc_right(oldcol, newcol, domotion, row)
{
...
#ifdef DEBUG
	if(outf) fprintf(outf,
		"_loc_right(%d, %d, %d), chars %d, ri %d, RI %d, ch %d\n",
...
#ifdef DEBUG
		if(outf) fprintf(outf,
			"chose column absolute cursor addressing\n");
#endif
		if (domotion)
...
#endif
		if (domotion)
			tputs(tparm(parm_right_cursor, newcol-oldcol),
								1, _outch);
		return c2;
	} else {
...
#ifdef DEBUG
		if(outf) fprintf(outf,
		"chose rights: ntabs %d, tabcol %d, nleft %d, nright %d\n",
	 ...
	 */
						...
						{
				wcode = rp->body[tabcol+i]&A_CHARTEXT;
#ifdef PHASE2
				watr = rp->atr_body[tabcol+i];
				if( !IS_NEXTATR( watr ) &&
				    (wlen = get_byteln( watr )) <= (nright-i) )
				{
					SP->curatr = &watr;
					_w_outch( wcode );
					i += (wlen - 1);
...
#else
				if( !IS_NEXTCHAR( wcode ) &&
				    (wlen = wcwidth( wcode )) <= (nright-i) )
				{
					_outch( wcode );
					i += (wlen - 1);
				}
...
#ifdef DEBUG
	if (outf && SP->cur_body[row+1]) fprintf(outf,
			"nd2, row %d col %d, char '%c'\n",
			row, i, SP->cur_body[row+1]->body[i]);
					...
					if (rp && rp->length > i)
#else
					if (cursor_right && (!notinsmode ||
						rp && SP->phys_gr !=
						(rp->body[i] & A_ATTRIBUTES)
						|| SP->phys_gr & BLANK_ATTRS))
							 /* dont know */
						tputs(cursor_right,1,_outch);
...
#ifdef WCHAR
					{
				wcode = rp->body[i]&A_CHARTEXT;
#ifdef PHASE2
				watr = rp->atr_body[i];
				if( !IS_NEXTATR( watr ) &&
				    (wlen = get_byteln( watr )) <= (newcol-i) )
				{
					SP->curatr = &watr;
					_w_outch( wcode );
					i += (wlen - 1);
	...
	if (cursor_left) {
		if (back_tab) {
			...
			if (tabcol < newcol) {
				...
				nright = newcol - tabcol;
				c1 = ntabs*_cost(Back_tab) +
					nright*_cost(Cursor_right);
			} else {
				/* some tabs plus some lefts */
				nleft = tabcol - newcol;
				c1 = ntabs*_cost(Back_tab) +
					nleft*_cost(Cursor_left);
			}
		} else {
...
#ifdef DEBUG
	if(outf) fprintf(outf,
		"_loc_left(%d, %d, %d), le %d, LE %d, ch %d\n",
...
#endif
		if (domotion)
			tputs(tparm(parm_left_cursor, oldcol-newcol),
								1, _outch);
		return c2;
	} else {
...
#ifdef DEBUG
		if(outf) fprintf(outf,
			"chose row absolute cursor addressing\n");
#endif
		if (domotion)
...
#endif
		if (domotion)
			tputs(tparm(parm_up_cursor, oldrow-newrow),
								1, _outch);
		return c2;
	} else {
...
#ifdef DEBUG
	if(outf) fprintf(outf,
		"_loc_down(%d, %d, %d, %d), do %d, DO %d, cv %d\n",
...
#endif
		if (domotion)
			tputs(tparm(parm_down_cursor, newrow-oldrow),
								1, _outch);
		return c2;
	} else {

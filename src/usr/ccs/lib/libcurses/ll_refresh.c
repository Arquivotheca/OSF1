...
static char rcsid[] = "@(#)$RCSfile: ll_refresh.c,v $ $Revision: 4.2.2.2 $ (DEC) $Date: 1993/06/12 21:48:13 $";
#endif
/*
 * HISTORY
 */
 ...
 *
 * FUNCTIONS:   _ll_refresh, _toss_cookies
 *
 *
 ...
 * 
 * ll_refresh.c  1.5  com/lib/curses,3.1,8943 10/16/89 23:24:26
 */
...
static _toss_cookies();		/* predefine to remove warnings -- TEJ */
 ...
 *      look like SP->std_body.
 *
 * EXECUTION ENVIRONMENT:
 *
 *      If use_idl is 1, this routine will call
 ...
 *      be used, and the insert/delete line features of the terminal will
 *      not be used.
 *
...
int	use_idl;
{
	...
	if(outf) fprintf(outf,
		"phys cursor at (%d,%d), want cursor at (%d,%d)\n",
...
#ifdef DEBUG
		if (outf) fprintf(outf,
		"InputPending %d, aborted ll_refresh at start\n",
		InputPending);
#endif	/* DEBUG */
		return 0;
	}
...
#ifdef DEBUG
	if (outf) fprintf(outf,
		"virt cursor at y=%d, x=%d, length %d\n",
		SP->virt_y, SP->virt_x, SP->std_body[SP->virt_y+1]->length);
#endif	/* DEBUG */
	if( SP->virt_y >= 0 &&
		SP->std_body[SP->virt_y+1]->length < SP->virt_x )
	{
...
#ifdef DEBUG
	if(outf) fprintf(outf,
	"at end, phys SP->curptr at (%d,%d), want SP->curptr at (%d,%d)\n",
		SP->phys_y, SP->phys_x, SP->virt_y, SP->virt_x);
#endif	/* DEBUG */
#ifdef notdef
	if(magic_cookie_glitch > 0)
 ...
 *      each character (7 for the character and 9 for attributes) but
 *      instead created some reserved "magic cookie" characters to tell
 *      the sca routine "you should change attributes now".  This would be
 *      fine except that these cookies take up a space in memory, and
 *      usually display as a blank.  This makes it impossible to display
 *      what the user really wanted, if he is using attributes for
 *      underlining, bold, etc.  Such terminals include the Teleray 1061,
 *      TVI 912 and 950, Teletype DataSpeed 40, and Adds Regent 40.
 *
 ...
 *      when attributes are changed.  However, it is unacceptable.
 *
 *      An approach is to simulate what the programmer (who wasn't thinking
 ...
 *      are changed on one line, this can result in losing several
 *      characters from the right of the line.)
 *

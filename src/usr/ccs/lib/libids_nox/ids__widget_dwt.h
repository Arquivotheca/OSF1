...
/* define DECWINDOWS if we want the DECWINDOWS function bindings to compile */
#define DECWINDOWS 
...
#define RenderIndex (DwtDwtCommonIndex + 1)
#define ImageIndex  (RenderIndex + 1)
#define PannedIndex (ImageIndex + 1)

/*
**  These macros are moved from ids$$macros.h which are DWT specific
*/
#define Callback_(callback)     (callback.ecallback != NULL)
...
#define IdsPartOffset_(index,part,variable) \
	index/**/Index, XtOffset(part/**/Part *,variable)
#else
#define HALFLONGWORD (sizeof(long) * 4)
#define IdsPartOffset_(index,part,variable) \
	(index/**/Index << HALFLONGWORD) + XtOffset(part/**/Part *,variable)
#define GetIdsPartOffset_(r,offset) \
	((r)->resource_offset & 0xFFFF) + \
         (*(offset))[ (r)_>resource_offset >> HALFLONGWORD];
#endif

    /*
    **  convenience macros for accessing DwtCOMMON part
    */
...
#define DefineWidgetParts_(c,w) c/**/Widget IdsBase_(w)=(c/**/Widget)w;\
	 Ids/**/c/**/Part IdsPart_(w)=(Ids/**/c/**/Part)(((char *) (w))+\
	    renderImageWidgetClassRec.render_image_class.offsets[RenderIndex])

#define DwtCommonPart_(w)   ((DwtCommonPart *)(((char *)(w))\
                            +renderImageWidgetClassRec.render_image_class\
                            .offsets[DwtDwtCommonIndex]))

#define ForePix_(w)     (DwtCommonPart_(w)->foreground_pixel)
#define BackGC_(w)      (DwtCommonPart_(w)->background_gc)
#define ForeGC_(w)      (DwtCommonPart_(w)->foreground_gc)
      
    /*
    **  Call RenderImageClass functions from subclasses.
    */
#define UpdateCallback_(o,ol,n,nl,name) \
        ((* renderImageWidgetClass->render_image_class.update_callback) \
                                            ((o),(ol),(n),(nl),(name)))
#define RenderImage_(w,e) \
        ((* renderImageWidgetClass->render_image_class.render_image)((w),(e)))

    /*
    **  Call ImageClass functions from subclasses.
    */
#define RedrawWindow_(w) \
        ((* imageWidgetClass->image_class.redraw_window)((w)))
#define RedrawRegion_(w,e) \
        ((* imageWidgetClass->image_class.redraw_region)((w),(e)))
#define ImageCallback_(w,b,r,e) \
        ((* imageWidgetClass->image_class.image_callback)((w),(b),(r),(e)))



typedef struct {		    /* Render Image Part of Class record    */
	DwtOffsetPtr	offsets;	    /* offsets to instance parts    */
...
typedef struct _RenderImageClassRec {	    /* Render Image Class record    */
	CoreClassPart		core_class;
	CompositeClassPart	composite_class;
	DwtCommonClassPart	dwtcommon_class;
	RenderImageClassPart	render_image_class;
} RenderImageClassRec, *RenderImageClass;
...
typedef struct {		/* Render Image Part of instance record	    */
	...
	DwtCallbackStruct	 xielist_callback;
	DwtCallbackStruct	 render_callback;
	DwtCallbackStruct	 work_notify_callback;
        DwtCallbackStruct        save_callback;
	DwtCallbackStruct	 error_callback;
...
typedef struct _imageClassRec {		    /* Image Class record	    */
	CoreClassPart		core_class;
	CompositeClassPart	composite_class;
	DwtCommonClassPart	dwtcommon_class;
	RenderImageClassPart	render_image_class;
	ImageClassPart		image_class;
} ImageClassRec, *ImageClass;
...
typedef struct _PannedImageClassRec {	    /* Panned Image Class record    */
	CoreClassPart		core_class;
	CompositeClassPart	composite_class;
	DwtCommonClassPart      dwtcommon_class;
	RenderImageClassPart	render_image_class;
...
} PannedImageClassRec, *PannedImageClass;
...
#ifndef RENDERIMAGE
external RenderImageClassRec	renderImageWidgetClassRec;
external RenderImageClass	renderImageWidgetClass;
#endif

#ifndef IMAGE
external ImageClassRec		imageWidgetClassRec;
external ImageClass		imageWidgetClass;
#endif

#ifndef PANNEDIMAGE
external PannedImageClassRec	pannedImageWidgetClassRec;
external PannedImageClass	pannedImageWidgetClass;
#endif

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: su_secure.c,v $ $Revision: 1.1.8.7 $ (DEC) $Date: 1993/08/24 22:33:05 $";
#endif 
/*
...
#include "sia_mech.h"
#define MSGSTR_SEC(n,s) GETMSG(MS_SIA_SU,n,s)
...
/* This global is here for siad_suauth to share. */
int su_crypt_alg;	/* encryption index to pass to dispcrypt */

#define	NO_WAY_TO_GET	"::"
#define OVERRIDE_USER   "root"
char *override_passwd = "OVERRIDE";
static char auditbuf[80];
static gid_t sysgroup = -1;
...
/*
 * Check if the passed-in pr_passwd is a "super-user".  This check
 * is based on SEC_SUCOMPAT, and on uid==0 if SEC_PRIV is not on.
 */
static int
is_suser(pr)
	register struct pr_passwd *pr;
{
	register priv_t *pvec;
	register int i;

#if !SEC_PRIV
	if (!pr->uflg.fg_uid || !pr->ufld.fd_uid)
		return 1;
#endif
	pvec = privvec(SEC_SUCOMPAT, -1);
	for (i = 0;  i < SEC_SPRIVVEC_SIZE;  i++)
		if (pvec[i])
			break;
	if (i >= SEC_SPRIVVEC_SIZE)
		return 0;	/* no such priv any more? */
	pvec += i;		/* make the matching easy on me */
	if (pr->uflg.fg_bprivs && (pr->ufld.fd_bprivs[i] & *pvec))
		return 1;
	if (pr->sflg.fg_bprivs && (pr->sfld.fd_bprivs[i] & *pvec))
		return 1;
	if (pr->uflg.fg_sprivs && (pr->ufld.fd_sprivs[i] & *pvec))
		return 1;
	if (pr->sflg.fg_sprivs && (pr->sfld.fd_sprivs[i] & *pvec))
		return 1;
	return 0;
}
	...
	struct pr_passwd **ppr;
{
...
#endif
				-1))) {
		show_error(MSGSTR_SEC(S_INSUFF_PRIV, "%s: insufficient privileges\n"), command_name);
		exit(1);
	}
	 ...
	 *
	 * We look up group "system", here, too, since getgrnam and getpw*
	 * use the same static buffer.
	 */
	desired_name = malloc(strlen(pwd->pw_name) + 1);
	if (desired_name == (char *) 0)  {
		show_error(MSGSTR_SEC(S_NO_MEM, "No memory to save %s\n"), pwd->pw_name);
		exit(1);
	}
	...
	if (uid >= 0)  {
		...
		if (pr == (struct pr_passwd *) 0) {
			audit_subsystem(
MSGSTR_SEC(S_GET_PROT, "get protected password entry of current user"),
MSGSTR_SEC(S_ABORT1, "user ID unknown, abort su"), 7 /* ET_OBJECT_UNAV */);
			show_error(MSGSTR_SEC(S_UNKNOWN_UID, "su: Your own ID is unknown.\n"));
			exit(1);
		}
		this_user = *pr;
	}
	else {
		show_error(MSGSTR_SEC(S_NO_FROM_RC, "su: may not be called from rc script\n"));
		exit(1);
	}
	...
	if (pr == (struct pr_passwd *) 0)  {
		audit_subsystem(
			...
			MSGSTR_SEC(S_ABORT2, "entry not found, abort su"), 7 /* ET_OBJECT_UNAV */);
	...
	if (pr->uflg.fg_oldcrypt)
		su_crypt_alg = pr->ufld.fd_oldcrypt;
	else if (pr->sflg.fg_oldcrypt)
		su_crypt_alg = pr->sfld.fd_oldcrypt;
	else
		su_crypt_alg = AUTH_CRYPT_BIGCRYPT;
	...
	if (!pr->uflg.fg_uid || !pr->uflg.fg_name ||
		...
		audit_subsystem(
			MSGSTR_SEC(S_VERIFY_NAME, "verify name, id fields for requested user match in protected password and /etc/password"), MSGSTR_SEC(S_ABORT3, "abort su"), 18 /* ET_SUBSYSTEM */);
		show_error(MSGSTR_SEC(S_UNKNOWN1, "su: Unknown id: %s\n"), desired_name);
		exit(1);
	}
...
#if !SEC_PRIV /*{*/
	/*
	 * Allow su-ing from root to anyone.
	 */
	else if (this_user.uflg.fg_uid && !this_user.ufld.fd_uid)
		pwd->pw_passwd = "";
#endif /*} !SEC_PRIV */
	 ...
	 * [Bozo alert on this one.  Should really check for SL & MAC
	 * domination, and ILB inclusion, and allow a password-validated
	 * access if the MAC requirements are satisfied.]
	 */
	else
		pwd->pw_passwd = NO_WAY_TO_GET;
#else /*}{ SEC_ENCODINGS */
	/*
	 * All other cases depend on password, except if target user is
	 * the super-user.
	 */
	else if (is_suser(pr)) {
	...
	register struct passwd *pwd;
{
	...
	is_proper =
	    (strcmp(pwd->pw_passwd,
		    dispcrypt(cleartext, pwd->pw_passwd, su_crypt_alg)) == 0);
	...
	if (access(log_name, 02) != 0)  {
		...
		if (cfs_status != CFS_GOOD_RETURN) {
			...
			audit_security_failure(OT_REGULAR, auditbuf,
				MSGSTR_SEC(S_ALLOWED, "su allowed to continue"), 7 /* ET_OBJECT_UNAV */);
			show_error(
	...
	char *log_name;
{
	...
	if (pr->uflg.fg_encrypt && pr->ufld.fd_encrypt[0] != '\0') {
		...
		if (expired)  {
			...
			audit_subsystem(
			    ...
			    11 /* ET_ACCESS_DENIAL */);
			fflush(stdout);
			show_error(MSGSTR_SEC(S_HAS_EXP, "Password has expired.\n"));
			show_error(MSGSTR_SEC(S_SET_NEW_PWD, "Setting a new password for it cannot be done from here.\n"));
			exit(2);
		}
	}
	...
	if (locked_out(pr))  {
		...
		audit_subsystem(MSGSTR_SEC(S_CHECK_ACC, "check for disabled account"), 
                  MSGSTR_SEC(S_ABORT4, "account disabled, abort su"), 11 /* ET_ACCESS_DENIAL */);
		show_error(
	...
	if (pr->uflg.fg_expdate) {
		if (pr->ufld.fd_expdate && pr->ufld.fd_expdate <= now) {
			pr->uflg.fg_retired = 1;
			pr->ufld.fd_retired = 1;
		}
	}
	else if (pr->sflg.fg_expdate) {
		if (pr->sfld.fd_expdate && pr->sfld.fd_expdate <= now) {
			pr->uflg.fg_retired = 1;
			pr->ufld.fd_retired = 1;
		}
	}
	...
	if (pr->uflg.fg_retired && pr->ufld.fd_retired) {
		audit_subsystem(MSGSTR_SEC(S_CHECK1, "check for retired account"), 
			MSGSTR_SEC(S_ABORT5, "account retired, abort su"), 11 /* ET_ACCESS_DENIAL */);
		show_error(
	...
	if (fake_login)  {
		...
		sprintf(succ_mesg, MSGSTR_SEC(S_LAST_SUCC, "Last   successful real login for %s: %s"),
		  pr->ufld.fd_name, ptime);
		if (pr->uflg.fg_suctty)
			sprintf(&succ_mesg[strlen(succ_mesg)], MSGSTR_SEC(S_ON, " on %s"), pr->ufld.fd_suctty);
		strncat(succ_mesg, "\n", sizeof succ_mesg);
		...
		sprintf(&succ_mesg[strlen(succ_mesg)], MSGSTR_SEC(S_LAST_UNSUCC, "Last unsuccessful real login for %s: %s"),
		pr->ufld.fd_name, ptime);
		if (pr->uflg.fg_unsuctty)
			sprintf(&succ_mesg[strlen(succ_mesg)], MSGSTR_SEC(S_ON, " on %s"), pr->ufld.fd_unsuctty);
		strncat(succ_mesg, "\n", sizeof succ_mesg);
		show_mesg(succ_mesg);
	}
}
	...
	uid_t luid;
{
	...
	if (setclrnce(clearance) == -1)  {
		...
		if ((clearance_ir == (mand_ir_t *) 0) ||
			...
			audit_subsystem(MSGSTR_SEC(S_GET_CLEAR, "get clearance for current user"), 
				MSGSTR_SEC(S_ABORT6, "cannot get clearance, abort su"),18 /* ET_SUBSYSTEM */);
			show_error(MSGSTR_SEC(S_CANT_GET_CL, "Cannot get your clearance.\n"));
			exit(1);
		}
		...
		if ((relation & (MAND_EQUAL | MAND_SDOM)) == 0) {
			...
			show_error(MSGSTR_SEC(S_CANT_SET1, "Cannot set user's clearance.\n"));
			...
			if (relation & MAND_ODOM)
  			...
  			MSGSTR_SEC(S_CURRENT1, "Current one and the new user's are incomparable.\n"));
				...
				show_error(MSGSTR_SEC(S_UNKNOWN2, "Unknown.\n"));
	...
	if (getpriv(SEC_MAXIMUM_PRIV, curr_sysprivs) < 0)  {
		audit_subsystem(MSGSTR_SEC(S_RETR_KERN, "retrieve kernel authorizations"), 
			MSGSTR_SEC(S_CANT_RETR, "cannot retrieve"), 18 /* ET_SUBSYSTEM */);
		show_error(MSGSTR_SEC(S_CANT_RETR_KERN, "Cannot retrieve kernel authorizations.\n"));
		exit(1);
	}
	...
	if (getpriv(SEC_BASE_PRIV, curr_baseprivs) < 0)  {
		audit_subsystem(MSGSTR_SEC(S_RETR_BASE, "retrieve base privileges"), 
			MSGSTR_SEC(S_CANT_RETR, "cannot retrieve"), 18 /* ET_SUBSYSTEM */);
		show_error(MSGSTR_SEC(S_CANT_RETR_BASE, "Cannot retrieve base privileges.\n"));
		exit(1);
	}
	...
	if (setbaseprivs(new_baseprivs) < 0)  {
		audit_subsystem(MSGSTR_SEC(S_SET_RESTR1, "set restricted base privileges in su"),
                        ...
                        18 /* ET_SUBSYSTEM */);
		show_error(MSGSTR_SEC(S_UNABLE2, "Unable to set account's base privileges.\n"));
		exit(1);
	}

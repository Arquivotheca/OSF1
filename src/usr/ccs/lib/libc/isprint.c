 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: isprint.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/05 21:02:17 $";
#endif 
/*
...
#pragma weak NCisprint = isprint

int 
isprint(int c)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
		return __iswctype_sb((wint_t)c, _ISPRINT, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
			( (wint_t)c, _ISPRINT, __lc_ctype);
}

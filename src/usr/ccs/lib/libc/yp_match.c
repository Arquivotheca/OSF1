 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: yp_match.c,v $ $Revision: 4.2.8.2 $ (DEC) $Date: 1993/06/08 00:28:23 $";
#endif
/*
 */
...
#pragma weak yp_match = __yp_match
#endif
#endif
#define NULL 0L
#include <sys/time.h>
	...
	int keylen, vallen;
{
	...
	bzero((char *) n, sizeof(*n));
	for (;;) {
		if ((n->domain = (char *) malloc((size_t) (1 + strlen(domain)))) == 0)
		    break;
		if ((n->map = (char *) malloc((size_t) (1 + strlen(map)))) == 0)
		    break;
		if ((n->key = (char *) malloc((size_t) keylen)) == 0)
		    break;
		if ((n->val = (char *) malloc((size_t) vallen)) == 0)
		    break;
		return (n);
	}
	freenode(n);
	...
	for (prev=0, cnt=0, c=head; c != 0; prev=c, c=c->next, cnt++) {
		if ((c->keylen == keylen) &&
			...
			if ((now.tv_sec - c->birth) > CACHETO) {
				...
				savesize = c->vallen + 2;
				*val = (char *) malloc((size_t) savesize);
				if (*val == 0) {
					return (YPERR_RESRC);
				}
	...
	int  *vallen;		/* return: bytes in val */
{
	...
	if (!retval && ((*val = (char *) malloc((size_t)

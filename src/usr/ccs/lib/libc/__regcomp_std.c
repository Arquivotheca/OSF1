 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: __regcomp_std.c,v $ $Revision: 1.1.5.9 $ (DEC) $Date: 1993/11/23 21:33:12 $";
#endif
/*
 ...
 * 
 * 1.11  com/lib/c/pat/__regcomp_std.c, , bos320, 9134320 8/13/91 14:49:14
 */
...
#define OPPOSITE(icase,pp,ucoll,wc,wc2)					\
			...
			if (icase != 0)					\
			  if (((wc2 = towupper(wc)) != wc) ||		\
			      ((wc2 = towlower(wc)) != wc))		\
			    {						\
			    _getcolval(phdl, &ucoll, wc2, "", MAX_NORDS); \
...
int
__regcomp_std(regex_t *preg, const char *pattern, int cflags, _LC_collate_t *phdl)
{
 ...
 */
		...
		cc_char:
			if (sblocale == 0)
				{
				wclen = mbtowc(&wc, (char *)ppat-1, mb_cur_max);
				if (wclen < 0)
					{
					preg->re_erroff = (uchar_t *)ppat - (uchar_t *)pattern - 1;
					RETURN (REG_ECHAR);
					}
				}
			if (icase == 0)
 ...
 */
		case '[':
			if ((strcmp(setlocale(LC_COLLATE,NULL),"C") == 0) || ((phdl->co_coltbl == NULL) && (MAX_PC - MIN_PC < BITMAP_LEN * NBBY))) 
				{
				...
				*pp++ = CC_BITMAP;
				...
				if (stat != 0)
					{
					preg->re_erroff = (uchar_t *)pto - (uchar_t *)pattern - 1;
					RETURN (stat);
					}
				ppat = pto;
				pto = pp;
				...
				*pp++ = CC_WBITMAP;
				if (isfirst++ == 0)
					...
					for (i = MIN_PC; i <= MAX_PC; i++)
						{
			    			_getcolval(phdl, &ucoll, i, "", MAX_NORDS);
						ucoll = byte_string_to_collation(ucoll);
						if (ucoll >= min_ucoll && ucoll <= max_ucoll)
							{
							delta = ucoll - min_ucoll;
							if ((*(pp + (delta >> 3)) &  __reg_bits[delta & 7]) != 0)
								if (sblocale != 0)
									pmap[i] = 1;
								else if (wctomb((char *)filecode, i) < 1)
									pmap[i & 0xff] = 1;
								else
									pmap[*filecode] = 1;
							}
 ...
 */
			case '{':
				...
				if (c2 == '\\' && *pri == '}')
					...
					{
					preg->re_erroff = (uchar_t *)pri - (uchar_t *)pattern - 1;
					RETURN (REG_BADBR);
					}
				else if (*pri == '\\' && pri[1] == '}')
					{
				...
				if (minri > maxri || maxri > RE_DUP_MAX || *pri == '*' || (*plastce & CR_MASK) != 0)
					{
					preg->re_erroff = (uchar_t *)ppat - (uchar_t *)pattern;
					RETURN (REG_BADBR);
					}
				maxri -= minri;
				ppat = pri;
 ...
 */
		case '|':
			if (ere == 0 || *ppat == ')' || *ppat == '\0' || ppat == (uchar_t *)pattern+1 ||
...
static int
bracketw(_LC_collate_t *phdl, uchar_t *ppat, uchar_t **pnext, uchar_t *pp, regex_t *preg, uchar_t *pmap)
	{
 ...
 *	d) collating symbol ([. .])
 */
	while ((wclen = mbtowc(&wc, (char *)pb, mb_cur_max)) > 0)
		{
		pb += wclen;
    		_getcolval(phdl, &min_ucoll, wc, "", MAX_NORDS);
		if (min_ucoll == 0)
			continue ;	/* Ignore the current character */
 ...
 */
		case ']':
			...
			if (neg != 0)
				...
				for (; pxor >= pp; pxor--)
					*pxor = ~*pxor;
    				_getcolval(phdl, &ucoll, '\0', "", MAX_NORDS);    
				ucoll = byte_string_to_collation(ucoll);
				if (ucoll >= MIN_UCOLL && ucoll <= MAX_UCOLL)
					CLEARBIT(pp, ucoll) 
				if ((preg->re_cflags & REG_NEWLINE) != 0)
					{
    					_getcolval(phdl, &ucoll, '\n', "", MAX_NORDS);   
					ucoll = byte_string_to_collation(ucoll);
					if (ucoll >= MIN_UCOLL && ucoll <= MAX_UCOLL)
						CLEARBIT(pp, ucoll)
					}
...
static int
intl_expr(_LC_collate_t *phdl, regex_t *preg, uchar_t type, uchar_t *pexpr,
	uchar_t *pp, wchar_t *pmin, wchar_t *pmax)
	{
	...
	wctype_t wct;		/* character class handle for iswctype	*/
	uchar_t	lcexpr[NL_NMAX*MB_LEN_MAX]; /* lowercase [. .]		*/
	wchar_t t_pcoll;	/* temporary collating weight value	*/
 ...
 */
	case '=':
		wclen = mbtowc(&wc, (char *)pexpr, MB_CUR_MAX);
		if (wclen < 0)
			return (REG_ECHAR);
		if (pexpr[wclen] != '\0')
			goto co_symbol;
    		_getcolval(phdl, &pcoll, wc, "", 0);
		pcoll = byte_string_to_collation(pcoll);
		if (pcoll < MIN_UCOLL || pcoll > MAX_UCOLL)
			return (REG_ECOLLATE);
    		_getcolval(phdl, pmax, wc, "", MAX_NORDS);     
		*pmax = byte_string_to_collation(*pmax);
		*pmin = *pmax;
		for (i = 1; i <= MAX_PC; i++)
		{

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: sia_c_pass.c,v $ $Revision: 1.1.15.5 $ (DEC) $Date: 1993/12/16 23:55:22 $";
#endif
/*****************************************************************************
* Usage:  int * sia_chg_password( (*sia_collect),username,argc,argv)
*
* Description: The purpose of this routine is to implement a password changing
...
* the user will be given a choice of changing the various passwords. 
...
#pragma weak sia_chg_password = __sia_chg_password
#endif
#endif
#ifdef uname
#undef uname
...
int
  sia_chg_password (int(*collect)(), char *username, int argc, char *argv[])
{
...
/*****************************************************************************************/
	...
	siauid=getuid();
        ...
        while(pkgind < SIASWMAX && sia_matrix[SIA_CHG_PASSWORD][pkgind].pkgnam != NULL)	/* first check if user is registered */
                {
		...
		if(pfp != NULL)
			{
			if ((siaderr = (*pfp)(username,CHGPASSWD)) == SIADSUCCESS)
				{
				/* save package index mapping */
				/* and creat the prompt string */
				sia_chg_password_array[collindex]=pkgind;
				sprintf(promptstrs[collindex],"%s", sia_matrix[SIA_CHG_PASSWORD][pkgind].pkgnam);
				collindex++;
				}
			}
		pkgind++;
		}
	if(collindex == 0)	/* no change password configuration */
		{
/* Changed to report error to user. DAL001
		SIALOG(MSGSTR(SIA_MSG_LOGERROR,"ERROR"),MSGSTR(SIA_MSG_NOCHGPASSWD,"No selection configured for changing password "));
*/
	...
	if(collindex > 1)      /* more than one option */
		...
		siacollerr=(*collect)(timeout,rendition,menutitle,num_prompts,prompts);
		if(siacollerr == SIACOLSUCCESS)
			{
			pkgind=-1;
			for(i=0;i<=collindex;i++)
				{
				if(prompts[i].result != NULL)
					{
					pkgind=sia_chg_password_array[i];
					}
				}
			if(pkgind == -1)
				{
	...
	else	pkgind=sia_chg_password_array[0];
	if(sia_matrix[SIA_CHG_PASSWORD][pkgind].fp == NULL)
		{ /* load was successful already for SIA_CHK_USER */
		if (sia_setupfp(SIA_CHG_PASSWORD, pkgind) == SIAFAIL)
			{
			SIATHREADREL(SIA_CHANGE_LOCK);
			return SIAFAIL;
			}
		}
	siaderr = (*sia_matrix[SIA_CHG_PASSWORD][pkgind].fp)((collect),username,argc,argv);
        if (!(siaderr & SIADSUCCESS))
                {
		(void) sia_audit(AUTH_EVENT, T_CHARP, MSGSTR(SIA_MSG_CHGPASSWD,"change password"),
					T_RESULT, -1, T_ERRNO, EACCES,
					T_LOGIN, username, 0);
                SIALOG(MSGSTR(SIA_MSG_LOGERROR,"ERROR"),MSGSTR(SIA_MSG_CHGPASSERR,"Failure to change password for %s"),username);
                SIATHREADREL(SIA_CHANGE_LOCK)
                return(SIAFAIL);
                }
        else    {
		(void) sia_audit(AUTH_EVENT, T_CHARP, MSGSTR(SIA_MSG_CHGPASSWD,"change password"),
					T_RESULT, 0,
					T_LOGIN, username, 0);
                SIALOG(MSGSTR(SIA_MSG_LOGEVENT,"EVENT"),MSGSTR(SIA_MSG_CHGPASSGOOD,"Successful password change for %s"),username);
                SIATHREADREL(SIA_CHANGE_LOCK)
                return(SIASUCCESS);
                }
}

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: siad_pw_adm.c,v $ $Revision: 1.1.13.3 $ (DEC) $Date: 1993/06/08 01:20:38 $";
#endif
/*
...
#pragma weak getpw = __getpw
#pragma weak putpw = __putpw
#pragma weak putpwent = __putpwent
#endif
#endif
#include <stdio.h>
#include <sys/stat.h>
...
int
putpwent(struct passwd *p, FILE *f)
{
...
static int
err_cleanup (struct passwd   *newp)
{
	if (pwfd != FAIL)
		close (pwfd);
	if (tmppwf != NULL)
...
static int
putpw_audit (struct passwd *p, int result)
{

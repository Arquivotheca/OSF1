 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: iswalnum.c,v $ $Revision: 1.1.6.2 $ (DEC) $Date: 1993/06/07 23:17:28 $";
#endif
/*
...
#pragma weak iswalnum = __iswalnum
#endif
#endif
...
iswalnum(wint_t pc)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
	 	return __iswctype_sb(pc, _ISALNUM, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
			( pc, _ISALNUM,__lc_ctype);
}

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: iscntrl.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/05 21:02:05 $";
#endif 
/*
...
#pragma weak NCiscntrl = iscntrl

int 
iscntrl(int c)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
		return __iswctype_sb((wint_t)c, _ISCNTRL, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
			((wint_t)c, _ISCNTRL, __lc_ctype);
}

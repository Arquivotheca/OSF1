 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: yp_bind.c,v $ $Revision: 4.3.9.2 $ (DEC) $Date: 1993/06/08 00:20:57 $";
#endif
/*
 */
...
#pragma weak usingypmap = __usingypmap
#pragma weak yp_bind = __yp_bind
#pragma weak yp_get_default_domain = __yp_get_default_domain
#pragma weak yp_ismapthere = __yp_ismapthere
#pragma weak yp_softbind = __yp_softbind
#pragma weak yp_unbind = __yp_unbind
#endif
#endif
#ifdef _NAME_SPACE_WEAK_STRONG
#ifdef stat
...
#else
#define	YPTIMEOUT 10			/* Total seconds for timeout */
 ...
 */
static unsigned int binder_port;	/* Initialize to "no port" */
	...
	int usertries;
{
	...
	if (!bfinvalid) {
		/* the sprintf below has been replaced by several */
		/* strcat calls to avoid having more of the stdio */
		/* library dragged in.  This keeps the size down  */
		/* of things like csh which */
		/* use their own printf routines.  CAR 7/5/88 */
		/* sprintf(bfname,"%s/%s.%d",CACHE_DIR, */
		/* 			domain,YPVERS); */

		char prbuf[11];
		register char *cp, *bfp;
		unsigned int n = YPVERS;
...
newborn()
{
	static pid_t mypid = 0;		/* Cached to detect forks */
	pid_t testpid;
	...
	int *err;
{
	...
	clnt_stat = (enum clnt_stat) clnt_call(client, PMAPPROC_GETPORT,
	    xdr_pmap, &portmap, xdr_u_int, &binder_port, bind_timeout);
	clnt_destroy(client);
	(void) close(sokt);
	...
	int *err;
{
	...
	if ((bf != -1) && !bfinvalid) {
		...
		if (bf != -1) {
			if (flock(bf,LOCK_EX+LOCK_NB) != 0) {
				(void) lseek(bf,(long)(sizeof(u_short)),SEEK_SET);

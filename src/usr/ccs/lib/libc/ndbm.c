 ...
 */
/*
 * HISTORY bacj out submit  4.2.8.4 and 4.2.8.3, this causes the
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: ndbm.c,v $ $Revision: 4.2.8.5 $ (DEC) $Date: 1993/12/03 00:04:03 $";
#endif
/*
 */
...
#include "pollution.h"
#define dbm_firsthash __dbm_firsthash
#define dbm_flush __dbm_flush
#define dbm_flushdir __dbm_flushdir
#define dbm_flushpag __dbm_flushpag
#pragma weak dbm_close = __dbm_close
#pragma weak dbm_close_status = __dbm_close_status
#pragma weak dbm_delete = __dbm_delete
#pragma weak dbm_do_nextkey = __dbm_do_nextkey
#pragma weak dbm_fetch = __dbm_fetch
#pragma weak dbm_firsthash = __dbm_firsthash
#pragma weak dbm_firstkey = __dbm_firstkey
#pragma weak dbm_flush = __dbm_flush
#pragma weak dbm_flushdir = __dbm_flushdir
#pragma weak dbm_flushpag = __dbm_flushpag
#pragma weak dbm_forder = __dbm_forder
#pragma weak dbm_nextkey = __dbm_nextkey
#pragma weak dbm_open = __dbm_open
#pragma weak dbm_setdefwrite = __dbm_setdefwrite
#pragma weak dbm_store = __dbm_store
#endif
	...
	DBM *db;
{
	...
	if (dbm_dirty(db)){ /*must page out the page*/
		(void) lseek(db->dbm_pagf, (long)(db->dbm_pagbno*PBLKSIZ), SEEK_SET);
	...
	DBM *db;
{
	...
	(void) lseek(db->dbm_dirf, (long)(db->dbm_dirbno*DBLKSIZ), SEEK_SET);
...
#ifdef	_THREAD_SAFE
	if (_rec_mutex_alloc((rec_mutex_t *)&db->dbm_lock) < 0) {
		_Seterrno(ENOMEM);
		goto bad1;
	}
#endif	/* _THREAD_SAFE */
	...
	datum key;
{
	...
	(void) lseek(db->dbm_pagf, (long)(db->dbm_blkno*PBLKSIZ), SEEK_SET);
	if (write(db->dbm_pagf, db->dbm_pagbuf, PBLKSIZ) != PBLKSIZ) {
	err:
		db->dbm_flags |= _DBM_IOERR;
		TS_UNLOCK(db->dbm_lock);
...
split:
	...
	(void) lseek(db->dbm_pagf, 
		(long)((db->dbm_blkno+db->dbm_hmask+1)*PBLKSIZ), SEEK_SET);
...
static const datum nullkey= {NULL, 0};
	...
	statb.st_size /= PBLKSIZ;
       ...
       for (;;) {
		if (db->dbm_blkptr != db->dbm_pagbno) {
			...
			(void) lseek(db->dbm_pagf, (long)(db->dbm_blkptr*PBLKSIZ), SEEK_SET);
	...
	long hash;
{
	...
	if (my_blkno != db->dbm_pagbno) {
		...
		(void) lseek(db->dbm_pagf, (long)(my_blkno*PBLKSIZ), SEEK_SET);
		if (read(db->dbm_pagf, db->dbm_pagbuf, PBLKSIZ) != PBLKSIZ)
			bzero(db->dbm_pagbuf, PBLKSIZ);
#ifdef DEBUG
		else if (chkblk(db->dbm_pagbuf) < 0)

 ...
 */
#ifndef lint
static char     *sccsid = "@(#)$RCSfile: rpc_prot.c,v $ $Revision: 4.2.7.2 $ (DEC) $Date: 1993/06/07 22:28:26 $";
#endif
/*
 */
...
#pragma weak xdr_accepted_reply = __xdr_accepted_reply
#pragma weak xdr_callhdr = __xdr_callhdr
#pragma weak xdr_des_block = __xdr_des_block
#pragma weak xdr_opaque_auth = __xdr_opaque_auth
#pragma weak xdr_rejected_reply = __xdr_rejected_reply
#pragma weak xdr_replymsg = __xdr_replymsg
#endif
#endif
#ifdef KERNEL
#include "param.h"
	...
	register struct accepted_reply *ar;
{
	...
	case PROG_MISMATCH:
		if (! xdr_u_int(xdrs, &(ar->ar_vers.low)))
			return (FALSE);
		return (xdr_u_int(xdrs, &(ar->ar_vers.high)));
	}
	...
	register struct rejected_reply *rr;
{
	...
	case RPC_MISMATCH:
		if (! xdr_u_int(xdrs, &(rr->rj_vers.low)))
			return (FALSE);
		return (xdr_u_int(xdrs, &(rr->rj_vers.high)));
	...
	register struct rpc_msg *cmsg;
{
	...
	if (
	    ...
	    return (xdr_u_int(xdrs, &(cmsg->rm_call.cb_vers)));

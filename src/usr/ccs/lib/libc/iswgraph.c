 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: iswgraph.c,v $ $Revision: 1.1.6.2 $ (DEC) $Date: 1993/06/07 23:18:17 $";
#endif
/*
...
#pragma weak iswgraph = __iswgraph
#endif
#endif
...
iswgraph(wint_t pc)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
		return __iswctype_sb(pc, _ISGRAPH, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
			( pc, _ISGRAPH, __lc_ctype);
}

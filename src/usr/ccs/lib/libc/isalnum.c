 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: isalnum.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/05 21:01:57 $";
#endif 
/*
...
#pragma weak NCisalnum = isalnum

int 
isalnum(int c)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
		return __iswctype_sb((wint_t)c, _ISALNUM, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
				((wint_t)c, _ISALNUM, __lc_ctype);
}

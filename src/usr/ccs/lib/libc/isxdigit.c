 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: isxdigit.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/05 21:02:32 $";
#endif 
/*
...
#pragma weak NCisxdigit = isxdigit

int 
isxdigit(int c)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
		return __iswctype_sb((wint_t)c, _ISXDIGIT, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
			( (wint_t)c, _ISXDIGIT, __lc_ctype);
}

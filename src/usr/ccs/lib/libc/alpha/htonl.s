 ...
 *
 *	a0	32-bit integer in host-byte order (hostlong)
 *
 * Convert a 32-bit integer from host-byte order (little endian) to Internet
 * network-byte order (big endian).
 *
 * The return value contains a 32-bit value in Internet network-byte order.
 *
 * NOTE: This routine and ntohl are identical. If you make a change here,
 * please make it in ntohl as well.
 */
	.align	4
LEAF(htonl)
	.prologue 0

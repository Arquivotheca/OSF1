 ...
 */
/*
 * @(#)$RCSfile: strcpy.s,v $ $Revision: 1.1.6.4 $ (DEC) $Date: 1993/11/18 15:39:09 $
 */
#include <alpha/asm.h>
 ...
 *
 * Copy the string pointed to by the "s2" parameter, including the
 * terminating null character, to the location pointed to by the
 * "s1" parameter. When operating on overlapping strings, the
 * behavior is unreliable.
 *
 * The return value contains a pointer to the destination string (s1).
 */
	.align	4
WEAK_LEAF(strcpy, NLstrcpy)
	.prologue 0
...
10:	extql	t0, a1, t8		# get low bytes of src
	...
	#	t1	bit mask of null bytes within t8

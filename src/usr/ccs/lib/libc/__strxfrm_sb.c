 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: __strxfrm_sb.c,v $ $Revision: 1.1.5.3 $ (DEC) $Date: 1993/06/08 01:22:17 $";
#endif
/*
 ...
 * 
 * 1.6  com/lib/c/str/__strxfrm_sb.c, libcstr, bos320, 9140320 9/24/91 15:46:06
 */
...
static size_t forward_xfrm_sb(_LC_collate_t *, const char *,
			char *, int , int , int );

static size_t forw_pos_xfrm_sb(_LC_collate_t *, const char *,
			char *, int , int , int );
...
size_t 
__strxfrm_sb( char *str_out, const char *str_in, size_t n, _LC_collate_t *hdl)
{
	...
	**********/
	if (sort_mod == 0) {
	    ...
	    ***********/
	    count = forward_xfrm_sb(hdl, str_in_ptr, str_out, count,
				     n, cur_order);
	}
	...
	****************/
	    ...
	    if (sort_mod & _COLL_POSITION_MASK) {
	    	count = forw_pos_xfrm_sb(hdl, str_in_ptr, str_out, count,
				     n, cur_order);

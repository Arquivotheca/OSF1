 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: isdigit.c,v $ $Revision: 4.2.5.3 $ (DEC) $Date: 1993/10/05 21:02:08 $";
#endif 
/*
...
#pragma weak NCisdigit = isdigit

int 
isdigit(int c)
{
	if (METHOD(__lc_ctype, iswctype) == NULL)
		return __iswctype_sb((wint_t)c, _ISDIGIT, __lc_ctype);
	else
		return METHOD(__lc_ctype, iswctype)
			((wint_t)c, _ISDIGIT, __lc_ctype);
}

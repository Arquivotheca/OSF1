...
**	ImgScaleFrame command.  It will scale an image.
**
**	define:	$ scale := $sys$disk:[]img_scale_motif_c.exe
**	usage: 	$ scale [-h] [-v] [-r] [-d] [-n] input_image_file_name [ouput_image_file_name]
**
**  ENVIRONMENT:
**
...
#define INPUT_IMG_NAME	"Scale Input Image"	/* input image name */
#define OUTPUT_IMG_NAME	"Scale Output Image"	/* output image name */
#define IMG_CLASS_NAME	"Scale Image"		/* image class name */
...
**		-h - save horizontal detail
**		-v - save vertical detail
**		-r - reverse preference
**		-d - disable preference
...
char *argv[];
{
    ...
    float scale_x = 0;			/* x scale factor */
    float scale_y = 0;			/* y scale factor */
    unsigned long scale_flags = 0;	/* ISL scale flags */
    ...
    */
    ...
    for (i = 1; i < argc; i++)
    {
	if (strcmp (argv[i], "-h") == 0)	/* save horizontal detail */
	    scale_flags |= ImgM_SaveHorizontal;
	else if (strcmp (argv[i], "-v") == 0)	/* save vertical detail */
	    scale_flags |= ImgM_SaveVertical;
	else if (strcmp (argv[i], "-r") == 0)	/* reverse preference */
	    scale_flags |= ImgM_ReversePreference;
	else if (strcmp (argv[i], "-d") == 0)	/* disable preference */
	    scale_flags |= ImgM_DisablePreference;
	else if (strcmp (argv[i], "-n") == 0)	/* nearest neighbor */
	    scale_flags |= ImgM_NearestNeighbor;
	else if (strcmp (argv[i], "-x") == 0)	/* x scale factor */
	    scale_x = (float) atof (argv[++i]);
	else if (strcmp (argv[i], "-y") == 0)	/* y scale factor */
	    scale_y = (float) atof (argv[++i]);
    ...
    /*
    ** It's an error not to have an image file or a scale factor.
    */

    if (input_filename == NULL || scale_x == 0)
	error_flag = TRUE;
    ...
    */
    ...
    {
	printf (
	    "usage:  scale -x scale_factor [-y scale_factor] [-h] [-v] [-r] \
[-d] [-n] input_image_filename [output_image_filename]\n", 
	    argv[0]);
	exit (1);
    }
    ...
    */
    ...
    output_fid = ImgScaleFrame (
			input_fid,		/* frame identifier to scale */
			&scale_x, &scale_y,	/* x and y scale factors */
			scale_flags);		/* flags */
    /*
...
{
    printf("scale -x scale_factor [-y scale_factor] [-h] [-v] [-r] [-d] [-n] \

...
**	ImgFlipFrame command.  It will flip an image vertically or
**	horizontally.
**
**	define:	$ flip := $sys$disk:[]img_flip_motif_c.exe
**	usage: 	$ flip [-h] [-v] input_image_file_name [ouput_image_file_name]
**
**  ENVIRONMENT:
**
...
#define INPUT_IMG_NAME	"Flip Input Image"	/* input image name */
#define OUTPUT_IMG_NAME	"Flip Output Image"	/* output image name */
#define FLIP_CLASS_NAME	"Flip Image"		/* flip image class name */
...
**	o Fliping the input image for output
...
**		-h - flip the image horizontally (default)
...
char *argv[];
{
    ...
    unsigned long flip_flags = 0;	/* ISL flip flags */
    ...
    */
    ...
    for (i = 1; i < argc; i++)
    {
	if (strcmp (argv[i], "-h") == 0)	/* horizontal flip */
	    flip_flags = ImgM_FlipHorizontal;
	else if (strcmp (argv[i], "-v") == 0)	/* vertical flip */
	    flip_flags = ImgM_FlipVertical;
    ...
    /*
    ** It's an error not to have an image file.
    */
...
input_image_filename [output_image_filename]\n", 
    ...
    */
    ...
    output_fid = ImgFlipFrame (
			input_fid,		/* frame identifier to flip */
			(unsigned long)0,	/* region of interest */
			flip_flags);		/* flags */
    /*
    ...
    */
    ...
    output_widget = XtAppCreateShell (
		    OUTPUT_IMG_NAME, FLIP_CLASS_NAME, topLevelShellWidgetClass,
...
{
    printf("flip [-h] [-v] input-image-filename [output-image-filename]\n");

 ...
 * OSF/1 Release 1.0
 */
/* @(#)parser.y	1.6  com/cmd/lang/lex,3.1,9021 9/7/89 18:37:12 */
/*
...
defns:	defns STR STR
	={	scopy($2,dp);
		...
		subs[dptr++] = dp;
		if(dptr >= DEFSIZE)
			error(MSGSTR(MANYDEFS, "Too many definitions"));
		dp += slength($3) + 1;
		if(dp >= dchar+DEFCHAR)
			error(MSGSTR(LONGDEFS, "Definitions too long"));
...
yylex(){
	...
	static unsigned char token[TOKENSIZE];
	static int iter;
	int first_char;
	int prev_prev;
				...
				case '%':
					...
					case '%':
						...
						name = (int *)myalloc(treesize,
							sizeof(*name));
						left = (long *)myalloc(treesize,
							sizeof(*left));
						right=(long *)myalloc(treesize,
							sizeof(*right));
						nullstr = (unsigned char *)
							myalloc(treesize,
							sizeof(*nullstr));
...
# endif
		...
		while(!eof){
			...
			case '\t':
				...
				if(peek == ' ' || peek == '\t' ||
					sectbegin == TRUE){
					warning(MSGSTR(EXSTATES, "Executable statements should occur right after %%"));
					continue;
					}
				x = NEWE;
				break;
			...
			case '{':	/* either iteration or definition */
				...
				else  		/* definition */
				{
				    ...
				    i = lookup(token,def);
				    if(i < 0)
					warning(MSGSTR(NODEF,
					  "Definition %s not found"),token);
				    else
				    {
					munput('s',subs[i]);
			...
			case '<':		/* start condition ? */
				...
				do {
					...
					if(i < 0) {
						warning(MSGSTR(NOSTART, "Undefined start condition %s"),token);
						continue;
						}
					*slptr++ = i+1;
					} while(c && c != '>');
			...
			case '[':
				...
				while(c != ']' && c){
					...
					/* range specified */
						...
						if(!(('A' <= j && k <= 'Z') ||
						     ('a' <= j && k <= 'z') ||
						     ('0' <= j && k <= '9')))
							warning(MSGSTR(NOPORTCHAR, "Non-portable Character Class"));
						for(n=j+1;n<=k;n++)
							symbol[n] = 1;	
						        /* implementation
							 * dependent */
				...
				if(p < ccptr)	/* found it */
					...
					if(ccptr >= ccl+CCLSIZE)
						error(MSGSTR(MANYCLASSES, "Too many large character classes"));
					}
				cclinter(x==CCL);
				break;
			case '\\':

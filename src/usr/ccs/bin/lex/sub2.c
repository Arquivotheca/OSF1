 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: sub2.c,v $ $Revision: 4.2.3.2 $ (DEC) $Date: 1992/04/03 18:38:30 $";
#endif 
/*
 */
/*
 ...
 *
 * FUNCTIONS: acompute, add, bprint, cfoll, cgoto, first,
		follow, layout, member, mkmatch, nextstate,
		notin, packtrans, padd, pccl, pfoll, pstate,
		rprint, shiftr, stprt, upone
 *
 * ORIGINS: 27
 *
 ...
 *
 * sub2.c	1.6  com/cmd/lang/lex,3.1,9021 12/14/89 17:04:28"; 
 */
...
first(v)	/* calculate set of positions with v as
		 * root which can be active initially */
  int v; {
	register int i;
	...
	if(i < NCH)i = 1;
	switch(i){
		case 1: case RCCL: case RNCCL: case RNULLS: case FINAL:
...
# endif
		...
		for(i = 1; i<NCH; i++){
			if(symbol[i]){
				...
				xstate = notin(stnum);
				if(xstate == -2)
					warning(MSGSTR(BADSTATE,
						"bad state  %d %o"),s,i);
				else if(xstate == -1){
					if(stnum >= nstates)
						error(MSGSTR(MANYSTATES,
						"Too many states %s"),
						(nstates == NSTATES ?
						MSGSTR(MANYSTATES2,
						"\nTry using %%n num"):""));
...
nextstate(s,c)
  int s,c; {
	...
	register unsigned char *temp, *tz;
...
# endif
	...
	for(i=0; i<st; i++){	/* get most similar state */
				 ...
				 * and state which is compressed by char if
				 * ours is not to be */
		...
		while(ach[j] && p < upper){
			...
			if(ast[j] != nexts[++p] || ast[j] == -1 ||
			   (cpackflg[st] && ach[j] != match[ach[j]]))diff++;
			j++;
			}
		while(ach[j]){
			diff++;
	...
	for(i=0;i<cnt;i++){
		if(name[*p] == FINAL)temp[k++] = left[*p];
			...
			if (left[*p] >nactions) {
				...
				nactions += NACTIONS;
				extra = (unsigned char *)
				    realloc(extra, nactions * sizeof(char));
				if (!extra)
			  		error(MSGSTR(ERTEXTS,
						"Too many right contexts"));
				memset(extra+save_size, NULL, NACTIONS * sizeof(char));
			}
			extra[left[*p]] = 1;
		}
...
# endif
	...
	if(optim){
		...
		if (chset==0) /* no chset, put out in normal order */
			...
			if (fbarr==0)
				error(MSGSTR(EREVERSE,
					"No space for char table reverse"),0);
			for(i=0; i<ZCH; i++)
				fbarr[i]=0;
			for(i=0; i<NCH; i++)

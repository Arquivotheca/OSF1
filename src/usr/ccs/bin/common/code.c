 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: code.c,v $ $Revision: 4.2.4.8 $ (DEC) $Date: 1993/11/22 21:21:06 $";
#endif 
/*
 */
/*
...
char *tempstrbuf; /* a temporary buffer for string collection */
char *saved_str_ptr; 
int lintXdlr;   /* if 1 then -Xdollar appeared on the CC command line */
#endif
...
bfcode( a, n ) int a[]; {
	...
	if( p->stype->ftn_parm != PNIL && !(p->sflags & SISOLDSTYLE)){
		if( lintvarg >= 0 )
	...
	while(id = ScanStab(id)) {
	  p = &stab[id];
		if( TOPTYPE(p->stype) == TNULL ) {
		  ...
		  cerror(TOOLSTR(M_MSG_241, "EJOBCODE:check error: %s"), 
				 stab[id].psname );
		}
		if( ( lineno = p->suse ) < 0 )
			lineno = - p->suse;
		...
		case STATIC:
			...
			WARNING( WUSAGE && WKNR, MESSAGE(7), p->psname);
		   }
...
#ifdef LINT
			WARNING( WUSAGE && vflag && !lintargu && !lintlib && WKNR,
...
#ifdef LINT
			WARNING( WUSAGE && vflag, MESSAGE(6), p->psname,
				stab[curftn].psname );
#else
...
char *bycode( t, i ){
	...
	if ( i >= strbufmax)
	   cerror("String constant too long; increase default option: -Wf,-XNl%d", strbufmax);
	if( t < 0 ){ /* end of the string */
	  /* NOTE !!!! for lint bycode is ALWAYS called so that the string is NULL 
	   * terminated. It is called ONLY from lxstr(). 
	   */
	    register int n = i;		 /* length of string */ /* index into the string buffer */
	    register char *sp;
	    register int offset = 0;
	    sp = (char *) getmem(i);
	    memcpy(sp,tempstrbuf,i);
	    return (sp);
...
#else 
bycode( t, i ){
#if	!defined (CXREF) && !defined (CFLOW)
	/* put byte i+1 in a string */
...
#endif
#ifdef LINT
	tempstrbuf = (char *) getmem(strbufmax); /* init the string buffer */
	lintXdlr = 0; 
#endif
#ifdef MSG
	catd = NLcatopen(MF_CTOOLS, NL_CAT_LOCALE);
#endif
	...
	for( i=1; i<argc; ++i ){
		...
		while( *++cp ){
		    ...
		    case 'X': /* flags local to pass 1 - historical since */
			     ...
			     **    To allow processing identifiers with $
			     **    we support -Xdollar passed to passes 1 & 2 !!!
			     **
			      */
			    while( *++cp ){
				switch( *cp ){
...
#if	defined (LINT) || defined (CFLOW)
			/* Initialize these in the case where 
			 * there are no # lineno "title" statements
			 */
			if (ifname = strrchr(argv[i], '/'))
			  ifname++;
			else
			  ifname = argv[i];
			if (!pfname)

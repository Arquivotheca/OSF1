 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: y3.c,v $ $Revision: 4.2.6.2 $ (DEC) $Date: 1993/08/02 19:43:42 $";
#endif
/*
...
output(){ /* print the output for the states */
	...
	fprintf( ftable, "static yytabelem yyexca[] ={\n" );
...
apack(p, n ) int *p;{ /* pack state i from temp1 into amem */
	...
	for( rr=amem; rr<=r; ++rr,++off ){  /* try rr */
		...
		for( qq=rr,pp=p; pp<=q; ++pp,++qq ){
			if( *pp ){
				if( qq > r ) error( MSGSTR(ACTOVR,
					"action table overflow") ); /*MSG*/
				if( qq>memp ) memp = qq;
				*qq = *pp;
				}
			}
	...
	error(MSGSTR(ACTOVR2, "no space in action table") ); /*MSG*/
	/* NOTREACHED */
	}
...
wract(i){ /* output state i */
	 ...
	 * be the error action */
	/* This previously referenced temp1[1].  Bug was reported in YACC
	 * in comp.compilers news group.
	 */
...
void
warray( const char *s, const int *v, int n ) {

	register i;

	fprintf( ftable, "static yytabelem %s[]={\n", s );
	for( i=0; i<n; ){

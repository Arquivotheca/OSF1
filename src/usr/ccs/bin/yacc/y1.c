 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: y1.c,v $ $Revision: 4.2.6.3 $ (DEC) $Date: 1993/10/07 19:16:28 $";
#endif
/*
 ...
 *
 *
 * (C) COPYRIGHT International Business Machines Corp. 1985, 1989 All Rights
 * Reserved 
 *
...
#define round(a,b)	((((unsigned long)(a)+(b))-1)&~(unsigned long)((b)-1))
...
struct item    *pstate[NSTATES + 2];    /* pointers to the descriptions of
                                         * the states */
int             tystate[NSTATES];       /* contains type information about
                                         * the states */
int             indgo[NSTATES]; /* index to the stored goto table */
int             tstates[NTERMS];/* states generated by terminal gotos */
int             ntstates[NNONTERM];     /* states generated by nonterminal
                                         * gotos */
...
int             temp1[TEMPSIZE];/* temporary storage, indexed by terms +
                                 * ntokens or states */
...
struct looksets *pfirst[NNONTERM + 2];  /* vector of pointers to first sets
                                         * for each nonterminal */
int             pempty[NNONTERM + 1];   /* vector of nonterminals
...
#endif
        lkst = (struct looksets *) calloc(LSETBLOCK, sizeof(struct looksets));
        if (!lkst)
...
{                               /* sorts last state,and sees if it equals
        ...
        pstate[nstate + 2] = p2;
        if (nstate + 1 >= NSTATES)
                error(MSGSTR(ESTATE, "too many states"));       /* MSG */
        if (c >= NTBASE) {
...
more:           ;
        ...
        if (nlset + 1 >= lsetsize) {
                ...
                */
                if (old_lkst != lkst) {
                        for (temp = (struct item*)&mem[0]; temp < (struct item*)&mem[memsize]; temp++) {

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: slabel.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/19 23:55:42 $";
#endif 
/*
 */
/*
...
static int SL_setup(), SL_init(), SL_free(), SL_exit();
...
static Scrn_parms SL_scrn;
static Menu_scrn  *MenuScrn;
...
#define SL_UNDER_DESC		1
#define SL_TEXT_DESC		2
#define CLASS_TITLE_DESC	3
#define COMPS_TITLE_DESC	4
#define CLASS_UNDER_DESC	5
#define COMPS_UNDER_DESC	6
#define CLASS_SCRTOG_DESC	7
#define COMPS_SCRTOG_DESC	8

/* if this/future requested */
#define SL_THIS_DESC		9
#define SL_FUTURE_DESC		10

/* if default requested */
#define SL_DEF_SL_TOGGLE_DESC	9
#define SL_DEF_SL_TITLE_DESC	10
#define SL_DEF_SL_DESC		11
 ...
 */
{ 0, 6, FLD_PROMPT, 0, FLD_OUTPUT,  NO, "Sensitivity Label/Clearance" },
{ 1, OUTSIDE_MARGIN, FLD_PROMPT, 0, FLD_OUTPUT,  NO,
	"----------- ---------------" },
...
static Scrn_desc SL_default_desc[] = {
{11, OUTSIDE_MARGIN, FLD_TOGGLE, 11, FLD_BOTH, NO, "Use Default" },
{11, 30,             FLD_PROMPT,  0, FLD_OUTPUT, NO,
...
static Menu_scrn SL_menu[] = {
	ROUT_DESC(1, SL_exit, 1, NULL),
	ROUT_DESC(2, SL_exit, 1, NULL),
	ROUT_DESC(3, SL_exit, 1, NULL),
	ROUT_DESC(4, SL_exit, 1, NULL),
	ROUT_DESC(5, SL_exit, 1, NULL),
};

static Scrn_hdrs SL_hdrs = {
	NULL,				/* title */
	cur_date, runner, cur_time,	/* lh, ch, rh */
	NULL, NULL, NULL,		/* lf, cf, rf */
	NULL, cmds_titem, cmds_line1	/* c3, c2, c1 */
};
...
int
aif_label(title, ir_buffer, this_ptr, future_ptr, use_default, default_sl)
	...
	mand_ir_t *default_sl;	/* pointer to IR if default SL */
{
	...
	if (ret) {
		if (!msg_error_cant_setup_labels)
			LoadMessage("msg_label_cant_setup_passwd",
				&msg_error_cant_setup_labels,
				&msg_error_cant_setup_labels_text);
		ErrorMessageOpen(-1, msg_error_cant_setup_labels,
		  0, NULL);
	...
	struct label_fillin *lf;
{
	...
	if (IsTyping) {
		...
		if (ir == NULL) {
			ErrorLabel = 1;
			pop_msg("Sensitivity label cannot be converted.",
				"Please enter a valid sensitivity label.");
		} else {
			ErrorLabel = 0;
...
initialize_just_sl()
{
	...
	SL_scrn.ndescs = NUMDESCS(SL_desc) - NUM_OPTIONAL_DESCS;
}
	...
	struct label_fillin *lf;
{
	...
	if (ret == 0 && HowCalled == ThisFuture) {
		if (lf->this == 0 && lf->future == 0) {
			pop_msg("Select at least one of Current Session",
				"or Future Sessions");
			ret = 1;
		}
	}

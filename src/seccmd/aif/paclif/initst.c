 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: initst.c,v $ $Revision: 1.1.4.2 $ (DEC) $Date: 1993/04/01 20:11:57 $";
#endif
/*
 * @OSF_COPYRIGHT@
 */
 ...
 *  *** OSF1_1_1 version ***
 * 
 * Revision 1.1.4.2  1992/06/11  17:04:23  hosking
 * 	bug 6057: ANSI C changes to allow '-pedantic' to be enabled
 * 	[1992/06/11  16:59:16  hosking]
 *
 * Revision 1.1.2.3  1992/04/05  18:19:44  marquard
 * 	paclif POSIX ACL interface program.
 * 	[1992/04/05  11:52:26  marquard]
 * 
 * $OSF_EndLog$
 */
/*
 ...
 *	@(#)initst.c	1.10 16:31:08 5/17/91 SecureWare
 */

/* #ident "@(#)initst.c	1.1 11:16:56 11/8/91 SecureWare" */
...
int	firstdesc;
{
	...
	for (sdp = screenp->sd, i = 0; i < screenp->ndescs; i++, sdp++) {
		...
		if (sdp->prompt &&
			...
			restorescreen();
			printf (
			MSGSTR(INITST_1, "Field %d had prompt \'%s\' but was not FLD_PROMPT.\n"),
			  i, sdp->prompt);
			exit (1);
		}

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: scrscr.c,v $ $Revision: 1.1.4.2 $ (DEC) $Date: 1993/04/01 20:13:57 $";
#endif
/*
 * @OSF_COPYRIGHT@
 */
 ...
 *  *** OSF1_1_1 version ***
 * 
 * Revision 1.1.4.2  1992/06/11  17:05:39  hosking
 * 	bug 6057: ANSI C changes to allow '-pedantic' to be enabled
 * 	[1992/06/11  17:00:22  hosking]
 *
 * Revision 1.1.2.3  1992/04/05  18:20:38  marquard
 * 	paclif POSIX ACL interface program.
 * 	[1992/04/05  11:54:01  marquard]
 * 
 * $OSF_EndLog$
 */
/*
 ...
 *	@(#)scrscr.c	1.13 16:31:28 5/17/91 SecureWare
 */

/* #ident "@(#)scrscr.c	1.1 11:18:43 11/8/91 SecureWare" */
...
register struct	scrn_desc	*sdp;
{
	 ...
	 * next field on screen.  Blank items in output regions and on
	 * single-entry scrolled toggle regions are OK.
	 */
...
/* scrlll backward in a scrolling region */
void
scrollupkey (stp)
register struct	state	*stp;
{
...
register struct	scrn_desc	*sdp;
{
	...
	if (!scrtogl1 && sdp->inout != FLD_OUTPUT) {
		stringtab = s_scrollreg (sdp->scrnstruct);
...
register struct scrn_desc	*sdp;
{
	...
	if (item < sdp->scrnstruct->filled && stringtab[item][0])
		scr_below_ind (stp->window, sdp, ON);
	else	scr_below_ind (stp->window, sdp, OFF);
	scr_moveto (stp, sdp);
	return;
}

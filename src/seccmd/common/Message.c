 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: Message.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/19 23:59:59 $";
#endif 
/*
 */
/*
	...
	long    timecheck;
{
	...
	if (fread(&timecheck_msg, sizeof(long), 1, cfp) != 1)
		goto corrupted;
	if (timecheck != (long) -1 && timecheck != timecheck_msg)
		goto bad_time_stamp;
	...
	char        **p_text;
{
	...
	messagep = (char **) Malloc(sizeof(char*) * (class_number + 1));
	if (! messagep) {
		Free (text);
		MemoryError();
	}

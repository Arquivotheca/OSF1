 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: UsrGrp.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:01:10 $";
#endif 
/*
 */
/*
...
/*
 * Routines that support user and group lists for audit
 *
 *	entry points:
	...
	AUDIT_USERS_GROUPS_STRUCT *aufill;
{
	...
	uid_t  id;
	struct pr_passwd *pr;
#define UNAMELEN sizeof(pr->ufld.fd_name)
	int     i;
	char    buf[80],
		*cp;
	int     return_value;
	...
	aufill->users = alloc_cw_table(aufill->au.uid_count, UNAMELEN);
	aufill->groups = alloc_cw_table(aufill->au.gid_count, NGROUPNAME);
	...
	AUDIT_USERS_GROUPS_STRUCT *aufill;
{
	...
	for (i = 0; i < aufill->nusers; i++)
		if (aufill->users[i][0] &&
		    pw_nametoid(aufill->users[i]) == (uid_t) -1) {
			ErrorMessageOpen(2830, msg_ausrgrp, 6,
			  aufill->users[i]);
			return 1;
		}
	return 0;
	...
	AUDIT_USERS_GROUPS_STRUCT *aufill;
{
	...
	for (i = 0; i < aufill->ngroups; i++)
		if (aufill->groups[i][0] &&
		    gr_nametoid(aufill->groups[i]) == (gid_t) -1) {
	...
	AUDIT_USERS_GROUPS_STRUCT *aufill;
{
	...
	*/
	...
	for (i = 0; i < gcount; i++) {
		...
		id = gr_nametoid(aufill->groups[i]);

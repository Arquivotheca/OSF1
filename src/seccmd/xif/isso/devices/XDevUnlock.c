 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XDevUnlock.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:10:51 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for explicitly unlocking a terminal/host
...
CREATE_SCREEN_ROUTINES (DevUnlockStart, DevUnlockOpen, DevUnlockClose,
	DevUnlockStop)
...
MakeWidgets() 
{
	...
	/* Form                                                               */
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "DevUnlock");
	XtAddCallback(form_widget, XmNhelpCallback,
		                  HelpDisplayOpen, "devices,DevUnlock");
		                         
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_devices_unlock_device", 
	...
	XmAnyCallbackStruct    *info;
{
	...
	/* Unlock the terminal/host */
	dv.tm.uflg.fg_lock = 1;
	dv.tm.ufld.fd_lock = 0;

	if (dv.tm.uflg.fg_max_tries)
		maxtries = dv.tm.ufld.fd_max_tries;
	else
		if (dv.tm.uflg.fg_max_tries)
			maxtries = dv.tm.sfld.fd_max_tries;
		else
			maxtries = 0;

	/* If too many login attempts reset */
	if (dv.tm.uflg.fg_nlogins)
		if (dv.tm.ufld.fd_nlogins > maxtries) {
			dv.tm.uflg.fg_nlogins = 1;
			dv.tm.ufld.fd_nlogins = 0;
		}
	...
	if (ret != SUCCESS)
		return;
	sa_audit_lock(ES_SET_TERM_UNLOCK, dv.tm.ufld.fd_devname);
	...
	XmAnyCallbackStruct    *info;
{
	...
	LoadMessage("msg_devices_unlock_device_no_match", 

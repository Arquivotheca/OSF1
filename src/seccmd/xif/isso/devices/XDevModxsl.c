 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XDevModxsl.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:10:44 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for DevModxsl function in role programs.
...
CREATE_SCREEN_ROUTINES (DevModxslStart, DevModxslOpen, DevModxslClose,
	DevModxslStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "DevModxsl");
	XtAddCallback(form_widget, XmNhelpCallback,
				 HelpDisplayOpen, "devices,DevModxsl");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_devices_modify_multi_level_max", &msg_header, 
			&msg_header_text);
	...
	CreateDevModxsl(work_area1_frame);
}

CREATE_CALLBACKS (msg_header[1], DevModxslClose) 
...
LoadVariables ()
{
	...
	/* If device's max sl set use that. If not then use the system default.
	 * If no system default then use syslo
	 */
	if (dv.dev.uflg.fg_max_sl) {
		sl      = mand_ir_to_er(dv.dev.ufld.fd_max_sl);
	...
	if (dv.dev.sflg.fg_max_sl) {
		sl      = mand_ir_to_er(dv.dev.sfld.fd_max_sl);
	...
	SetDevModxsl(dv.dev.uflg.fg_max_sl, dflt_sl, device_sl,
...
/* Called from SL code */
void
DevModxslOK(w, ptr, info, sl, sl_given)
	Widget 		w;
	caddr_t		ptr;
	caddr_t 	info;
	char		*sl;
...
ValidateEntries ()
{
	...
	if (set_slabel) {
		...
		if (! dev->uflg.fg_max_sl)
			dev->ufld.fd_max_sl = mand_alloc_ir();
		dev->uflg.fg_max_sl = 1;
		mand_copy_ir (m, dev->ufld.fd_max_sl);
	}
	else

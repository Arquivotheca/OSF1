 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XDevDefnsl.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:09:41 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for DevDefnsl function in role programs.
...
CREATE_SCREEN_ROUTINES (DevDefnslStart, DevDefnslOpen, DevDefnslClose,
	DevDefnslStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "DevDefnsl");
	XtAddCallback(form_widget, XmNhelpCallback,
					 HelpDisplayOpen, "devices,DevDefnsl");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_devices_default_multi_level_min", 
	...
	CreateDevDefnsl(work_area1_frame);
}

CREATE_CALLBACKS (msg_header[1], DevDefnslClose) 
...
LoadVariables ()
{
	...
	if (sd.df.devg.fg_min_sl) {
		sl      = mand_ir_to_er(sd.df.devd.fd_min_sl);
	...
	/* Set the user name */
	SetDevDefnsl(dflt_sl, mand_syshi->class, mand_syshi->cat);
	free(dflt_sl);
	return (ret);
}
...
ValidateEntries ()
{
	...
	if (! sdef->devg.fg_min_sl)
		sdef->devd.fd_min_sl = mand_alloc_ir();
	sdef->devg.fg_min_sl = 1;
	mand_copy_ir (m, sdef->devd.fd_min_sl);
	mand_free_ir (m);

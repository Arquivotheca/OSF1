 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XDevDefxsl.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:09:50 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for DevDefxsl function in role programs.
...
CREATE_SCREEN_ROUTINES (DevDefxslStart, DevDefxslOpen, DevDefxslClose,
	DevDefxslStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "DevDefxsl");
	XtAddCallback(form_widget, XmNhelpCallback,
					 HelpDisplayOpen, "devices,DevDefxsl");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_devices_default_multi_level_max", 
	...
	CreateDevDefxsl(work_area1_frame);
}

CREATE_CALLBACKS (msg_header[1], DevDefxslClose) 
...
LoadVariables ()
{
	...
	if (sd.df.devg.fg_max_sl) {
		sl      = mand_ir_to_er(sd.df.devd.fd_max_sl);
	...
	SetDevDefxsl(dflt_sl, mand_syshi->class, mand_syshi->cat);
	free(dflt_sl);
	return (ret);
}
...
ValidateEntries ()
{
	...
	if (! sdef->devg.fg_max_sl)
		sdef->devd.fd_max_sl = mand_alloc_ir();
	sdef->devg.fg_max_sl = 1;
	mand_copy_ir (m, sdef->devd.fd_max_sl);
	mand_free_ir (m);

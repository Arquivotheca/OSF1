 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XDevModssl.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:10:41 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for DevModssl function in role programs.
...
CREATE_SCREEN_ROUTINES (DevModsslStart, DevModsslOpen, DevModsslClose,
	DevModsslStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "DevModssl");
	XtAddCallback(form_widget, XmNhelpCallback,
				 HelpDisplayOpen, "devices,DevModssl");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_devices_modify_single_level_sl", 
	...
	CreateDevModssl(work_area1_frame);
}

CREATE_CALLBACKS (msg_header[1], DevModsslClose) 
...
LoadVariables ()
{
	...
	/* If device's cur sl set use that. If not then use the system default.
	 * If no system default then use syslo
	 */
	if (dv.dev.uflg.fg_cur_sl) {
		sl      = mand_ir_to_er(dv.dev.ufld.fd_cur_sl);
	...
	if (dv.dev.sflg.fg_cur_sl) {
		sl      = mand_ir_to_er(dv.dev.sfld.fd_cur_sl);
	...
	SetDevModssl(dv.dev.uflg.fg_cur_sl, dflt_sl, device_sl,
...
/* Called from SL code */
void
DevModsslOK(w, ptr, info, sl, sl_given)
	Widget 		w;
	caddr_t		ptr;
	caddr_t 	info;
	char		*sl;
...
ValidateEntries ()
{
	...
	if (set_slabel) {
		...
		if (! dev->uflg.fg_cur_sl)
			dev->ufld.fd_cur_sl = mand_alloc_ir();
		dev->uflg.fg_cur_sl = 1;
		mand_copy_ir (m, dev->ufld.fd_cur_sl);
	}
	else

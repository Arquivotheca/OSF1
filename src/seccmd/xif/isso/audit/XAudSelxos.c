 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XAudSelxos.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:08:06 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for AudSelxos function in role programs.
...
/* N.B. The Close routine is defined separately. AudSelxosJunk is not
 * used anywhere */
CREATE_SCREEN_ROUTINES (AudSelxosStart, AudSelxosOpen, AudSelxosJunk,
	AudSelxosStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "AudSelxos");
	XtAddCallback(form_widget, XmNhelpCallback,
					 HelpDisplayOpen, "audit,AudSelxos");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	LoadMessage ("msg_audit_selection_max_object_sl", &msg_header, 
		&msg_header_text);
	...
	CreateAudSelxos(work_area1_frame);
}
...
CREATE_CALLBACKS (msg_header[1], AudSelxosClose) 
...
LoadVariables ()
{
	...
	sl      = mand_ir_to_er(selection_fillin.olevel_max);
	dflt_sl = (char *) Malloc(strlen(sl) + 1);
	strcpy (dflt_sl, sl);

	/* Set the user name */
	SetAudSelxos(dflt_sl, mand_syshi->class, mand_syshi->cat);
	free(dflt_sl);
	return (SUCCESS);
}
...
ValidateEntries ()
{
	...
	if (! m)
		return (FAILURE);
	mand_copy_ir (m, selection_fillin.olevel_max);
	mand_free_ir (m);
	return (SUCCESS);
}

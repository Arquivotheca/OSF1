 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XAudSelnos.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:07:59 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for AudSelnos function in role programs.
...
/* N.B. The Close routine is defined separately. AudSelnosJunk is not
 * used anywhere */
CREATE_SCREEN_ROUTINES (AudSelnosStart, AudSelnosOpen, AudSelnosJunk,
	AudSelnosStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "AudSelnos");
	XtAddCallback(form_widget, XmNhelpCallback,
					 HelpDisplayOpen, "audit,AudSelnos");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_audit_selection_min_object_sl", 
	...
	CreateAudSelnos(work_area1_frame);
}
...
CREATE_CALLBACKS (msg_header[1], AudSelnosClose) 
...
LoadVariables ()
{
	...
	sl      = mand_ir_to_er(selection_fillin.olevel_min);
	dflt_sl = (char *) Malloc(strlen(sl) + 1);
	strcpy (dflt_sl, sl);

	SetAudSelnos(dflt_sl, mand_syshi->class, mand_syshi->cat);
	free(dflt_sl);
	return (SUCCESS);
}
...
ValidateEntries ()
{
	...
	if (! m)
		return (FAILURE);
	mand_copy_ir (m, selection_fillin.olevel_min);
	mand_free_ir (m);
	return (SUCCESS);
}

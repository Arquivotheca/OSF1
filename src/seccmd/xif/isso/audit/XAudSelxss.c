 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XAudSelxss.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:08:10 $";
#endif 
/*
 */
/*
...
/*
	...
	function:
		X windows front end for AudSelxss function in role programs.
...
/* N.B. The Close routine is defined separately. AudSelxssJunk is not
 * used anywhere */
CREATE_SCREEN_ROUTINES (AudSelxssStart, AudSelxssOpen, AudSelxssJunk,
	AudSelxssStop)
...
MakeWidgets() 
{
	...
	/**********************************************************************/
	form_widget = CreateForm(mother_form, "AudSelxss");
	XtAddCallback(form_widget, XmNhelpCallback,
					 HelpDisplayOpen, "audit,AudSelxss");
	
	/**********************************************************************/
	/* Title label                                                        */
	/**********************************************************************/
	if (! msg_header)
		LoadMessage ("msg_audit_selection_max_subject_sl", 
	...
	CreateAudSelxss(work_area1_frame);
}
...
CREATE_CALLBACKS (msg_header[1], AudSelxssClose) 
...
LoadVariables ()
{
	...
	sl      = mand_ir_to_er(selection_fillin.slevel_max);
	dflt_sl = (char *) Malloc(strlen(sl) + 1);
	strcpy (dflt_sl, sl);

	SetAudSelxss(dflt_sl, mand_syshi->class, mand_syshi->cat);
	free(dflt_sl);
	return (SUCCESS);
}
...
ValidateEntries ()
{
	...
	if (! m)
		return (FAILURE);
	mand_copy_ir (m, selection_fillin.slevel_max);
	mand_free_ir (m);
	return (SUCCESS);
}

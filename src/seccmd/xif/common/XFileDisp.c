 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: XFileDisp.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:04:54 $";
#endif 
/*
 */
/*
...
MakeWidgets() {
	...
	form_widget = XtCreateWidget("FileDisplay", 
			xmFormWidgetClass, main_form, args, n);
	XtAddCallback(form_widget, XmNunmapCallback, 
			FileDisplayCallback, NULL);
	...
	title_widget = XmCreateLabelGadget(form_widget, "Title", args, n);
	XtManageChild(title_widget);
	
	/**********************************************************************/
	/* Display text of file through scrolled list                         */
	/**********************************************************************/

	n = 0;
	XtSetArg(args[n], XmNtopAttachment,      XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNtopWidget,             title_widget); n++;
	XtSetArg(args[n], XmNleftAttachment,       XmATTACH_FORM); n++; 
	...
	scrolled_window_widget = XtCreateManagedWidget(
			"ScrolledArea", xmScrolledWindowWidgetClass, 
			form_widget, args, n);
	...
	list_widget = XtCreateManagedWidget("List",
			xmListWidgetClass, scrolled_window_widget, args, n);
	...
	XtSetArg(args[n], XmNtopAttachment,  XmATTACH_WIDGET); n++;
	XtSetArg(args[n], XmNtopWidget,        scrolled_window_widget); n++;
	XtSetArg(args[n], XmNleftAttachment,   XmATTACH_FORM); n++;
	action_button_widget = XmCreateRowColumn(form_widget, "Button", 
			args, n);
	...
	XtSetArg(args[n], XmNlabelString,                xmstring); n++;
	XtSetArg(args[n], XmNalignment,         XmALIGNMENT_CENTER); n++;
	w = XmCreatePushButtonGadget(action_button_widget, 
			"Button", args, n);
	XtManageChild(w);
	XtAddCallback(w, XmNactivateCallback, FileDisplayCallback, "OK");
	XmStringFree(xmstring);
	...
	XtManageChild(w);
	XtAddCallback(w, XmNactivateCallback, FileDisplayCallback, "Print");
	XmStringFree(xmstring);
}

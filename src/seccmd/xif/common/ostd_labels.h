 ...
 */
/*	
 *	@(#)$RCSfile: ostd_labels.h,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/20 00:05:45 $
 */ 
/*
 */
/*
 ...
 * This include file originated as /usr/include/std_labels.h
 * All items removed have ifdef NOT_USED around them
 * This include file is needed to simulate a DIA style B1 environment
 * so that common code can be shared between the DIA style and the
 * regular style B1 environment.
 * Therefore we only use this if we are using MAC_OB. 
 * This file creates an environment * similar to SEC_ENCODINGS so we 
 * can maintain common code in XSL.c
 */
...
/* XXXX - These three tables are not used so don't define them.
 * This helps to track possible errors at the compilation stage */
#if NOT_USED
...
/* The following have been rearranged because some are not used */
/* XXX */
#ifdef NOT_USED

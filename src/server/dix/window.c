    ...
    BoxPtr	pBox;
{
    ...
    if (pBoundRgn = wBoundingShape (pWin)) {
	    (*pScreen->TranslateRegion) (pRgn, -pWin->origin.x,
					          -pWin->origin.y);
	    (*pScreen->TranslateRegion) (pBoundRgn, pWin->parent->origin.x, 
					    pWin->parent->origin.y);
	    (*pScreen->Intersect) (pRgn, pRgn, pBoundRgn);
	    (*pScreen->TranslateRegion) (pBoundRgn, -pWin->parent->origin.x, 
					    -pWin->parent->origin.y);
	    (*pScreen->TranslateRegion) (pRgn, pWin->origin.x,
					          pWin->origin.y);

...
#ifdef MODE_SWITCH
    /* Bug fix of server crash for the i18n keyboard support.
     * 1-Feb-1993 K.Ichimaru
     */
    InitHandleKeybindingParameter();
    i = HandleKeyBinding(keyc, pKeySyms); /* ignore return status */
#endif
    dev->key = keyc;
    return TRUE;
}
    ...
    ClientPtr client;
{
...
#ifdef MODE_SWITCH
    register KeyClassPtr Keyc = inputInfo.keyboard->key;
    register int modMappingChanged;
#endif
	...
	return BadAlloc;
#ifdef MODE_SWITCH
    /* If we have successfully caused the keyboard mapping as well as the
     * modifier mapping changed, we need to let all clients know.  So send
     * two mappingNotify events out instead of the usual one.
     */
    modMappingChanged = HandleKeyBinding(Keyc, &keysyms);
    SendMappingNotify(MappingKeyboard, stuff->firstKeyCode, stuff->keyCodes);
    if(modMappingChanged)
        SendMappingNotify(MappingModifier, 0, 0);
#else
    SendMappingNotify(MappingKeyboard, stuff->firstKeyCode, stuff->keyCodes);
#endif
...
#define DO_ALL    (-1)
	...
	return BadLength;
    vlist = (CARD32 *)&stuff[1];		/* first word of values */

    ...
    ClientPtr client;
{
    ...
    result = GrabDevice(client, inputInfo.keyboard, 
			(unsigned) stuff->keyboardMode,
			(unsigned) stuff->pointerMode, 
			(Window) stuff->grabWindow,
			(unsigned) stuff->ownerEvents, (Time) stuff->time,

...
#if LONG_BIT == 64
/* The protocol restricts time information to 32 bits, so all of
 * these have to be changed. Perhaps we should just define them
 * as CARD32?
 */
...
Dispatch()
{
    ...
    register int	nready;
#ifdef __alpha /* THIS NEEDS TO BE LONG!!! */
    register int	**icheck = checkForInput;
#else
...
#ifdef SMT
            /*
             * The shared memory transport depends on ReadRequestFromClient()
             * being called with the oldbuf argument either pointing to the
             * last request buffer allocated, or on being null.  This
             * allows the freeing of the last request buffer to be done
             * in ReadRequestToClient(), avoiding an explicit test/call
             * in the inner dispatch loop which would have to be tested
             * by all clients regardless of whether they use shared memory
             * or not.  Deferring the freeing of the last request buffer
             * until the next request is to be executed may ultimately
             * benefit extensions (e.g. PEX) which DMA directly from
             * request buffers, though additional work will be needed
             * to make this possible.
             */
            client->requestBuffer = NULL;
#endif SMT
...
#ifdef SMT
            SmtFreeLastRequest(clients[clientReady[nready]]);
#endif SMT
	    FlushAllOutput();
	}
    }
    ...
    register ClientPtr client;
{
    ...
    if (!(pParent = (WindowPtr)LookupWindow((XID)(stuff->parent), client)))
        return BadWindow;
	...
	for ( i = 0; i < len; i ++ )
	{
        ...
        pWin = CreateWindow((Window)stuff->wid, pParent, stuff->x,
			      stuff->y, stuff->width, stuff->height, 
			      stuff->borderWidth, stuff->class,
			      (Mask) stuff->mask, changes,
			      (int)stuff->depth, 
			      client, (VisualID) stuff->visual, &result);
    }
    else
        pWin = CreateWindow(stuff->wid, pParent, stuff->x,
			      stuff->y, stuff->width, stuff->height, 
			      stuff->borderWidth, stuff->class,
			      (Mask) stuff->mask, (XID *) &stuff[1], 
			      (int)stuff->depth, 
			      client, (VisualID) stuff->visual, &result);
    if (pWin)
    {
    ...
    register ClientPtr client;
{
	...
	for ( i = 0; i < len; i ++ )
	{
        ...
        result =  ConfigureWindow(pWin, (Mask)stuff->mask, changes,
			      client);
    }
    else
    ...
    register ClientPtr client;
{
	...
	/* Window is a MD type, not a protocol type. */
	childIDs = (CARD32 *) ALLOCATE_LOCAL(numChildren * sizeof(CARD32));
	if (!childIDs)
	    return BadAlloc;
	for (pChild = pWin->lastChild; pChild != pHead; pChild =pChild->prevSib)
	    childIDs[curChild++] = (CARD32) pChild->drawable.id;
    }
    
    reply.nChildren = numChildren;
    reply.length = (numChildren * sizeof(CARD32)) >> 2;
    
    WriteReplyToClient(client, sizeof(xQueryTreeReply), &reply);
    if (numChildren)
    {
    	client->pSwapReplyFunc = Swap32Write;
	WriteSwappedDataToClient(client, numChildren*sizeof(CARD32), childIDs);
    ...
    register ClientPtr client;
{
    ...
    err = OpenFont(client, (XID)stuff->fid, (Mask) 0,
    ...
    register ClientPtr client;
{
	...
	for ( i = 0; i < len; i ++ )
	{
        ...
        pGC = (GC *)CreateGC(pDraw, (Mask) stuff->mask, 
			 changes, &error);
    } 
    else
        pGC = (GC *)CreateGC(pDraw, (Mask) stuff->mask, 
    ...
    register ClientPtr client;
{
	...
	for ( i = 0; i < len; i ++ )
	{
        ...
        result = DoChangeGC(pGC, (Mask) stuff->mask, changes, 0);
    }
    else
        result = DoChangeGC(pGC, (Mask) stuff->mask, (XID *) &stuff[1], 0);
    ...
    register ClientPtr client;
{
    ...
    if (things)
        (*pGC->ops->FillPolygon) (pDraw, pGC, (int)stuff->shape,
			 (int)stuff->coordMode, things,
			 (DDXPointPtr) &stuff[1]);
    return(client->noClientException);
}
    ...
    register ClientPtr client;
{
    ...
    /* handle 64-bit case where stuff is not 64-bit aligned */
        ...
        if(!(tmpImage = (char *) ALLOCATE_LOCAL(length*stuff->height)))
            return (BadAlloc);
	bcopy((char *)&stuff[1],tmpImage,length*stuff->height);
    }
    else
    ...
    if ((unsigned long)&stuff[1] & (sizeof(long)-1) ||
        length != lengthProto ) 
    ...
    register ClientPtr	client;
{
	...
	while ((linesPerBuf > 1) && (length & (sizeof(long)-1)))
	{
	    linesPerBuf--;
	    length -= widthBytesLine;
	}
	while (length & (sizeof(long)-1))
	{
	    linesPerBuf++;
	    length += widthBytesLine;
	}
...
#define FontShiftSize 5
    ...
    VALIDATE_DRAWABLE_AND_GC((Drawable)stuff->drawable, pDraw, pGC, client);
	...
	else	/* print a string */
	{
		...
		pElt + TextEltHeader);
	    xorg = (* polyText)(pDraw, pGC, xorg, (int)stuff->y, (int)*pElt,
		pElt + TextEltHeader);
	    pElt = pNextElt;
	}
    }
    ...
    register ClientPtr client;
{
    ...
    locCM = (Colormap * )
	ALLOCATE_LOCAL(pWin->drawable.pScreen->maxInstalledCmaps *
		     sizeof(Colormap));
    if ( !locCM )
    {
    ...
    nummaps = (*pWin->drawable.pScreen->ListInstalledColormaps)
        (pWin->drawable.pScreen, locCM);
    /* problem here. There is no type available by which we can name
     * the Xproto.h version of Colormap, so CARD32 will have to do.
     */
    for (i = 0; i < nummaps; i += 1)
	((CARD32 *) &preply[1])[i] = (CARD32) locCM[i];
    preply->nColormaps = nummaps;
    preply->length = nummaps;
    WriteReplyToClient(client, sizeof (xListInstalledColormapsReply), preply);
    client->pSwapReplyFunc = Swap32Write;
    WriteSwappedDataToClient(client, nummaps * sizeof(CARD32), &preply[1]);
    DEALLOCATE_LOCAL(preply);
    DEALLOCATE_LOCAL(locCM);
    return(client->noClientException);
}
    ...
    register ClientPtr client;
{
	...
	acr.blue = stuff->blue;
	localPix = 0;
	retval = AllocColor(pmap, &acr.red, &acr.green, &acr.blue,
				&localPix, client->index);
	acr.pixel = localPix;
	if(retval)
	{
            if (client->noClientException != Success)
    ...
    register ClientPtr client;
{
				    ...
				    (Bool)stuff->contiguous, ppixels, pmasks))
	{
        ...
        {
	    register int i;
	    length = ((long)npixels + (long)nmasks) * sizeof(CARD32);
	    ppixels32 = (CARD32 *)ALLOCATE_LOCAL(length);
	    pmasks32 = ppixels32 + npixels;
	    for ( i = 0; i < npixels; i ++ )
	    {
	        ppixels32[i] = (CARD32)ppixels[i];
	    }
	    for ( i = 0; i < nmasks; i ++ )
	    {
	        pmasks32[i] = (CARD32)pmasks[i];
	    }
        }
	else {
		ppixels32 = (CARD32 *)ppixels;
		pmasks32 = (CARD32 *)pmasks;
	}
	...
	client->pSwapReplyFunc = Swap32Write;
	WriteSwappedDataToClient(client, length, ppixels32);
	DEALLOCATE_LOCAL(ppixels);
	DEALLOCATE_LOCAL(ppixels32);
        return (client->noClientException);        
    }
    else
    {
    ...
    register ClientPtr client;
{
	...
	/* This is for the case where long != 32 bits since
	 * AllocColorPlanes expects Pixel and these are CARD32
	 */
	acpr.redMask 	= rmask;
	acpr.greenMask 	= gmask;
	acpr.blueMask 	= bmask;
	    ...
	    length = (long)npixels * sizeof(CARD32);
	    ppixels32 = (CARD32 *)ALLOCATE_LOCAL(length);
	    for ( i = 0; i < npixels; i ++ )
	    {
	        ppixels32[i] = (CARD32)ppixels[i];
    ...
    register ClientPtr client;
{
	...
	if ( sizeof(Pixel) != sizeof(CARD32) )
	{
	    pixels = (Pixel *)ALLOCATE_LOCAL(count * sizeof(Pixel));
	    if(!pixels && count)
		return(BadAlloc);
	    ppixel = (CARD32 *) &stuff[1];
	    for (i = 0; i < count; i += 1) 
		pixels[i] = (Pixel) *ppixel++;
	    retval = FreeColors(pcmp, client->index, count,
	        pixels, stuff->planeMask);
	    if (pixels) 
		DEALLOCATE_LOCAL(pixels);
	} 
	else
	    retval =  FreeColors(pcmp, client->index, count,
	        (Pixel *)&stuff[1], stuff->planeMask);
    ...
    register ClientPtr client;
{
	...
	if (sizeof(Pixel) != sizeof(CARD32))
	{
	    ...
	    for (i = 0; i < count; i += 1) 
		pixels[i] = (Pixel) *ppixel++;
	    retval = QueryColors(pcmp, count, pixels, prgbs);
	    if (pixels) 
		DEALLOCATE_LOCAL(pixels);
	}
	else
	    retval = QueryColors(pcmp, count, (Pixel *)&stuff[1], prgbs);

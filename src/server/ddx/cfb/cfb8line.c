...
# ifndef POLYSEGMENT
	    PixelType    *t;
    	    ...
    	    register unsigned long startmask, endmask; 
			  /* can't use e and mask interchangeably in 
			   * 64-bit world. e is an error term and mask
			   * must be a full 64-bits.
			   */
...
#  endif
		...
		if (x1_or_len)
		{
		    maskpartialbits(y1_or_e1, x1_or_len, startmask)
		    RROP_SOLID_MASK((unsigned long *) addrp, startmask);
		}
	    }
	    else
	    {
	    	maskbits(y1_or_e1, x1_or_len, startmask, endmask, x1_or_len)
	    	if (startmask)
	    	{
		    RROP_SOLID_MASK((unsigned long *) addrp, startmask);
		    addrp += PPW;
		...
		RROP_SPAN(addrp, x1_or_len)
	    	if (endmask) {
		    RROP_SOLID_MASK((unsigned long *) addrp, endmask);
		}
	    }
# ifndef POLYSEGMENT
	    addrp = t;

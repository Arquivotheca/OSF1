...
#define ResetTileBits {\
	...
	leftShift = xoff << ((LONG_BIT == 32 ? 5 : 6 ) -PWSH); \
	rightShift = LONG_BIT - leftShift; \
    }\
}
    ...
    unsigned long   planemask;
{
    ...
    widthSrc = tile->devKind / sizeof(long);
    narrowTile = FALSE;
    if (widthSrc == 1)
    {
    ...
    tileEndLeftShift = (tileEndPart) << ((LONG_BIT == 32 ? 5 : 6 ) -PWSH);
    tileEndRightShift = LONG_BIT - tileEndLeftShift;
	...
	leftShiftStart = (xoffStart) << ((LONG_BIT == 32 ? 5 : 6 ) -PWSH);
	rightShiftStart = LONG_BIT - leftShiftStart;

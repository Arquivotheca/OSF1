...
int fSorted;
{
	...
	else
	{
	    ...
	    nextPartBits = (x & (sizeof(long)-1)) + w;
    ...
    int 	fSorted;
{
	...
	RotBitsLeft (bits, (x & ((LONG_BIT-1) & ~(sizeof(long)-1))));
...
#undef SwitchBitsLoop
	...
	else
	{
		...
		*dst = 
		    MaskRRopPixels(*dst, xor, startmask);
		dst++;
		RotBitsLeft (bits, LONG_BIT/8);
	    }
	    while (nlw--)
    ...
    int 	fSorted;
{
	...
	if (cfb8StippleRRop == GXcopy)
	{
		...
		if (startmask)
		{
		    *dst = *dst & ~startmask | GetFourPixels (bits) & startmask;
		    dst++;
		    RotBitsLeft (bits, LONG_BIT/8);
		}
		while (nlw--)
		...
		if (endmask)
		{
		    *dst = *dst & ~endmask | GetFourPixels (bits) & endmask;
		}
	    }
	    else
	    {

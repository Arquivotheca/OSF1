...
static void miArcJoin(), miArcCap(), miFreeArcs();
extern void miRoundCap();
...
double	a;
{
	...
	if (floor (a/90) == a/90) {
		...
		case 2: return (double)0;
		case 3: return (double)-1;
		}
	}
...
double	v;
{
    ...
    if (v == (double)-1.0)
	return (double)-90.0;
    return asin(v) * ((double)180.0 / M_PI);
}
	...
	GCPtr	pGC;
{
	...
	for (;;) {
		...
		if (isDashed) {
			...
			CARD16 length = parcs[i].width;
			
			backwards = 0;
			selfJoin = data[i].selfJoin &&
 				    (iphase == 0 || isDoubleDash);
			/*
			 * add dashed segments to each bucket
			 */
			...
			while (length) {
			    ...
			    this_element = (length > dashRemaining) ? dashRemaining : length;
			    if (iphase == 0 || isDoubleDash) {
				xarc.width = this_element;
				arc = addArc (&arcs[iphase].arcs, 
					      &arcs[iphase].narcs,
					      &arcSize[iphase], &xarc);
				if (!arc)
				    goto arcfail;
			    ...
			    length -= this_element;
			    xarc.x += this_element;
			    if (this_element < dashRemaining) {
				dashRemaining -= this_element;
			    } else {
				++iDash;
				if (iDash == pGC->numInDashList)
				    iDash = 0;
			...
			if (parcs[i].width == 0) {
				...
				prevphase = 0;
			    arc = addArc (&arcs[prevphase].arcs, 
					  &arcs[prevphase].narcs,
	...
	struct accelerators	*acc;
{
	...
	if (acc->h2mw2 == (double)0.0) {
		if (scan_y > (double)0.0 && !left || 
			scan_y < (double)0.0 && left)

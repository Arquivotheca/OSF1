...
/*
   ...
   pixmap.devPrivate = (unsigned long *)pointer_to_bits
...
#define mfbGetIntWidth(pDrawable) cbGetTypedWidth(pDrawable, int)
...
#define mfbGetIntWidthAndPointer(pDrawable, width, pointer) \
    mfbGetTypedWidthAndPointer(pDrawable, width, pointer, unsigned int, unsigned int)
...
typedef struct _pos{
    ...
    int topEdge;
    int bottomEdge;
    unsigned long *pdstBase;	/* longword with character origin */

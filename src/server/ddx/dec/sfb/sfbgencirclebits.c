...
static char *BuildSystemHeader= "$Header: /alphabits/u3/x11/ode/rcs/x11/src/server/ddx/dec/sfb/sfbgencirclebits.c,v 1.1.3.3 92/02/21 11:44:05 David_Coleman Exp $";	/* BuildSystemHeader */
...
{							    \
    CommandWord mask;					    \
    mask = SFBLEFTBUSMASK(xorg - x, SFBBUSALL1)		    \
	    & SFBRIGHTBUSMASK(xorg - x + slw, SFBBUSALL1);  \
    bits[yorg - y] |= mask;				    \
...
main() {
    ...
    printf(
      "/* DO NOT MODIFY.  AUTOMATICALLY GENERATED BY sfbgencirclebits.c */\n");
    printf("\n");
    printf("#include \"sfb.h\"\n");
    printf("#include \"sfbcirclebits.h\"\n");
    printf("\n");
    printf("\n");

    for (diameter = 0; diameter < SFBBUSBITS; diameter++) {
	MakeZerCircle(diameter);
    }

    printf("\n");
    printf("CommandWord *zeroCircles[SFBBUSBITS] = {");
    for (diameter = 0; diameter < SFBBUSBITS; diameter++) {
	if ((diameter&3) == 0) printf("\n");
	printf("    zeroCircle%d", diameter);
	if (diameter == SFBBUSBITS-1) printf("\n"); else printf(",");
    }
    printf("};\n\n");
    ...
    printf("\n");
    printf("CommandWord *solidCircles[SFBBUSBITS+1] = {");
    for (diameter = 0; diameter <= SFBBUSBITS; diameter++) {
	if ((diameter&3) == 0) printf("\n");
	printf("    solidCircle%d", diameter);
	if (diameter == SFBBUSBITS) printf("\n"); else printf(",");
    }
    printf("};\n\n");
}

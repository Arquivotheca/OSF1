    ...
    char *	argv[];
{
...
#if 0  /* backing store is not working */
    miInitializeBackingStore (pScr, &qdBSFuncRec);
#endif
    ...
    pScr->defColormap = FakeClientID(0);
    if (CreateColormap(pScr->defColormap, pScr, pVisual, &pColormap,
	(pVisual->class & DynamicClass) ? AllocNone : AllocAll,
	0) != Success) {
	ErrorF( "qdScreenInit: cannot create default colormap\n");
	return FALSE;
    }

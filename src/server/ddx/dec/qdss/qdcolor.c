    ...
    ColormapPtr     pcmap;
{
	...
	for (r = 0; r < nr; r++)
		...
		for (b = 0; b < nb; b++)
		{
			...
			pcmap->red[pixel].co.local.red = red;
			pcmap->red[pixel].co.local.green = green;
			pcmap->red[pixel].co.local.blue = blue;
		    } else {
			/* TrueColor does not need nested loops XXX */
			pcmap->red[r].co.local.red = red;
			pcmap->green[g].co.local.green = green;
			pcmap->blue[b].co.local.blue = blue;
		    }
		}
	break;
      case StaticGray:

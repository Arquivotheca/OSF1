 ...
 *
 * $Header: /usr/sde/x11/rcs/x11/src/./server/ddx/x386/x386Events.c,v 1.2 91/12/15 12:42:16 devrcs Exp $
 */
     ...
     int buttons, dx, dy;
{
  ...
  if (x386Info.emulate3Buttons)
  ...
  else
    {
       ...
       */
      change = buttons ^ x386Info.lastButtons;
      while (change)
	{
	  id = ffs(change);
	  change &= ~(1 << (id-1));
...
x386VTSwitch()
{
  ...
  CursorPtr cursor;
  XID cursorID;
  static WindowPtr pWin;
  static XID Wid;

  if (x386VTSema) {
    
    /*
     * this is modeled after ideas from server/dix/windows.c
     */
    attributes[0] = WindowTable[0]->drawable.pScreen->blackPixel;
    attributes[1] = xTrue;
    ...
    pWin =
      CreateWindow(Wid,
		   ...
		   (ushort)screenInfo.screens[0]->width + RANDOM_WIDTH,
		   (ushort)screenInfo.screens[0]->height + RANDOM_WIDTH,
		   0, InputOutput, mask, attributes, 0, serverClient,
		   wVisual (WindowTable[0]), &result);
    
    if (!pWin) return;
    
    if (!AddResource(pWin->drawable.id, RT_WINDOW, (pointer)pWin)) return;

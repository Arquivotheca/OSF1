...
#ifdef  XDPS
char *xDPSFakeRequest = (char *)0;
int xDPSFakeRequestSize = 0;
  /* This is defined by the X/DPS integration. It is a DPS extension    */
  /* request for PSGiveInput. It is returned by ReadRequestFromClient.  */
#endif  XDPS
    ...
    ClientPtr client;
{
    ...
    if (client == serverClient)
      {
      client->requestBuffer = (pointer)xDPSFakeRequest;
      client->req_len = xDPSFakeRequestSize;
      YieldControl();
      return(xDPSFakeRequestSize);
      }
#endif  XDPS
...
#ifdef SMT
    /*
     * It would be really nice if the client structure had
     * per-transport vectors (e.g. have a per-client
     * ReadRequestFromClient function).  A lot of cruft
     * comes from pretending that all transports should behave
     * like socket-based transports.  In order to deal with the
     * wakeup requests from the client (which are the only ones
     * which continue to go over the socket), when the shared
     * memory transport runs out of requests, we fall through
     * to the socket-reading code.  Since the only function
     * of the wakeup requests is to ensure that the server is
     * not asleep, it is safe to give priority to requests
     * arriving via shared memory.
     */
    if (oc->pSmt) {
        result =
            (*oc->pSmt->ReadRequestFromClient)(client);
        if (client->requestBuffer == NULL)

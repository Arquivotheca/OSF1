...
static int tmpi;    /* Store converted value in tmpi to avoid re-evaluation */
 
#define FABS(x) (((tmpx = (x)) < 0.0) ? -tmpx : tmpx)
 
#define CEIL(x) (((tmpi = (Word) (tmpx = (x))) < tmpx) ? ++tmpi : tmpi)
 
#define FLOOR(x) (((tmpi = (Word) (tmpx = (x))) > tmpx) ? --tmpi : tmpi)

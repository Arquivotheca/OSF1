...
int
_bdfReadFont(pFont, file, bit, byte, glyph, scan, ink)
    FontPtr     pFont;
    FontFilePtr file;
    int         bit,
                byte,
                ...
                ink;
{
    ...
    if (ink && pFont->info.constantMetrics)
...
{
/*
 * set the ink parameter to TRUE to enable creation of ink metrics.
 */
    return (_bdfReadFont(pFont, file, bit, byte, glyph, scan, TRUE));
}

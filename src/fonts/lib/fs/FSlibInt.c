...
{
    char        buf[BUFSIZE];
    int        pend_not_register;	/* because can't "&" a register
					 * variable */
    register int pend;
    register fsEvent *ev;
    ...
    register long size;
{
    ...
    int        skip = 0;
    int        svrbufsize = (svr->bufptr - svr->buffer);
    int        padsize = padlength[size & 3];
    int        total = svrbufsize + size + padsize;
    int        todo = total;
    int numtimes=0;
     ...
     */
    while (total) {
	int        before = skip;	/* amount of whole thing written */
	int        remain = todo;	/* amount to try this time, <= total */
	int         i = 0;
	int        len;
...
_FSReply(svr, rep, extra, discard)
    register FSServer *svr;
    register fsReply *rep;
    long         extra;		/* number of 32-bit words expected after the
				 * reply */
				 ...
				 * words? */
{
    ...
    _FSFlush(svr);
    while (1) {
	_FSRead(svr, (char *) rep, (int) SIZEOF(fsReply));
	...
	case FS_Reply:
		 ...
		 */
		_FSRead(svr, (char *) NEXTPTR(rep,fsReply), ((int) extra) << 2);
		return (1);
	    }
	    if (extra < rem_length) {
	...
	case FS_Error:
		...
		_FSRead(svr, (char *) &err + SIZEOF(fsReply),
			(int) (SIZEOF(fsError) - SIZEOF(fsReply)));
...
#define SCRATCHSIZE 2048
    ...
    while (n > 0) {
	register int bytes_read = (n > SCRATCHSIZE) ? SCRATCHSIZE : n;

...
# 
#
# @(#)dbgmon.sh	3.2	(ULTRIX/OSF)	6/30/91
#
# *****************************************************************
# *                                                               *
# *    Copyright (c) Digital Equipment Corporation, 1991, 1994    *
...
# *****************************************************************
#
# This is the script for building dbgmon for standalone kernel debugging.
...
# REVISION HISTORY:
#
# 01-Apr-1991	Don Dutile
#	Creation of this file.  Modified stand.sh to get to initial rev.
#
#
...
if [ $# != 2 ]
then
    echo "dbgmon.sh <target_machine> <destination_root>"
    echo "Example:  dbgmon.sh DS3100 /usr/projects/mypool/obj"
    echo "       :  dbgmon.sh all_sys /usr/projects/mypool/obj"
    exit 1
fi

export MAKEFILEPATH SOURCEBASE SOURCEDIR OBJECTDIR

#(cd kernel/dec/debug/dbgmon; make -cpkEF $1 DESTROOT=$2)
(cd kernel/dec/debug/dbgmon; make -cdpkEF $1 DESTROOT=$2)
#(cd kernel/dec/debug/dbgmon; make -ckEF $1 DESTROOT=$2)

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: wt.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/19 22:10:13 $";
#endif 
/*
 */
/*
...
struct isa_driver wtdriver = {wtprobe, 0, wtattach, "wt", 0, 0, 0};

#if	NWT > 1
/* There is no way this code supports multiple drives */
#undef	NWT
#define NWT	1
#endif

struct isa_dev *wtdev[NWT];
static ihandler_t wthandlers[NWT];
...
struct isa_dev *dev;
{
	...
	ihandler_id_t *wthandler_id;

        wtdev[unit] = dev;

	wthandlers[unit].ih_level = dev->dev_pic;
	wthandlers[unit].ih_handler = dev->dev_intr[0];
	wthandlers[unit].ih_resolver = i386_resolver;
	wthandlers[unit].ih_rdev = dev;
	wthandlers[unit].ih_hparam[0].intparam = unit;
	wthandlers[unit].ih_stats.intr_type = INTR_DEVICE;
	wthandlers[unit].ih_stats.intr_cnt = 0;
	if ((wthandler_id = handler_add(&wthandlers[unit])) != NULL)
		handler_enable(wthandler_id);
	else
		panic("Unable to add wt tape interrupt handler");
...
register struct buf *bp;
{
	...
	adr1 = pmap_extract(current_thread()->task->map->pmap,bp->b_un.b_addr);
#ifdef DEBUG
	debug("bpaddr %x\n", adr1);
#endif
	ucnt1 = bp->b_bcount;
...
struct uio	*uio;
{
	...
	BUF_LOCK(&rwtbuf);
	error = physio(wtrawio, &rwtbuf, dev, B_READ, wt_minphys, uio);
	BUF_UNLOCK(&rwtbuf);
	return(error);
}
...
struct uio	*uio;
{
	...
	BUF_LOCK(&rwtbuf);
	error = physio(wtrawio, &rwtbuf, dev, B_WRITE, wt_minphys, uio);
	BUF_UNLOCK(&rwtbuf);
	return(error);
}
...
int	dev, flag;
{
	...
	printf("wtopen ...\n");
#endif
        if ((unsigned)unit >= NWT || !wtdev[unit] || !wtdev[unit]->dev_alive)
                return ENXIO;
...
#endif
		delay(10000);	/* Can we do something else here XXX */
	}

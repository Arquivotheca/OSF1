 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: pvinstall.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/19 22:29:28 $";
#endif 
/*
 */
/*
...
struct	lv_installpv	ipv;
	...
	printf("%s: Installing physical %s volume on volume group %s\n",
	       progname, pvol, path);
	 ...
	 */
	if (ioctl(fd, LVM_INSTALLPV, &ipv)) {
		return(report_error(stderr,"attachpv", errno));
	}

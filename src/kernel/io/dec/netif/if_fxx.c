...
/*
#ifndef lint
static char *rcsid = "@(#)$RCSfile: if_fxx.c,v $ $Revision: 1.1.7.9 $ (DEC) $Date: 1993/12/20 21:48:31 $";
#endif
 */
/*
 ...
 */
  ...
  */
/*
 * This file is used to generate both fta.c and faa.c
 * This file is included in both fta.c and faa.c,
 * each of which simply define a few preprocessor symbols
 * before including this file.
 */
#if NFTA > 0 || NFAA > 0 || defined(BINARY)

#include <data/if_fxx_data.c>
...
int	FXX/**/attach(), FXX/**/intr(), FXX/**/output(), FXX/**/probe();
int	FXX/**/init(), FXX/**/ioctl(), FXX/**/reset(), FXX/**/status(), FXX/**/tint();
int	FXX/**/_service_rcv_q(), FXX/**/_service_unsol_q();
int	FXX/**/_service_cmd_rsp_q(), FXX/**/_rcv_mbuf_alloc(), FXX/**/_error_recovery();
short	FXX/**/_cmd_req(), FXX/**/_poll_cmd_req();
#ifdef VFAA
int	FXX/**/intr_0(), FXX/**/intr_1(), FXX/**/intr_2(), FXX/**/intr_3();
#endif
#ifdef SMT
int	FXX/**/_service_smt_q();
#endif /* SMT */

struct  driver FXX/**/driver =
	{ FXX/**/probe, 0 ,FXX/**/attach, 0 , 0, 0, 0,0, ADAP, FXX/**/info };
extern  int ether_output();
u_char  FXX/**/_invalid[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
#define CONVTOPHYS(virtual, physaddr) { \
		if(svatophys((virtual), ((vm_offset_t *)(physaddr))) == KERN_INVALID_ADDRESS) {\
			panic("fxx: CONVTOPHYS: Invalid physical address!\n"); \
		} \
}
...
#ifdef VFAA
#define ENABLE_PROD_INTR (0x80000000)
#endif
#ifdef VFTA
#define ENABLE_PROD_INTR (0x0)
#endif

/*
 * Name:	fxxprobe();
...
{
    struct fxx_softc *sc = &FXX/**/_softc[ctlr->ctlr_num];
...
#ifdef VFAA
#define COBRA_DIV2(x) ((x) >> 1)
/* these are for fbus only, and are below the mode_base about to be added in to basereg*/
/*  So, we compute the addresses prior to basereg += mode_base */
    sc->addr_error_hi   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_ERROR_HI));  
    sc->addr_error_lo   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_ERROR_LO));  
    sc->addr_faddr_hi   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_FADDR_HI));  
    sc->addr_faddr_lo   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_FADDR_LO));  
    sc->addr_bytelane   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_BYTELANE));  
#endif /* VFAA */
#ifdef VFTA
    sc->pdqsw = turbosw;
#endif
...
#ifdef VFAA
/* alpha uses quad words, but cobra uses fbus, which uses longwords */
    sc->addr_portreset   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_PORT_RESET));  
    sc->addr_hostdata    = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_HOST_DATA));
    sc->addr_portctrl    =	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_PORT_CTRL));
    sc->addr_portdataA   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_PORT_DATA_A));
    sc->addr_portdataB   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_PORT_DATA_B));
    sc->addr_portstatus  = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_PORT_STATUS));
    sc->addr_intrtype0   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_HOST_INT_TYPE_0));
    sc->addr_intenbX     = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_HOST_INT_ENB_X));
    sc->addr_type2prod   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_TYPE_2_PROD));
    sc->addr_cmdrspprod  = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_CMD_RSP_PROD));
    sc->addr_cmdreqprod  = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_CMD_REQ_PROD));
    sc->addr_hostsmtprod = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_SMT_HOST_PROD));
    sc->addr_unsolprod   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_UNSOL_PROD));
/* interrupt address and interrupt data are fbus specific */
    sc->addr_intr_addr   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_INTR_ADDR));
    sc->addr_intr_data   = 	(u_int *)(sc->basereg + COBRA_DIV2(BI_KR_INTR_DATA));
/* debug:  printf("faa sc->basereg = %lx\n", sc->basereg); */
#endif /* VFAA */
    /*
	...
	bcopy(FXX/**/_invalid, &sc->is_multi[i][0], 8);
	sc->is_muse[i] = 0;
    }
...
#ifdef __alpha
    /*
     * Enable the the tc option. Otherwise the interrupts from the
     * adapter will not get serviced.
     */
#ifdef VFTA
     tc_enable_option(ctlr);
#endif
#endif /* __alpha */
    return_status = FXX/**/_transition_state(sc, unit, PI_DRIVERINIT);
...
struct controller *ctlr;
{
	...
	u_short voff;
	u_long *newvec;
/* is this necessary? */	vm_offset_t p;
	...
	ifp->if_start = FXX/**/output;
	ifp->if_init = FXX/**/init;
	ifp->if_ioctl = FXX/**/ioctl;
	ifp->if_reset = FXX/**/reset;
	 ...
	 * descriptor blocks.
	 */
	if (FXX/**/_get_physalign_memory(sc) == NULL)
		return(0);
	 ...
	 */
	if (FXX/**/_transition_state(sc, ifp->if_unit, PI_PORTINIT) == NULL) {
...
#endif
#ifdef VFAA
	 ifp->if_version = "DEC DEFAA (PDQ) FDDI Interface";
#endif
	...
	printf("%s%d: %s, Hardware address: %s , Firmware rev: %c%c%c%c \n", 
		...
		fw[3], fw[2], fw[1], fw[0]);
#ifdef VFAA
		/* get 4 scb vectors, aligned on a 4scb block boundary */
	do {
		if(allocvec(1,&newvec) != KERN_SUCCESS) {
			printf("faaattach: unable to allocvec\n");
			return(0);
		}
	}while ((vecoffset(newvec) < 0x900) || (vecoffset(newvec) & 0x30));
	voff = vecoffset(newvec);
	if(allocvec(3,&newvec) != KERN_SUCCESS) {
		printf("faaattach: unable to allocvec\n");
		return(0);
	}
	intrsetvec(voff + 0*sizeof(struct scbentry), &FXX/**/intr_0, ifp->if_unit);
	intrsetvec(voff + 1*sizeof(struct scbentry), &FXX/**/intr_1, ifp->if_unit);
	intrsetvec(voff + 2*sizeof(struct scbentry), &FXX/**/intr_2, ifp->if_unit);
	intrsetvec(voff + 3*sizeof(struct scbentry), &FXX/**/intr_3, ifp->if_unit);

	intreq_reg.function = FBUS_INTREQ_REG;
	intreq_reg.out_flags = 0;
	intreq_reg.in_flags = 0;
	intreq_reg.next_function = NULL;
	intreq_reg.input_data = 0;
	if (get_info(&intreq_reg)) 
		FTAREGWR(sc->addr_intr_addr, intreq_reg.output_data);
	else
		printf("if_fxx attach: Could not get the interrupt request register address.\n");

	FTAREGWR(sc->addr_intr_data, (long)voff);
	FTAIOSYNC();
#endif /* VFAA */
...
/*
 * Name:	fxx_get_physalign_memory(sc);
 *
 * Arguments:
...
FXX/**/intr_0(unit) int unit; { FXX/**/intr(unit); }
FXX/**/intr_1(unit) int unit; { FXX/**/intr(unit); }
FXX/**/intr_2(unit) int unit; { FXX/**/intr(unit); }
FXX/**/intr_3(unit) int unit; { FXX/**/intr(unit); }
/*
 * Name:	fxxintr(unit);
 *
 * Arguments:
...
int unit;
{
    register struct fxx_softc *sc = &FXX/**/_softc[unit];
    struct ifnet *ifp = &sc->is_if;
    int s;
    u_int status_reg;
 ...
 get_out:
	...
	if ((type0_reg & PI_HOST_INT_0_M_PM_PAR_ERR) ||
		...
		printf("%s%d: Halts threshold exceeded.\n",
		       ADAP, ifp->if_unit);	
		FXX/**/_transition_state(sc, unit, PI_BROKEN);
	        splx(s);
		return(0);
	    }
	    thread_wakeup((vm_offset_t)&sc->error_recovery_flag);
	...
	struct fxx_softc *sc = &FXX/**/_softc[unit];
	/*
 ...
 *	fxx_cmd_req();
 *	fxx_transition_state();
 *	fxx_print_comp_status();
 *
 * Return Codes:
 *	
 */
FXX/**/ioctl(ifp, cmd, data)
...
caddr_t data;
{
    struct fxx_softc *sc = &FXX/**/_softc[ifp->if_unit];
    struct protosw *pr;
    ...
    switch (cmd) {
	  ...
	  if (ifp->if_flags & IFF_RUNNING) {
	      ...
	      if (!FXX/**/_transition_state(sc, ifp->if_unit, PI_PORTINIT)) {
	       ...
	       */
	      FXX/**/init(ifp->if_unit);
	      if (FXX/**/_set_initial_chars(ifp->if_unit) == NULL) 
		      return(EIO);
	  }
	  ...
	  if (sc->rsp_thread_started == TRUE) {
	      retval = FXX/**/_cmd_req(cmdbuf,
				   sc,
				   PI_CMD_K_CHARS_SET);
	  } else {
	      retval = FXX/**/_poll_cmd_req(cmdbuf,
					sc,
					PI_CMD_K_CHARS_SET);
	  }
	  if (retval == NULL) {
	  ...
	  if (((PI_RSP_HEADER *)cmdbuf->rsp_buf)->status != PI_RSP_K_SUCCESS) {
	      ...
	      printf("%s%d: Enable Internal Loop Back Failed.\n",
		     ADAP, ifp->if_unit);
	      FXX/**/_print_comp_status(((PI_RSP_HEADER *)cmdbuf->rsp_buf)->status);
	  ...
	  kmem_free(kernel_map, 
		    data, 
		    sizeof(PI_CMD_CHARS_SET_REQ));
	  if (!FXX/**/_transition_state(sc, ifp->if_unit, PI_OPERATIONAL)) {
	...
	return_value = FXX/**/_addr_filter_set(sc, 
					   PDQ_ADD_ADDR, 
	...
	if (sc->driver_state != PI_UPGRADE || 
	    ...
	    switch(ctr->ctr_type) {
	      ...
	      case FDDIMIB_PORT:
	      case FDDIMIB_ATTA:
		if (FXX/**/_fmib_fill(sc, ctr, ctr->ctr_type) == NULL)
			return(EIO);
		break;
		...
		if (FXX/**/_smtmib_fill(sc, ctr, ctr->ctr_type) == NULL)
			return(EIO);
		break;
		
	      case FDDIDECEXT_MIB:
		if (FXX/**/_get_decext_mib(ifp->if_unit) != ESUCCESS)
			return(EIO);
		if (sc->dec_ext != NULL) {
		...
		break;
	      ...
	      case FDDI_STATUS:
		return(FXX/**/_get_status(sc, &ctr->sts_fddi));
		break;
	      default:
	      case CTR_FDDI:
		ctr->ctr_type = CTR_FDDI;
		FXX/**/_get_cntrs(sc, &ctr->ctr_fddi);
		break;
	    }
	} else {
	    return(EACCES);
	...
	break;
	  ...
	  if (FXX/**/debug > 1)
		  printf("SIOCIFRESET\n");
	  if (FXX/**/_reinitialize(ifp, TRUE) == EIO)
		  return(EIO);
	  break;
      }
...
#define SWAP_INT(x)  ((x & 0xff000000)>>24)|((x & 0x00ff0000)>>8) \
	...
	if (PI_STATE_K_UPGRADE != ((FTAREGRD(sc->addr_portstatus) & 
	    ...
	    simple_lock(&sc->xmt_indexes_lock);
	    simple_lock(&sc->counter_update_lock);
	    if (FXX/**/_transition_state(sc, ifp->if_unit, PI_UPGRADE) == NULL) {
	...
	for (i = ife->ife_blklen, k = 0; i > 0; i = i - 8, k = k + 8) {
		...
		if (count++ > 1000) {
		    ...
		    FXX/**/_transition_state(sc, ifp->if_unit, PI_BROKEN);
		    splx(s);
		    return(EIO);
		}	
	    }
...
int unit;
{
    struct fxx_softc *sc = &FXX/**/_softc[unit];
    struct ifnet *ifp = &sc->is_if;
    FXX/**/_reinitialize(ifp, TRUE);
}


/*
 * Name:	fxx_reinitialize();
 *
 * Arguments:
 *	ifp:	The ifnet pointer.
 *	reset_only: Is it only a reset only or is it re-allocating of all
 ...
 *
 */
FXX/**/_reinitialize(ifp, reset_only)
struct ifnet *ifp;
int reset_only;
{
    struct fxx_softc *sc = &FXX/**/_softc[ifp->if_unit];
    ...
    if (FXX/**/_transition_state(sc, ifp->if_unit, PI_DRIVERINIT) == NULL) {
	printf("%s%d: Could not initialize network adapter.\n",
	       ADAP, ifp->if_unit);
	simple_unlock(&sc->counter_update_lock);
	simple_unlock(&sc->xmt_indexes_lock);
 ...
 *
 * Functional Description:
 *	It schedules read status command which updates the fxx_status 
...
int unit;
{
    ...
    if (FTAREGRD(sc->addr_portctrl) & PI_PCTRL_M_HALT) {
    	if (sc->fxx_status == NULL)
	    return(EIO);
        s = splimp();
        sc->fxx_status->phy_led[0] = 2;   /* Red */
        sc->fxx_status->link_state = 1;   /* Off Ready */
        sc->fxx_status->dup_addr_flag = 0;/* Unknown */
        sc->fxx_status->purger_state = 0; /* Purger Off */
        sc->fxx_status->phy_state[0] = 2; /* Off Ready */
	if (sc->dec_ext == NULL) {
	    splx(s);
	    return(EIO);
	}
    ...
    if (((PI_RSP_HEADER *)cmdbuf->rsp_buf)->status != PI_RSP_K_SUCCESS) {
	printf("%s%d: %sstatus() - Can not get status characteristics\n",
	       ADAP, ifp->if_unit, ADAP);
	kmem_free(kernel_map, req_buff, sizeof(NODATA_CMD));
	kmem_free(kernel_map, cmdbuf->rsp_buf, PAGESZ);/*SNU*/
	kmem_free(kernel_map, cmdbuf, sizeof(struct cmd_buf));
	return(EIO);
     ...
     */
    s = splimp();
    if (sc->fxx_status != NULL)  /* this is not the first update */
	kmem_free(kernel_map, sc->fxx_status, PAGESZ);/*SNU to be changedto 512*/
    sc->fxx_status = (PI_CMD_STATUS_CHARS_GET_RSP *)(cmdbuf->rsp_buf);
    splx(s);
    kmem_free(kernel_map, req_buff, sizeof(NODATA_CMD));
    kmem_free(kernel_map, cmdbuf, sizeof(struct cmd_buf));
     ...
     */
    if (FXX/**/_get_decext_mib(unit) != ESUCCESS)
...
int unit;
{
    ...
    if (sc->rsp_thread_started == TRUE) {
	retval = FXX/**/_cmd_req(ext_cmdbuf,
			     sc,
			     PI_CMD_K_DEC_EXT_MIB_GET);
    } else { /* we poll for a response */
	retval = FXX/**/_poll_cmd_req(ext_cmdbuf,
				  sc,
				  PI_CMD_K_DEC_EXT_MIB_GET);
    }
    if (retval == NULL) {
...
struct fxx_softc *sc;
{
	...
	printf("%s_selftest begin adap_stat=%x\n", ADAP, adap_state);
    }
#endif
    /*
	...
	if (count > 10000) { /* we couldn't initialize the adapter */
	    ...
	    FXX/**/_print_halt_reason(sc, halt_reason); 
	    return(0);
    	} else {
	    continue;
        }
 ...
 *      none.
 */
FXX/**/_print_halt_reason(sc, reason)
struct fxx_softc *sc;
short reason;
{
    struct ifnet *ifp = &sc->is_if;
...
short state;
{
...
#ifdef DEBUG
	if (FXX/**/debug > 1)
		printf("transition state to PI_DRIVERINIT\n");
#endif
	/*
	 ...
	 */
 	 ...
 	 */
 	FXX/**/_set_dma_burst_size(sc, PI_PDATA_B_DMA_BURST_SIZE_16);
	sc->prev_state = sc->driver_state;
	sc->driver_state = PI_DRIVERINIT;
	return(sizeof(struct fxx_softc));
	break;
...
#ifdef DEBUG
	  if (FXX/**/debug > 1)
		printf("transition state to PI_PORTINIT\n");
#endif
	...
	if (((status_reg & PI_PSTATUS_M_STATE) >> PI_PSTATUS_V_STATE) != 
...
#ifdef DEBUG
	  if (FXX/**/debug > 1)
		printf("transition state to PI_OPERATIONAL\n");
#endif
	  ...
	  if (((PI_RSP_HEADER *)cmdbuf->rsp_buf)->status != PI_RSP_K_SUCCESS){
	      printf("%s%d: %s_transition_state() - Start Command Failed.\n",
			ADAP, ifp->if_unit, ADAP);
	      kmem_free(kernel_map, cmdbuf->rsp_buf, PAGESZ);/*SNU*/
	      kmem_free(kernel_map, cmdbuf, sizeof(struct cmd_buf));
	      kmem_free(kernel_map, req_buff, sizeof(NODATA_CMD));
...
#ifdef DEBUG
	  if (FXX/**/debug > 1)
		printf("transition state to PI_UPGRADE\n");
#endif

	  /*
	   * Bring the adapter into DMA Unavailable state.
	   */
	  if (FXX/**/_reinitialize(ifp, PI_UPGRADE) != ESUCCESS)
		  return(0);
	  /*
...
int unit;
{
     ...
     * 6) Host SMT queue
     *
     * "fxx_rcv_descr_init()" initializes descriptors of all the receive
     * queues - rcv data queue, command response queue, unsolicited
     * queue and smt host queue.
     *
     * "fxx_xmt_descr_init()" initializes descriptors for the command
    ...
    FXX/**/_xmt_descr_init(sc->cmd_req_blk, NPDQCREQ);

    /* Allocate the buffers */
    FXX/**/_alloc_buffs(unit);
    ifp->if_flags |= IFF_RUNNING;
...
FXX/**/_error_recovery()
{
     ...
     */
    sc = (struct fxx_softc *)thread->reply_port;
    ifp = &sc->is_if;
	...
	thread_block();
	FXX/**/_reinitialize(ifp, FALSE);
	if (FXX/**/_transition_state(sc, ifp->if_unit, PI_OPERATIONAL) == NULL) {
	...
	if (q_type == SMT_Q) {
		...
		FTAREGWR((sc->addr_hostsmtprod), 
			 (FTAREGRD(sc->addr_hostsmtprod) & 
			  ~PI_TYPE_1_PROD_M_PROD) | ENABLE_PROD_INTR |
 ...
 *	in the fxxinit routine. It normally is unable to allocate all the
 *	mbufs required.
 *
 * Calls:
 *	None.
 ...
 *	      yet available. Once the system is up, fxx_cmd_req() is used.
 *
 * Calls:
 *	None.
 * 
...
register short command;
{
	...
	break;
      ...
      default:
	printf("%s_cmd_req: Unknown command requested.\n", ADAP);
	return(NULL);
	break;
    }
...
FXX/**/_service_cmd_rsp_q()
{
	...
	while (cmdrsp_cindex != sc->cons_rsp_index) {
		 ...
		 * up. In this case we just go back to sleep.
		 */
...
register int unit;
{
...
#ifdef DEBUG
	printf("unsol_report->event_type = %d\n",unsol_report->event_type );
#endif
	...
	if (unsol_report->event_type == PI_UNSOL_TYPE_K_EVENT_REPORT) {
	    ...
	    FXX/**/print_event_args(unsol_report, sc);
	    /*
    ...
    FTAREGWR((sc->addr_unsolprod),
	     ((FTAREGRD(sc->addr_unsolprod) &  ~PI_TYPE_1_PROD_M_PROD) | 
	      (unsol_pindex << PI_TYPE_1_PROD_V_PROD)) | ENABLE_PROD_INTR );
    FTAIOSYNC();
    FTAREGWR((sc->addr_unsolprod), 
	     ((FTAREGRD(sc->addr_unsolprod) &  ~PI_TYPE_1_PROD_M_COMP) | 
	      (unsol_cindex << PI_TYPE_1_PROD_V_COMP)) | ENABLE_PROD_INTR );
    FTAIOSYNC();
}
...
register struct ifnet *ifp;
{
	...
	if(
	 (cpu == DEC_7000) &&
	 (physaddr&3)  && (frag_len = 4-(physaddr&3)) && (m->m_len > frag_len)
	) { /* if unaligned data */
		/* This code is the software workaround
		   for the DEFAA data corruption problem of unaligned data.
		   This hardware bug should be fixed in a later hardware rev.
		   On the FLAG adapter (DEC_7000 systembus-to-fbus),
		   Leading unaligned data can be safely handled only
		   in amounts of 3bytes or less.
		   An extra descriptor is used for the leading 1,2,or3 bytes.
		 */
	    u_long hi_boundary;
		...
		xmt_blk->long_1 = (frag_len) << PI_XMT_DESCR_V_SEG_LEN;
		...
		xmt_blk->buff_lo = (u_int)physaddr + frag_len;
		xmt_blk->long_1 = (m->m_len - frag_len) << PI_XMT_DESCR_V_SEG_LEN;
		xmt_blk->long_1 |= (u_int)(((physaddr + frag_len) >> 32) & PI_XMT_DESCR_M_BUFF_HI);
		/* save the address for later use, while freeing up mbufs */
		...
		if (physaddr2 == physaddr + first_page_len) {
		    /* This is 2 contiguous pages, with fragment in another page up front */
		    xmt_blk->buff_lo = (u_int)physaddr;
		    xmt_blk->long_1 = (frag_len) << PI_XMT_DESCR_V_SEG_LEN;
		    xmt_blk->long_1 |= (u_int)((physaddr >> 32) & PI_XMT_DESCR_M_BUFF_HI);
		    xmt_blk->long_1 |=  PI_XMT_DESCR_M_SOP;
		    ...
		    /*
		     * We have to produce 3 descriptors, for this mbuf,
			...
			/* If fragment is the only thing on first page,
			   skip the second descriptor of the first page. */
			if( first_page_len != frag_len) {
			  xmt_blk->buff_lo = (u_int)physaddr + frag_len;
			  xmt_blk->long_1 = (first_page_len - frag_len) << PI_XMT_DESCR_V_SEG_LEN;
			  xmt_blk->long_1 |= (u_int)(((physaddr + frag_len) >> 32) & PI_XMT_DESCR_M_BUFF_HI);
			  sc->xmt_vaddr[xmt_pindex].vaddr = NULL;
			  xmt_pindex = (xmt_pindex + 1) & NPDQXMT_MASK;
	...
	} /* end of else clause of 'if unaligned data' */
#else
	{
	    u_int hi_boundary;
	    vm_offset_t physaddr2;
	...
	while (m) {
		 ...
		 * This algorithm should be re-written for the
		 * MIPS platform.
		 */
...
short unit;
{
	...
	if(!(PI_FMC_DESCR_M_RCC_FLUSH & fmc_descr)) {
		...
		if (mp) { /* There is mbuf available */
		    ...
		    FTAIOSYNC();
    		    simple_unlock(&sc->alloc_mbufs_lock);
		    FXX/**/read(sc, mp, len); /* pass the packet up */
		    bp[rcv_cindex].phymbuf = 0;
		    bp[rcv_cindex].rmbuf = NULL;
		    bp[rcv_cindex].mdata = NULL;
		 ...
		 */
		ifp->if_flags &= ~IFF_UP;
		printf("%s%d: Network Adapter halted for re-initialization.\n",
		       ADAP, ifp->if_unit);	
		if (++(sc->halt_count) > MAX_HALTS) {
		    printf("%s%d: Halts threshold exceeded.\n",
			   ADAP, ifp->if_unit);	
...
/** NOTE: This code is untested and uncompiled. In essence this is identical
          to the fxx_service_rcv_q() routine.***********/
/** NOTE: References to registers will need to be changed to use FTAREGWR/RD */
/*
 * Name:	fxx_service_smt_q()
 *
 * Arguments:
 *  	unit:		The unit to recive data on.
 *			
 ...
 * to fxx_service_rcv_q(), but has been seperated out to make things
 * work faster.
...
short unit;
{
	...
	if(!(PI_FMC_DESCR_M_RCC_FLUSH & fmc_descr.descr)) {
		...
		*(sc->addr_hostsmtprod) = 
		 ...
		 * RCC is zero. Check for other problems.
		 * CHECK FSC before doing anything!!
		 * This part has to be re-written ***SNU***
		 */
...
int type;
{
    ...
    if (((PI_RSP_HEADER *)cmdbuf->rsp_buf)->status != PI_RSP_K_SUCCESS){
	printf("%s%d: %s_fmib_fill() - Can't get MIB variables.\n",
			ADAP, ifp->if_unit, ADAP);
	kmem_free(kernel_map, cmdbuf->rsp_buf, PAGESZ);/*SNU*/
	kmem_free(kernel_map, cmdbuf, sizeof(struct cmd_buf));
	kmem_free(kernel_map, req_buff, sizeof(NODATA_CMD));
	...
	bcopy((u_char *)(&mib_value->smt_station_id.octet_3_0), 
			&fmib->fmib_smt.smt_stationid[0],4);
	bcopy((u_char *)(&mib_value->smt_station_id.octet_7_4), 
			&fmib->fmib_smt.smt_stationid[4],4);
	...
	bcopy(&(((u_char *)(&sc->fxx_status->una))[0]),
	      &fmib->fmib_mac.mac_upstream[0],6);
	bcopy(&(((u_char *)(&sc->fxx_status->una_old))[0]), 
 ...
 *	None.
 */
FXX/**/_smtmib_fill(sc, mibreq, type)
register struct fxx_softc *sc;
...
u_short type;
{
    ...
    if (((PI_RSP_HEADER *)cmdbuf->rsp_buf)->status != PI_RSP_K_SUCCESS){
	printf("%s%d: %s_smtmib_fill() - Can't get MIB variables.\n",
			ADAP, ifp->if_unit, ADAP);
	kmem_free(kernel_map, cmdbuf->rsp_buf, PAGESZ);/*SNU*/
	kmem_free(kernel_map, cmdbuf, sizeof(struct cmd_buf));
	kmem_free(kernel_map, req_buff, sizeof(NODATA_CMD));
...
struct fstatus *ctr;
{
     ...
     */
    if ((return_value = FXX/**/status(ifp->if_unit)) != ESUCCESS)
	    return(return_value);
...
struct fstat *ctr;
{
	...
	(cntrs->user_buff_unavailable.ls & 0xffff0000))
	    ...
	    ctr->fst_sysbuf = fstc_sysbuf = *(u_short*)(&cntrs->user_buff_unavailable.ls);
...
u_char *e_addr;
{
    ...
    switch(cmd) {
	  ...
	  for (i = PI_CMD_ADDR_FILTER_K_FIRST + 1; 
	       ...
	       * case we need it.
	       */
	      if ((j < 0) && (bcmp(sc->is_multi[i], FXX/**/_invalid, 8) == 0))
		      j = i;
	  }
  ...
  add:
	  ...
	  if ((i < PI_CMD_ADDR_FILTER_K_SIZE - 1) && (--sc->is_muse[i] == 0)) {
		  bcopy(FXX/**/_invalid, sc->is_multi[i], 8);
		  sc->is_inuse--;
	  } else {
	      splx(s);
	      return(EINVAL);
 ...
 * all the buffers have to freed before they get re-initialized.
 *
 * Calls:
 * 	None.
 *
...
int unit;
{
    ...
    sc->fxx_debug.cmdcnt.set_addr_filter++;
	 ...
	 * adapter currently.
	 */
	if (FXX/**/status(ifp->if_unit) == EIO) {
	    sc->char_val.valid = 0;
	    printf("%s%d: Can not get characteristic values.\n",
		   ADAP, ifp->if_unit);
	} else if (sc->fxx_status != NULL) {
	    /*
	     * Copy in the default characteristic values.
	     */
	    sc->char_val.treq = sc->fxx_status->t_req;
	    sc->char_val.tvx = sc->fxx_status->tvx;
	    sc->char_val.rtoken = sc->fxx_status->token_timeout;
	    sc->char_val.ring_purger = sc->fxx_status->purger_enb;
	    sc->char_val.cnt_interval = sc->fxx_status->cntr_interval;
	    sc->char_val.lem0 = sc->fxx_status->link_error_est[0];
	    sc->char_val.loopback = sc->fxx_status->loopback;
	    if (sc->dec_ext == NULL)
	...
	if (sc->char_val.valid) {
		...
		printf("%s%d: Can not set characteristic value.\n",
		       ADAP, ifp->if_unit);
		FXX/**/_print_comp_status(((PI_RSP_HEADER *)rsp_buf)->status);
		kmem_free(kernel_map, cmdbuf->rsp_buf, PAGESZ);/*SNU*/
		kmem_free(kernel_map, cmdbuf, sizeof(struct cmd_buf));
		kmem_free(kernel_map, 
			  char_set, 
 ...
 *	Failure:	0
 */
FXX/**/_set_dma_burst_size(sc, size)
struct fxx_softc *sc;
int size;
{
    int count;
...
#ifdef VFAA
    /* DEFAA requires setting burst size by writing to logical module control
       register (addr base+516) */
    /* printf("faa_set_dma_burst_size: warning not setting burst size yet\n"); */
    return(1);
#endif
...
#ifdef DEBUG
	printf("portctrl returns %x\n", FTAREGRD(sc->addr_portctrl));
#endif
 	return(0);
    } 
    return(1);
	...
	/* This code is associated with software workaround
	   with the DEFAA data corruption problem of unaligned data.
	   This hardware bug should be fixed in a later hardware rev. */
int sys_uac;
 /* For testing purposes, create unaligned data by shifting the data. */
fxx_debug_shift_mdata(m, shift_down)
struct mbuf *m;
int shift_down;
{
	caddr_t cp;
	int count;
	sys_uac = 0;
	if(M_TRAILINGSPACE(m) >= shift_down) {
		for(cp = (caddr_t)m->m_data + m->m_len - 1 , count = m->m_len;
			count; cp--, count--) 
				*(cp+shift_down) = *cp;
		m->m_data += shift_down;
	}
}
#endif /* DEBUG_FRAGMENT */

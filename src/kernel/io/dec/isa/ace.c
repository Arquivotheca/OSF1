 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: ace.c,v $ $Revision: 1.1.6.2 $ (DEC) $Date: 1993/06/24 22:40:27 $";
#endif
...
#define NO_PROM_PUTS	/* won't be needed when a real console exists
			 * with a real callback for PUTS.  In the meantime
			 * a hack routine xxputs() is used here. */
...
int	acestart(), acexint(), ace_rint(), aceparam(); 
int	ttrstrt();
...
#ifdef NO_PROM_PUTS
#define	LOGGING_COM_BASE	ACE_COMMA_BASE
#endif /* NO_PROM_PUTS */
...
#define ACE_READ(base, off, val)   val = inVti((base)+(off))
#define ACE_WRITE(base, off, val)  outVti((base)+(off),(val))
...
int ctrl;
{
    /* the intialization is done through ace_cons_init, so
...
int ctrl;
{
    ...
    acesoftCAR = 0x03;		/* Defaults line to no-modem */
    acedefaultCAR = 0x03;	/* Defaults line to no-modem */
    for (i = 0; i < NACELINE; i++) {
	    ace_brk[i] = 0;
	    queue_init(&ace_tty[i].t_selq);

	    /* If kdebug is using the non-console tty port don't touch it. */
	    if ((kdebug_state() == 1) && (i == ACE_KDEBUG_UNIT))
		continue;
...
int flag;
{
  ...
  if ((tp->t_state & TS_BUSY) == 0) { 
    ACE_WRITE( rsp, ACE_IE, ACE_IE_RDINT | ACE_IE_RLSINT);
  }
 ...
 * corresponding to lines 0 & 1.  This rouine determines the source of
 ...
 */
/*
 * NOTE: this routine gets called for interrupts on either line.  Newer
 * versions of the PAL will vector separate interrupt service routines.
 * In that case the parameter should be the controller number.  Use that to
 * remove the need for some of the tests here which differentiate which line
...
u_int ctrl; /*NOTUSED*/
{
    register u_long rsp;
    register struct ace_softc *sc = acesc;
    register int unit0 = 0;    
    register int unit1 = 1;
    register int intr_id0;
    register int intr_id1;

    /* first who interrupted? check line 0 first */

    rsp = sc->ace_regs[unit0];
    ACE_READ( rsp, ACE_IIR, intr_id0 );
    rsp = sc->ace_regs[unit1];
    ACE_READ( rsp, ACE_IIR, intr_id1 );
    ...
    if( !IP( intr_id0 ) ){
        /* check line 1 */
	if( !IP( intr_id1 ) ){
	   printf("about to panic, id0 %x id1 %x\n",
		intr_id0,intr_id1);
	   panic( "spurious interrupt in aceintr\n" );
	 }
    }
     ...
     */
   while( IP( intr_id0 ) || IP(intr_id1) ){
     if( RLS_INT( intr_id0 ) ){		/* Receiver line status */
         acerint( unit0 );
     }
     if( RDA_INT( intr_id0 ) ){		/* Received Data Available */
         acerint( unit0 );
     }
     if( THRE_INT( intr_id0 ) ){	/* Ready to transmit */
         acexint( unit0 );
     }
     if( MS_INT( intr_id0 ) ){		/* Modem status interrupt */
         ace_modem_int( unit0 );
     }

     if( RLS_INT( intr_id1 ) ){		/* Receiver line status */
         acerint( unit1 );
     }
     if( RDA_INT( intr_id1 ) ){		/* Received Data Available */
         acerint( unit1 );
     }
     if( THRE_INT( intr_id1 ) ){	/* Ready to transmit */
         acexint( unit1 );
     }
     if( MS_INT( intr_id1 ) ){		/* Modem status interrupt */
         ace_modem_int( unit1 );
     }
...
{	
     ...
     * is empty.
     */
    ACE_READ( rsp, ACE_IE, c_mask );
    c_mask |= (ACE_IE_RDINT | ACE_IE_THREINT | ACE_IE_RLSINT);
    ACE_WRITE( rsp, ACE_IE, c_mask);
...
#ifdef NO_PROM_PUTS
/*
 * This routine (xxputs) is currently called from the prom_puts routine from
 * arch/alpha/pc_prom.c.  Instead of calling the conventional console routine
 * this gets called.  Consequently all the stuff here labeled with the
 * #ifdef NO_PROM_PUTS can go away once a real console PUTS is in place.
 */
xxputs( char *c )
{
    while( *c ) wputc(*c++ );
 ...
 * ace_putc.
 *
 * This routine is called from pc_prom.c to perform the function that
 * is supposed to be done with a call to PROM_PUTC.  So until we have a
 * real console to call back to this routine will be used.
 * Once printstate gets set appropriately this routine shouldn't get called.
 * The ace_putc routine is used later in the game.
 */
w_putc(unit, c)
     int unit;
     register int c;
{
    ...
    register int timo = 1000000;
        ...
        /* wait for empty transmit hold register */
        ACE_READ( LOGGING_COM_BASE, ACE_LSR, status );
        while( (status&ACE_LSR_THRE)  == 0 ){
	  ACE_READ( LOGGING_COM_BASE, ACE_LSR, status );
		    if (--timo == 0) {
			panic("w_putc: timo expired.\n");
		    }
	}

        /* write data to transmit register */
        ACE_WRITE( LOGGING_COM_BASE, ACE_THR, c );
}
#endif /* NO_PROM_PUTS */

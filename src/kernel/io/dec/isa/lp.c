 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: lp.c,v $ $Revision: 1.1.5.2 $ (DEC) $Date: 1993/06/24 22:41:06 $";
#endif
...
#define LPT_BASE_ADDR	0x3BC		/* CSR Base Address - note this can
...
/*
 * Macros used to access the device registers.
 */
#define LPT_READ(regname, value) \
	value = inVti((LPT_BASE_ADDR) + (regname))
#define LPT_WRITE(regname, value) \
	outVti((LPT_BASE_ADDR) + (regname), (value))
/*
...
{
#ifdef JENSEN_LPT
	/* Since the parallel port on Jensen is in the combo chip which is
	 * present on every system its a safe bet to return a status indicating
	 * that it exists.
	 */
	return(1);
#endif /* JENSEN_LPT */
	...
	struct controller *ctlr;
{
...
#ifdef JENSEN_LPT
	if (unit != 0) {
		printf("lpattach: invalid unit %d.\n",unit);
		return(0);	
	}
#endif /* JENSEN_LPT */
	lpc = &lp_ctrl[unit];
	...
	/* An afterthought - a potential problem with initializing
	 * the printer in the attach routine is that suppose the printer is
	 * powered-down when the system boots up.  Will it still work if you
	 * turn it on after probe time?  A little testing could easily answer
	 * this question.
	 */
	/* 
	...
	int flag;
{
	...
	if ((unit >= NLP) || ((lpc->flags & LPF_OPEN) == 0))
		return(ENXIO);

#ifdef 0
	lpout(dev,"\f",1);	/* not sure if really needed */
#endif /* 0 */

 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: cam_special.c,v $ $Revision: 1.1.7.2 $ (DEC) $Date: 1993/12/15 20:50:34 $";
#endif
 ...
 *	In the scmn_FindCmdEntry() function, do not save a pointer to	*
 *	the command table located in the special argument block sa_sph	*
 *	field.  This field is used to specify a specific command table	*
 *	to search, rather than searching all command tables.  By saving	*
 *	this pointer, if the caller did not reset this field to zero,	*
 *	then later commands would fail since the initial command table	*
 *	was always being searched (but no others).			*
 *									*
 ...
 */
extern void scmn_SpecialInit(), scmn_AddSpecialCmds(), scmn_SpecialCleanup();
static int scmn_SetupSpecial(), scmn_CheckCmdAccess(), scmn_FinishSpecial();
static int scmn_ccbwait();
...
struct special_args *sap;
{
  ...
  ("[%d/%d/%d] scmn_FreeArgsBuffer: Freeing args buffer at 0x%lx of %d bytes.\n",
	...
	ccmn_rel_dbuf (sap, (U32) sizeof(struct special_args));
	return (SUCCESS);
}

/************************************************************************
...
("[%d/%d/%d] scmn_SpecialCmd: cmd_flags = 0x%x, user_buffer = 0x%lx, user_length = 0x%d\n",
	...
	/*
	 * Since the CCB wait function requires a peripheral driver structure
	 * and a pointer to a peripheral working set, check for these unless
	 * the user has specified no callback or no waiting (caller will do).
	 * [ NOTE:  These always exist if our ccmn_* functions are used. ]
	 */
	if ( ((sap->sa_flags & SA_NO_WAIT_FOR_IO) == 0) &&
	     ((ccb->cam_ch.cam_flags & CAM_DIS_CALLBACK) == 0) ) {
	    register PDRV_WS *pws = (PDRV_WS *)ccb->cam_pdrv_ptr;
	    if ( (pws == NULL) || (pws->pws_pdrv == NULL) ) {
		scmn_SpecialCleanup (sap);
		PRINTD (sap->sa_bus, sap->sa_target, sap->sa_lun, CAMD_CMD_EXP,
    ("[%d/%d/%d] scmn_SpecialCmd: Peripheral WS & driver structures are required.\n",
			sap->sa_bus, sap->sa_target, sap->sa_lun, spc->spc_cmdp));
		return (EINVAL);
	    }
	}
...
register struct special_args *sap;
{
    ...
    ("[%d/%d/%d] scmn_SpecialCleanup: Freeing data buffer at 0x%lx of %d (0x%x) bytes.\n",
		sap->sa_bus, sap->sa_target, sap->sa_lun,
	        sap->sa_data_buffer, sap->sa_data_length, sap->sa_data_length));
	   ccmn_rel_dbuf (sap->sa_data_buffer, (U32)sap->sa_data_length);
    ...
    ("[%d/%d/%d] scmn_SpecialCleanup: Freeing sense buffer at 0x%lx of %d (0x%x) bytes.\n",
		sap->sa_bus, sap->sa_target, sap->sa_lun,
	        sap->sa_sense_buffer, sap->sa_sense_length, sap->sa_sense_length));
	    ccmn_rel_dbuf (sap->sa_sense_buffer, (U32)sap->sa_sense_length);
...
("[%d/%d/%d] scmn_SpecialCleanup: Freeing I/O parameters buffer at 0x%lx of %d (0x%x) bytes.\n",
		sap->sa_bus, sap->sa_target, sap->sa_lun,
		sap->sa_iop_buffer, sap->sa_iop_length, sap->sa_iop_length));
	    ccmn_rel_dbuf (sap->sa_iop_buffer, sap->sa_iop_length);
	    sap->sa_iop_length = 0;
  ...
  ("[%d/%d/%d] scmn_SetupSpecial: Allocated I/O parameters buffer at 0x%lx of %d (0x%x) bytes.\n",
	...
	if (sp->sp_sense_length && sp->sp_sense_buffer) {
	    if ( ((sap->sa_flags & SA_SYSTEM_REQUEST) == 0) &&
		 (!CAM_IS_KUSEG(sp->sp_sense_buffer)) ) {
		PRINTD (sap->sa_bus, sap->sa_target, sap->sa_lun, CAMD_ERRORS,
	...
	if (sp->sp_user_length && sp->sp_user_buffer) {
	    if ( ((sap->sa_flags & SA_SYSTEM_REQUEST) == 0) &&
		 (!CAM_IS_KUSEG(sp->sp_user_buffer)) ) {
		PRINTD (sap->sa_bus, sap->sa_target, sap->sa_lun, CAMD_ERRORS,
...
("[%d/%d/%d] scmn_CheckCmdAccess: sap = 0x%lx, spc = 0x%lx, file_flags = 0x%x\n",
	 ...
	 */
#if	SEC_BASE
	if ( (sap->sa_cmd_flags & SPC_SUSER) && !privileged(SEC_FILESYS, 0) )
	    return (EACCES);

#else	/* SEC_BASE */
	if ( (sap->sa_cmd_flags & SPC_SUSER) && suser(u.u_cred, &u.u_acflag) )
	    return (EACCES);
...
register CCB_SCSIIO *ccb;
{
	...
	switch (ccbh->cam_status&CSM) {
		...
		if (mpsleep((caddr_t)ccb, priority, "Zzzzzz",0,(void *)0,0)) {
		    return (EINTR);
		} else {
		    return (RETRYABLE);
		}
...
register struct special_args *sap;
{
	...
	/*
	 * Start the I/O request via the driver start routine.
	 * We expect the CAM status normally returned by xpt_action().
	 */
	cam_status = (*sap->sa_start)(ccb);

	/*
	 * Return now if we're not waiting or callback is disabled.
	 */
	if ( (sap->sa_flags & SA_NO_WAIT_FOR_IO) ||
	     (ccbh->cam_flags & CAM_DIS_CALLBACK) ) {
	    return (status);
	}

	/*
	 * If the request never got started, then we don't wait for the
	 * CCB, since the callback and subsequent wakeup won't happen.
	 */
	if (cam_status == CAM_REQ_INPROG) {
	    ...
	    if ((status = scmn_ccbwait (ccb, priority)) == SUCCESS) {
	...
	} /* End 'if (cam_status == CAM_REQ_INPROG) {' */
...
register CCB_SCSIIO *ccb;
{
	...
	if ( (accb->cam_ch.cam_status&CSM) != CAM_REQ_CMP) {
	     ...
	     */
	    status = scmn_ccbwait (ccb, priority);
	}
...
struct special_args *sap;
{
	...
	ccb->cam_dxfer_len = (U32) sap->sa_user_length;
	ccb->cam_sense_ptr = (u_char *) sap->sa_sense_buffer;

	/*
	 * Ensure the callback flag is reset, in case retrying.
	 */
	((PDRV_WS *)ccb->cam_pdrv_ptr)->pws_flags &= ~PWS_CALLBACK;
 ...
 * command which involves data movement to a user data or sense buffer,	*
...
/************************************************************************
 *
 *  ROUTINE NAME: scmn_ccbwait()
 *
 *  FUNCTIONAL DESCRIPTION:
 *	This function waits for a SCSI I/O CCB request to complete.
 *
 *  FORMAL PARAMETERS:
...
int priority;
{
	...
	PRINTD(ccb->cam_ch.cam_path_id, ccb->cam_ch.cam_target_id, 
		...
		("[%d/%d/%d] scmn_ccbwait: ENTER - CCB = 0x%lx, priority = 0x%x\n",
		ccb->cam_ch.cam_path_id, ccb->cam_ch.cam_target_id,
	...
	while ( (CAM_STATUS(ccb) == CAM_REQ_INPROG) ||
		((pws->pws_flags & PWS_CALLBACK) == 0) ) {
	    if (priority > PZERO)  {
		if (PDRV_SMP_SLEEPUNLOCK(ccb, (priority | PCATCH), pd))  {
		    PDRV_LOWER_IPL(spl);
		    PRINTD(ccb->cam_ch.cam_path_id, ccb->cam_ch.cam_target_id, 
			ccb->cam_ch.cam_target_lun, (CAMD_COMMON | CAMD_INOUT),
		("[%d/%d/%d] scmn_ccbwait: interrupted sleep for CCB 0x%lx\n",
			ccb->cam_ch.cam_path_id, ccb->cam_ch.cam_target_id,
			ccb->cam_ch.cam_target_lun, ccb));
		    return(EINTR);
		}
	...
	PRINTD(ccb->cam_ch.cam_path_id, ccb->cam_ch.cam_target_id, 
		ccb->cam_ch.cam_target_lun, (CAMD_COMMON | CAMD_INOUT),
		("[%d/%d/%d] scmn_ccbwait: EXIT - SUCCESS\n",
		ccb->cam_ch.cam_path_id, ccb->cam_ch.cam_target_id,

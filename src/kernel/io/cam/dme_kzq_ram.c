 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: dme_kzq_ram.c,v $ $Revision: 1.1.7.2 $ (DEC) $Date: 1993/09/21 21:51:41 $";
#endif
 ...
 *    11/01/91  janet   Moved Mipsmate functions to dme_mipsmate_kzq_ram.c
 ...
 *    10/12/91	rln	PreEFT changes: 1) Change get_segment => ramkzq_get_seg
 *    10/11/91	rln	First pass kzq support.
...
#include <io/cam/sim_kzq.h>		/* DEC KZQ specific definitions */
#include <io/cam/sim_common.h>
#include <io/cam/dme_kzq_ram.h>
...
/*
 * For the KZQ allocate only 1KB and 4Kb segments. 
 *
...
U32 ramkzq_get_segment();
U32 ramkzq_allocate_segments();
U32 ramkzq_dme_init();
U32 ramkzq_dme_setup();
U32 ramkzq_dme_end();
U32 ramkzq_dme_pause();
U32 ramkzq_dme_resume();
U32 ramkzq_dme_save();
U32 ramkzq_dme_restore();
U32 ramkzq_dme_modify();
U32 ramkzq_dme_clear();
U32 ramkzq_dme_map_copy_write();
U32 ramkzq_dme_map_buf();
U32 ramkzq_dme_init_segment();
U32 ramkzq_dme_reinit_segment();
U32 ramkzq_dme_bump_sglist();
U32 ramkzq_dme_toggle_buffer();
U32 ramkzq_dme_buffer_map_move();    
U32 ramkzq_lck_maps();    
void sc_free();
static char* allocate_list();
static U32 free_segment();
 ...
 *
 * For KZQ subtract off the base address of the RAM buffer shift left (*2)
 * one position and then add back in base address.
 */
#define DME_MOD_ADDRESS(addr,softc) (u_short*) (((U32) (addr) - \
		       (U32)SIMKZQ_GET_RAM_BUF(sim_softc)\
     		       <<1) + (U32)SIMKZQ_GET_RAM_BUF(sim_softc))


#define SIM_LOG_KZQ_ALL SIM_LOG_ALL_SIM_WS
    ...
    SIM_SOFTC* sim_softc;
{
    ...
    REGISTER SIMKZQ_REG *kzqreg;/* Address of KZQ CSR's			     */
    DME_KZQ_STRUCT *ldme;
    U32 mem_addr;		/* Ram buffer qbus address(start);	     */

    SIM_MODULE(ramkzq_dme_init);
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_init): Initialize the dme subsystem\n"));

    if ( sim_softc->simd_init )
      {
	CAM_ERROR(module,
		  "(ramkzq_dme_kzq_init): duplicate init call\n",
		   (SIM_LOG_KZQ_ALL|SIM_LOG_HBA_CSR),
		   sim_softc,NULL,NULL);
    ...
    /*
     * Get the kzq register address
     */
    kzqreg = SIMKZQ_GET_CSR(sim_softc);	   /* Address of KZQ CSR's           */
    ...
    if ( !sim_softc->dme )
      {
	CAM_ERROR(module,
		  "ramkzq_dme_init: Can't allocate memory for DME_STRUCT.\n",
		   (SIM_LOG_KZQ_ALL|SIM_LOG_HBA_CSR),
		   sim_softc,NULL,NULL);
    ...
    sim_softc->dme->extension = (void *)sc_alloc(sizeof(DME_KZQ_STRUCT));

    if ( !sim_softc->dme->extension )
      {
	  CAM_ERROR(module,
		    "ramkzq_dme_init: Can't allocate memory for DME_KZQ_STRUCT",
		   (SIM_LOG_KZQ_ALL|SIM_LOG_HBA_CSR),
		   sim_softc,NULL,NULL);
	/* 
	 * Free our resource gotten 
	 */
	sc_free( sim_softc->dme, sizeof(DME_STRUCT));

	return CAM_FAILURE;
      }

    ldme = (DME_KZQ_STRUCT *) sim_softc->dme->extension;
     ...
     * verifies that it is in a valid address range, and then
     * reserves the appropriate qbus mapping registers.  It then 
     * shifts the bits into place and converts to Qbus virtual by 
     * adding in 'qmem'
     */
    mem_addr = (U32)kzqreg->vector;
    if ((mem_addr & 0x400) == 0) {
	printf("kzq%d must have W8 removed\n",sim_softc->cntlr);
	/* 
	 * Free our resource gotten 
	 */
	sc_free( sim_softc->dme->extension, sizeof(DME_KZQ_STRUCT));
	sc_free( sim_softc->dme, sizeof(DME_STRUCT));
	return(CAM_FAILURE);
    }

    if ((mem_addr & 0xf800) < 0xe000) {
	if ((mem_addr & 0xf800) < 0xc000){
	    printf("kzq%d W9-W13 should be above 300000\n",sim_softc->cntlr);
	}
	if (ramkzq_lck_maps(((mem_addr & 0xf800) >>3) + 1, 
			sim_softc ) == 0 ){
	    printf("kzq%d memory cannot be mapped; change W9-W13",
			sim_softc->cntlr);
	    /* 
	     * Free our resource gotten 
	     */
    ...
    mem_addr = (((mem_addr & 0xf800)<<6) + (U32)qmem);
    ldme->SVA = (u_char *)mem_addr;
     ...
     */
    retval = ramkzq_allocate_segments(sim_softc);
 ...
 * FUNCTIONAL DESCRIPTION:
 * 
 * This function is called during ramkzq_dme_init() to divide up the RAM 
 ...
 **/
U32
ramkzq_allocate_segments(sim_softc)

    SIM_SOFTC* sim_softc;
{
    U32 retval;
    char* ram_buf_ptr;		  	/* Addr of free DMA RAM buffer */
    DME_KZQ_STRUCT *ldme = (DME_KZQ_STRUCT *) sim_softc->dme->extension;
    ...
    char* ram_buf_ptr;			/* Addr of free DMA RAM space */
{
     ...
     * the caller (ramkzq_dme_init) some flexibility, during the allocation of
    ...
    {
	dme_desc.segment = (SEGMENT_ELEMENT *)sc_alloc(
					sizeof(SEGMENT_ELEMENT));
    ...
    
{
    ...
    /*
     * Clear the segment buffer flags and then
     * make sure that the segment flags have been cleaned up.
     */
    dme_desc->segment->buf0.flags = NULL;
    dme_desc->segment->buf1.flags = NULL;
 ...
 *
 **/
U32 ramkzq_get_segment (sim_softc,dme_desc)
    ...
    DME_DESCRIPTOR* dme_desc;    
{
    ...
    /* 
     * Since the maximum transfer size for the KZQ is 8KB. The following
	...
	if (retval != CAM_REQ_CMP)
	{
	    ...
	    */
           retval = ramkzq_get_segment(sim_softc,&temp_desc);
 ...
 **/
U32
ramkzq_dme_setup( sim_ws, count, buf, dir, dme_desc )
    ...
    
{
    ...
    SIM_MODULE(ramkzq_dme_setup);
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (RAMKZQ_DME_SETUP) Setup a dma transfer\n"));
     ...
     */
    retval = ramkzq_get_segment(sim_softc,dme_desc);
    ...
    if (CAM_REQ_CMP != ramkzq_dme_init_segment(dme_desc,seg_ptr))
    {
	CAM_ERROR(module,
		  "(ramkzq_dme_setup): Failed to setup can't init segment\n",
		  (SIM_LOG_KZQ_ALL|SIM_LOG_HBA_CSR),
		  sim_softc,sim_softc->active_io,
		  sim_softc->active_interrupt_context->hba_intr);
	panic("ramkzq_dme_setup: Can't initialize segment\n");
    };
...
/*
 * Description of odd byte handling in the KZQ.
 *
 * The KZQ handles odd byte alligned transfer with the aid of one interrupt
 * bit (OBB) and one register (DMABYTE). Since the KZQ can only perform word
 ...
 *
 * The KZQ DME handles odd byte alligned transfer by having dme_pause detect
 * the condition of a transfer ending on an odd byte boundry and by having
 * dme_resume setup the KZQ appropriately when a transfer resumes at an odd
 * byte boundry.
 *
 *
 * More details TBD.
 ...
 * from the DME subsystem when the DME is active. ramkzq_dme_pause() will 
 ...
 *
 * This function must be called before a ramkzq_dme_save, ramkzq_dme_end or 
 * ramkzq_dme_resume call may be executed by the HBA.
 *
 * FORMAL PARAMETERS:  	       
 *    dme_desc		- Address of descriptor that describes this DMA request
 *
 ...
 **/
U32
ramkzq_dme_pause(dme_desc)
    
    REGISTER DME_DESCRIPTOR* dme_desc;
    
{ 			/* ramkzq_dme_pause */

    REGISTER KZQ_INTR *kzq_intr;	/* Address of interrupt state struct */
     ...
     */
    xfer_cnt = segbuf_ptr->act_pend_cnt - (U32) kzqreg->dmlotc;
     ...
     * segment has an odd byte pending. The ramkzq_dme_resume() function 
	 ...
	 */
	seg_ptr->pending_byte = kzqreg->dmabyte;
...
/**
 * ramkzq_dme_save - Maybe this should be a macro!
 *
 * FUNCTIONAL DESCRIPTION:
 *
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (RAMKZQ_DME_save) Save the state of this request.\n"));
...
/**
 * ramkzq_dme_restore - Maybe this should be a macro!
 *
 * FUNCTIONAL DESCRIPTION:
 *
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (RAMKZQ_DME_restore) Restore saved data pointer to the active.\n"));
 ...
 *
 * This routine calls ramkzq_dme_reinit segment which handles scatter gather,
    ...
    
{
    ...
    u_short kzq_cmd;			/* KZQ command to be issued to chip  */

    SIM_MODULE(ramkzq_dme_resume);
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_resume) entered the resume DMA function.\n"));
	...
	PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_resume:) Entered with ZERO segment element"));
	 ...
	 */
	if ( CAM_REQ_CMP != ramkzq_dme_reinit_segment(dme_desc,seg_ptr))
	{
	    CAM_ERROR(module,
     ...
     * KZQ has been completed, issue a "transfer info" command to the KZQ
     ...
     * load the KZQ DMABYTE register with the first (odd byte) of the
     * transfer.
     *
     * If the last transfer (datain) ended on an odd byte boundry the last byte
     * was left in the KZQ. Ramkzq_Dme_pause() removed this byte from
     * the the KZQ and saved it in the seg_ptr. This is done for  odd byte 
     * disconnect where the KZQ doesn't transfer the last byte into
     * memory. The DME must handle this case by reloading this byte into the
     * KZQ before resuming the transfer. Since the count of pending bytes 
     * INCLUDES this pending byte, when KZQ starts up again, it will transfer
     ...
     * If the starting address is odd we must first load the KZQ dmabyte reg.
     */
	 ...
	 */
	seg_ptr->flags &= ~SEG_ODD_DISCONNECT; 
	kzqreg->dmabyte = (u_short) *((u_char*)segbuf_ptr->act_ram_ptr);
     ...
     */
    kzqreg->dmlotc = segbuf_ptr->act_pend_cnt;

    /*
     * Calculate the RAM buffer address, the KZQ only can address 18 bits.
     ...
     * buffer to get the address to be loaded into the KZQ.
     */
     ...
     */
    kzqreg->dmaddrl = (ram_buf_addr & 0xffff);
    kzqreg->dmaddrh = (ram_buf_addr >> 16);



    /*
     * Insure that all the previously setup registers are written to the KZQ.
     */
    WBFLUSH(); 
     ...
     */
    kzq_cmd = ((kzq_intr->cstat &  KZQ_CSTAT_STATE) |
	       (kzq_intr->dstat & KZQ_DSTAT_PHASE)|
	       (KZQ_COMM_INFO_XFER | KZQ_COMM_DMA));
	...
	if (!(dme_desc->flags & DME_WRITE))
	    ramkzq_dme_buffer_map_move(dme_desc,seg_ptr,seg_ptr->pbuf,1);
    };
 ...
 * if ramkzq_dme_setup() has been called.
 *
 *
 * FORMAL PARAMETERS:  	       
 *    dme_desc		- Address of descriptor that describes this DMA request
    ...
    
{
    ...
    REGISTER SIMKZQ_REG *kzqreg;	/* Address of KZQ's CSR's              */
    U32 retval;			/* Return value                      */
    U32 xfer_cnt;		        /* Number of bytes really transfered */
    ...
    {
	PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	       ("[b/t/l] (ramkzq_dme_end:) Entered with ZERO segment element, return."));
	return(retval);
    }
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (RAMKZQ_DME_end) Complete a DME transaction deallocate resourses.\n"));
 ...
 * RETURN VALUE:        	CAM_REQ_CMP - All is well.
 *
 * SIDE EFFECTS:        	This function calls ramkzq_dme_init, to init the DME
 *
 * ADDITIONAL INFORMATION:      NONE
 *
 **/
U32
ds5500_kzq_dme_attach(sim_softc)
    ...
    
{
     ...
     */
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ds5500_kzq_dme_attach) XXXXXXX\n"));

    if ( CAM_REQ_CMP == ramkzq_dme_init(sim_softc))
    {
	/* initialize the data mover engine	*/
	sim_softc->dme->vector.dme_init = ramkzq_dme_init;
	
	/* setup for a transfer			*/
	sim_softc->dme->vector.dme_setup = ramkzq_dme_setup;
	
	/* end a data transfer			*/
	sim_softc->dme->vector.dme_end  = ramkzq_dme_end;
	
	/* pause the data mover engine		*/
	sim_softc->dme->vector.dme_pause= ramkzq_dme_pause;
	
	 /* continue a transfer			*/
	sim_softc->dme->vector.dme_resume = ramkzq_dme_resume;
	
	/* save the pointers			*/
	sim_softc->dme->vector.dme_save = ramkzq_dme_save;
	
	/* pause the data mover engine		*/
	sim_softc->dme->vector.dme_restore = ramkzq_dme_restore;
	
	/* modify the active data pointers	*/
	sim_softc->dme->vector.dme_modify  = ramkzq_dme_modify;
 ...
 **/
U32
ramkzq_dme_modify (dme_desc,offset)
    DME_DESCRIPTOR* dme_desc;
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (RAMKZQ_DME_MODIFY) Not fully supported\n"));
...
U32
ramkzq_dme_map_buf(dme_desc,seg_ptr,segbuf_ptr)
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_map_buf): Double map a buffer.\n"));
 ...
 **/
U32
ramkzq_dme_map_copy_write(dme_desc,segbuf_ptr)
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_map_copy_write): Double map a buffer and copy\n"));
    
    /*
     * Since this is an P0 buffer, we must first map the buffer before doing
     * the buffer copy into the RAM buffer.
     */
    if (ramkzq_dme_map_buf(dme_desc,seg_ptr,segbuf_ptr) != CAM_REQ_CMP)
	return(CAM_REQ_CMP_ERR);
 ...
 **/
U32
ramkzq_dme_map_copy_read(dme_desc,segbuf_ptr)
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_map_copy_read): Double map a buffer and copy\n"));
 ...
 * structure.
 *
 * The ramkzq_dme_init_segment() functions main purpose is to correctly initialize
 ...
 **/
U32
ramkzq_dme_init_segment(dme_desc,seg_ptr)
     ...
     SEGMENT_ELEMENT *seg_ptr;    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_init_segment) setup the segment for this request\n"));
	 ...
	 * buffered, since the KZQ's can only <= 8KB DMA operations.
	 */
	 ...
	 */
	retval = ramkzq_dme_buffer_map_move(dme_desc,seg_ptr,segbuf_ptr,0);
...
/**
 * ramkzq_dme_reinit_segment -
 *
 * FUNCTIONAL DESCRIPTION:
 *
 * Reinitialize  the segment element based on the passed DME_DESC and SIM_WS. 
 * This function is called by ramkzq_dme_resume() to initialize this requests segment
 ...
 **/
U32
ramkzq_dme_reinit_segment(dme_desc,seg_ptr)
    ...
    REGISTER SEGMENT_ELEMENT *seg_ptr;    
{
    ...
    SIM_MODULE(ramkzq_dme_reinit_segment);

    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_reinit_segment) setup the segment for this request\n"));
	...
	if (ramkzq_dme_toggle_buffer(dme_desc,seg_ptr,&segbuf_ptr) == CAM_REQ_CMP)
	{
	    /*
	     * Now that we have setup the active buffer, for write copy the 
	     * first buffer within the segment into the RAM buffer. For reads
	...
	{			/* Just toggle buffer */
	    ...
	    if (ramkzq_dme_toggle_buffer(dme_desc,seg_ptr,&segbuf_ptr) ==
		CAM_REQ_CMP)
	    {
		/*
	...
	{			/* Else Scatter Gather */
	    ...
	    if (ramkzq_dme_toggle_buffer(dme_desc,seg_ptr,&segbuf_ptr) == CAM_REQ_CMP)
	    {
		if (ramkzq_dme_bump_sglist(dme_desc,seg_ptr,segbuf_ptr)==CAM_REQ_CMP)
		{
		    
		    /*
		     * The ramkzq_dme_dump_sglist returned the address of a new uesr
		     * buffer and the size of that buffer. The
		     * ramkzq_dme_toggle_buffer returned the address in the RAM 
		     ...
		     */
		    retval =
			ramkzq_dme_buffer_map_move(dme_desc,seg_ptr,segbuf_ptr,0);
 ...
 **/
U32
ramkzq_dme_bump_sglist(dme_desc, seg_ptr, segbuf_ptr)
    ...
    REGISTER SEGMENT_BUFFER  *segbuf_ptr;/* Address of segment buffer  */
{
    ...
    SIM_MODULE(ramkzq_dme_bump_sglist);
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_bump_sglist): entered ramkzq_dme_bump_sglist\n"));	    
  ...
  **/
U32
ramkzq_dme_toggle_buffer (dme_desc, seg_ptr, segbuf_ptr)
    ...
    
{
    ...
    SIM_MODULE(ramkzq_dme_toggle_buffer);

    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_toggle_buffer): enter toggle buffer\n"));	    
    ...
    *segbuf_ptr   = seg_ptr->abuf;	/* Now point at active             */
     ...
     * actual copy will occur at the end of the ramkzq_dme_resume() function 
 ...
 **/
U32
ramkzq_dme_buffer_map_move(dme_desc,seg_ptr,segbuf_ptr,copy_on_read)
    ...
    U32 copy_on_read;			/* Read copy no copy flag    */
{
    ...
    SIM_MODULE(ramkzq_dme_buffer_map_move);

    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,
	   ("[b/t/l] (ramkzq_dme_buffer_map_move) Move data to/from ram buffer.\n"));
	...
	{				/* If a write operation */
	     ...
	     */
	    retval = ramkzq_dme_map_copy_write(dme_desc,segbuf_ptr);
	}
	else		/* Read operation */
	{
	    ...
	    if (copy_on_read)
	    {
		retval = ramkzq_dme_map_copy_read(dme_desc,segbuf_ptr);
	    }
	    else  retval = ramkzq_dme_map_buf(dme_desc,seg_ptr,segbuf_ptr);
     ...
     * which may be changed and that are used by the ramkzq_dme_resume() 
     * and ramkzq_dme_pause() functions.
     */
    ...
    
{
    ...
    PRINTD(NOBTL,NOBTL,NOBTL,CAMD_DMA_FLOW,("[b/t/l] (RAMKZQ_DME_XXXXX) XXXXXXX\n"));
    return(retval);
};			/* ramkzq_dme_XXXXX */
 ...
 *
 * FUNCTIONAL DESCRIPTION:
 * Reserve the mapping registers needed by the KZQSA to prevent
 * other devices from using the bus addresses where the KZQ memory is.
 *
 * FORMAL PARAMETERS:  		
 * U32 addr	 	
 *	The start address of where we lock
 * SIM_SOFTC *sim_softc; 
 *	So we can figure out which uba to reserve the maps from.
 *
 * IMPLICIT INPUTS:     	NONE
 *
 * IMPLICIT OUTPUTS:		NONE
 *
 * RETURN VALUE:        	
 * U32 NULL if maps could not be gotten 
 * U32 addr if sucess
 *
 * SIDE EFFECTS:        	NONE
 *
...
U32 ramkzq_lck_maps(addr, sim_softc)
	U32 addr;
	SIM_SOFTC *sim_softc;
{
	...
	um = sim_softc->um_probe;

	/* 
	 * do it this way Since none of the uba structs
	 * are filled in at this time....
	 */
	uhp = &uba_hd[numuba];
	return((U32)rmget(uhp->uq_map, 256, addr));
}

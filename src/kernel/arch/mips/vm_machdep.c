 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: vm_machdep.c,v $ $Revision: 1.2.4.3 $ (DEC) $Date: 1992/10/13 12:24:23 $";
#endif 
/*
 */
/*
 ...
 */
/* 
 * derived from vm_machdep.c	2.1	(ULTRIX/OSF)	12/3/90";
 */
/*
...
vm_machdep_init()
{
	...
	if (mapsize > size) {
		mapsize = size;
		printf("vm_machdep_init: warning reducing bp_map_size to %d\n",
			 size);
	}
 ...
 * the same memory type (KSEG0, KSEG1 or * KSEG2).
 */
...
get_cluster_bp(register struct buf *dp, int rw)
{
	...
	register enum {kseg0_mem, kseg1_mem, kseg2_mem} mem_type;
	vm_prot_t prot;
	...
	lbp = dp->av_forw;
	if (IS_KSEG0(lbp->b_un.b_addr)) mem_type = kseg0_mem;
	else if (IS_KSEG1(lbp->b_un.b_addr)) mem_type = kseg1_mem;
	else if (IS_KSEG2(lbp->b_un.b_addr)) mem_type = kseg2_mem;
	else panic("get_cluster_bp: kesgu memory not supported");
	...
	for (; lbp != dp; lbp = lbp->av_forw) {
		if (mem_type == kseg2_mem) 
			...
			case kseg0_mem:
				pa = K0_TO_PHYS(pa);
				ret = pmap_load(va, pa, size, 
					prot, TB_SYNC_NONE);
				if (prot & VM_PROT_WRITE) pmap_clear_modify(pa);
				break;
			case kseg1_mem:
				pa = K1_TO_PHYS(pa);
				ret = pmap_map_io(pa, size, va, 
					prot, TB_SYNC_NONE);
			}
			if (ret != KERN_SUCCESS) goto failed;
			va += size;
...
/*
 * TB strategy is to unload TB only when buffers are freed.
 * This avoids polluting the TB with translations that are
 * possibly never used.  Doing this also defers TB maintenace
 * until after the double mapping is nolonger required.
 */
...
bp_mapin(register struct buf *bp)
{
	...
	/*
	 * On the mips architecture we use KSEG0 to avoid extra mappings.
	 */

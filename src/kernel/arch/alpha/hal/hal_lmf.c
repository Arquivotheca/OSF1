 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: hal_lmf.c,v $ $Revision: 1.1.14.6 $ (DEC) $Date: 1993/10/19 21:49:55 $";
#endif
...
/* MIPS CPU types will not run under this kernel.  The ULTRIX code for
 * these cpus has been left in place, but it is conditional under the
 * MIPS_CPU_TYPE flag.  The following define sets that flag to 0 and
 * effectively removes the code. -- carlosc
 */
#define MIPS_CPU_TYPE 1
...
/* For production, the constants LMF_DEBUG and lmf_debug must be defined
 * here as 0. -- carlosc
 */
#define LMF_DEBUG 0
int lmf_debug = 0;
...
#include <mach/machine.h>   /* from OSF, for FLAMINGO vs SANDPIPER (cpu_subtype) */

/*
 *	Machine dependent kernel part of the Ultrix LMF.  The get/setlminfo
 *	functions are called from the get/setinfo system calls.
 */
...
/*  NOTE.  At the time this module is incorporated in a release, set_lmf_smm
 *  must be updated to reflect the current product line. -- carlosc
 */
...
set_lmf_smm() {
	...
	static unsigned short isis_tbl[] = 
		{ 4,
		  SMM_M5810, SMM_M5820,
		  SMM_M5830, SMM_M5840 };
        ...
        static unsigned short cobra_h_tbl[] = 
		{ 2,
		  SMM_A4000_610, SMM_A4000_620 };

        static unsigned short cobra_b_tbl[] = 
		{ 2,
		  SMM_A4000_810, SMM_A4000_820 };

        static unsigned short lazer_ruby_tbl[] = 
		{ 4,
		  SMM_A7000_610, SMM_A7000_620,
                  SMM_A7000_630, SMM_A7000_640 };

        static unsigned short blazer_ruby_tbl[] = 
		{ 4,
		  SMM_A10000_610, SMM_A10000_620,
                  SMM_A10000_630, SMM_A10000_640 };
	...
	case DS_CMAX:			/* CMAX = ? */
        ...
        /* Switch for Alpha CPU Types */

        /* The following switch uses fictitious values for some of
         * the CPU types; that is because the classification provided
         * in cpuconf.h is imcomplete at this time (Aug 92).  These
         * fictitious types are defined in the immediately following
         * defines.  The defines and the cases should be edited to
         * use actual values as they become available.
         */

#define ALPHA_COBRAa 	101	/* Cobra-B */
#define ALPHA_RUBYa	105	/* Blazer/Ruby */
#define DEC_3000_300a	106	/* Pelican S */

        /* For some CPU types, lmf_smm is set to SMM_UNKNOWN; when
         * the SMM is known for these CPU types, it should replace
         * SMM_UNKNOWN.  The object is to eventually eliminate all the
         * defines here.
         */

#define SMM_UNKNOWN        0  

	/* If the rpb_vers is not 5 or greater the rpb
	 * is old and does not have smm data in it.  This way
	 * new platform support does NOT NEED to change this
	 * code unless the data in the console HWRPB data is
	 * not correct.
	 */
	if(rpb->rpb_vers < 5 ){
		switch (cpu) {
		case ALPHA_ADU:			/* Not Used */
			lmf_smm = SMM_UNKNOWN;
			break;
	
		case DEC_4000:			/* Cobra-H = A4000_610 and 620 */
			set_smp_smm(cobra_h_tbl);
			break;
	
		case ALPHA_COBRAa:		/* Cobra-B = A4000_810 and 820 */
			set_smp_smm(cobra_b_tbl);
			break;
	
		case DEC_3000_500:		/* Flamingo W = A3000_500W */
			switch (machine_slot[master_cpu].cpu_subtype) {
				case CPU_SUBTYPE_DEC_3000_400:
					lmf_smm = SMM_A3000_400W;
					break;
				case CPU_SUBTYPE_DEC_3000_500:
					lmf_smm = SMM_A3000_500W;
					break;
				default:
					lmf_smm = SMM_UNKNOWN;
					break;
			...
			break;
		case DEC_3000_300:		/* Pelican W = A3000_300W */
			lmf_smm = SMM_A3000_300W;
			break;
	
		case DEC_3000_300a:		/* Pelican S = A3000_300S */
			lmf_smm = SMM_A3000_300S;
			break;
	
		case DEC_2000_300:		/* Jensen temp = A???? */
			lmf_smm = SMM_A2000_300;
			break;
	
		case DEC_7000:			/* Lazer systems */
			switch (machine_slot[master_cpu].cpu_subtype) {
			case CPU_SUBTYPE_DEC_7000:
			    set_smp_smm(lazer_ruby_tbl);
			    break;
			  case CPU_SUBTYPE_DEC_10000:
			    set_smp_smm(blazer_ruby_tbl);
			    break;
			  default:
			    lmf_smm = SMM_UNKNOWN;
			    break;
		...
		lmf_smm = get_platform_smm();
	}
#if LMF_DEBUG
                if( lmf_debug )
                        printf("set_lmf_smm: lmf_smm=%d\n", lmf_smm);
#endif

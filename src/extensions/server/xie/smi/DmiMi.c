...
**  ABSTRACT:
**
**	This module implements the connection between the XIE server DMI
**      (Digital Machine Independent) DDX, and a hardware-specific DDX.
**      The hardware-specific DDX uses this interface to obtain DDX services
**      which it does not implement.  If there is no hardware-specific DDX,
**      the DIX requests DMI DDX services using the interface in the module
**      DmiDdx.c
**      
...
**  External References
*/
extern	int	DmiUsingCL();
extern	int	SmiReqFullImg();
...
**	This independence will allow substitution of an MI layer in the server
...
#ifdef	CLIENT				/* CLIENT needs to use Xie	    */
...
**
*******************************************************************************/
externaldef(xieMiAlign) card32 (*xieMiAlign[])() = {
        NULL,                   /* 0 */
        NULL,
        NULL,
        NULL,

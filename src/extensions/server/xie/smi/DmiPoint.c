...
**	values using a lookup table.  The resulting values can have more or
...
**
*****************************************************************************/
int 	    DmiCreatePoint(pipe,srcsnk,dstsnk,idc,luts)
Pipe		     pipe;
PipeSinkPtr	     srcsnk;
PipeSinkPtr	     dstsnk;
UdpPtr		     idc;
...
**
*****************************************************************************/
int 	DmiPoint( srcudp, dstudp, idc, lut )
UdpPtr		srcudp;
UdpPtr		dstudp;
UdpPtr	 	idc;
UdpPtr		lut;

{
    ...
    */
    ...
    /* 
    **	If the ROI lies within the src/dst area, copy the areas which 
    **	don't need to be modified: top, bottom, left, right
    */
    status = Success;
    if( y1 > Y1 )
...
UdpPtr		lut;
{
    ...
    **	inner loops.
    **	For each dst type, a switch is done on the src type to ensure
    **	efficient code is used.
    */
    if( cpp == NULL )
	...
	case XieK_UBAVU:
	    ...
	    **	Source format is not considered
	    */
	...
	case XieK_ABU :			
	    ...
	    */
	    switch( DdxGetTyp_( src ) )
	    {
		case XieK_UBAV  :	/* table/dst == bytes, src bits/LU  */
		case XieK_UBAVU :
		    for (scanline = y1; scanline <= y2; scanline++)
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			...
			for( pixel = x1; pixel <= x2; pixel++ )
			{
			    pxlvalue = GET_VALUE_(src->UdpA_Base, srcpxloff, 
						    srcmask);
			    pxlvalue = ((unsigned char *)
					    lut->UdpA_Base)[pxlvalue];
		...
		case XieK_ABU :		/* table, dst, src are bytes         */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
			...
			{
			    pxlvalue = 	*((unsigned char *)
					(src->UdpA_Base + (srcpxloff >> 3)));
			    *((unsigned char *)
				(dst->UdpA_Base + (dstpxloff >> 3))) =
				 ((unsigned char *)lut->UdpA_Base)[pxlvalue];
		...
		case XieK_AWU :		/* table/dst are bytes, src is short */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
	    ...
	    break; /* End table bytes, dst bytes, src type switch */
	...
	case XieK_AWU :
	    ...
	    */
	    switch( DdxGetTyp_( src ) )
	    {
		case XieK_UBAV  :	/* tbl=bytes, dst=short, src bits/LU  */
		case XieK_UBAVU :
	 	    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
		...
		case XieK_ABU :	/* tbl = byte, dst = short, src = bytes	    */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
		...
		case XieK_AWU :	    /* tbl = byte, dst = short, src = short  */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
	    ...
	    break; /* End table bytes, dst shorts, src type switch */
	} /* End table bytes, dst type switch */
    ...
    */
	...
	case XieK_ABU :
	    ...
	    */
	    switch( DdxGetTyp_( src ) )
	    {
		case XieK_UBAV  :	/* tbl = short, dst = bytes, src bits/LU  */
		case XieK_UBAVU :
		    for (scanline = y1; scanline <= y2; scanline++)
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			...
			for( pixel = x1; pixel <= x2; pixel++ )
			{
			    ...
			    pxlvalue = ((unsigned short *)
					    lut->UdpA_Base)[pxlvalue];
			    PUT_VALUE_(dst->UdpA_Base, dstpxloff, pxlvalue, 
					    dstmask);
		...
		case XieK_ABU :	    /* tbl = short, dst = byte, src = byte   */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
			...
			for (pixel = x1; pixel <= x2; pixel++)
			{
			    ...
			    *((unsigned char *)
				(dst->UdpA_Base + (dstpxloff >> 3))) = 
				  ((unsigned short *)lut->UdpA_Base)[pxlvalue];
		...
		case XieK_AWU :	    /* tbl = short, dst = byte, src = short */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
		    ...
		    break;
	    } /* End tbl = short, dst = byte, src type switch */
	    break;
	...
	case XieK_AWU :
	    ...
	    */
	    switch( DdxGetTyp_( src ) )
	    {
		case XieK_UBAV  :	/* tbl=short, dst=short, src bits/LU */
		case XieK_UBAVU :
	 	    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
		...
		case XieK_ABU :	    /* tbl = short, dst = short, src = bytes */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
		...
		case XieK_AWU :	    /* tbl = short, dst = short, src = short */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)*dst->UdpL_PxlStride;
		    ...
		    break;
	    } /* End tbl = short, dst = short, src type swich */
	    } /* End tbl = short, dst type switch	      */
	} /* End if then */

    else
	{
    ...
    */
	...
	case XieK_ABU :
	    ...
	    */
	    switch( DdxGetTyp_( src ) )
	    {
		case XieK_UBAV  :   /* tbl = long, dst = byte, src bits/LU   */
		case XieK_UBAVU :
		    for (scanline = y1; scanline <= y2; scanline++)
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
		...
		case XieK_ABU :	    /* tbl = long, dst = byte, src = byte    */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)
					* src->UdpL_PxlStride;
			dstpxloff = dstpxlstart+(x1-dst->UdpL_X1)
					* dst->UdpL_PxlStride;
			...
			for (pixel = x1; pixel <= x2; pixel++)
			{
			    ...
			    *((unsigned char *) 
				(dst->UdpA_Base + (dstpxloff >> 3))) =
				  ((unsigned long *)lut->UdpA_Base)[pxlvalue];
		...
		case XieK_AWU :	    /* tbl = long, dst = byte, src = short   */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)
					* src->UdpL_PxlStride;
		    ...
		    break;
	    } /* End tbl = long, dst = byte, src type switch */
	    break;
	...
	case XieK_AWU :
	    ...
	    */
	    switch( DdxGetTyp_( src ) )
	    {
		case XieK_UBAV  :   /* tbl = long, dst = short, src bits/LU  */
		case XieK_UBAVU :
		    for (scanline = y1; scanline <= y2; scanline++)
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)*src->UdpL_PxlStride;
		...
		case XieK_ABU :     /* tbl = long, dst = short, src = byte  */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)
					* src->UdpL_PxlStride;
		...
		case XieK_AWU :	    /* tbl = long, dst = short, src = short  */
		    for( scanline = y1; scanline <= y2; scanline++ )
		    {
			srcpxloff = srcpxlstart+(x1-src->UdpL_X1)
					* src->UdpL_PxlStride;

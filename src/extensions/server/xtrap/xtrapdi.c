 ...
 */
#ifndef lint
static char RCSID[] = "$Header: /alphabits/u3/x11/ode/rcs/x11/src/extensions/server/xtrap/xtrapdi.c,v 1.1.2.3 92/04/27 14:05:09 Leela_Obilichetti Exp $";
#endif

/*-----------------*
 *  Include Files  *
...
#ifdef VXT
globalref int_function ProcVector[256L];           /* The real ProcVector */
#else
extern int_function ProcVector[256L];           /* The real ProcVector */
#endif
...
extern void_function ReplySwapVector[256L];/* WriteReplyToClient & sDispatch */
extern void_function EventSwapVector[128L];   /* WriteEventsToClient & sDisp */
...
# define P(s) ()
#endif
static void _SwapProc P((int (**f1 )(), int (**f2 )()));
static void sXETrapEvent P((xETrapDataEvent *from , xETrapDataEvent *to ));
static int add_accelerator_node P((ClientPtr client , ClientList *accel ));
static void remove_accelerator_node P((ClientPtr client , ClientList *accel ));
static void update_protocol P((xXTrapGetReq *reqptr , ClientPtr client ));
#ifdef COLOR_REPLIES
static void GetSendColorRep P((ClientPtr client , xResourceReq *req ));
static void GetSendNamedColorRep P((ClientPtr client , xResourceReq *req ));
static void GetSendColorCellsRep P((ClientPtr client , xResourceReq *req ));
static void GetSendColorPlanesRep P((ClientPtr client , xResourceReq *req ));
#endif
#ifdef X11R3
static unsigned long *Xcalloc P((unsigned long amount ));
#endif
#undef P

/*
 ...
 */
#ifdef FUNCTION_PROTOS
void XETrapDestroyEnv(pointer value, XID id)
#else
void XETrapDestroyEnv(value, id)
    ...
    if (XETrapKbdDev == NULL)
    {
        ...
        else
        {
            ...
            EventProcVector[KeyPress] =
                (int_function)XETrapKbdDev->processInputProc;
    ...
    if (XETrapPtrDev == NULL)
    {
        ...
        else
        {
            ...
            EventProcVector[ButtonPress] = 
                (int_function)XETrapPtrDev->processInputProc;
...
void DEC_XTRAPInit()
{
    ...
    EventSwapVector[extEntry->eventBase + XETrapData] = sXETrapEvent;
...
#if defined X11R3 && !defined VMSDW_V3
        (XETrapClass = CreateNewResourceClass()) == 0L ||
#endif
...
#else
int XETrapReset(request, client)
    xXTrapReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapGetAvailable(request, client)
...
#else
int XETrapGetCurrent(request, client)
    xXTrapReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapGetStatistics(request, client)
    xXTrapReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapConfig(request, client)
...
#else
int XETrapStartTrap(request, client)
    xXTrapReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapStopTrap(request, client)
    xXTrapReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapGetVersion(request, client)
    xXTrapGetReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapGetLastInpTime(request, client)
    xXTrapReq *request;
    ClientPtr client;
#endif
{
...
#else
int XETrapRequestVector(client)
    ClientPtr client;
#endif
{
    int status = True;
...
#else
                    pdata->hdr.win_x = window_ptr->absCorner.x;
                    pdata->hdr.win_y = window_ptr->absCorner.y;
#endif
                }
            }
...
#else
int XETrapKeyboard(x_event,keybd,count)
    xEvent *x_event;                
    DevicePtr keybd;
    int count;   /* Used in X11R4 only for batch of events */
#endif
{                     
...
#if ((!defined X11R3 || defined VMSDW_V3) && (defined VERBOSE))
    if (count != 1L)
...
#else
int XETrapPointer(x_event,ptrdev,count)
    xEvent *x_event;
    DevicePtr ptrdev;
    int count;   /* Used in X11R4 only for batch of events */
#endif
...
#else
void XETrapStampAndMail(x_event)
    xEvent *x_event;
#endif
{
    XETrapDatum data;
...
#else
int XETrapEventVector(client, x_event)
    ClientPtr client;
    xEvent    *x_event;
#endif
{
...
#else
void sReplyXTrapDispatch(client,size,reply)
    ClientPtr client;
    int       size;
    char      *reply;
#endif
...
#else
int XETrapWriteXLib(penv, data, nbytes)
    XETrapEnv *penv;
    BYTE      *data;
    CARD32    nbytes;
#endif
...
#else
static void update_protocol(reqptr, client)
    xXTrapGetReq *reqptr;
    ClientPtr client;
#endif
{
 ...
 *  DESCRIPTION:
 *
 *      This function performs the DECwindows specific memory allocation
 *      since it does NOT follow Sheiffler/Gettys standard of Xmalloc
 *      and does NOT even have the equivalent Xcalloc call!
 *
 *      This version of the function has been copied directly from the
 *      X11R4 sources in the server/os/4.2bsd/utils.c module.
 *
 */
#ifdef FUNCTION_PROTOS
static unsigned long *Xcalloc(unsigned long amount)
#else
static unsigned long *Xcalloc(amount)
...
#else
static void sXETrapEvent(from,to)
    xETrapDataEvent *from, *to;
#endif
{
    to->type = from->type;
...
#else
static int add_accelerator_node(client, accel)
    ClientPtr  client;
    ClientList *accel;
#endif
{
...
#else
static void remove_accelerator_node(client, accel)
    ClientPtr  client;
    ClientList *accel;
#endif
{
...
#else
static void GetSendColorRep(client, req)
    ClientPtr client;
    xResourceReq *req;
#endif
...
#else
static void GetSendNamedColorRep(client, req)
    ClientPtr client;
    xResourceReq *req;
#endif
...
#else
static void GetSendColorCellsRep(client, req)
    ClientPtr client;
    xResourceReq *req;
#endif
...
#else
static void GetSendColorPlanesRep(client, req)
    ClientPtr client;
    xResourceReq *req;
#endif

 ...
 */
#ifndef lint
static char RCSID[] = "$Header: /alphabits/u3/x11/ode/rcs/x11/src/extensions/server/xtrap/xtrapdiswap.c,v 1.1.2.3 92/11/06 16:23:36 Jim_Ludwig Exp $";
#endif

#include "X.h"
#define NEED_REPLIES
...
#endif
{
    register char n;
    swaps(&(request->length),n);
    swaps(&(request->config.max_pkt_size),n);
...
#endif
{
    ...
    swapl(&(rep->data.xport_msk_out),n);
#ifndef _XINPUT
    swapl(&(rep->data.xport_msk_in),n);
#endif
...
#endif
{
    ...
    swaps(&(rep->hdr.sequenceNumber),n);
    swapl(&(rep->hdr.length),n);
    swaps(&(rep->data.config.max_pkt_size),n);
    (void)WriteToClient(client,size,reply);
    return;
}
#ifdef FUNCTION_PROTOS

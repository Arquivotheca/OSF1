 ...
 */
#ifndef lint
static char *rcsid = "@(#)$RCSfile: libpip.c,v $ $Revision: 1.1.6.2 $ (DEC) $Date: 1993/06/18 18:17:44 $";
#endif
/***********************************************************
...
#include <X11/Xserver/pmagro.h>

#include "misc.h" /* GET MAXSCREENS SYMBOL */
#include <X11/Xserver/rop.h> /* RopPtr STRUCT DEFINITION SHARED WITH ROP.C */
#include "libpip.h"
#include "i2c.h"
...
#define I2CDELAY 5
static int i2cDelay = I2CDELAY;
static int *waitPtr = &i2cDelay;
static int waitMem[I2CDELAY];

/* max and min values of duty cycle register for this machine */
static int dutyCycleMin;
static int dutyCycleMax;
...
pip_priority(prop, prio)
     ...
     unsigned int prio;
{
  if (prio > dutyCycleMax) prio = dutyCycleMax;
  if (prio < dutyCycleMin) prio = dutyCycleMin;
  * (unsigned int *) prop->dutyCycle = prio;
}
     ...
     int		type;	/* =  type of video source: see libpip.h */
{
  ...
  /*
   *    Set limits for TurboChannel duty cycle.  The minimum value was
   *    established empirically on a Flamingo.  The maximum is as defined in
   *    the TX/PIP specification.
   */

  dutyCycleMin = 0xa0;
  dutyCycleMax = 0xb0;

  /* index is from logical screen number. Convert to physical here */
  prop = &ropInfo[index];
  pregs = (Pip_Device_Regs *)prop->pipRegisters;

  if (!pip_installed(index)) return (RopPtr)NULL;

  /* UGLY, NO VERY UGLY */

  waitPtr = (int *)prop->SelectionPlane;
  ...
  pip_priority(prop, 256);      /* set low video priority */

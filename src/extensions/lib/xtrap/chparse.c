 ...
 */
int chparse(max_delay, rest_delay, state,
    private, param, nparam, inter, ninter, final)
int		max_delay;
int		rest_delay;
...
dumpsequence(state, c, private, param, nparam, inter, ninter, final, column)
int            state;
int		c;
int		private;	/* Sequence private char, 'X' if error	*/
int		param[];	/* numeric param, starting at param[1]	*/
...
int		max_delay;
{
	...
	if (bptr >= bend) {
	    ...
	    count = select(fileno(stdin) + 1, &fdmask, NULL, NULL, &timeout);
	    if (count < 0)
		return (ERROR);
	    else if (count == 0)
		return (TIMEOUT);

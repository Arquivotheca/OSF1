...
#else
static unsigned long _XSetLastRequestRead();
#endif
...
#else
static Bool event_to_wire(dpy,libevent,netevent)
    Display *dpy;
    XEvent *libevent;
    xEvent *netevent;
#endif
...
#else
Bool XETrapQueryExtension(dpy,event_base_return,error_base_return,opcode_return)
    Display *dpy;
...
#else
unsigned long _XSetLastRequestRead(dpy, rep)

    ...
    Multibuffer *buffers;
{
    ...
    if ( sizeof(Multibuffer) != sizeof(CARD32) ) {
	protoBuffers = (CARD32 *)Xmalloc(count * sizeof(CARD32));
	if (! protoBuffers) {
	    _XEatData(dpy, (unsigned long) (count * sizeof(CARD32)));
	    UnlockDisplay(dpy);
	    SyncHandle();
	    return (0);
	}
        for (i = 0; i < count; i++) 
	    protoBuffers[i] = (CARD32)buffers[i];
    }
    else
	protoBuffers = (CARD32 *)buffers;
    ...
    req->length += count;
    PackData32 (dpy, protoBuffers, count * sizeof (CARD32));
    ...
    XmbufWindowAttributes *attr;
{
	...
	int protobytes = rep.length * sizeof(CARD32);
	int nbytes = rep.length * sizeof(Multibuffer);

	attr->buffers = (Multibuffer *) Xmalloc((unsigned) nbytes);

	if ( sizeof(Multibuffer) != sizeof(CARD32) )  
	    protoBuffers = (CARD32 *) Xmalloc((unsigned) protobytes);
	else
	    protoBuffers = (CARD32 *)attr->buffers;

	protobytes = rep.length << 2;
	if (! attr->buffers || ! protoBuffers) {
	    if ( attr->buffers ) Xfree(attr->buffers);
	    if ( protoBuffers )  Xfree(protoBuffers);
	    _XEatData(dpy, (unsigned long) protobytes);
	    UnlockDisplay(dpy);
	    SyncHandle();
	    return (0);
	}
	_XRead32 (dpy, (char *) protoBuffers, protobytes);

	if ( sizeof(Multibuffer) != sizeof(CARD32) )  {
	    register int i;
	    for ( i = 0; i < rep.length; i++ )
		attr->buffers[i] = (CARD32)protoBuffers[i];
	    Xfree(protoBuffers);
	}
    }
    ...
    XmbufSetWindowAttributes *attr;
{
	...
	CARD32 values[1];	/* one per element in if stmts below */
	CARD32 *v = values;

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: lvdisplay.c,v $ $Revision: 4.2.3.2 $ (DEC) $Date: 1993/01/08 15:41:47 $";
#endif 
/*
 */
/*
...
#define USAGE	"Usage: lvdisplay  [-v] \
 LogicalVolumePath... \n"
...
main(int argc, char **argv)
{
   ...
   /* Just loop on the logical volume names, and call the library routine */
   dump_lv_title();
   for (i = 0; i < LogicalVolumePath_cnt; i++) {

      lv_path = LogicalVolumePath[i];
      if (dump_lv(lv_path, vflag? VERBOSE : TERSE) != OK) {
	 print_prgname();
	 fprintf(stderr, MSG_CANT_DUMP_LV, lv_path);
	 return(1);
      }
   }

 ...
 */
#ifndef lint
static char	*sccsid = "@(#)$RCSfile: test19.c,v $ $Revision: 4.2 $ (DEC) $Date: 1991/09/19 23:44:15 $";
#endif 
/*
 */
/*
...
char testpurpose[] = "test bad pointer to environment list (EFAULT)";
	...
	char *argv[];
{
	...
	/********** Must pass argv else envp won't be read! **********/
	ltprintf("calling exec_with_loader(0, \"%s\", \"%s\", argv, %d)\n",
		LOADER, COMMAND, BAD_POINTER);
	if (exec_with_loader(0, LOADER, COMMAND, argv,
	    (char **)BAD_POINTER) != -1) {
